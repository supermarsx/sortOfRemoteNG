...
impl AgileEncryptionInfo {
    ...
    pub fn decrypt(
        ...
        match self.key_data_hash_algorithm.as_str() {
            "SHA512" => {
                // PATCH (sortOfRemoteNG): the upstream condition
                // `block_start < (total_size - SEGMENT_LENGTH)` exits the
                // loop one iteration too early *and* underflows when
                // `total_size < SEGMENT_LENGTH`, leaving the last-block
                // code to decrypt two segments with a single IV and
                // corrupting the trailing bytes (or panicking in debug
                // builds). Rewritten as "loop while there's still more
                // than one segment of plaintext left to consume" so
                // every full segment goes through the loop with its
                // matching block_index and the last-block path only
                // handles the final partial segment.
                while total_size > SEGMENT_LENGTH + (block_start - 8) {
