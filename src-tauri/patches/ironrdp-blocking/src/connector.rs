    ...
    S: Read + Write,
{
    ...
    if connector.should_perform_credssp() {
        info!("CredSSP: starting perform_credssp_step");
        ...
        info!("CredSSP: perform_credssp_step completed successfully");
    }

    info!(connector.state = connector.state.name(), "Remaining of connection sequence");

    let result = loop {
        info!(connector.state = connector.state.name(), "single_sequence_step: entering");
    ...
    S: Read + Write,
{
    ...
    loop {
        ...
        let Some(next_pdu_hint) = sequence.next_pdu_hint() else {
            info!("CredSSP: sequence finished (no more PDU hints), breaking loop");
            break;
        };
        ...
        info!(
            ...
            hint = ?next_pdu_hint,
            "CredSSP: waiting for PDU from server"
        );
    ...
    S: Read + Write,
{
    ...
    let written = if let Some(next_pdu_hint) = connector.next_pdu_hint() {
        info!(
            connector.state = connector.state.name(),
            hint = ?next_pdu_hint,
            "single_sequence_step: reading PDU"
        );

        let pdu = framed
            .read_by_hint(next_pdu_hint)
            .map_err(|e| {
                info!(connector.state = connector.state.name(), "READ FAILED in single_sequence_step");
                ironrdp_connector::custom_err!("read frame by hint", e)
            })?;
