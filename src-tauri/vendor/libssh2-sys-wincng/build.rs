...
fn main() {
    ...
    assert!(
        Path::new("libssh2/src/session.c").is_file(),
        "Missing vendored libssh2 source; restore the complete published libssh2-sys package. Automatic submodule/network checkout is disabled."
    );
    ...
    if target.contains("windows") {
        ...
        if env::var_os("CARGO_FEATURE_OPENSSL_ON_WIN32").is_some() {
            ...
            // sortOfRemoteNG requires Windows 10+: upstream disables native ECC
            // only for older Windows compatibility. Keep this crate-scoped.
            cfg.define("LIBSSH2_ECDSA_WINCNG", None);
        }
    } else {
