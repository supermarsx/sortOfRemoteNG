...
impl JiraClient {
    pub fn from_config(cfg: &JiraConnectionConfig) -> JiraResult<Self> {
        ...
        Ok(Self { http, base_url: base, api_version: cfg.api_version.clone(), auth_header })
    }

    fn default_headers(&self) -> header::HeaderMap {
        let mut h = header::HeaderMap::new();
        h.insert(header::AUTHORIZATION, header::HeaderValue::from_str(&self.auth_header).unwrap_or_else(|_| header::HeaderValue::from_static("")));
    ...
    pub(crate) async fn get_with_params<T: DeserializeOwned>(&self, url: &str, params: &[(String, String)]) -> JiraResult<T> {
        let resp = self.http.get(url).headers(self.default_headers()).query(params).send().await?;
        self.handle_response(resp).await
    }

    pub(crate) async fn post<T: DeserializeOwned, B: serde::Serialize>(&self, url: &str, body: &B) -> JiraResult<T> {
        let resp = self.http.post(url).headers(self.default_headers()).json(body).send().await?;
        self.handle_response(resp).await
    }

    pub(crate) async fn post_empty(&self, url: &str) -> JiraResult<()> {
        let resp = self.http.post(url).headers(self.default_headers()).send().await?;
        self.handle_empty(resp).await
    }

    pub(crate) async fn post_unit<B: serde::Serialize>(&self, url: &str, body: &B) -> JiraResult<()> {
        let resp = self.http.post(url).headers(self.default_headers()).json(body).send().await?;
        self.handle_empty(resp).await
    }

    pub(crate) async fn put<T: DeserializeOwned, B: serde::Serialize>(&self, url: &str, body: &B) -> JiraResult<T> {
        let resp = self.http.put(url).headers(self.default_headers()).json(body).send().await?;
        self.handle_response(resp).await
    }

    pub(crate) async fn put_unit<B: serde::Serialize>(&self, url: &str, body: &B) -> JiraResult<()> {
        let resp = self.http.put(url).headers(self.default_headers()).json(body).send().await?;
        self.handle_empty(resp).await
    }
    ...
    async fn handle_response<T: DeserializeOwned>(&self, resp: Response) -> JiraResult<T> {
        ...
        if status.is_success() {
            ...
            serde_json::from_str(&text).map_err(|e| JiraError::new(JiraErrorKind::ParseError, format!("{}: {}", e, &text[..text.len().min(200)])))
        } else {
    ...
    fn status_error(&self, status: StatusCode, body: String) -> JiraError {
        ...
        JiraError::new(kind, format!("{}: {}", status, &body[..body.len().min(500)]))
    }
    ...
    pub async fn ping(&self) -> JiraResult<crate::types::JiraConnectionStatus> {
        ...
        match self.http.get(&url).headers(self.default_headers()).send().await {
            Ok(resp) if resp.status().is_success() => {
                let body: serde_json::Value = resp.json().await.unwrap_or_default();
                Ok(crate::types::JiraConnectionStatus {
                    connected: true,
                    server_title: body.get("serverTitle").and_then(|v| v.as_str()).map(String::from),
                    version: body.get("version").and_then(|v| v.as_str()).map(String::from),
                    deployment_type: body.get("deploymentType").and_then(|v| v.as_str()).map(String::from),
                    message: Some("Connected".into()),
                })
            }
            Ok(resp) => Ok(crate::types::JiraConnectionStatus {
                connected: false, server_title: None, version: None, deployment_type: None,
