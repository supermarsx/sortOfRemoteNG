...
impl FilterManager {
    pub async fn get(client: &JiraClient, filter_id: &str) -> JiraResult<JiraFilter> {
        client
            .get(&client.api_url(&format!("/filter/{}", filter_id)))
            .await
    }
    ...
    pub async fn update(
        ...
        client
            .put(&client.api_url(&format!("/filter/{}", filter_id)), req)
            .await
    }

    pub async fn delete(client: &JiraClient, filter_id: &str) -> JiraResult<()> {
        client
            .delete(&client.api_url(&format!("/filter/{}", filter_id)))
            .await
    }
    ...
    pub async fn search(client: &JiraClient, name: &str) -> JiraResult<Vec<JiraFilter>> {
        ...
        client
            .get_with_params(&client.api_url("/filter/search"), &params)
            .await
    }
}
