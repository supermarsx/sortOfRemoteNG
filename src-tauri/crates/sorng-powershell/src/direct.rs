...
/// Build a safe VM target argument, escaping single quotes and validating VM ID format.
fn sanitize_vm_target(config: &PsDirectConfig) -> Result<String, String> {
    if let Some(ref id) = config.vm_id {
        if !id.chars().all(|c| c.is_ascii_hexdigit() || c == '-') {
            return Err("Invalid VM ID: must be a GUID (hex digits and hyphens only)".to_string());
        }
        Ok(format!("-VMId '{}'", id.replace('\'', "''")))
    } else {
        Ok(format!(
            "-VMName '{}'",
            config.vm_name.as_deref().unwrap_or("").replace('\'', "''")
        ))
    }
}
