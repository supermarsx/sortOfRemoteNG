...
fn validate_command_name(name: &str) -> Result<(), String> {
        ...
        && !name.is_empty()
    {
        ...
        Err("Invalid command name: contains disallowed characters".to_string())
    }
}

/// Validates a file path doesn't contain shell metacharacters beyond basic path characters.
fn validate_file_path(path: &str) -> Result<(), String> {
    let dangerous = ['`', '$', ';', '|', '{', '}', '(', ')'];
    if path.chars().any(|c| dangerous.contains(&c)) {
        Err("File path contains disallowed shell metacharacters".to_string())
    } else {
        Ok(())
    }
}
