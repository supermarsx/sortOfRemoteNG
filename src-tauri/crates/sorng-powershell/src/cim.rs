...
use uuid::Uuid;
/// Sanitize input for use in PowerShell single-quoted strings.
/// Escapes single quotes and rejects dangerous characters.
fn sanitize_ps_input(input: &str) -> Result<String, String> {
    // Reject null bytes and control characters
    if input.chars().any(|c| c.is_control() && c != '\t') {
        return Err("Input contains control characters".to_string());
    }
    // Escape single quotes (double them for PS single-quoted strings)
    Ok(input.replace('\'', "''"))
}

fn sanitize_ps_filter(input: &str) -> Result<String, String> {
    let dangerous = ['$', '`', ';', '|', '{', '}'];
    if input
        .chars()
        .any(|c| dangerous.contains(&c) || (c.is_control() && c != '\t'))
    {
        return Err("Filter contains disallowed characters".to_string());
    }
...
impl CimSessionManager {
    ...
    pub async fn get_instances(
        ...
        let safe_class = sanitize_ps_input(&params.class_name)?;
        let safe_ns = sanitize_ps_input(namespace)?;
        ...
        if let Some(ref filter) = params.filter {
            let safe_filter = sanitize_ps_filter(filter)?;
            script.push_str(&format!(" -Filter \"{}\"", safe_filter));
        }
        ...
        if !params.property.is_empty() {
            let props = params
                .property
                .iter()
                .map(|p| format!("'{}'", sanitize_ps_input(p).unwrap_or_default()))
                .collect::<Vec<_>>()
                .join(", ");
    ...
    pub async fn invoke_method(
        ...
        let safe_class = sanitize_ps_input(&params.class_name)?;
        let safe_ns = sanitize_ps_input(namespace)?;
        let safe_method = sanitize_ps_input(&params.method_name)?;

        let mut script = format!(
            "Invoke-CimMethod -ClassName '{}' -Namespace '{}' -MethodName '{}'",
            safe_class, safe_ns, safe_method
        );
    ...
    pub async fn register_event(
        ...
        let safe_ns = sanitize_ps_input(namespace)?;
        let safe_query = sanitize_ps_filter(&params.query)?;
        let safe_dialect = sanitize_ps_input(&params.query_dialect)?;
