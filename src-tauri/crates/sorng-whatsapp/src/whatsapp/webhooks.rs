...
mod tests {
    ...
    fn test_parse_text_message_webhook() {
        ...
        let WaWebhookEvent::IncomingMessage(evt) = &events[0] else {
            unreachable!("Expected IncomingMessage")
        };
    ...
    fn test_parse_status_update_webhook() {
        ...
        let WaWebhookEvent::StatusUpdate(evt) = &events[0] else {
            unreachable!("Expected StatusUpdate")
        };
