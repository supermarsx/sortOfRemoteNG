...
impl HealthManager {
    ...
    pub async fn get_dashboard(client: &SmcClient) -> SmcResult<SmcDashboard> {
        let system_info = crate::system::SystemManager::get_system_info(client)
            .await
            .ok();
        let bmc_info = crate::system::SystemManager::get_bmc_info(client)
            .await
            .ok();
        let power_state = crate::power::PowerManager::get_power_state(client)
            .await
            .ok();
        let health = Self::get_health_rollup(client).await.ok();
        let thermal = crate::thermal::ThermalManager::get_thermal_summary(client)
            .await
            .ok();
        let power_metrics = crate::power::PowerManager::get_power_metrics(client)
            .await
            .ok();
        let event_log = crate::event_log::EventLogManager::get_event_log(client)
            .await
            .ok();

        let platform = bmc_info
            .as_ref()
