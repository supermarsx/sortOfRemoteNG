//! Trust Center store handle for the legacy web TLS client.
//!
...
/// an error and the effective policy reads `Strict`: a BMC handshake fails
...
mod tests {
    ...
    fn tls_identity(fingerprint: &str) -> Identity {
        Identity::Tls(Box::new(CertIdentity {
            ...
            subject: Some("CN=bmc.example".into()),
            issuer: Some("CN=bmc.example".into()),
    ...
    fn pins_into_the_active_database_trust_file() {
        ...
        let _guard = install_active_runtime_for_tests(databases.clone(), "smc-db");

        trust_store_handle()
            .trust(
                "bmc.example:443".into(),
                "tls".into(),
                tls_identity("aa11"),
                false,
            )
        ...
        assert!(
            databases.join("smc-db.trust.json").exists(),
    ...
    fn fails_closed_without_an_active_database() {
        ...
        assert!(store
            .verify("bmc.example:443", "tls", tls_identity("aa11"))
            .is_err());
    }
}
