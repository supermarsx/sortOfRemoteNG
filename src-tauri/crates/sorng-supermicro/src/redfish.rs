...
impl SmcRedfishClient {
    ...
    pub async fn get_bmc_info(&self) -> SmcResult<SmcBmcInfo> {
        ...
        Ok(SmcBmcInfo {
            ...
            bmc_mac_address: mgr
                ...
                .and(None), // Would need a follow-up call
            ipmi_version: mgr
                .get("Oem")
                .and_then(|o| o.get("Supermicro"))
                ...
                if let Some(uri) = member.get("@odata.id").and_then(|v| v.as_str()) {
                    if let Ok(stor) = self.get_json(uri).await {
                        if let Some(controllers) =
                            stor.get("StorageControllers").and_then(|v| v.as_array())
                        {
                                    ...
                                    if let Ok(drv) = self.get_json(du).await {
                                        disks.push(PhysicalDisk {
                                            ...
                                            slot: drv
                                                ...
                                                .and_then(|v| v.as_u64())
                                                .map(|v| v as u32),
                                            predicted_life_left_percent: drv
    ...
    pub async fn get_network_adapters(&self) -> SmcResult<Vec<NetworkAdapter>> {
        let col = self
            .get_json("/redfish/v1/Systems/1/EthernetInterfaces")
            .await?;
    ...
    pub async fn get_bmc_network(&self) -> SmcResult<Vec<NetworkAdapter>> {
        let col = self
            .get_json("/redfish/v1/Managers/1/EthernetInterfaces")
            .await?;
        let mut adapters = Vec::new();
    ...
    pub async fn get_virtual_media_status(&self) -> SmcResult<Vec<VirtualMediaStatus>> {
        let col = self.get_json("/redfish/v1/Managers/1/VirtualMedia").await?;
        let mut items = Vec::new();
    ...
    pub async fn get_event_log(&self) -> SmcResult<Vec<EventLogEntry>> {
        let col = self
            .get_json("/redfish/v1/Managers/1/LogServices/Log1/Entries")
            .await?;
    ...
    pub async fn get_audit_log(&self) -> SmcResult<Vec<EventLogEntry>> {
        ...
        let result = self
            .get_json("/redfish/v1/Managers/1/LogServices/AuditLog/Entries")
            .await;
        match result {
            Ok(col) => {
                let mut entries = Vec::new();
    ...
    pub async fn get_users(&self) -> SmcResult<Vec<UserAccount>> {
        let col = self.get_json("/redfish/v1/AccountService/Accounts").await?;
    ...
    pub async fn get_boot_config(&self) -> SmcResult<BootConfig> {
        ...
        Ok(BootConfig {
            boot_mode: boot
                ...
                .to_string(),
            boot_order,
            current_boot_source: boot
                .get("BootSourceOverrideTarget")
                .and_then(|v| v.as_str())
                .map(String::from),
            uefi_secure_boot: sys
                .get("SecureBoot")
    ...
    pub async fn set_boot_override(&self, target: &str, mode: Option<&str>) -> SmcResult<()> {
        ...
        if let Some(m) = mode {
            boot.as_object_mut().expect("json! macro creates an Object").insert(
    ...
    pub async fn get_security_status(&self) -> SmcResult<SmcSecurityStatus> {
        let np = self
            .get_json("/redfish/v1/Managers/1/NetworkProtocol")
            .await
    ...
    pub async fn get_console_info(&self) -> SmcResult<SmcConsoleInfo> {
        ...
        let gfx = mgr
            .get("GraphicalConsole")
            .unwrap_or(&serde_json::Value::Null);
        let enabled = gfx
            .get("ServiceEnabled")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let max_sessions = gfx
    ...
    pub async fn get_html5_ikvm_url(&self) -> SmcResult<String> {
        if !self.platform.supports_html5_ikvm() {
            return Err(SmcError::console(
                "HTML5 iKVM not supported on this platform",
            ));
        }
                ...
                if let Some(members) = col.get("Members").and_then(|v| v.as_array()) {
                    for p in members {
                        policies.push(NodeManagerPolicy {
                            policy_id: p.get("PolicyId").and_then(|v| v.as_u64()).unwrap_or(0)
                                as u32,
                            ...
                            power_limit_watts: p
                                ...
                                .and_then(|v| v.as_u64())
                                .unwrap_or(0) as u32,
                            correction_time_ms: p
                                .get("CorrectionTimeMs")
                                .and_then(|v| v.as_u64())
                                .unwrap_or(6000)
                                as u32,
                            trigger_type: json_str(p, "TriggerType")
                                .unwrap_or_else(|| "Always".into()),
                            reporting_period_secs: p
                                .get("ReportingPeriodSecs")
                                .and_then(|v| v.as_u64())
                                .unwrap_or(1)
                                as u32,
                        });
