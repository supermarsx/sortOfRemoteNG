...
const MAX_SHARE_RECIPIENTS: usize = 256;
...
impl PassboltSecrets {
    ...
    pub async fn get(
        ...
        debug!("Fetching Passbolt resource secret");
        let resp: ApiResponse<Secret> = client
            .get(&format!(
                "/secrets/resource/{}.json",
                PassboltApiClient::encode_path_segment(resource_id)?
            ))
            .await?;
        Ok(resp.body)
    }
    ...
    pub fn decrypt(pgp: &PgpContext, secret: &Secret) -> Result<DecryptedSecret, PassboltError> {
        let mut plaintext = pgp.decrypt(&secret.data)?;
        if plaintext.len() > MAX_SECRET_PLAINTEXT_BYTES {
            plaintext.zeroize();
            return Err(PassboltError::crypto(
                "Decrypted Passbolt secret exceeds the configured size limit",
            ));
        }
    ...
    pub fn encrypt_for_user(
        ...
        let mut json = serde_json::to_string(secret)
            .map_err(|e| PassboltError::encryption(format!("Failed to serialize secret: {}", e)))?;
        if json.len() > MAX_SECRET_PLAINTEXT_BYTES {
            json.zeroize();
            return Err(PassboltError::bad_request(
                "Passbolt secret exceeds the configured size limit",
            ));
        }
        let result = pgp.encrypt_for_user(&json, user_id);
        json.zeroize();
        result
    }
    ...
    pub fn encrypt_for_server(
        ...
        Err(PassboltError::crypto(
            "Current-user Passbolt secret encryption is unavailable; refusing to encrypt password data to the server authentication key",
        ))
    }
    ...
    pub fn build_secret_data(
        ...
        let _ = (pgp, password, description, totp);
        Err(PassboltError::crypto(
            "Current-user Passbolt secret encryption is unavailable; refusing to construct plaintext for an unsupported encryption path",
        ))
    }
    ...
    pub fn build_share_secrets(
        ...
        if user_ids.is_empty() || user_ids.len() > MAX_SHARE_RECIPIENTS {
            return Err(PassboltError::bad_request(
                "Passbolt share recipient count is invalid",
            ));
        }
...
-----END PGP PRIVATE KEY BLOCK-----";
    ...
    fn roundtrip_context() -> PgpContext {
        ...
        let fingerprint = crate::passbolt::crypto::parse_armored_key(TEST_PUBLIC_KEY, false)
            .unwrap()
            .fingerprint
            .clone();
        pgp.set_user_key(TEST_PRIVATE_KEY, "").unwrap();
        pgp.set_server_key(TEST_PUBLIC_KEY, &fingerprint).unwrap();
        pgp
    }
