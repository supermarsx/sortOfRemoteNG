...
- **Vendor crates** (`sorng-*-vendor`) are first-party wrappers that group a
  ...
  versions via `Cargo.toml` and group dependency APIs. Cargo already caches
  unchanged dependencies; a re-export wrapper does not prevent ordinary
  downstream recompilation or independently isolate generic code generation. A wrapper
...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------- |
| `sorng-rdp-vendor`         | `ironrdp` 0.14 (+ `ironrdp-blocking` 0.8, `ironrdp-svc` 0.6, `ironrdp-dvc` 0.5, `ironrdp-core` 0.1, `ironrdp-cliprdr[-native]` 0.5), `openh264` 0.9.8             | Keep the IronRDP generic tree behind one first-party, rlib-only dependency boundary. Release builds use the separately staged, hard-linked OpenH264 2.6 shared library; 0.9.8 makes the libloading-only build skip the bundled C++ source, while source-backed decoding remains available only for development. | 2026-08-30 | rdp-team      |
| `sorng-aws-vendor`         | `quick-xml` 0.41 (serialize), `percent-encoding` 2.3, `hmac` 0.12, `sha2` 0.10, `hex` 0.4                                                                         | Rlib-only re-exports for the ordinary `sorng-aws` Rust dependency. No runtime-loading or bundle contract exists for a companion wrapper DLL.                                                                                                                                                                    | 2026-04-20 | cloud-team    |
| `sorng-compression-vendor` | `zstd` 0.13, `flate2` 1.0                                                                                                                                         | Rlib-only compression re-exports used by `sorng-recording`; underlying native dependency features and C linkage remain unchanged. No companion wrapper DLL is loaded or bundled.                                                                                                                                | 2026-04-20 | platform-team |
...
- On 14 September 2026, the unused AWS/compression companion `dylib` outputs
  were removed while retaining their `rlib` APIs, dependency versions and
  features. The AWS version above now matches the existing manifest; this was
  a documentation correction, not an upstream resync. See the
  [build review](../../docs/performance/build-review-2026-09-14.md) for linkage
  evidence and the distinction from real native-runtime libraries.
...
- `npm run stage:opkssh-vendor -- --enable [--release]` or `SORNG_ENABLE_OPKSSH_VENDOR_BUNDLE=1` stages the native library into `src-tauri/crates/sorng-opkssh-vendor/bundle/opkssh/<platform>-<arch>/`. The staging helper also prepends `SORNG_OPKSSH_VENDOR_GO` or the standard Scoop Go install path to the cargo subprocess `PATH` when present so the embedded bridge can build even if the calling shell has a stale Go PATH. Direct helper use without `--enable` stays disabled and scrubs stale staged artifacts.
- `src-tauri/tauri.conf.json` maps `crates/sorng-opkssh-vendor/bundle/opkssh/` into `$RESOURCE/opkssh/`, and production Tauri/Docker builds now run the staging helper with `--enable` before bundling. If the pinned checkout or Go toolchain is unavailable, the staged wrapper remains metadata-only and the app keeps CLI fallback explicit.
