...
pub struct SyslogService {
    hosts: HashMap<String, SyslogHost>,
}

impl SyslogService {
    pub fn new() -> SyslogServiceState {
    ...
    pub fn add_host(&mut self, h: SyslogHost) -> Result<(), SyslogError> {
        if self.hosts.contains_key(&h.id) {
            return Err(SyslogError::Other(format!("Host {} exists", h.id)));
        }
        self.hosts.insert(h.id.clone(), h);
        Ok(())
    }
    pub fn remove_host(&mut self, id: &str) -> Result<SyslogHost, SyslogError> {
        self.hosts
            .remove(id)
            .ok_or_else(|| SyslogError::HostNotFound(id.into()))
    }
    pub fn get_host(&self, id: &str) -> Result<&SyslogHost, SyslogError> {
        self.hosts
            .get(id)
            .ok_or_else(|| SyslogError::HostNotFound(id.into()))
    }
    pub fn list_hosts(&self) -> Vec<&SyslogHost> {
...
mod tests {
    ...
    use chrono::Utc;
    fn th(id: &str) -> SyslogHost {
        SyslogHost {
            id: id.into(),
            name: id.into(),
            ssh: None,
            use_sudo: true,
            backend: SyslogBackend::Rsyslog,
            created_at: Utc::now(),
            updated_at: Utc::now(),
        }
    }
