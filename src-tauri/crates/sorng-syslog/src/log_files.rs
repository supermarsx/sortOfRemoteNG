...
pub async fn list_log_files(host: &SyslogHost) -> Result<Vec<LogFile>, SyslogError> {
    let stdout = client::exec_ok(host, "find", &["/var/log", "-maxdepth", "2", "-type", "f", "-printf", "%p\\t%s\\t%m\\n"]).await?;
    ...
    for line in stdout.lines() {
        ...
        files.push(LogFile {
            ...
            is_compressed: name.ends_with(".gz") || name.ends_with(".xz") || name.ends_with(".bz2") || name.ends_with(".zst"),
        });
    }
    Ok(files)
}
...
pub async fn grep_file(host: &SyslogHost, path: &str, pattern: &str) -> Result<String, SyslogError> {
