...
pub async fn restart(host: &SyslogHost) -> Result<(), SyslogError> {
    client::exec_ok(host, "systemctl", &["restart", "rsyslog"]).await?; Ok(())
}
...
pub fn parse_rsyslog_conf(content: &str) -> Result<RsyslogConfig, SyslogError> {
    ...
    for line in content.lines() {
        ...
        if line.starts_with("module(") || line.starts_with("$ModLoad") {
            ...
            templates.push(RsyslogTemplate { name: String::new(), template_type: String::new(), content: line.to_string() });
    ...
    Ok(RsyslogConfig { version: None, modules, global_directives: global, rules, templates })
}
...
fn parse_rule_line(line: &str) -> Option<RsyslogRule> {
    ...
    let facility = match fac_str {
        "kern" => SyslogFacility::Kern, "user" => SyslogFacility::User,
        "mail" => SyslogFacility::Mail, "daemon" => SyslogFacility::Daemon,
        "auth" => SyslogFacility::Auth, "syslog" => SyslogFacility::Syslog,
        "cron" => SyslogFacility::Cron, "authpriv" => SyslogFacility::Authpriv,
        "*" => SyslogFacility::Any, _ => SyslogFacility::User,
    };
    let severity = match sev_str {
        "emerg" => SyslogSeverity::Emergency, "alert" => SyslogSeverity::Alert,
        "crit" => SyslogSeverity::Critical, "err" | "error" => SyslogSeverity::Error,
        "warning" | "warn" => SyslogSeverity::Warning, "notice" => SyslogSeverity::Notice,
        "info" => SyslogSeverity::Info, "debug" => SyslogSeverity::Debug,
        "*" => SyslogSeverity::Any, _ => SyslogSeverity::Info,
    };

    Some(RsyslogRule { facility, severity, action, template: None, raw_line: line.to_string() })
}
