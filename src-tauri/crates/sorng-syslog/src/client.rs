...
use crate::types::{SshAuth, SshConfig, SyslogHost};
use log::debug;
...
pub async fn exec(
    ...
    program: &str,
    args: &[&str],
) -> Result<(String, String, i32), SyslogError> {
    debug!(
        "syslog: executing {} command",
        if host.ssh.is_some() {
            "remote"
        } else {
            "local"
    ...
    let (stdout, stderr, code) = if let Some(ssh) = &host.ssh {
    ...
    if stderr.contains("Permission denied") {
        return Err(SyslogError::PermissionDenied(program.to_string()));
    }
...
pub async fn exec_ok(
    ...
    if code != 0 {
        return Err(SyslogError::CommandFailed {
...
async fn exec_local(
    ...
    prog: &str,
    args: &[&str],
) -> Result<std::process::Output, SyslogError> {
    Ok(if sudo {
        Command::new("sudo")
            .arg("-n")
            .arg("--")
...
async fn exec_remote(
    ...
    let raw = session
        ...
        .await
        .map_err(|error| SyslogError::SshError(bounded_error(error)))?;
    parse_framed_output(raw, &marker).map_err(SyslogError::SshError)
}

fn validate_ssh_config(ssh: &SshConfig) -> Result<(), SyslogError> {
    if ssh.host.trim().is_empty() || ssh.username.trim().is_empty() || ssh.port == 0 {
        return Err(SyslogError::SshError(
...
fn auth_material(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), SyslogError> {
    match &ssh.auth {
