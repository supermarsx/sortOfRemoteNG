...
mod tests {
    ...
    fn test_facility_codes() {
        assert_eq!(facility_code(&SyslogFacility::Kern), 0);
        assert_eq!(facility_code(&SyslogFacility::Local7), 23);
    }
    #[test]
    fn test_severity_codes() {
        assert_eq!(severity_code(&SyslogSeverity::Emergency), 0);
        assert_eq!(severity_code(&SyslogSeverity::Debug), 7);
    }
    #[test]
    fn test_priority() {
        assert_eq!(
            priority(&SyslogFacility::Auth, &SyslogSeverity::Warning),
            4 * 8 + 4
        );
    }
}
