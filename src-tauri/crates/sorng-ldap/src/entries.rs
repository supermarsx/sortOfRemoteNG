...
pub async fn search(host: &LdapHost, opts: &LdapSearchOpts) -> Result<LdapSearchResult, LdapError> {
    let scope = match opts.scope { LdapScope::Base => "base", LdapScope::One => "one", LdapScope::Sub => "sub" };
    let mut args = vec!["-x", "-H", &host.ldap_uri, "-b", &opts.base_dn, "-s", scope];
    let bind_dn_ref;
    let bind_pw_ref;
    if let Some(ref dn) = host.bind_dn { bind_dn_ref = dn.clone(); args.push("-D"); args.push(&bind_dn_ref); }
    if let Some(ref pw) = host.bind_password { bind_pw_ref = pw.clone(); args.push("-w"); args.push(&bind_pw_ref); }
    args.push(&opts.filter);
    for a in &opts.attributes { args.push(a); }
...
pub async fn add(host: &LdapHost, entry: &LdapEntry) -> Result<(), LdapError> {
    ...
    let cmd = format!("echo '{}' | ldapadd -x -H {} -D '{}' -w '{}'",
        escaped, host.ldap_uri, host.bind_dn.as_deref().unwrap_or(""), host.bind_password.as_deref().unwrap_or(""));
...
pub async fn delete(host: &LdapHost, dn: &str) -> Result<(), LdapError> {
    ...
    let bind_dn_ref; let bind_pw_ref;
    if let Some(ref d) = host.bind_dn { bind_dn_ref = d.clone(); args.push("-D"); args.push(&bind_dn_ref); }
    if let Some(ref p) = host.bind_password { bind_pw_ref = p.clone(); args.push("-w"); args.push(&bind_pw_ref); }
    args.push(dn);
    let refs: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
    client::exec_ok(host, "ldapdelete", &refs).await?; Ok(())
}
...
pub fn parse_ldif_search(output: &str) -> LdapSearchResult {
    ...
    for line in output.lines() {
        if line.starts_with('#') || line.trim().is_empty() {
            if !dn.is_empty() {
                ...
                entries.push(LdapEntry { dn: dn.clone(), object_classes: oc, attributes: attrs.clone() });
                dn.clear(); attrs.clear();
            }
            continue;
        }
        if line.starts_with("dn:") { dn = line[3..].trim().to_string(); }
    ...
    if !dn.is_empty() {
        let oc = attrs.get("objectClass").cloned().unwrap_or_default();
        entries.push(LdapEntry { dn, object_classes: oc, attributes: attrs });
    }
    let total = entries.len() as u32;
    LdapSearchResult { entries, referrals: Vec::new(), total }
}

fn entry_to_ldif(entry: &LdapEntry) -> String {
    let mut lines = vec![format!("dn: {}", entry.dn)];
    for oc in &entry.object_classes { lines.push(format!("objectClass: {oc}")); }
    for (k, vals) in &entry.attributes {
        if k == "objectClass" { continue; }
        for v in vals { lines.push(format!("{k}: {v}")); }
    }
    lines.join("\n")
}
