...
pub async fn list_users(host: &LdapHost) -> Result<Vec<LdapUser>, LdapError> {
    let opts = LdapSearchOpts {
        ...
        scope: LdapScope::Sub,
        filter: "(objectClass=posixAccount)".into(),
        attributes: vec![
            "uid".into(),
            "cn".into(),
            "sn".into(),
...
pub async fn create_user(host: &LdapHost, opts: &CreateLdapUserOpts) -> Result<(), LdapError> {
    ...
    if let Some(ref gn) = opts.given_name {
        attrs.insert("givenName".into(), vec![gn.clone()]);
    }
    if let Some(ref m) = opts.mail {
        attrs.insert("mail".into(), vec![m.clone()]);
    }
    if let Some(un) = opts.uid_number {
        attrs.insert("uidNumber".into(), vec![un.to_string()]);
    }
    if let Some(gn) = opts.gid_number {
        attrs.insert("gidNumber".into(), vec![gn.to_string()]);
    }
    if let Some(ref hd) = opts.home_directory {
        attrs.insert("homeDirectory".into(), vec![hd.clone()]);
    }
    if let Some(ref ls) = opts.login_shell {
        attrs.insert("loginShell".into(), vec![ls.clone()]);
    }
    let entry = LdapEntry {
        dn,
        object_classes: vec![
...
fn entry_to_user(entry: &LdapEntry) -> LdapUser {
    ...
    LdapUser {
        ...
        cn: get("cn"),
        sn: get("sn"),
        given_name: get_opt("givenName"),
        display_name: get_opt("displayName"),
        mail: get_opt("mail"),
        uid_number: get_opt("uidNumber").and_then(|v| v.parse().ok()),
        gid_number: get_opt("gidNumber").and_then(|v| v.parse().ok()),
        home_directory: get_opt("homeDirectory"),
        login_shell: get_opt("loginShell"),
        member_of: entry
            .attributes
            .get("memberOf")
            .cloned()
