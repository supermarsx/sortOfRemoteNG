...
impl AwsService {
    ...
    pub async fn connect_aws(&mut self, config: AwsConnectionConfig) -> Result<String, String> {
        ...
        // Derive signing key
        fn sign(key: &[u8], msg: &str) -> Result<Vec<u8>, String> {
        ...
        let signature = {
            let mut mac = Hmac::<Sha256>::new_from_slice(&k_signing)
                .map_err(|e| format!("HMAC signing error: {}", e))?;
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&amz_date)
                .map_err(|e| format!("Invalid header value for x-amz-date: {}", e))?,
        );
        headers.insert(
            "Authorization",
            HeaderValue::from_str(&authorization_header)
                .map_err(|e| format!("Invalid header value for Authorization: {}", e))?,
        );
        if let Some(token) = &session_token {
            headers.insert(
                "x-amz-security-token",
                HeaderValue::from_str(token)
                    .map_err(|e| format!("Invalid header value for security token: {}", e))?,
            );
        }
        ...
        let text = resp
            .text()
            .await
            .map_err(|e| format!("Failed to read AWS STS response: {}", e))?;
        ...
        let identity = identity
            .ok_or_else(|| format!("Failed to parse AWS identity from response: {}", text))?;
