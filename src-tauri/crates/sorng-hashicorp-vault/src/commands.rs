...
#[tauri::command]
pub async fn vault_disconnect(state: State<'_, VaultServiceState>, id: String) -> CmdResult<()> {
    state.lock().await.disconnect(&id).map_err(map_err)
}

#[tauri::command]
pub async fn vault_list_connections(state: State<'_, VaultServiceState>) -> CmdResult<Vec<String>> {
...
#[tauri::command]
pub async fn vault_seal(state: State<'_, VaultServiceState>, id: String) -> CmdResult<()> {
...
pub async fn vault_kv_undelete(
    ...
    state
        .lock()
        .await
        .kv_undelete(&id, &mount, &path, versions)
        .await
        .map_err(map_err)
}
...
pub async fn vault_kv_destroy(
    ...
    state
        .lock()
        .await
        .kv_destroy(&id, &mount, &path, versions)
        .await
        .map_err(map_err)
}
...
pub async fn vault_transit_create_key(
    ...
    state
        .lock()
        .await
        .transit_create_key(&id, &name, key_type.as_deref())
        .await
        .map_err(map_err)
}
...
pub async fn vault_transit_encrypt(
    ...
    state
        .lock()
        .await
        .transit_encrypt(&id, &name, &plaintext, context.as_deref())
        .await
        .map_err(map_err)
}
...
pub async fn vault_transit_decrypt(
    ...
    state
        .lock()
        .await
        .transit_decrypt(&id, &name, &ciphertext, context.as_deref())
        .await
        .map_err(map_err)
}
...
pub async fn vault_transit_verify(
    ...
    state
        .lock()
        .await
        .transit_verify(&id, &name, &input, &signature)
        .await
        .map_err(map_err)
}
...
pub async fn vault_pki_issue_cert(
    ...
    state
        .lock()
        .await
        .pki_issue_cert(&id, &mount, &role, &params)
        .await
        .map_err(map_err)
}
...
pub async fn vault_pki_revoke_cert(
    ...
    state
        .lock()
        .await
        .pki_revoke_cert(&id, &mount, &serial)
        .await
        .map_err(map_err)
}
...
pub async fn vault_pki_create_role(
    ...
    state
        .lock()
        .await
        .pki_create_role(&id, &mount, &name, &config)
        .await
        .map_err(map_err)
}
...
pub async fn vault_enable_auth(
    ...
    state
        .lock()
        .await
        .enable_auth(&id, &path, &auth_type, config.as_ref())
        .await
        .map_err(map_err)
}
...
pub async fn vault_userpass_create(
    ...
    state
        .lock()
        .await
        .userpass_create(&id, &mount, &username, &password, &policies)
        .await
        .map_err(map_err)
}
...
pub async fn vault_userpass_delete(
    ...
    state
        .lock()
        .await
        .userpass_delete(&id, &mount, &username)
        .await
        .map_err(map_err)
}
...
pub async fn vault_write_policy(
    ...
    state
        .lock()
        .await
        .write_policy(&id, &name, &policy_text)
        .await
        .map_err(map_err)
}
...
pub async fn vault_enable_audit(
    ...
    state
        .lock()
        .await
        .enable_audit(&id, &path, &audit_type, &options)
        .await
        .map_err(map_err)
}
...
pub async fn vault_renew_token(
    ...
    state
        .lock()
        .await
        .renew_token(&id, &token, increment.as_deref())
        .await
        .map_err(map_err)
}
...
pub async fn vault_renew_lease(
    ...
    state
        .lock()
        .await
        .renew_lease(&id, &lease_id, increment.as_deref())
        .await
        .map_err(map_err)
}
...
pub async fn vault_mount_engine(
    ...
    state
        .lock()
        .await
        .mount_secret_engine(&id, &path, &engine_type, config.as_ref())
        .await
        .map_err(map_err)
}
