...
pub async fn exec(
    ...
    if stderr.contains("Permission denied") {
        return Err(ProcError::PermissionDenied(program.to_string()));
    }
...
fn ssh_error(message: impl Into<String>) -> ProcError {
...
fn validate_ssh_identity(ssh: &SshConfig) -> Result<(), ProcError> {
