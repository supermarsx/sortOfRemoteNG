...
pub fn strip_ansi_with_map(input: &str) -> (String, Vec<usize>) {
    ...
    for m in ANSI_RE.find_iter(input) {
        ...
        for (i, _) in visible.char_indices() {
            ...
            let ch =
                &input[orig_pos..orig_pos + input[orig_pos..].chars().next().expect("orig_pos within input bounds").len_utf8()];
            for _ in 0..ch.len() {
    ...
    for (i, _) in visible.char_indices() {
        let orig_pos = last_end + i;
        let ch = &input[orig_pos..orig_pos + input[orig_pos..].chars().next().expect("orig_pos within input bounds").len_utf8()];
        for _ in 0..ch.len() {
            index_map.push(orig_pos);
        }
        clean.push_str(ch);
