...
/// Result of executing a script on the remote server as a proper file.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct ScriptExecutionResult {
    ...
    pub stderr: String,
    pub exit_code: i32,
    /// The temp file path used on the remote server (for diagnostics).
...
mod tests {
    ...
    fn ssh_connection_config_default_fields() {
        ...
        assert!(config.openvpn_config.is_none());
    }

    #[test]
    fn ssh_connection_config_with_openvpn_and_jump_hosts() {
        // Test that openvpn_config and jump_hosts can coexist
        let config = SshConnectionConfig {
            ...
            proxy_chain: None,
            mixed_chain: None,
            openvpn_config: Some(OpenVPNConfig {
                connection_id: "vpn-1".to_string(),
    ...
    #[test]
    fn mixed_chain_config_serialization() {
        let chain = MixedChainConfig {
            hops: vec![
                ChainHop::Proxy(ProxyConfig {
                ...
                ChainHop::SshJump(JumpHostConfig {
                    host: "jump.example.com".to_string(),
                    port: 22,
                    username: "jumpuser".to_string(),
                    password: None,
                    private_key_path: Some("/keys/jump.pem".to_string()),
                    private_key_passphrase: None,
                    agent_forwarding: true,
                    totp_secret: None,
                    keyboard_interactive_responses: vec![],
            ...
            hop_timeout_ms: 15000,
        };

        let json = serde_json::to_string(&chain).unwrap();
        let deserialized: MixedChainConfig = serde_json::from_str(&json).unwrap();

        assert_eq!(deserialized.hops.len(), 2);
        assert_eq!(deserialized.hop_timeout_ms, 15000);

        // Verify first hop is proxy
        let ChainHop::Proxy(p) = &deserialized.hops[0] else {
            unreachable!("Expected Proxy hop");
        };
        assert_eq!(p.host, "proxy.example.com");
        assert_eq!(p.port, 1080);

        // Verify second hop is SSH jump
        let ChainHop::SshJump(j) = &deserialized.hops[1] else {
            unreachable!("Expected SshJump hop");
        };
        assert_eq!(j.host, "jump.example.com");
        assert!(j.agent_forwarding);
    }

    #[test]
    fn chain_hop_address() {
        let proxy_hop = ChainHop::Proxy(ProxyConfig {
        ...
        assert_eq!(proxy_hop.address(), ("proxy.test".to_string(), 3128));

        let ssh_hop = ChainHop::SshJump(JumpHostConfig {
            host: "ssh.test".to_string(),
            port: 2222,
            username: "user".to_string(),
            password: None,
            private_key_path: None,
        ...
        assert_eq!(ssh_hop.address(), ("ssh.test".to_string(), 2222));
    }

    #[test]
    fn chain_hop_label() {
        let proxy = ChainHop::Proxy(ProxyConfig {
            proxy_type: ProxyType::Socks5,
            host: "proxy.test".to_string(),
            port: 1080,
            username: None,
        ...
        assert!(proxy.label().contains("proxy.test:1080"));

        let ssh = ChainHop::SshJump(JumpHostConfig {
        ...
        assert!(ssh.label().contains("admin@jump.test:22"));
    }

    #[test]
    fn proxy_chain_config_serialization() {
        let chain = ProxyChainConfig {
            proxies: vec![
                ProxyConfig {
                    proxy_type: ProxyType::Socks5,
                ...
                ProxyConfig {
                    proxy_type: ProxyType::Http,
                    host: "http-proxy.test".to_string(),
                    port: 3128,
                    username: None,
                    password: None,
                },
    ...
    #[test]
    fn proxy_chain_mode_variants() {
        let strict = ProxyChainMode::Strict;
        let dynamic = ProxyChainMode::Dynamic;
        let random = ProxyChainMode::Random;

        let json_strict = serde_json::to_string(&strict).unwrap();
        let json_dynamic = serde_json::to_string(&dynamic).unwrap();
        let json_random = serde_json::to_string(&random).unwrap();

        assert_ne!(json_strict, json_dynamic);
        assert_ne!(json_dynamic, json_random);

        let parsed: ProxyChainMode = serde_json::from_str(&json_strict).unwrap();
        assert!(matches!(parsed, ProxyChainMode::Strict));
    }

    #[test]
    fn jump_host_config_per_hop_auth() {
        let jump = JumpHostConfig {
            ...
            preferred_host_key_algorithms: vec!["ssh-ed25519".to_string()],
        };
    ...
    #[test]
    fn mixed_chain_status() {
        let status = MixedChainStatus {
            total_hops: 3,
            ssh_jump_count: 2,
            proxy_count: 1,
            hops: vec![
                ChainHopInfo {
                    index: 0,
                    label: "SOCKS5 proxy.test:1080".to_string(),
                    hop_type: "proxy".to_string(),
                    host: "proxy.test".to_string(),
                    port: 1080,
                },
                ChainHopInfo {
                    index: 1,
                    label: "SSH user@jump1.test:22".to_string(),
                    hop_type: "ssh_jump".to_string(),
                    host: "jump1.test".to_string(),
                    port: 22,
                },
            ],
        };

        assert_eq!(status.total_hops, 3);
        assert_eq!(status.ssh_jump_count, 2);
        assert_eq!(status.proxy_count, 1);
        assert_eq!(status.hops.len(), 2);
    }
}
