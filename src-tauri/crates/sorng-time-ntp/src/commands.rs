...
pub async fn time_get_host(
    ...
    state.lock().await.get_host(&host_id).cloned().map_err(map_err)
}

#[tauri::command]
pub async fn time_list_hosts(
    state: State<'_, TimeNtpServiceState>,
) -> CmdResult<Vec<TimeHost>> {
...
pub async fn time_get_status(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::timedatectl::get_time_status(&host).await.map_err(map_err)
}
...
pub async fn time_set_timezone(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::timedatectl::set_timezone(&host, &tz).await.map_err(map_err)
}
...
pub async fn time_list_timezones(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::timedatectl::list_timezones(&host).await.map_err(map_err)
}
...
pub async fn time_set_time(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    let dt = time_str.parse::<chrono::DateTime<chrono::Utc>>()
        .map_err(|e| format!("Invalid datetime: {e}"))?;
    crate::timedatectl::set_time(&host, dt).await.map_err(map_err)
}
...
pub async fn time_set_ntp(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::timedatectl::set_ntp(&host, enabled).await.map_err(map_err)
}
...
pub async fn time_get_chrony_config(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::chrony::get_chrony_config(&host).await.map_err(map_err)
}
...
pub async fn time_chrony_add_server(
    ...
    crate::chrony::add_server(&host, &cfg).await.map_err(map_err)
}
...
pub async fn time_chrony_remove_server(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::chrony::remove_server(&host, &address).await.map_err(map_err)
}
...
pub async fn time_ntpd_remove_server(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::ntpd::remove_server(&host, &address).await.map_err(map_err)
}
...
pub async fn time_sync_hwclock_from_system(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::hwclock::set_hwclock_from_system(&host).await.map_err(map_err)
}
...
pub async fn time_sync_system_from_hwclock(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::hwclock::set_system_from_hwclock(&host).await.map_err(map_err)
}
...
pub async fn time_get_hwclock_drift(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::hwclock::get_hwclock_drift(&host).await.map_err(map_err)
}
...
pub async fn time_detect_ntp(
    ...
    let host = state.lock().await.get_host(&host_id).cloned().map_err(map_err)?;
    crate::detect::detect_ntp_implementation(&host).await.map_err(map_err)
}
