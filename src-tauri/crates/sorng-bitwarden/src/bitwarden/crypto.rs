...
pub fn stretch_key(master_key: &[u8; 32]) -> Result<([u8; 32], [u8; 32]), BitwardenError> {
    use hkdf::Hkdf;
    ...
    hkdf.expand(b"enc", &mut enc_key)
        .map_err(|_| BitwardenError::crypto("HKDF expand failed for encryption key"))?;
    hkdf.expand(b"mac", &mut mac_key)
        .map_err(|_| BitwardenError::crypto("HKDF expand failed for MAC key"))?;
...
pub fn hmac_sha256(key: &[u8], data: &[u8]) -> Result<Vec<u8>, BitwardenError> {
    use hmac::Mac;
    let mut mac = Hmac::<Sha256>::new_from_slice(key)
        .map_err(|_| BitwardenError::crypto("Invalid HMAC key length"))?;
    mac.update(data);
    Ok(mac.finalize().into_bytes().to_vec())
}

/// Verify HMAC-SHA256.
pub fn verify_hmac_sha256(key: &[u8], data: &[u8], expected_mac: &[u8]) -> Result<bool, BitwardenError> {
...
pub fn encrypt_to_cipher_string(
    ...
    Ok(build_cipher_string(&components))
}
...
mod tests {
    ...
    fn stretch_key_produces_different_keys() {
        ...
        let (enc, mac) = stretch_key(&master).unwrap();
        assert_ne!(enc, mac);
    ...
    fn hmac_sha256_different_key() {
        let data = b"test data";
        let mac1 = hmac_sha256(b"key1", data).unwrap();
        let mac2 = hmac_sha256(b"key2", data).unwrap();
        assert_ne!(mac1, mac2);
    }
    ...
    fn verify_hmac_valid() {
        ...
        assert!(verify_hmac_sha256(key, data, &mac).unwrap());
    }
    ...
    fn verify_hmac_invalid() {
        ...
        let data = b"test data";
        let mac = hmac_sha256(key, data).unwrap();
        assert!(!verify_hmac_sha256(key, b"modified", &mac).unwrap());
    }
    ...
    fn encrypt_decrypt_cipher_string_roundtrip() {
        ...
        let (enc_key, mac_key) = stretch_key(&master).unwrap();

        let plaintext = b"This is a secret vault entry!";
        let cipher_string = encrypt_to_cipher_string(plaintext, &enc_key, &mac_key).unwrap();
    ...
    fn decrypt_with_wrong_key_fails() {
        ...
        let (enc1, mac1) = stretch_key(&master1).unwrap();

        let master2 = derive_master_key("pass2", "user@test.com", 1000);
        let (enc2, mac2) = stretch_key(&master2).unwrap();

        let cipher_string = encrypt_to_cipher_string(b"secret", &enc1, &mac1).unwrap();
