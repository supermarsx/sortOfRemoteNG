...
pub async fn ps_get_content_filter_config(
    ...
    let cmd = "Get-ContentFilterConfig | Select-Object Identity,Enabled,\
...
pub async fn ps_get_connection_filter_config(
    ...
    let cmd = "Get-HostedConnectionFilterPolicy -Identity Default | \
         Select-Object Identity,IPAllowList,IPBlockList,EnableSafeList";
    client.run_ps_json(cmd).await.or_else(|_| {
        // on-prem fallback
        Ok(ConnectionFilterConfig::default())
    })
}
...
pub async fn ps_get_sender_filter_config(
    ...
    let cmd = "Get-SenderFilterConfig | Select-Object Identity,Enabled,BlockedSenders,\
