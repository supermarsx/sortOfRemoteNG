...
use crate::types::{KernelHost, SshAuth, SshConfig};
...
pub async fn exec(
    ...
    program: &str,
    args: &[&str],
) -> Result<(String, String, i32), KernelError> {
    ...
    if stderr.contains("Permission denied") {
        return Err(KernelError::PermissionDenied(program.to_string()));
    }

    Ok((stdout, stderr, exit_code))
}

/// Execute and require exit code 0. Returns stdout.
...
/// Execute a shell command, returning (stdout, stderr, exit_code) without
/// failing on non-zero exit codes.
pub async fn exec_shell_raw(
    host: &KernelHost,
    shell_cmd: &str,
) -> Result<(String, String, i32), KernelError> {
...
fn ssh_error(message: impl Into<String>) -> KernelError {
    KernelError::SshError(message.into())
}

fn ssh_credentials(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), KernelError> {
...
fn validate_ssh_identity(ssh: &SshConfig) -> Result<(), KernelError> {
