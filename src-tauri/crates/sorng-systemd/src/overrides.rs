...
pub async fn list_overrides(host: &SystemdHost, unit: &str) -> Result<Vec<UnitOverride>, SystemdError> {
    ...
    for file in stdout.lines() {
        ...
        let content = client::exec_ok(host, "cat", &[&path]).await.unwrap_or_default();
...
pub async fn set_override(host: &SystemdHost, unit: &str, name: &str, content: &str) -> Result<(), SystemdError> {
    let dir = format!("/etc/systemd/system/{unit}.d");
...
pub async fn remove_override(host: &SystemdHost, unit: &str, name: &str) -> Result<(), SystemdError> {
