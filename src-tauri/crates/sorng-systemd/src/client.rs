...
use crate::types::{SshAuth, SshConfig, SystemdHost};
use log::debug;
...
pub async fn exec(
    ...
    if stderr.contains("Permission denied") {
        return Err(SystemdError::PermissionDenied(program.to_string()));
    }
...
fn ssh_error(message: impl Into<String>) -> SystemdError {
    SystemdError::SshError(message.into())
}

fn ssh_credentials(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), SystemdError> {
...
fn validate_ssh_identity(ssh: &SshConfig) -> Result<(), SystemdError> {
