...
impl IloService {
    ...
    pub async fn get_license(&self) -> IloResult<IloLicense> {
        LicenseManager::new(self.require_client()?)
            .get_license()
            .await
    }
    ...
    pub async fn get_security_status(&self) -> IloResult<IloSecurityStatus> {
        SecurityManager::new(self.require_client()?)
            .get_security_status()
            .await
    }
    ...
    pub async fn get_federation_groups(&self) -> IloResult<Vec<IloFederationGroup>> {
        FederationManager::new(self.require_client()?)
            .get_groups()
            .await
    }
