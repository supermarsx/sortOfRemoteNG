...
impl<'a> PowerManager<'a> {
    ...
    pub async fn get_power_state(&self) -> IloResult<String> {
        ...
        if let Ok(ipmi) = self.client.require_ipmi() {
            ...
            return Ok(if status.power_on { "On".to_string() } else { "Off".to_string() });
        }

        Err(IloError::unsupported("No protocol available for power state"))
    }
    ...
    pub async fn power_action(&self, action: PowerAction) -> IloResult<()> {
        ...
        if let Ok(ribcl) = self.client.require_ribcl() {
            match action {
                ...
                PowerAction::Nmi => return Err(IloError::unsupported("NMI not supported via RIBCL")),
        ...
        Err(IloError::unsupported("No protocol available for power action"))
    }
    ...
    pub async fn get_power_metrics(&self) -> IloResult<BmcPowerMetrics> {
        if let Ok(rf) = self.client.require_redfish() {
            ...
            let current_watts = pwr.pointer("/PowerControl/0/PowerConsumedWatts")
                .and_then(|v| v.as_f64());

            let min_watts = pwr.pointer("/PowerControl/0/PowerMetrics/MinConsumedWatts")
                .and_then(|v| v.as_f64());

            let max_watts = pwr.pointer("/PowerControl/0/PowerMetrics/MaxConsumedWatts")
                .and_then(|v| v.as_f64());
        ...
        if let Ok(ribcl) = self.client.require_ribcl() {
            ...
            let current = data.get("PRESENT_POWER_READING")
                .and_then(|v| v.as_str())
        ...
        Err(IloError::unsupported("No protocol available for power metrics"))
    }
}
