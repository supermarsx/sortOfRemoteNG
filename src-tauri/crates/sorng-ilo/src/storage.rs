...
impl<'a> StorageManager<'a> {
    ...
    pub async fn get_controllers(&self) -> IloResult<Vec<BmcStorageController>> {
        ...
        Err(IloError::unsupported("No protocol available for storage controllers"))
    }

    fn parse_smart_storage(&self, data: &serde_json::Value) -> IloResult<Vec<BmcStorageController>> {
        let mut controllers = Vec::new();
    ...
    pub async fn get_virtual_disks(&self) -> IloResult<Vec<BmcVirtualDisk>> {
        ...
        Err(IloError::unsupported("No protocol available for virtual disks"))
    }

    fn parse_smart_logical_drives(&self, data: &serde_json::Value) -> IloResult<Vec<BmcVirtualDisk>> {
        let mut disks = Vec::new();
        if let Some(members) = data.get("Members").and_then(|v| v.as_array()) {
            for ctrl in members {
                if let Some(lds) = ctrl.pointer("/Links/LogicalDrives")
                    .and_then(|v| v.as_array()) {
                    for ld in lds {
                        disks.push(BmcVirtualDisk {
                            id: ld.get("Id").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                            name: ld.get("LogicalDriveName")
    ...
    pub async fn get_physical_disks(&self) -> IloResult<Vec<BmcPhysicalDisk>> {
                    ...
                    if let Some(drv_arr) = s.get("Drives").and_then(|v| v.as_array()) {
                        for d in drv_arr {
                            drives.push(BmcPhysicalDisk {
                                ...
                                name: d.get("Name").and_then(|v| v.as_str()).unwrap_or("Drive").to_string(),
        ...
        Err(IloError::unsupported("No protocol available for physical disks"))
    }

    fn parse_smart_physical_drives(&self, data: &serde_json::Value) -> IloResult<Vec<BmcPhysicalDisk>> {
        let mut drives = Vec::new();
        if let Some(members) = data.get("Members").and_then(|v| v.as_array()) {
            for ctrl in members {
                if let Some(pds) = ctrl.pointer("/Links/PhysicalDrives")
                    .and_then(|v| v.as_array()) {
                    for pd in pds {
                        drives.push(BmcPhysicalDisk {
                            id: pd.get("Id").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                            name: pd.get("Name").and_then(|v| v.as_str()).unwrap_or("Drive").to_string(),
                            manufacturer: pd.get("Manufacturer")
                                .and_then(|v| v.as_str()).map(|s| s.to_string()),
                            model: pd.get("Model")
                                .and_then(|v| v.as_str()).map(|s| s.to_string()),
                            serial_number: pd.get("SerialNumber")
                                .and_then(|v| v.as_str()).map(|s| s.to_string()),
                            capacity_bytes: pd.get("CapacityMiB")
                                .and_then(|v| v.as_u64())
                                .map(|v| v * 1024 * 1024),
