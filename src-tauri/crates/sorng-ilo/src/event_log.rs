...
impl<'a> EventLogManager<'a> {
    ...
    pub async fn get_ilo_event_log(&self) -> IloResult<Vec<BmcEventLogEntry>> {
        ...
        Err(IloError::unsupported("No protocol available for iLO event log"))
    }
    ...
    fn parse_ribcl_log(&self, data: &serde_json::Value, _log_type: &str) -> IloResult<Vec<BmcEventLogEntry>> {
        let mut entries = Vec::new();

        if let Some(arr) = data.as_array() {
            for (i, entry) in arr.iter().enumerate() {
                let timestamp = entry.get("DATE")
                    .and_then(|v| v.as_str())
                    .map(|d| {
                ...
                let severity = entry.get("SEVERITY")
                    .and_then(|v| v.as_str()).unwrap_or("Informational");

                let message = entry.get("DESCRIPTION")
    ...
    pub async fn clear_iml(&self) -> IloResult<()> {
        ...
        Err(IloError::unsupported("No protocol available for clearing IML"))
    }
    ...
    pub async fn clear_ilo_event_log(&self) -> IloResult<()> {
        ...
        Err(IloError::unsupported("No protocol available for clearing iLO event log"))
    }
}
