...
impl RoutingManager {
    ...
    pub async fn get_route(client: &PfsenseClient, id: &str) -> PfsenseResult<StaticRoute> {
        let resp: ApiResponse<StaticRoute> = client
            .api_get(&format!("routing/static_route/{id}"))
            .await?;
        Ok(resp.data)
    }
    ...
    pub async fn update_route(
        ...
        let resp: ApiResponse<StaticRoute> = client
            .api_put(&format!("routing/static_route/{id}"), route)
            .await?;
        Ok(resp.data)
    }
    ...
    pub async fn delete_route(client: &PfsenseClient, id: &str) -> PfsenseResult<()> {
        client
            .api_delete_void(&format!("routing/static_route/{id}"))
            .await
    }

    pub async fn apply_routes(client: &PfsenseClient) -> PfsenseResult<serde_json::Value> {
        client
            .api_post("routing/apply", &serde_json::json!({}))
            .await
    }
    ...
    pub async fn update_gateway(
        ...
        name: &str,
        gw: &Gateway,
    ) -> PfsenseResult<Gateway> {
        let resp: ApiResponse<Gateway> = client
            .api_put(&format!("routing/gateway/{name}"), gw)
            .await?;
        Ok(resp.data)
    }
    ...
    pub async fn delete_gateway(client: &PfsenseClient, name: &str) -> PfsenseResult<()> {
        client
            .api_delete_void(&format!("routing/gateway/{name}"))
            .await
    }
    ...
    pub async fn get_gateway_group(
        client: &PfsenseClient,
        name: &str,
    ) -> PfsenseResult<GatewayGroup> {
        let resp: ApiResponse<GatewayGroup> = client
            .api_get(&format!("routing/gateway/group/{name}"))
            .await?;
        Ok(resp.data)
    }

    pub async fn create_gateway_group(
        client: &PfsenseClient,
        group: &GatewayGroup,
    ) -> PfsenseResult<GatewayGroup> {
        let resp: ApiResponse<GatewayGroup> =
            client.api_post("routing/gateway/group", group).await?;
        Ok(resp.data)
    }

    pub async fn update_gateway_group(
        client: &PfsenseClient,
        name: &str,
        group: &GatewayGroup,
    ) -> PfsenseResult<GatewayGroup> {
        let resp: ApiResponse<GatewayGroup> = client
            .api_put(&format!("routing/gateway/group/{name}"), group)
            .await?;
        Ok(resp.data)
    }
    ...
    pub async fn delete_gateway_group(client: &PfsenseClient, name: &str) -> PfsenseResult<()> {
        client
            .api_delete_void(&format!("routing/gateway/group/{name}"))
            .await
    }
    ...
    pub async fn get_routing_table(
        client: &PfsenseClient,
    ) -> PfsenseResult<Vec<RoutingTableEntry>> {
        let resp: ApiListResponse<RoutingTableEntry> =
            client.api_get("diagnostics/routing_table").await?;
        Ok(resp.data)
    }
}
