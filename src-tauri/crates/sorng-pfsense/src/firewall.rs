...
impl FirewallManager {
    ...
    pub async fn get_rule(client: &PfsenseClient, id: &str) -> PfsenseResult<FirewallRule> {
        let resp: ApiResponse<FirewallRule> =
            client.api_get(&format!("firewall/rule/{id}")).await?;
        Ok(resp.data)
    }
    ...
    pub async fn update_rule(
        ...
        let resp: ApiResponse<FirewallRule> =
            client.api_put(&format!("firewall/rule/{id}"), rule).await?;
        Ok(resp.data)
    }
    ...
    pub async fn apply_rules(client: &PfsenseClient) -> PfsenseResult<serde_json::Value> {
        client
            .api_post("firewall/apply", &serde_json::json!({}))
            .await
    }
    ...
    pub async fn get_alias(client: &PfsenseClient, name: &str) -> PfsenseResult<FirewallAlias> {
        let resp: ApiResponse<FirewallAlias> =
            client.api_get(&format!("firewall/alias/{name}")).await?;
        Ok(resp.data)
    }
    ...
    pub async fn update_alias(
        ...
        let resp: ApiResponse<FirewallAlias> = client
            .api_put(&format!("firewall/alias/{name}"), alias)
            .await?;
        Ok(resp.data)
    }
    ...
    pub async fn delete_alias(client: &PfsenseClient, name: &str) -> PfsenseResult<()> {
        client
            .api_delete_void(&format!("firewall/alias/{name}"))
            .await
    }
    ...
    pub async fn get_state_count(client: &PfsenseClient) -> PfsenseResult<u64> {
        ...
        let count = raw.get("data").and_then(|d| d.as_u64()).unwrap_or(0);
        Ok(count)
    }
