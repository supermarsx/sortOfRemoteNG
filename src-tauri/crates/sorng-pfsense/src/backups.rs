...
impl BackupManager {
    ...
    pub async fn download(client: &PfsenseClient, id: &str) -> PfsenseResult<Vec<u8>> {
        client
            .api_get_bytes(&format!("diagnostics/backup/download/{id}"))
            .await
    }
    ...
    pub async fn restore(
        ...
        let body = serde_json::json!({
            "config": base64::engine::general_purpose::STANDARD.encode(config_data),
    ...
    pub async fn delete(client: &PfsenseClient, id: &str) -> PfsenseResult<()> {
        client
            .api_delete_void(&format!("diagnostics/backup/{id}"))
            .await
    }

    pub async fn get(client: &PfsenseClient, id: &str) -> PfsenseResult<BackupEntry> {
        let resp: ApiResponse<BackupEntry> =
            client.api_get(&format!("diagnostics/backup/{id}")).await?;
        Ok(resp.data)
    }
    ...
    pub async fn download_current(
        ...
        let raw: serde_json::Value = client
            .api_post("diagnostics/backup/download", &body)
            .await?;
        let data_str = raw.get("data").and_then(|v| v.as_str()).unwrap_or("");
