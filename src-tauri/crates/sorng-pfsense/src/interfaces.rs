...
impl InterfaceManager {
    ...
    pub async fn get(client: &PfsenseClient, name: &str) -> PfsenseResult<NetworkInterface> {
        let resp: ApiResponse<NetworkInterface> = client.api_get(&format!("interface/{name}")).await?;
        Ok(resp.data)
    }

    pub async fn create(client: &PfsenseClient, iface: &InterfaceConfig) -> PfsenseResult<NetworkInterface> {
        let resp: ApiResponse<NetworkInterface> = client.api_post("interface", iface).await?;
        Ok(resp.data)
    }

    pub async fn update(client: &PfsenseClient, name: &str, iface: &InterfaceConfig) -> PfsenseResult<NetworkInterface> {
        let resp: ApiResponse<NetworkInterface> = client.api_put(&format!("interface/{name}"), iface).await?;
        Ok(resp.data)
    }
