...
pub async fn exec(
    ...
    if stderr.contains("Permission denied") || stderr.contains("not permitted") {
        return Err(PamError::PermissionDenied(
            "operation denied; verify noninteractive sudo policy".to_string(),
        ));
    }
...
pub async fn file_exists(host: &PamHost, path: &str) -> Result<bool, PamError> {
    let (stdout, _, _) = exec(
        ...
        &[
            "-c",
            &format!("test -f {} && echo yes || echo no", shell_quote(path)),
        ],
    )
    .await?;
    Ok(stdout.trim() == "yes")
...
fn ssh_error(message: impl Into<String>) -> PamError {
    PamError::SshError(message.into())
}

fn ssh_credentials(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), PamError> {
...
fn validate_ssh_identity(ssh: &SshConfig) -> Result<(), PamError> {
