...
pub fn nv12_strided_to_rgba(nv12: &[u8], nv12_stride: usize, width: u32, height: u32) -> Vec<u8> {
    let w = width as usize;
    let h = height as usize;
    let mut rgba = vec![0u8; w * h * 4];
...
pub fn yuv420_planar_to_rgba_inner_into(
    ...
    let u_needed = if uv_h > 0 {
        (uv_h - 1) * u_stride + uv_w
    } else {
        0
    };
    let v_needed = if uv_h > 0 {
        (uv_h - 1) * v_stride + uv_w
    } else {
        0
    };
...
pub fn detected_tier() -> &'static str {
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx512bw") {
            return "AVX-512BW (16px/zmm)";
        }
        if is_x86_feature_detected!("avx2") {
            return "AVX2 (16px/ymm)";
        }
        if is_x86_feature_detected!("sse4.1") {
            return "SSE4.1 (8px, native mullo)";
        }
        if is_x86_feature_detected!("ssse3") {
            return "SSSE3 (8px)";
        }
        if is_x86_feature_detected!("sse2") {
            return "SSE2 (BGRA only)";
        }
    }
    "Scalar"
}
...
pub fn bgra_to_rgba_inplace(data: &mut [u8]) {
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx512bw") {
            unsafe {
                bgra_to_rgba_avx512(data, pixel_count);
            }
            return;
        }
        if is_x86_feature_detected!("avx2") {
            unsafe {
                bgra_to_rgba_avx2(data, pixel_count);
            }
            return;
        }
        if is_x86_feature_detected!("ssse3") {
            unsafe {
                bgra_to_rgba_ssse3(data, pixel_count);
            }
            return;
        }
        if is_x86_feature_detected!("sse2") {
            unsafe {
                bgra_to_rgba_sse2(data, pixel_count);
            }
            return;
        }
    }
...
unsafe fn bgra_to_rgba_sse2(data: &mut [u8], pixel_count: usize) {
    ...
    for i in 0..chunks {
        ...
        let result = _mm_or_si128(
            _mm_or_si128(r_shifted, g),
            _mm_or_si128(b_shifted, alpha_mask),
        );
...
unsafe fn bgra_to_rgba_ssse3(data: &mut [u8], pixel_count: usize) {
    use std::arch::x86_64::*;
    let chunks = pixel_count / 4;
    let shuf = _mm_set_epi8(15, 12, 13, 14, 11, 8, 9, 10, 7, 4, 5, 6, 3, 0, 1, 2);
...
unsafe fn bgra_to_rgba_avx2(data: &mut [u8], pixel_count: usize) {
    ...
    let shuf = _mm256_set_epi8(
        15, 12, 13, 14, 11, 8, 9, 10, 7, 4, 5, 6, 3, 0, 1, 2, 15, 12, 13, 14, 11, 8, 9, 10, 7, 4,
        5, 6, 3, 0, 1, 2,
    );
...
unsafe fn bgra_to_rgba_avx512(data: &mut [u8], pixel_count: usize) {
    ...
    let shuf = _mm512_set_epi8(
        ...
        15, 12, 13, 14, 11, 8, 9, 10, 7, 4, 5, 6, 3, 0, 1, 2, 15, 12, 13, 14, 11, 8, 9, 10, 7, 4, 5,
        6, 3, 0, 1, 2, 15, 12, 13, 14, 11, 8, 9, 10, 7, 4, 5, 6, 3, 0, 1, 2, 15, 12, 13, 14, 11, 8,
        9, 10, 7, 4, 5, 6, 3, 0, 1, 2,
    );
...
fn dispatch_nv12(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx512bw") {
            unsafe {
                simd_impl::nv12_to_rgba_avx512(
                    y_plane, uv_plane, y_stride, uv_stride, width, height, rgba,
                );
            }
            return;
        }
...
fn dispatch_i420(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx512bw") {
            unsafe {
                simd_impl::i420_to_rgba_avx512(
                    y, u, v, y_stride, u_stride, v_stride, width, height, rgba,
                );
            }
            return;
        }
...
mod scalar {
            ...
            while col + 1 < width {
                ...
                for dc in 0..2u32 {
                    ...
                    unsafe {
                        ...
                        *rgba.get_unchecked_mut(d0 + 1) =
                            ((YC * y0 + gu_gv) >> 8).clamp(0, 255) as u8;
                    ...
                    unsafe {
                        ...
                        *rgba.get_unchecked_mut(d1 + 1) =
                            ((YC * y1 + gu_gv) >> 8).clamp(0, 255) as u8;
                ...
                for &row in &[row0, row1] {
                    ...
                    unsafe {
                        ...
                        *rgba.get_unchecked_mut(d + 1) =
                            ((YC * yv + GU * u + GV * v) >> 8).clamp(0, 255) as u8;
                        *rgba.get_unchecked_mut(d + 2) =
                            ((YC * yv + BU * u) >> 8).clamp(0, 255) as u8;
    ...
    pub(super) fn i420_to_rgba_scalar(
        ...
        for rp in 0..row_pairs {
            ...
            if col < width {
                ...
                for &row in &[row0, row1] {
                    ...
                    let d = (row * width + col) * 4;
                    unsafe {
                        *rgba.get_unchecked_mut(d) =
                            ((YC * yv + RV * v_val) >> 8).clamp(0, 255) as u8;
                        *rgba.get_unchecked_mut(d + 1) =
                            ((YC * yv + GU * u_val + GV * v_val) >> 8).clamp(0, 255) as u8;
                        *rgba.get_unchecked_mut(d + 2) =
                            ((YC * yv + BU * u_val) >> 8).clamp(0, 255) as u8;
                        *rgba.get_unchecked_mut(d + 3) = 255;
                    }
                }
            }
        ...
        if !height.is_multiple_of(2) {
            scalar_odd_row_i420(
                y, u_plane, v_plane, y_stride, u_stride, v_stride, width, height, rgba,
            );
        }
    }
...
mod simd_impl {
    ...
    pub(super) unsafe fn nv12_to_rgba_ssse3(
        ...
        for rp in 0..row_pairs {
            ...
            scalar_tail_nv12(
                y_plane, uv_plane, y_stride, uv_stride, width, row0, row1, rp, col_start, rgba,
            );
        }
        if !height.is_multiple_of(2) {
            scalar::scalar_odd_row_nv12(
                y_plane, uv_plane, y_stride, uv_stride, width, height, rgba,
            );
        }
    }
    ...
    pub(super) unsafe fn i420_to_rgba_ssse3(
        ...
        for rp in 0..row_pairs {
            ...
            let col_start = chunks * 8;
            scalar_tail_i420(
                y, u_plane, v_plane, y_stride, u_stride, v_stride, width, row0, row1, rp,
                col_start, rgba,
            );
        }
        if !height.is_multiple_of(2) {
            scalar::scalar_odd_row_i420(
                y, u_plane, v_plane, y_stride, u_stride, v_stride, width, height, rgba,
            );
        }
    }
    ...
    unsafe fn convert_8px_i420(
        ...
        let u_raw = _mm_cvtsi32_si128(std::ptr::read_unaligned(
            u_plane.as_ptr().add(u_off) as *const i32
        ));
        let v_raw = _mm_cvtsi32_si128(std::ptr::read_unaligned(
            v_plane.as_ptr().add(v_off) as *const i32
        ));
    ...
    unsafe fn yuv_to_rgba_row_8px(
        ...
        let g_lo = _mm_srai_epi32(
            _mm_sub_epi32(
                _mm_sub_epi32(mullo32(y_lo, c256), mullo32(u_lo, c88)),
                mullo32(v_lo, c183),
            ),
            8,
        );
        let g_hi = _mm_srai_epi32(
            _mm_sub_epi32(
                _mm_sub_epi32(mullo32(y_hi, c256), mullo32(u_hi, c88)),
                mullo32(v_hi, c183),
            ),
            8,
        );
    ...
    unsafe fn yuv_to_rgba_row_16px_avx2(
        ...
        let r_lo = _mm256_srai_epi32(
            _mm256_add_epi32(
                _mm256_mullo_epi32(y_lo, c256),
                _mm256_mullo_epi32(v_lo, c359),
            ),
            8,
        );
        let r_hi = _mm256_srai_epi32(
            _mm256_add_epi32(
                _mm256_mullo_epi32(y_hi, c256),
                _mm256_mullo_epi32(v_hi, c359),
            ),
            8,
        );
        let g_lo = _mm256_srai_epi32(
            _mm256_sub_epi32(
                _mm256_sub_epi32(
                    _mm256_mullo_epi32(y_lo, c256),
                    _mm256_mullo_epi32(u_lo, c88),
                ),
                _mm256_mullo_epi32(v_lo, c183),
            ),
            8,
        );
        let g_hi = _mm256_srai_epi32(
            _mm256_sub_epi32(
                _mm256_sub_epi32(
                    _mm256_mullo_epi32(y_hi, c256),
                    _mm256_mullo_epi32(u_hi, c88),
                ),
                _mm256_mullo_epi32(v_hi, c183),
            ),
            8,
        );
    ...
    unsafe fn yuv_to_rgba_row_16px_avx512(
        ...
        let r = _mm512_srai_epi32(
            _mm512_add_epi32(
                _mm512_mullo_epi32(y_32, c256),
                _mm512_mullo_epi32(v_32, c359),
            ),
            8,
        );
        let g = _mm512_srai_epi32(
            _mm512_sub_epi32(
                _mm512_sub_epi32(
                    _mm512_mullo_epi32(y_32, c256),
                    _mm512_mullo_epi32(u_32, c88),
                ),
                _mm512_mullo_epi32(v_32, c183),
            ),
            8,
        );
        let b = _mm512_srai_epi32(
            _mm512_add_epi32(
                _mm512_mullo_epi32(y_32, c256),
                _mm512_mullo_epi32(u_32, c454),
            ),
            8,
        );
    ...
    unsafe fn convert_8px_nv12_sse41(
        ...
        let y0 = _mm_cvtepu8_epi16(_mm_loadl_epi64(
            y_plane.as_ptr().add(y0_off) as *const __m128i
        ));
        let y1 = _mm_cvtepu8_epi16(_mm_loadl_epi64(
            y_plane.as_ptr().add(y1_off) as *const __m128i
        ));
    ...
    unsafe fn yuv_to_rgba_row_8px_sse41(
        ...
        let r_lo = _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(y_lo, c256), _mm_mullo_epi32(v_lo, c359)),
            8,
        );
        let r_hi = _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(y_hi, c256), _mm_mullo_epi32(v_hi, c359)),
            8,
        );
        let g_lo = _mm_srai_epi32(
            _mm_sub_epi32(
                _mm_sub_epi32(_mm_mullo_epi32(y_lo, c256), _mm_mullo_epi32(u_lo, c88)),
                _mm_mullo_epi32(v_lo, c183),
            ),
            8,
        );
        let g_hi = _mm_srai_epi32(
            _mm_sub_epi32(
                _mm_sub_epi32(_mm_mullo_epi32(y_hi, c256), _mm_mullo_epi32(u_hi, c88)),
                _mm_mullo_epi32(v_hi, c183),
            ),
            8,
        );
        let b_lo = _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(y_lo, c256), _mm_mullo_epi32(u_lo, c454)),
            8,
        );
        let b_hi = _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(y_hi, c256), _mm_mullo_epi32(u_hi, c454)),
            8,
        );
        ...
        for col in col_start..width {
            ...
            for &row in &[row0, row1] {
                ...
                unsafe {
                    ...
                    *rgba.get_unchecked_mut(d + 1) =
                        ((super::YC * yv + gu_gv) >> 8).clamp(0, 255) as u8;
                    *rgba.get_unchecked_mut(d + 2) =
                        ((super::YC * yv + bu) >> 8).clamp(0, 255) as u8;
                    *rgba.get_unchecked_mut(d + 3) = 255;
                }
            }
        }
...
mod tests {
    ...
    fn bgra_to_rgba_basic() {
        let mut data = vec![0, 128, 255, 200, 50, 100, 150, 0];
