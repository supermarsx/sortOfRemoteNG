...
impl LdapManager {
    ...
    pub async fn list(
        ...
        let resp = match search {
            Some(s) => {
                client
                    .get_with_params("/ldap-servers", &[("search", s)])
                    .await?
            }
    ...
    pub async fn update(
        ...
        let body = serde_json::to_value(req)?;
        let resp = client
            .put(&format!("/ldap-servers/{}", server_id), &body)
            .await?;
        let server: WarpgateLdapServer = serde_json::from_value(resp)?;
        Ok(server)
    }
    ...
    pub async fn delete(client: &WarpgateClient, server_id: &str) -> WarpgateResult<()> {
        client
            .delete(&format!("/ldap-servers/{}", server_id))
            .await?;
        Ok(())
    }
    ...
    pub async fn get_users(
        client: &WarpgateClient,
        server_id: &str,
    ) -> WarpgateResult<Vec<LdapUser>> {
        let resp = client
            .get(&format!("/ldap-servers/{}/users", server_id))
            .await?;
    ...
    pub async fn import_users(
        ...
        let body = serde_json::to_value(req)?;
        let resp = client
            .post(&format!("/ldap-servers/{}/import-users", server_id), &body)
            .await?;
