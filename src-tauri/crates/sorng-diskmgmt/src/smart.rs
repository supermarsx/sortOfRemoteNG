...
fn parse_smartctl(output: &str, device: &str) -> SmartInfo {
    let mut info = SmartInfo {
        device: device.into(), model: None, serial: None, firmware: None,
        passed: true, temperature_c: None, power_on_hours: None,
        reallocated_sectors: None, pending_sectors: None, offline_uncorrectable: None,
        attributes: Vec::new(),
    };
    ...
    for line in output.lines() {
        ...
        if line.starts_with("Device Model:") || line.starts_with("Model Number:") {
        ...
        } else if line.contains("PASSED") { info.passed = true; }
        else if line.contains("FAILED") { info.passed = false; }
        else if line.starts_with("ID#") { in_attrs = true; continue; }
            ...
            if let Some(attr) = parse_smart_attr(line) {
                match attr.id {
                    ...
                    194 | 190 => info.temperature_c = Some(attr.raw.trim().split_whitespace().next().and_then(|v| v.parse().ok()).unwrap_or(0)),
...
fn parse_smart_attr(line: &str) -> Option<SmartAttribute> {
    ...
    Some(SmartAttribute {
        id: cols[0].parse().ok()?, name: cols[1].into(),
        value: cols[3].parse().unwrap_or(0), worst: cols[4].parse().unwrap_or(0),
        threshold: cols[5].parse().unwrap_or(0), raw: cols[9..].join(" "),
