...
pub async fn disk_usage(host: &DiskHost) -> Result<Vec<DiskUsage>, DiskError> {
    let stdout = client::exec_ok(
        host,
        "df",
        &["-B1", "--output=target,size,used,avail,pcent"],
    )
    .await?;
    Ok(stdout
        .lines()
        ...
        .filter_map(|line| {
            ...
            Some(DiskUsage {
                path: cols[0].into(),
                total_bytes: cols[1].parse().unwrap_or(0),
                used_bytes: cols[2].parse().unwrap_or(0),
                avail_bytes: cols[3].parse().unwrap_or(0),
...
pub async fn dir_size(host: &DiskHost, path: &str) -> Result<DirectorySize, DiskError> {
    ...
    Ok(DirectorySize {
        path: path.into(),
        size_bytes: bytes,
        size_human: crate::blocks::humanize_bytes(bytes),
    })
}
