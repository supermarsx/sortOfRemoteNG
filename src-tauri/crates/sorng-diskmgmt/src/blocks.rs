...
pub async fn list_block_devices(host: &DiskHost) -> Result<Vec<BlockDevice>, DiskError> {
    let stdout = client::exec_ok(
        ...
        &[
            ...
            "-P",
            "-o",
            "NAME,TYPE,SIZE,MODEL,SERIAL,VENDOR,TRAN,RO,RM,HOTPLUG,STATE",
        ],
    )
    .await?;
...
fn parse_lsblk_line(line: &str) -> Option<BlockDevice> {
    ...
    let dtype = match get("TYPE").as_str() {
        ...
        "part" => BlockDeviceType::Part,
        "lvm" => BlockDeviceType::Lvm,
        "raid0" | "raid1" | "raid5" | "raid6" | "raid10" => BlockDeviceType::Raid,
        "loop" => BlockDeviceType::Loop,
        "crypt" => BlockDeviceType::Crypt,
        "rom" => BlockDeviceType::Rom,
    ...
    Some(BlockDevice {
        ...
        ro: get("RO") == "1",
        rm: get("RM") == "1",
        hotplug: get("HOTPLUG") == "1",
        state: Some(get("STATE")).filter(|s| !s.is_empty()),
        children: Vec::new(),
    })
}
