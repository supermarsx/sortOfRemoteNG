...
use crate::types::{DiskHost, SshAuth, SshConfig};
use log::debug;
...
pub async fn exec(
    ...
    program: &str,
    args: &[&str],
) -> Result<(String, String, i32), DiskError> {
    debug!(
        "disk: executing {} command",
        if host.ssh.is_some() {
            "remote"
        } else {
            "local"
...
pub async fn read_file(host: &DiskHost, path: &str) -> Result<String, DiskError> {
...
async fn exec_remote(
    ...
    let raw = session
        ...
        .await
        .map_err(|error| DiskError::SshError(bounded_error(error)))?;
    parse_framed_output(raw, &marker).map_err(DiskError::SshError)
}

fn validate_ssh_config(ssh: &SshConfig) -> Result<(), DiskError> {
    if ssh.host.trim().is_empty() || ssh.username.trim().is_empty() || ssh.port == 0 {
        return Err(DiskError::SshError(
...
fn auth_material(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), DiskError> {
    match &ssh.auth {
