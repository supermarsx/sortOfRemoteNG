...
mod tests {
    ...
    fn policy_value_string_serde() {
        ...
        let PolicyValue::String(s) = back else {
    ...
    fn policy_value_dword_serde() {
        ...
        let json = serde_json::to_string(&v).unwrap();
        let back: PolicyValue = serde_json::from_str(&json).unwrap();
        let PolicyValue::Dword(n) = back else {
            unreachable!("Expected Dword variant")
        };
        assert_eq!(n, 42);
    }
    ...
    fn policy_value_qword_serde() {
        ...
        let json = serde_json::to_string(&v).unwrap();
        let back: PolicyValue = serde_json::from_str(&json).unwrap();
        let PolicyValue::Qword(n) = back else {
            unreachable!("Expected Qword variant")
        };
        assert_eq!(n, u64::MAX);
    }
    ...
    fn policy_value_binary_serde() {
        ...
        let json = serde_json::to_string(&v).unwrap();
        let back: PolicyValue = serde_json::from_str(&json).unwrap();
        let PolicyValue::Binary(b) = back else {
            unreachable!("Expected Binary variant")
        };
