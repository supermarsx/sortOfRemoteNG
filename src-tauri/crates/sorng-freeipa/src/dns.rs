...
impl DnsManager {
    ...
    pub async fn create_zone(
        ...
        let result = client
            .rpc::<DnsZone>("dnszone_add", vec![serde_json::json!(req.idnsname)], serde_json::Value::Object(map))
            .await?;
        Ok(result.result)
    }
    ...
    pub async fn add_record(
        ...
        if let Some(obj) = opts.as_object_mut() {
            obj.insert(attr_key, serde_json::json!(req.record_data));
        }
        if let Some(ttl) = req.ttl {
            if let Some(obj) = opts.as_object_mut() {
                obj.insert("dnsttl".into(), serde_json::json!(ttl));
            }
        }
        let result = client
            .rpc::<DnsRecord>(
