...
impl RdsClient {
    ...
    pub async fn describe_db_instances(
        ...
        marker: Option<&str>,
    ) -> AwsResult<(Vec<DBInstance>, Option<String>)> {
    ...
    pub async fn create_db_instance(&self, input: &CreateDBInstanceInput) -> AwsResult<DBInstance> {
        ...
        params.insert(
            ...
            input.db_instance_identifier.clone(),
        );
        params.insert(
            "DBInstanceClass".to_string(),
            input.db_instance_class.clone(),
        );
        params.insert("Engine".to_string(), input.engine.clone());
        if let Some(ref v) = input.engine_version {
            params.insert("EngineVersion".to_string(), v.clone());
        }
        if let Some(ref u) = input.master_username {
            params.insert("MasterUsername".to_string(), u.clone());
        }
        if let Some(ref p) = input.master_user_password {
            params.insert("MasterUserPassword".to_string(), p.clone());
        }
        if let Some(ref n) = input.db_name {
            params.insert("DBName".to_string(), n.clone());
        }
        if let Some(s) = input.allocated_storage {
            params.insert("AllocatedStorage".to_string(), s.to_string());
        }
        if let Some(ref az) = input.availability_zone {
            params.insert("AvailabilityZone".to_string(), az.clone());
        }
        if let Some(m) = input.multi_az {
            params.insert("MultiAZ".to_string(), m.to_string());
        }
        if let Some(p) = input.publicly_accessible {
            params.insert("PubliclyAccessible".to_string(), p.to_string());
        }
        if let Some(ref st) = input.storage_type {
            params.insert("StorageType".to_string(), st.clone());
        }
        if let Some(e) = input.storage_encrypted {
            params.insert("StorageEncrypted".to_string(), e.to_string());
        }
        if let Some(ref k) = input.kms_key_id {
            params.insert("KmsKeyId".to_string(), k.clone());
        }
        if let Some(b) = input.backup_retention_period {
            params.insert("BackupRetentionPeriod".to_string(), b.to_string());
        }
        if let Some(ref sg) = input.db_subnet_group_name {
            params.insert("DBSubnetGroupName".to_string(), sg.clone());
        }
        for (i, sg_id) in input.vpc_security_group_ids.iter().enumerate() {
            params.insert(
                format!("VpcSecurityGroupIds.member.{}", i + 1),
                sg_id.clone(),
            );
        }
        if let Some(a) = input.auto_minor_version_upgrade {
            params.insert("AutoMinorVersionUpgrade".to_string(), a.to_string());
        }
        if let Some(iops) = input.iops {
            params.insert("Iops".to_string(), iops.to_string());
        }
        if let Some(d) = input.deletion_protection {
            params.insert("DeletionProtection".to_string(), d.to_string());
        }
    ...
    pub async fn delete_db_instance(
        ...
        params.insert(
            ...
            skip_final_snapshot.to_string(),
        );
        if !skip_final_snapshot {
            params.insert(
                "FinalDBSnapshotIdentifier".to_string(),
                format!(
                    "{}-final-{}",
                    db_instance_id,
        ...
        Ok(blocks
            ...
            .filter_map(|b| {
                Some(DBParameterGroup {
                    ...
                    db_parameter_group_family: client::xml_text(b, "DBParameterGroupFamily")
                        .unwrap_or_default(),
        ...
        Ok(blocks
            ...
            .filter_map(|b| {
                Some(DBSubnetGroup {
                    ...
                    db_subnet_group_description: client::xml_text(b, "DBSubnetGroupDescription")
                        .unwrap_or_default(),
                    vpc_id: client::xml_text(b, "VpcId").unwrap_or_default(),
                    subnet_group_status: client::xml_text(b, "SubnetGroupStatus")
                        .unwrap_or_default(),
                    subnets: client::xml_blocks(b, "Subnet")
                        .iter()
                        .filter_map(|s| {
                            Some(Subnet {
                                subnet_identifier: client::xml_text(s, "SubnetIdentifier")?,
                                subnet_availability_zone: client::xml_text(s, "AvailabilityZone"),
                                subnet_status: client::xml_text(s, "SubnetStatus")
                                    .unwrap_or_default(),
                            })
                        })
                        .collect(),
        ...
        blocks
            ...
            .filter_map(|b| {
                ...
                Some(DBInstance {
                    ...
                    endpoint,
                    allocated_storage: client::xml_text(b, "AllocatedStorage")
                        .and_then(|v| v.parse().ok())
                        .unwrap_or(0),
                    instance_create_time: client::xml_text(b, "InstanceCreateTime"),
                    preferred_backup_window: client::xml_text(b, "PreferredBackupWindow"),
                    backup_retention_period: client::xml_text(b, "BackupRetentionPeriod")
                        .and_then(|v| v.parse().ok())
                        .unwrap_or(0),
                    availability_zone: client::xml_text(b, "AvailabilityZone"),
                    multi_az: client::xml_text(b, "MultiAZ")
                        .map(|v| v == "true")
                        .unwrap_or(false),
                    publicly_accessible: client::xml_text(b, "PubliclyAccessible")
                        .map(|v| v == "true")
                        .unwrap_or(false),
                    storage_type: client::xml_text(b, "StorageType"),
                    storage_encrypted: client::xml_text(b, "StorageEncrypted")
                        .map(|v| v == "true")
                        .unwrap_or(false),
                    kms_key_id: client::xml_text(b, "KmsKeyId"),
                    db_cluster_identifier: client::xml_text(b, "DBClusterIdentifier"),
                    db_subnet_group_name: client::xml_text(b, "DBSubnetGroupName"),
                    vpc_security_groups: client::xml_blocks(b, "VpcSecurityGroupMembership")
                        .iter()
                        .filter_map(|sg| {
                        ...
                        .collect(),
                    auto_minor_version_upgrade: client::xml_text(b, "AutoMinorVersionUpgrade")
                        .map(|v| v == "true")
                        .unwrap_or(true),
                    iops: client::xml_text(b, "Iops").and_then(|v| v.parse().ok()),
                    max_allocated_storage: client::xml_text(b, "MaxAllocatedStorage")
                        .and_then(|v| v.parse().ok()),
                    deletion_protection: client::xml_text(b, "DeletionProtection")
                        .map(|v| v == "true")
                        .unwrap_or(false),
                    tags: vec![],
                })
        ...
        blocks
            ...
            .filter_map(|b| {
                Some(DBCluster {
                    ...
                    db_cluster_members: client::xml_blocks(b, "DBClusterMember")
                        .iter()
                        .filter_map(|m| {
                            Some(DBClusterMember {
                                db_instance_identifier: client::xml_text(
                                    m,
                                    "DBInstanceIdentifier",
                                )?,
                                is_cluster_writer: client::xml_text(m, "IsClusterWriter")
                                    .map(|v| v == "true")
                                    .unwrap_or(false),
                                db_cluster_parameter_group_status: client::xml_text(
                                    m,
        ...
        blocks
            ...
            .filter_map(|b| {
                Some(DBSnapshot {
                    ...
                    db_instance_identifier: client::xml_text(b, "DBInstanceIdentifier")
                        .unwrap_or_default(),
                    ...
                    master_username: client::xml_text(b, "MasterUsername"),
                    engine_version: client::xml_text(b, "EngineVersion"),
                    encrypted: client::xml_text(b, "Encrypted")
                        .map(|v| v == "true")
                        .unwrap_or(false),
                })
            })
    ...
    fn db_instance_serde() {
        let inst = DBInstance {
            ...
            endpoint: Some(Endpoint {
                address: Some("mydb.abc.us-east-1.rds.amazonaws.com".to_string()),
                port: 3306,
                hosted_zone_id: None,
            }),
            ...
            vpc_security_groups: vec![VpcSecurityGroupMembership {
                vpc_security_group_id: "sg-123".to_string(),
