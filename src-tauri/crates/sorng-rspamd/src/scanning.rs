...
impl ScanManager {
    ...
    pub async fn check_message(client: &RspamdClient, message: &str) -> RspamdResult<RspamdScanResult> {
    ...
    pub async fn check_file(client: &RspamdClient, path: &str) -> RspamdResult<RspamdScanResult> {
        ...
        let content = tokio::fs::read_to_string(path).await
            .map_err(|e| RspamdError::new(crate::error::RspamdErrorKind::ProcessError, format!("read file {path}: {e}")))?;
        let raw: serde_json::Value = client.post_body("/checkv2", &content).await?;
        Self::parse_scan_result(&raw)
    }

    /// POST /learnspam — train Bayes classifier with spam message
    pub async fn learn_spam(client: &RspamdClient, message: &str) -> RspamdResult<RspamdBayesLearnResult> {
    ...
    pub async fn learn_ham(client: &RspamdClient, message: &str) -> RspamdResult<RspamdBayesLearnResult> {
    ...
    pub async fn fuzzy_add(client: &RspamdClient, message: &str, flag: u32, weight: f64) -> RspamdResult<()> {
        debug!("RSPAMD fuzzy_add flag={flag} weight={weight}");
        let url = format!("/fuzzyadd");
        let resp_text = client.post_raw(&url, message).await
            .map_err(|e| {
                // Add flag & weight headers via raw post; fall back to query params
                e
            })?;
    ...
    fn parse_scan_result(raw: &serde_json::Value) -> RspamdResult<RspamdScanResult> {
        ...
        let score = raw.get("score")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.0);
        let required_score = raw.get("required_score")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.0);
        let is_skipped = raw.get("is_skipped")
        ...
        let message_id = raw.get("message-id")
            .or_else(|| raw.get("message_id"))
            .and_then(|v| v.as_str())
            .map(String::from);
        let subject = raw.get("subject")
            .and_then(|v| v.as_str())
            .map(String::from);
        ...
        // Parse urls
        let urls = raw.get("urls")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
            .unwrap_or_default();

        // Parse emails
        let emails = raw.get("emails")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
            .unwrap_or_default();
