...
impl FuzzyManager {
    ...
    pub async fn status(client: &RspamdClient) -> RspamdResult<Vec<RspamdFuzzyStatus>> {
        ...
        let raw: serde_json::Value = client.get("/plugins/fuzzy/status").await
            .or_else(|_| -> RspamdResult<serde_json::Value> {
    ...
    pub async fn check(client: &RspamdClient, message: &str) -> RspamdResult<Vec<RspamdSymbolResult>> {
        ...
        if !status.is_success() {
            return Err(RspamdError::api(format!("HTTP {}: {body_text}", status.as_u16())));
        }
    ...
    async fn parse_fuzzy_status(raw: &serde_json::Value, client: &RspamdClient) -> RspamdResult<Vec<RspamdFuzzyStatus>> {
