...
impl SymbolManager {
    ...
    fn parse_symbols(raw: &serde_json::Value) -> RspamdResult<Vec<RspamdSymbol>> {
        ...
        if let Some(groups) = raw.as_array() {
            for group in groups {
                let group_name = group.get("group")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string();
                if let Some(syms) = group.get("rules").and_then(|v| v.as_array()) {
                    for sym in syms {
                        symbols.push(RspamdSymbol {
                            ...
                            description: sym.get("description").and_then(|v| v.as_str()).map(String::from),
    ...
    fn parse_groups(raw: &serde_json::Value) -> RspamdResult<Vec<RspamdSymbolGroup>> {
        ...
        if let Some(arr) = raw.as_array() {
            for group in arr {
                ...
                let enabled = group.get("enabled").and_then(|v| v.as_bool()).unwrap_or(true);

                let symbols = group.get("rules")
