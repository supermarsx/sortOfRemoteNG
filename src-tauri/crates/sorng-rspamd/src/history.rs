...
impl HistoryManager {
    ...
    pub async fn get(client: &RspamdClient, limit: Option<u64>, offset: Option<u64>) -> RspamdResult<RspamdHistory> {
    ...
    pub async fn get_by_id(client: &RspamdClient, entry_id: &str) -> RspamdResult<RspamdHistoryEntry> {
    ...
    fn parse_history(raw: &serde_json::Value) -> RspamdResult<RspamdHistory> {
        let rows_val = raw.get("rows")
            .or(Some(raw));

        let rows = match rows_val {
            Some(serde_json::Value::Array(arr)) => {
                arr.iter().map(Self::parse_entry).collect()
            }
            _ => Vec::new(),
        };
    ...
    fn parse_entry(item: &serde_json::Value) -> RspamdHistoryEntry {
        ...
        RspamdHistoryEntry {
            ...
            timestamp: item.get("time").or_else(|| item.get("timestamp")).and_then(|v| v.as_f64()),
            ip: item.get("ip").and_then(|v| v.as_str()).map(String::from),
            action: item.get("action").and_then(|v| v.as_str()).map(String::from),
            ...
            scan_time_ms: item.get("scan_time")
