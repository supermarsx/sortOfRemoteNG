...
impl WolService {
    ...
    pub async fn wake_on_lan(
        ...
        task::spawn_blocking(move || {
            ...
            let mac_bytes = (0..6)
                .map(|i| {
                    u8::from_str_radix(&mac[i * 2..i * 2 + 2], 16)
            ...
            if let Some(pwd) = password {
                ...
                let pwd_bytes = (0..6)
                    .map(|i| {
                        u8::from_str_radix(&pwd_clean[i * 2..i * 2 + 2], 16)
                            .map_err(|_| "Invalid SecureOn password".to_string())
                    })
                    .collect::<Result<Vec<_>, _>>()?;
            ...
            socket
                .send_to(&packet, format!("{}:{}", broadcast, wol_port))
