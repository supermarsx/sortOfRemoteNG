...
mod tests {
    ...
    fn test_to_sp_parity() {
        match to_sp_parity(&Parity::None) {
            serialport::Parity::None => {}
            _ => panic!("Expected None parity"),
        }
        match to_sp_parity(&Parity::Odd) {
            serialport::Parity::Odd => {}
            _ => panic!("Expected Odd parity"),
        }
        match to_sp_parity(&Parity::Even) {
            serialport::Parity::Even => {}
            _ => panic!("Expected Even parity"),
        }
    }

    #[test]
    fn test_to_sp_stop_bits() {
        match to_sp_stop_bits(&StopBits::One) {
            serialport::StopBits::One => {}
            _ => panic!("Expected One stop bit"),
        }
        match to_sp_stop_bits(&StopBits::Two) {
            serialport::StopBits::Two => {}
            _ => panic!("Expected Two stop bits"),
        }
    }

    #[test]
    fn test_to_sp_flow_control() {
        match to_sp_flow_control(&FlowControl::None) {
            serialport::FlowControl::None => {}
            _ => panic!("Expected None flow control"),
        }
        match to_sp_flow_control(&FlowControl::XonXoff) {
            serialport::FlowControl::Software => {}
            _ => panic!("Expected Software flow control"),
        }
        match to_sp_flow_control(&FlowControl::RtsCts) {
            serialport::FlowControl::Hardware => {}
            _ => panic!("Expected Hardware flow control"),
        }
    }
}
