...
impl FrameCompositor for SoftbufferCompositor {
    fn update_region(&mut self, image_data: &[u8], fb_width: u16, x: u16, y: u16, w: u16, h: u16) {
        ...
        // Expand bounding dirty rect (clamped to desktop dimensions)
        self.dirty_left = self.dirty_left.min(x);
        self.dirty_top = self.dirty_top.min(y);
        self.dirty_right = self.dirty_right.max(x.saturating_add(w)).min(self.desk_w);
        self.dirty_bottom = self.dirty_bottom.max(y.saturating_add(h)).min(self.desk_h);
        self.dirty = true;
    }
