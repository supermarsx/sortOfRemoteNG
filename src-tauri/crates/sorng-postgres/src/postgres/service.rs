...
impl PostgresService {
    ...
    /// Validate a SQL identifier (database, schema, table name).
    fn validate_sql_identifier(name: &str) -> Result<(), PgError> {
    ...
    pub async fn create_database(
        ...
        Self::validate_sql_identifier(name)?;
        if let Some(o) = owner {
            Self::validate_sql_identifier(o)?;
        }
        let pool = self.get_pool(id)?.clone();
    ...
    pub async fn show_settings(
        ...
        let pool = self.get_pool(id)?.clone();
        let (sql, bind_val) = match filter {
            Some(f) => {
                let pattern = format!("%{}%", f);
                ("SELECT name, setting, unit, category, short_desc AS description FROM pg_settings WHERE name ILIKE $1 ORDER BY name".to_string(), Some(pattern))
            }
            None => ("SELECT name, setting, unit, category, short_desc AS description FROM pg_settings ORDER BY name".to_string(), None),
        };
        let query = sqlx::query(&sql);
        let query = if let Some(ref val) = bind_val {
            query.bind(val)
        } else {
            query
        };
