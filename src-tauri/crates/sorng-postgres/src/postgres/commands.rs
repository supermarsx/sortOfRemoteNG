//! Tauri commands for the PostgreSQL integration.

use crate::postgres::service::PostgresServiceState;
use crate::postgres::types::*;

// ── Connection ──────────────────────────────────────────────────────

#[tauri::command]
pub async fn pg_connect(
    state: tauri::State<'_, PostgresServiceState>,
    config: PgConnectionConfig,
) -> Result<String, String> {
    let mut svc = state.lock().await;
    svc.connect(config).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_disconnect(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.disconnect(&session_id).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_disconnect_all(
    state: tauri::State<'_, PostgresServiceState>,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.disconnect_all().await;
    Ok(())
}

#[tauri::command]
pub async fn pg_list_sessions(
    state: tauri::State<'_, PostgresServiceState>,
) -> Result<Vec<SessionInfo>, String> {
    let svc = state.lock().await;
    Ok(svc.list_sessions())
}

#[tauri::command]
pub async fn pg_get_session(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<SessionInfo, String> {
    let svc = state.lock().await;
    svc.get_session(&session_id).map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_ping(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<bool, String> {
    let svc = state.lock().await;
    svc.ping(&session_id).map_err(|e| e.message)
}

// ── Query execution ─────────────────────────────────────────────────

#[tauri::command]
pub async fn pg_execute_query(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    sql: String,
) -> Result<QueryResult, String> {
    let mut svc = state.lock().await;
    svc.execute_query(&session_id, &sql).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_execute_statement(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    sql: String,
) -> Result<QueryResult, String> {
    let mut svc = state.lock().await;
    svc.execute_statement(&session_id, &sql).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_explain_query(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    sql: String,
) -> Result<Vec<ExplainNode>, String> {
    let mut svc = state.lock().await;
    svc.explain_query(&session_id, &sql).await.map_err(|e| e.message)
}

// ── Schema ──────────────────────────────────────────────────────────

#[tauri::command]
pub async fn pg_list_databases(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<Vec<DatabaseInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_databases(&session_id).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_schemas(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<Vec<SchemaInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_schemas(&session_id).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_tables(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
) -> Result<Vec<TableInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_tables(&session_id, &schema).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_describe_table(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
) -> Result<Vec<ColumnDef>, String> {
    let mut svc = state.lock().await;
    svc.describe_table(&session_id, &schema, &table).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_indexes(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
) -> Result<Vec<IndexInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_indexes(&session_id, &schema, &table).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_foreign_keys(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
) -> Result<Vec<ForeignKeyInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_foreign_keys(&session_id, &schema, &table).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_views(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
) -> Result<Vec<ViewInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_views(&session_id, &schema).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_routines(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
) -> Result<Vec<RoutineInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_routines(&session_id, &schema).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_triggers(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
) -> Result<Vec<TriggerInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_triggers(&session_id, &schema).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_sequences(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
) -> Result<Vec<SequenceInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_sequences(&session_id, &schema).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_extensions(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<Vec<ExtensionInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_extensions(&session_id).await.map_err(|e| e.message)
}

// ── DDL ─────────────────────────────────────────────────────────────

#[tauri::command]
pub async fn pg_create_database(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    name: String,
    owner: Option<String>,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.create_database(&session_id, &name, owner.as_deref()).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_drop_database(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    name: String,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.drop_database(&session_id, &name).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_create_schema(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    name: String,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.create_schema(&session_id, &name).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_drop_schema(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    name: String,
    cascade: bool,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.drop_schema(&session_id, &name, cascade).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_drop_table(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.drop_table(&session_id, &schema, &table).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_truncate_table(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.truncate_table(&session_id, &schema, &table).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_vacuum_table(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
    analyze: bool,
) -> Result<(), String> {
    let mut svc = state.lock().await;
    svc.vacuum_table(&session_id, &schema, &table, analyze).await.map_err(|e| e.message)
}

// ── Data CRUD ───────────────────────────────────────────────────────

#[tauri::command]
pub async fn pg_get_table_data(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
    limit: Option<u32>,
    offset: Option<u32>,
) -> Result<QueryResult, String> {
    let mut svc = state.lock().await;
    svc.get_table_data(&session_id, &schema, &table, limit, offset).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_insert_row(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
    columns: Vec<String>,
    values: Vec<String>,
) -> Result<u64, String> {
    let mut svc = state.lock().await;
    svc.insert_row(&session_id, &schema, &table, &columns, &values).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_update_rows(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
    columns: Vec<String>,
    values: Vec<String>,
    where_clause: String,
) -> Result<u64, String> {
    let mut svc = state.lock().await;
    svc.update_rows(&session_id, &schema, &table, &columns, &values, &where_clause).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_delete_rows(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
    where_clause: String,
) -> Result<u64, String> {
    let mut svc = state.lock().await;
    svc.delete_rows(&session_id, &schema, &table, &where_clause).await.map_err(|e| e.message)
}

// ── Export / Import ─────────────────────────────────────────────────

#[tauri::command]
pub async fn pg_export_table(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
    options: ExportOptions,
) -> Result<String, String> {
    let mut svc = state.lock().await;
    svc.export_table(&session_id, &schema, &table, &options).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_export_schema(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    options: ExportOptions,
) -> Result<String, String> {
    let mut svc = state.lock().await;
    svc.export_schema(&session_id, &schema, &options).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_import_sql(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    sql_content: String,
) -> Result<u64, String> {
    let mut svc = state.lock().await;
    svc.import_sql(&session_id, &sql_content).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_import_csv(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    schema: String,
    table: String,
    csv_content: String,
    has_header: bool,
) -> Result<u64, String> {
    let mut svc = state.lock().await;
    svc.import_csv(&session_id, &schema, &table, &csv_content, has_header).await.map_err(|e| e.message)
}

// ── Administration ──────────────────────────────────────────────────

#[tauri::command]
pub async fn pg_show_settings(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    filter: Option<String>,
) -> Result<Vec<ServerSetting>, String> {
    let mut svc = state.lock().await;
    svc.show_settings(&session_id, filter.as_deref()).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_show_activity(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<Vec<PgStatActivity>, String> {
    let mut svc = state.lock().await;
    svc.show_activity(&session_id).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_terminate_backend(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    pid: i32,
) -> Result<bool, String> {
    let mut svc = state.lock().await;
    svc.terminate_backend(&session_id, pid).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_cancel_backend(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    pid: i32,
) -> Result<bool, String> {
    let mut svc = state.lock().await;
    svc.cancel_backend(&session_id, pid).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_roles(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<Vec<PgRole>, String> {
    let mut svc = state.lock().await;
    svc.list_roles(&session_id).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_list_tablespaces(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<Vec<TablespaceInfo>, String> {
    let mut svc = state.lock().await;
    svc.list_tablespaces(&session_id).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_server_uptime(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
) -> Result<String, String> {
    let mut svc = state.lock().await;
    svc.server_uptime(&session_id).await.map_err(|e| e.message)
}

#[tauri::command]
pub async fn pg_database_size(
    state: tauri::State<'_, PostgresServiceState>,
    session_id: String,
    database: String,
) -> Result<String, String> {
    let mut svc = state.lock().await;
    svc.database_size(&session_id, &database).await.map_err(|e| e.message)
}
