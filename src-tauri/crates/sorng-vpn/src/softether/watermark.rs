//! SoftEther protocol watermark blob.
//!
//! This is the 1411-byte binary payload that SoftEther VPN clients POST to
//! `/vpnsvc/connect.cgi` (HTTP_VPN_TARGET2) as the first message of the
//! SSL-VPN handshake. The byte string is a valid GIF89a image that encodes
//! the SoftEther logo — it acts as a watermark / magic cookie that the
//! server matches to distinguish legitimate SoftEther clients from random
//! HTTP hits, and it doubles as a DPI / proxy-passthrough signal: middle
//! boxes that inspect the POST body see an innocuous `image/jpeg`
//! (Content-Type per `HTTP_CONTENT_TYPE3`) payload and pass it through.
//!
//! # Source
//!
//! Byte values copied verbatim from upstream
//! <https://github.com/SoftEtherVPN/SoftEtherVPN_Stable/blob/master/src/Cedar/WaterMark.c>
//! (`BYTE WaterMark[]`). SoftEtherVPN_Stable is Apache License 2.0, which
//! permits verbatim redistribution with attribution. The header comment on
//! `WaterMark.c` labels the blob "Digital watermark image data (JPEG)"; the
//! bytes are actually a GIF89a image (see opening `47 49 46 38 39 61` =
//! `GIF89a`). A verbatim copy is required here — changing any byte breaks
//! protocol interop: the SoftEther server calls
//! `CompareWaterMark(buf, size)` to gate the connection.
//!
//! # Size note
//!
//! The SE-1 executor brief quoted "1412 bytes" for the blob; the upstream
//! source actually defines 1411 bytes (see `SizeOfWaterMark()` in
//! `WaterMark.c`, which returns `sizeof(WaterMark)`). We follow the source.
//!
//! # Use
//!
//! Consumers (currently `softether::mod::softether_watermark_handshake`)
//! send this buffer as the HTTP POST body with `Content-Type: image/jpeg`.
//! Per upstream `ClientUploadSignature` in `Cedar/Protocol.c`, the real
//! client also appends up to `HTTP_PACK_RAND_SIZE_MAX * 2` (= 2000) bytes
//! of cryptographic random padding after the watermark. We omit the
//! padding — the server's `CompareWaterMark` check only inspects the
//! leading `SizeOfWaterMark()` bytes and ignores the tail.

/// SoftEther VPN client watermark.
///
/// A 1411-byte GIF89a image that the SoftEther server recognises as a
/// valid VPN client greeting. See module docs for details.
...
mod tests {
    ...
    /// Length matches upstream `sizeof(WaterMark)`. If this changes the
    /// server will reject our handshake: `CompareWaterMark` compares the
    /// exact byte range `&buf[..SizeOfWaterMark()]`.
    #[test]
    fn watermark_size_matches_upstream() {
        assert_eq!(WATERMARK.len(), 1411);
    }

    /// The first byte is `'G'` (0x47), the start of the GIF89a magic
    /// (`47 49 46 38 39 61` = "GIF89a"). Any other first byte means the
    /// blob has been corrupted in transit from the C source and the
    /// server will reject us.
    #[test]
    fn watermark_starts_with_gif_magic() {
        assert_eq!(&WATERMARK[..6], b"GIF89a");
    }

    /// GIF89a trailer byte.
    #[test]
    fn watermark_ends_with_gif_trailer() {
        assert_eq!(WATERMARK[WATERMARK.len() - 1], 0x3B);
    }
}
