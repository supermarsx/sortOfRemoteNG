...
impl<'a> NetworkManager<'a> {
    ...
    pub async fn list_adapters(&self) -> IdracResult<Vec<NetworkAdapter>> {
        if let Ok(rf) = self.client.require_redfish() {
            let col: serde_json::Value = rf
                ...
                .await
                .map_err(|_| IdracError::not_found("Trying alternate path"))?;
        ...
        Err(IdracError::unsupported(
            "NIC listing requires Redfish or WSMAN",
        ))
    }
    ...
    pub async fn list_ports(&self) -> IdracResult<Vec<NetworkPort>> {
        if let Ok(rf) = self.client.require_redfish() {
            ...
            return Ok(members
                ...
                .map(|p| NetworkPort {
                    ...
                    vlan_id: p
                        .get("VLAN")
                        .and_then(|v| v.get("VLANId"))
                        .and_then(|v| v.as_u64())
                        .map(|n| n as u32),
                    vlan_enabled: p
                        .get("VLAN")
                        .and_then(|v| v.get("VLANEnable"))
        ...
        Err(IdracError::unsupported(
            "Port listing requires Redfish or WSMAN",
        ))
    }
    ...
    pub async fn get_idrac_network_config(&self) -> IdracResult<IdracNetworkConfig> {
        if let Ok(rf) = self.client.require_redfish() {
            ...
            return Ok(IdracNetworkConfig {
                ...
                ipv4_subnet: iface
                    ...
                    .and_then(|v| v.as_array())
                    .and_then(|a| a.first())
                    .and_then(|addr| addr.get("SubnetMask"))
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                ipv4_gateway: iface
                    .get("IPv4Addresses")
                    .and_then(|v| v.as_array())
                    .and_then(|a| a.first())
                    .and_then(|addr| addr.get("Gateway"))
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                ipv4_dhcp_enabled: iface
                    .get("DHCPv4")
                    .and_then(|v| v.get("DHCPEnabled"))
                    .and_then(|v| v.as_bool()),
                ipv6_address: iface
                    .get("IPv6Addresses")
                    .and_then(|v| v.as_array())
                    ...
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                ipv6_prefix_length: iface
                    .get("IPv6Addresses")
                    .and_then(|v| v.as_array())
                    .and_then(|a| a.first())
                    .and_then(|addr| addr.get("PrefixLength"))
                    .and_then(|v| v.as_u64())
                    .map(|n| n as u32),
                ipv6_gateway: iface
        ...
        Err(IdracError::unsupported(
            "iDRAC network config requires Redfish",
        ))
    }
    ...
    pub async fn update_idrac_network_config(
        ...
        if ipv4_address.is_some() || ipv4_subnet.is_some() || ipv4_gateway.is_some() {
            ...
            if let Some(gw) = ipv4_gateway {
                addr.insert(
                    "Gateway".to_string(),
                    serde_json::Value::String(gw.to_string()),
                );
            }
            body.insert("IPv4Addresses".to_string(), serde_json::json!([addr]));
        }
