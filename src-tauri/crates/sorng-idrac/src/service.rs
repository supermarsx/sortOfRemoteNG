...
impl IdracService {
    ...
    fn require_client(&self) -> IdracResult<&IdracClient> {
        self.client
            ...
            .ok_or_else(|| {
                IdracError::connection("Not connected to iDRAC. Call idrac_connect first.")
            })
    }
    ...
    pub async fn list_virtual_disks(
        ...
        StorageManager::new(self.require_client()?)
            .list_virtual_disks(controller_id)
            .await
    }
    ...
    pub async fn list_physical_disks(
        ...
        StorageManager::new(self.require_client()?)
            .list_physical_disks(controller_id)
            .await
    }
    ...
    pub async fn delete_virtual_disk(
        ...
        StorageManager::new(self.require_client()?)
            .delete_virtual_disk(controller_id, volume_id)
            .await
    }
    ...
    pub async fn assign_hotspare(
        ...
        StorageManager::new(self.require_client()?)
            .assign_hotspare(controller_id, disk_id, hotspare_type, target_volume)
            .await
    }
    ...
    pub async fn initialize_virtual_disk(
        ...
        StorageManager::new(self.require_client()?)
            .initialize_virtual_disk(controller_id, volume_id, init_type)
            .await
    }
    ...
    pub async fn update_idrac_network_config(
        ...
        NetworkManager::new(self.require_client()?)
            .update_idrac_network_config(ipv4, subnet, gw, dhcp)
            .await
    }
    ...
    pub async fn list_jobs(&self) -> IdracResult<Vec<LifecycleJob>> {
        LifecycleManager::new(self.require_client()?)
            .list_jobs()
            .await
    }
    ...
    pub async fn wait_for_job(
        ...
        LifecycleManager::new(self.require_client()?)
            .wait_for_job(job_id, timeout_secs, poll_secs)
            .await
    }
    ...
    pub async fn change_user_password(&self, slot_id: &str, password: &str) -> IdracResult<()> {
        UserManager::new(self.require_client()?)
            .change_password(slot_id, password)
            .await
    }
    ...
    pub async fn import_certificate(&self, cert_pem: &str, cert_type: &str) -> IdracResult<()> {
        CertificateManager::new(self.require_client()?)
            .import_certificate(cert_pem, cert_type)
            .await
    }
    ...
    pub async fn replace_ssl_certificate(&self, cert_pem: &str, key_pem: &str) -> IdracResult<()> {
        CertificateManager::new(self.require_client()?)
            .replace_ssl_certificate(cert_pem, key_pem)
            .await
    }
    ...
    pub async fn get_power_telemetry(&self) -> IdracResult<PowerTelemetry> {
        TelemetryManager::new(self.require_client()?)
            .get_power_telemetry()
            .await
    }
    ...
    pub async fn get_telemetry_report(
        ...
        TelemetryManager::new(self.require_client()?)
            .get_telemetry_report(report_id)
            .await
    }
    ...
    pub async fn racadm_execute(&self, command: &str) -> IdracResult<RacadmResult> {
        RacadmManager::new(self.require_client()?)
            .execute(command)
            .await
    }
