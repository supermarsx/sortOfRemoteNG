...
impl ImageManager {
    ...
    pub async fn list(
        ...
        if opts.all.unwrap_or(false) {
            q.push(("all", "true".to_string()));
        }
        if opts.digests.unwrap_or(false) {
            q.push(("digests", "true".to_string()));
        }
    ...
    pub async fn remove(
        ...
        if q.is_empty() {
            ...
            let qs: Vec<String> = q.iter().map(|(k, v)| format!("{}={}", k, v)).collect();
            client
                .delete(&format!("/images/{}?{}", name, qs.join("&")))
                .await
        }
    }
    ...
    pub async fn commit(
        ...
        let mut path = format!(
            ...
            container_id, repo, tag
        );
        if let Some(c) = comment {
            path.push_str(&format!("&comment={}", c));
        }
