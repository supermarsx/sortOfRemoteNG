//! Vendor-neutral IPMI-over-LAN client (pure Rust, UDP port 623).
//!
//! Implements IPMI 1.5/2.0 session-less mode for the common operations
//! every BMC supports: chassis power, device ID, FRU, SEL, basic sensors.
...
impl IpmiClient {
    ...
    pub async fn get_chassis_status(&self) -> BmcResult<IpmiChassisStatus> {
        let data = self
            .send_ipmi_command(netfn::CHASSIS, cmd::GET_CHASSIS_STATUS, &[])
            .await?;

        if data.len() < 3 {
            return Err(BmcError::ipmi("Chassis status response too short"));
        }
    ...
    pub async fn get_fru(&self) -> BmcResult<IpmiFru> {
        let area_info = self
            .send_ipmi_command(netfn::STORAGE, cmd::GET_FRU_INVENTORY_AREA_INFO, &[0x00])
            .await?;

        if area_info.len() < 2 {
            return Err(BmcError::ipmi("FRU area info too short"));
        }
    ...
    /// Read sensors (simplified — full SDR parsing is complex).
    pub async fn get_sensors(&self) -> BmcResult<Vec<IpmiSensor>> {
        log::info!("IPMI sensor reading via SDR is limited; use Redfish for full sensor data");
        Ok(Vec::new())
    }

    /// Clear the SEL.
    pub async fn clear_sel(&self) -> BmcResult<()> {
        let _reservation = self
            .send_ipmi_command(netfn::STORAGE, 0x42, &[])
            .await
            .unwrap_or_default();
    ...
    /// Send an IPMI command via RMCP/IPMI-over-LAN (session-less mode).
    pub async fn send_ipmi_command(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if len < 20 {
                return Err(BmcError::ipmi("IPMI response too short"));
            }

            let msg_start = 14;
            if len <= msg_start + 7 {
                return Err(BmcError::ipmi("IPMI response data section too short"));
            }

            let completion_code = buf[msg_start + 6];
            if completion_code != 0x00 {
                return Err(BmcError::ipmi(format!(
        ...
        .await
        .map_err(|e| BmcError::ipmi(format!("IPMI task panicked: {e}")))?
    }
