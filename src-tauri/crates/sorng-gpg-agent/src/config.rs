...
impl Default for GpgConfigManager {
    fn default() -> Self {
        Self::new()
    }
}
...
impl GpgConfigManager {
    ...
    pub async fn detect_gpg(&mut self) -> Result<String, String> {
        ...
        for name in &["gpg", "gpg2", "gpg.exe", "gpg2.exe"] {
            let output = Command::new(name).args(["--version"]).output().await;
            ...
            if output.status.success() {
                ...
                for line in stdout.lines() {
                    ...
                    if parts.len() >= 3 && parts[0] == "gpg-agent" {
                        ...
                        info!("Detected gpg-agent binary: {}", self.gpg_agent_binary);
    ...
    pub async fn get_gpg_home(&mut self) -> Result<String, String> {
        ...
        if output.status.success() {
            self.home_dir = String::from_utf8_lossy(&output.stdout).trim().to_string();
    ...
    pub async fn read_config(&mut self) -> Result<GpgAgentConfig, String> {
        ...
        let mut config = GpgAgentConfig {
            home_dir: home.clone(),
            gpg_binary: self.gpg_binary.clone(),
            gpg_agent_binary: self.gpg_agent_binary.clone(),
            scdaemon_binary: self.scdaemon_binary.clone(),
            ..GpgAgentConfig::default()
        };
    ...
    pub async fn write_gpg_conf(&self, settings: &HashMap<String, String>) -> Result<bool, String> {
        let path = format!("{}/gpg.conf", self.home_dir);
