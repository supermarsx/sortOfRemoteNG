...
pub async fn update(host: &PkgHost) -> Result<String, PkgError> {
    client::exec_ok(host, "apt-get", &["update", "-qq"]).await
}
pub async fn upgrade(host: &PkgHost) -> Result<String, PkgError> {
    client::exec_ok(host, "apt-get", &["upgrade", "-y", "-qq"]).await
}
pub async fn dist_upgrade(host: &PkgHost) -> Result<String, PkgError> {
    client::exec_ok(host, "apt-get", &["dist-upgrade", "-y", "-qq"]).await
}
...
pub async fn autoremove(host: &PkgHost) -> Result<String, PkgError> {
    client::exec_ok(host, "apt-get", &["autoremove", "-y", "-qq"]).await
}
    ...
    Ok(stdout
        ...
        .filter_map(|l| {
            ...
            Some(PackageUpdate {
                ...
                new_version: parts.get(1).unwrap_or(&"").to_string(),
                repo: None,
                security: false,
            })
        })
