...
use crate::types::{PkgHost, SshAuth, SshConfig};
use log::debug;
...
pub async fn exec(
    ...
    program: &str,
    args: &[&str],
) -> Result<(String, String, i32), PkgError> {
    debug!(
        "packages: executing {} command",
        if host.ssh.is_some() {
            "remote"
        } else {
            "local"
...
async fn exec_local(
    ...
    prog: &str,
    args: &[&str],
) -> Result<std::process::Output, PkgError> {
    Ok(if sudo {
        Command::new("sudo")
            .arg("-n")
            .arg("--")
...
async fn exec_remote(
    ...
    let raw = session
        ...
        .await
        .map_err(|error| PkgError::SshError(bounded_error(error)))?;
    parse_framed_output(raw, &marker).map_err(PkgError::SshError)
}

fn validate_ssh_config(ssh: &SshConfig) -> Result<(), PkgError> {
    if ssh.host.trim().is_empty() || ssh.username.trim().is_empty() || ssh.port == 0 {
        return Err(PkgError::SshError(
...
fn auth_material(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), PkgError> {
    match &ssh.auth {
