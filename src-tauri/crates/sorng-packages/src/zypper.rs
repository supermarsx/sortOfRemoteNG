...
pub async fn update(host: &PkgHost) -> Result<String, PkgError> { client::exec_ok(host, "zypper", &["refresh"]).await }
pub async fn upgrade(host: &PkgHost) -> Result<String, PkgError> { client::exec_ok(host, "zypper", &["update", "-y"]).await }
pub async fn list_installed(host: &PkgHost) -> Result<Vec<Package>, PkgError> {
    let stdout = client::exec_ok(host, "rpm", &["-qa", "--queryformat", "%{NAME}|%{VERSION}|%{ARCH}|%{SUMMARY}\\n"]).await?;
...
pub async fn search(host: &PkgHost, query: &str) -> Result<Vec<Package>, PkgError> {
    ...
    Ok(stdout.lines().skip(5).filter_map(|l| { let p: Vec<&str> = l.split('|').collect(); if p.len() < 4 { return None; } Some(Package { name: p[1].trim().into(), version: p[3].trim().into(), architecture: p.get(4).map(|s| s.trim().to_string()), description: None, installed: p[0].contains('i'), repo: p.get(5).map(|s| s.trim().to_string()), size: None, install_date: None }) }).collect())
}
