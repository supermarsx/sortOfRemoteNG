...
pub fn detect_product() -> VmwResult<VmwHostInfo> {
    let (product, version, build, install_path) = if cfg!(target_os = "windows") {
        detect_windows()
...
fn detect_windows() -> VmwResult<(VmwProduct, Option<String>, Option<String>, String)> {
    ...
    for (key_path, is_workstation) in &paths {
        ...
        {
            let version: Option<String> = hklm.get_value("ProductVersion").ok();
            let build: Option<String> = hklm.get_value("BuildNumber").ok();
            ...
            let product = if *is_workstation {
                ...
                let lic: Option<String> = hklm.get_value("License.ws.e1.bType").ok();
...
pub fn get_default_vm_dir() -> String {
    if let Ok(prefs) = read_preferences() {
        if let Some(dir) = prefs
            ...
            .or_else(|| prefs.settings.get("prefsvmx.defaultvmpath"))
        {
            return dir.clone();
        }
    }
