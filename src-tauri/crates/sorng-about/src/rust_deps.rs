...
fn categorize(name: &str) -> &'static str {
    match name {
        ...
        // Serialization
        n if n == "serde"
            || n.starts_with("serde_")
            || n == "bincode"
            || n == "toml"
            || n.starts_with("toml_")
            || n == "csv"
            || n.starts_with("quick-xml")
            || n == "rmp"
            || n.starts_with("rmp-")
            || n == "ciborium"
            || n.starts_with("ciborium-")
            || n == "postcard"
            || n == "ron"
            || n == "erased-serde"
            || n == "serdect"
        ...
        n if n == "hyper"
            ...
            || n == "mime"
            || n == "mime_guess"
            || n.starts_with("cookie")
            || n.starts_with("headers") =>
        {
            "HTTP & Networking"
        }

        // TLS / SSL
        n if n.starts_with("native-tls")
            || n.starts_with("openssl")
            || n == "schannel"
            || n.starts_with("security-framework")
            || n.starts_with("tokio-native-tls")
            || n.starts_with("tokio-rustls")
            || n.starts_with("hyper-tls")
            || n.starts_with("hyper-rustls") =>
        {
            "TLS & SSL"
        }

        // SSH
        n if n.starts_with("russh")
            || n == "ssh-key"
            || n == "ssh-encoding"
            || n == "ssh-cipher"
            || n.starts_with("thrussh") =>
        {
            "SSH"
        }

        // Database
        n if n.starts_with("sqlx")
            || n.starts_with("diesel")
            || n == "rusqlite"
            || n.starts_with("mysql")
            || n.starts_with("postgres")
            || n.starts_with("redis")
            || n.starts_with("mongodb")
            || n == "r2d2"
            || n == "deadpool"
            || n.starts_with("deadpool-")
            || n == "sea-orm"
            || n.starts_with("sea-")
            || n == "libsqlite3-sys" =>
        {
        ...
        n if n == "proc-macro2"
            ...
            || n == "build-data"
            || n == "vergen"
            || n.starts_with("vergen-") =>
        {
            "Build & Macros"
        }

        // Container / Orchestration
        n if n.starts_with("bollard")
            || n.starts_with("kube")
            || n == "k8s-openapi"
        ...
        // Concurrency
        n if n == "crossbeam"
            || n.starts_with("crossbeam-")
            || n == "rayon"
            || n.starts_with("rayon-")
            || n == "parking_lot"
            || n.starts_with("parking_lot_")
            || n == "dashmap"
            || n == "flume"
            || n == "kanal"
            || n == "once_cell"
