...
use crate::types::{SshAuth, SshConfig, UserMgmtHost};
use log::debug;
...
pub async fn exec(
    ...
    if stderr.contains("Permission denied") || stderr.contains("not permitted") {
        return Err(UserMgmtError::PermissionDenied(format!(
            "`{program}` failed; try enabling sudo"
        )));
    }
...
fn ssh_error(message: impl Into<String>) -> UserMgmtError {
    UserMgmtError::SshError(message.into())
}

fn ssh_credentials(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), UserMgmtError> {
...
fn validate_ssh_identity(ssh: &SshConfig) -> Result<(), UserMgmtError> {
