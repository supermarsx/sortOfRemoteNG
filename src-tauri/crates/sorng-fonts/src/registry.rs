...
impl FontRegistry {
    ...
    pub fn by_css_family(&self, css: &str) -> Option<&FontMetadata> {
        self.fonts.iter().find(|f| f.css_family == css || f.name == css)
    }
    ...
    pub fn by_subcategory(&self, sub: FontSubcategory) -> Vec<&FontMetadata> {
        self.fonts.iter().filter(|f| f.subcategory == Some(sub)).collect()
    }
    ...
    pub fn with_nerd_font(&self) -> Vec<&FontMetadata> {
        self.fonts.iter().filter(|f| f.nerd_font_available).collect()
    }

    /// Fonts pre-installed on the current OS.
    pub fn preinstalled(&self) -> Vec<&FontMetadata> {
        self.fonts.iter().filter(|f| {
    ...
    pub fn search(&self, query: &FontSearchQuery) -> Vec<&FontMetadata> {
        ...
        let mut results: Vec<&FontMetadata> = self.fonts.iter().filter(|f| {
            // Category filter.
            ...
            if let Some(sub) = query.subcategory {
                if f.subcategory != Some(sub) { return false; }
            }
            // Ligatures filter.
            if query.ligatures_only && !f.ligatures { return false; }
            // Nerd Font filter.
            if query.nerd_font_only && !f.nerd_font_available { return false; }
            // Free filter.
            if query.free_only && !f.is_free { return false; }
            // Preinstalled filter.
            if query.preinstalled_only && !f.preinstalled { return false; }
            // Text search.
            if !q_lower.is_empty() {
                let haystack = format!(
                    "{} {} {} {}",
                ...
                if !haystack.contains(&q_lower) { return false; }
            }
            true
        }).collect();
    ...
    pub fn stats(&self) -> FontStats {
        FontStats {
            ...
            monospace_fonts: self.fonts.iter().filter(|f| f.category == FontCategory::Monospace).count(),
            sans_serif_fonts: self.fonts.iter().filter(|f| f.category == FontCategory::SansSerif).count(),
            serif_fonts: self.fonts.iter().filter(|f| f.category == FontCategory::Serif).count(),
            display_fonts: self.fonts.iter().filter(|f| f.category == FontCategory::Display).count(),
            system_fonts: self.fonts.iter().filter(|f| f.category == FontCategory::System).count(),
...
fn build_registry() -> Vec<FontMetadata> {
    ...
    fonts.push(mono("cascadia-code", "Cascadia Code", "Cascadia Code",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("CaskaydiaCove Nerd Font"), Some("CaskaydiaCove"),
        14.0, 1.2, 0.0, vec![200, 300, 400, 600, 700], true, true,
        plat(true, false, false, true), true, "OFL-1.1",
    ...
    fonts.push(mono("consolas", "Consolas", "Consolas",
        Some(FontSubcategory::Terminal), false, true,
        Some("Caskaydia Cove Nerd Font"), None,
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(true, false, false, false), true, "Proprietary",
        None,
        "Classic Windows monospace font, clear and compact",
        vec!["windows", "classic", "microsoft", "terminal"],
        2004, 2));

    fonts.push(mono("courier-new", "Courier New", "Courier New",
        Some(FontSubcategory::Terminal), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(true, true, true, false), true, "Proprietary",
        None,
        "Universal courier-style monospace, available everywhere",
        vec!["universal", "classic", "courier", "wide"],
        1990, 15));

    fonts.push(mono("menlo", "Menlo", "Menlo",
        Some(FontSubcategory::Terminal), false, true,
        Some("Menlo Nerd Font"), None,
        13.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, true, false, false), true, "Proprietary",
        None,
        "macOS default terminal font, based on Bitstream Vera Sans Mono",
        vec!["macos", "apple", "terminal", "vera"],
        2009, 5));

    fonts.push(mono("sf-mono", "SF Mono", "SF Mono",
        Some(FontSubcategory::Terminal), false, true,
        None, None,
        13.0, 1.2, 0.0, vec![300, 400, 500, 600, 700, 800, 900], true, false,
        plat(false, true, false, false), true, "Proprietary",
    ...
    fonts.push(mono("lucida-console", "Lucida Console", "Lucida Console",
        Some(FontSubcategory::Terminal), false, false,
        None, None,
        13.0, 1.2, 0.0, vec![400], false, false,
        plat(true, false, false, false), true, "Proprietary",
        None,
    ...
    fonts.push(mono("fira-code", "Fira Code", "Fira Code",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("FiraCode Nerd Font"), Some("FiraCode"),
        14.0, 1.2, 0.0, vec![300, 400, 500, 600, 700], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("jetbrains-mono", "JetBrains Mono", "JetBrains Mono",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("JetBrainsMono Nerd Font"), Some("JetBrainsMono"),
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700, 800], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("source-code-pro", "Source Code Pro", "Source Code Pro",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("SauceCodePro Nerd Font"), Some("SauceCodePro"),
        14.0, 1.2, 0.0, vec![200, 300, 400, 500, 600, 700, 900], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("ubuntu-mono", "Ubuntu Mono", "Ubuntu Mono",
        Some(FontSubcategory::Terminal), false, true,
        Some("UbuntuMono Nerd Font"), Some("UbuntuMono"),
        15.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, true, true), true, "UFL-1.0",
        Some("https://design.ubuntu.com/font"),
        "Ubuntu's monospace font, wide and readable",
        vec!["ubuntu", "linux", "canonical", "wide"],
        2010, 10));

    fonts.push(mono("deja-vu-sans-mono", "DejaVu Sans Mono", "DejaVu Sans Mono",
        Some(FontSubcategory::Terminal), false, true,
        Some("DejaVuSansMono Nerd Font"), Some("DejaVuSansM"),
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, true, false), true, "Bitstream Vera + Public Domain",
        Some("https://dejavu-fonts.github.io/"),
        "Extended Vera Sans Mono with massive Unicode coverage",
        vec!["dejavu", "unicode", "linux", "vera"],
        2004, 12));

    fonts.push(mono("hack", "Hack", "Hack",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("Hack Nerd Font"), Some("Hack"),
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, false, true), false, "MIT + Bitstream Vera",
    ...
    fonts.push(mono("inconsolata", "Inconsolata", "Inconsolata",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("Inconsolata Nerd Font"), Some("Inconsolata"),
        14.0, 1.2, 0.0, vec![200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("roboto-mono", "Roboto Mono", "Roboto Mono",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("RobotoMono Nerd Font"), Some("RobotoMono"),
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700], true, true,
        plat(false, false, false, true), false, "Apache-2.0",
    ...
    fonts.push(mono("ibm-plex-mono", "IBM Plex Mono", "IBM Plex Mono",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("BlexMono Nerd Font"), Some("BlexMono"),
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("victor-mono", "Victor Mono", "Victor Mono",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("VictorMono Nerd Font"), Some("VictorMono"),
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("anonymous-pro", "Anonymous Pro", "Anonymous Pro",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("AnonymousPro Nerd Font"), Some("AnonymousPro"),
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("fira-mono", "Fira Mono", "Fira Mono",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("FiraMono Nerd Font"), Some("FiraMono"),
        14.0, 1.2, 0.0, vec![400, 500, 700], true, false,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("iosevka", "Iosevka", "Iosevka",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("Iosevka Nerd Font"), Some("Iosevka"),
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("cascadia-mono", "Cascadia Mono", "Cascadia Mono",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("CaskaydiaMono Nerd Font"), Some("CaskaydiaMono"),
        14.0, 1.2, 0.0, vec![200, 300, 400, 600, 700], true, true,
        plat(true, false, false, true), true, "OFL-1.1",
        Some("https://github.com/microsoft/cascadia-code"),
    ...
    fonts.push(mono("space-mono", "Space Mono", "Space Mono",
        Some(FontSubcategory::CodingPlain), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("overpass-mono", "Overpass Mono", "Overpass Mono",
        Some(FontSubcategory::CodingPlain), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![300, 400, 500, 600, 700], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("pt-mono", "PT Mono", "PT Mono",
        Some(FontSubcategory::CodingPlain), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("droid-sans-mono", "Droid Sans Mono", "Droid Sans Mono",
        Some(FontSubcategory::Terminal), false, true,
        Some("DroidSansMono Nerd Font"), Some("DroidSansM"),
        14.0, 1.2, 0.0, vec![400], false, false,
        plat(false, false, false, true), false, "Apache-2.0",
    ...
    fonts.push(mono("liberation-mono", "Liberation Mono", "Liberation Mono",
        Some(FontSubcategory::Terminal), false, true,
        Some("LiterationMono Nerd Font"), Some("LiterationMono"),
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, true, false), true, "OFL-1.1",
    ...
    fonts.push(mono("monaco", "Monaco", "Monaco",
        Some(FontSubcategory::Terminal), false, false,
        None, None,
        12.0, 1.2, 0.0, vec![400], false, false,
        plat(false, true, false, false), true, "Proprietary",
        None,
    ...
    fonts.push(mono("oxygen-mono", "Oxygen Mono", "Oxygen Mono",
        Some(FontSubcategory::CodingPlain), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![400], false, false,
        plat(false, false, true, true), false, "OFL-1.1",
        Some("https://fonts.google.com/specimen/Oxygen+Mono"),
        "KDE's monospace font, rounded and friendly",
        vec!["kde", "linux", "rounded", "friendly"],
        2012, 30));

    fonts.push(mono("share-tech-mono", "Share Tech Mono", "Share Tech Mono",
        Some(FontSubcategory::Terminal), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![400], false, false,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("courier-prime", "Courier Prime", "Courier Prime",
        Some(FontSubcategory::Terminal), false, false,
        None, None,
        14.0, 1.3, 0.0, vec![400, 700], true, false,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("meslo-lg", "Meslo LG", "Meslo LG",
        Some(FontSubcategory::NerdFont), false, true,
        Some("MesloLGS Nerd Font"), Some("MesloLGS"),
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, false, false), false, "Apache-2.0",
    ...
    fonts.push(mono("noto-sans-mono", "Noto Sans Mono", "Noto Sans Mono",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("NotoSansMono Nerd Font"), Some("NotoSansM"),
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, true, true), false, "OFL-1.1",
        Some("https://fonts.google.com/noto"),
        "Google Noto monospace — massive Unicode/CJK/emoji coverage",
        vec!["google", "noto", "unicode", "cjk", "international", "variable"],
        2014, 28));

    fonts.push(mono("comic-shanns", "Comic Shanns", "Comic Shanns",
        Some(FontSubcategory::Retro), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![400, 700], false, false,
        plat(false, false, false, false), false, "MIT",
    ...
    fonts.push(mono("fantasque-sans-mono", "Fantasque Sans Mono", "Fantasque Sans Mono",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("FantasqueSansMono Nerd Font"), Some("FantasqueSansM"),
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, false, false), false, "OFL-1.1",
    ...
    fonts.push(mono("monaspace-neon", "Monaspace Neon", "Monaspace Neon",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("Monaspace Neon Nerd Font"), None,
        14.0, 1.2, 0.0, vec![200, 300, 400, 500, 600, 700, 800], true, true,
        plat(false, false, false, false), false, "OFL-1.1",
        Some("https://monaspace.githubnext.com/"),
        "GitHub Next's texture-healing monospace superfamily — Neon variant",
        vec!["github", "texture-healing", "modern", "ligatures", "variable"],
        2023, 31));

    fonts.push(mono("monaspace-argon", "Monaspace Argon", "Monaspace Argon",
        Some(FontSubcategory::CodingLigatures), true, false,
        None, None,
        14.0, 1.2, 0.0, vec![200, 300, 400, 500, 600, 700, 800], true, true,
        plat(false, false, false, false), false, "OFL-1.1",
        Some("https://monaspace.githubnext.com/"),
    ...
    fonts.push(mono("geist-mono", "Geist Mono", "Geist Mono",
        Some(FontSubcategory::CodingPlain), false, true,
        Some("GeistMono Nerd Font"), Some("GeistMono"),
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, false, false), false, "OFL-1.1",
    ...
    fonts.push(mono("rec-mono", "Recursive Mono", "Recursive Mono",
        Some(FontSubcategory::CodingLigatures), true, false,
        None, None,
        14.0, 1.2, 0.0, vec![300, 400, 500, 600, 700, 800, 900, 1000], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(mono("commit-mono", "Commit Mono", "Commit Mono",
        Some(FontSubcategory::CodingPlain), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![400, 700], true, false,
        plat(false, false, false, false), false, "OFL-1.1",
    ...
    fonts.push(mono("intel-one-mono", "Intel One Mono", "Intel One Mono",
        Some(FontSubcategory::CodingPlain), false, false,
        None, None,
        14.0, 1.2, 0.0, vec![300, 400, 500, 600, 700], true, true,
        plat(false, false, false, false), false, "OFL-1.1",
    ...
    fonts.push(mono("maple-mono", "Maple Mono", "Maple Mono",
        Some(FontSubcategory::CodingLigatures), true, true,
        Some("Maple Mono NF"), None,
        14.0, 1.2, 0.0, vec![100, 200, 300, 400, 500, 600, 700], true, true,
        plat(false, false, false, false), false, "OFL-1.1",
    ...
    fonts.push(mono("terminus", "Terminus", "Terminus",
        Some(FontSubcategory::Retro), false, true,
        Some("Terminess Nerd Font"), Some("Terminess"),
        14.0, 1.0, 0.0, vec![400, 700], false, false,
        plat(false, false, true, false), false, "OFL-1.1",
    ...
    fonts.push(mono("proggy-clean", "ProggyClean", "ProggyClean",
        Some(FontSubcategory::Retro), false, true,
        Some("ProggyClean Nerd Font"), Some("ProggyClean"),
        13.0, 1.0, 0.0, vec![400], false, false,
        plat(false, false, false, false), false, "MIT",
    ...
    fonts.push(sans("inter", "Inter", "Inter",
        Some(FontSubcategory::UiGeneral),
        vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(sans("segoe-ui", "Segoe UI", "Segoe UI",
        Some(FontSubcategory::PlatformNative),
        vec![300, 350, 400, 600, 700, 800, 900], true, false,
        plat(true, false, false, false), true, "Proprietary",
        None,
        "Windows system font",
    ...
    fonts.push(sans("sf-pro", "SF Pro", "SF Pro",
        Some(FontSubcategory::PlatformNative),
        vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, true, false, false), true, "Proprietary",
        Some("https://developer.apple.com/fonts/"),
    ...
    fonts.push(sans("roboto", "Roboto", "Roboto",
        Some(FontSubcategory::UiGeneral),
        vec![100, 300, 400, 500, 700, 900], true, true,
        plat(false, false, true, true), false, "Apache-2.0",
    ...
    fonts.push(sans("noto-sans", "Noto Sans", "Noto Sans",
        Some(FontSubcategory::UiGeneral),
        vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, true, true), false, "OFL-1.1",
        Some("https://fonts.google.com/noto"),
    ...
    fonts.push(sans("open-sans", "Open Sans", "Open Sans",
        Some(FontSubcategory::Humanist),
        vec![300, 400, 500, 600, 700, 800], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(sans("lato", "Lato", "Lato",
        Some(FontSubcategory::Humanist),
        vec![100, 300, 400, 700, 900], true, false,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(sans("geist", "Geist", "Geist",
        Some(FontSubcategory::UiGeneral),
        vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, false, false), false, "OFL-1.1",
        Some("https://vercel.com/font"),
    ...
    fonts.push(sans("ibm-plex-sans", "IBM Plex Sans", "IBM Plex Sans",
        Some(FontSubcategory::UiGeneral),
        vec![100, 200, 300, 400, 500, 600, 700], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
        Some("https://www.ibm.com/plex/"),
    ...
    fonts.push(sans("nunito", "Nunito", "Nunito",
        Some(FontSubcategory::Geometric),
        vec![200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(serif("georgia", "Georgia", "Georgia",
        Some(FontSubcategory::TraditionalSerif),
        vec![400, 700], true, false,
        plat(true, true, false, true), true, "Proprietary",
        None,
        "Classic screen serif, excellent readability on monitors",
        vec!["classic", "screen", "readable"],
        1993, 1));

    fonts.push(serif("source-serif-pro", "Source Serif Pro", "Source Serif 4",
        Some(FontSubcategory::TraditionalSerif),
        vec![200, 300, 400, 600, 700, 900], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
    ...
    fonts.push(serif("ibm-plex-serif", "IBM Plex Serif", "IBM Plex Serif",
        Some(FontSubcategory::TraditionalSerif),
        vec![100, 200, 300, 400, 500, 600, 700], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
        Some("https://www.ibm.com/plex/"),
    ...
    fonts.push(serif("noto-serif", "Noto Serif", "Noto Serif",
        Some(FontSubcategory::TraditionalSerif),
        vec![100, 200, 300, 400, 500, 600, 700, 800, 900], true, true,
        plat(false, false, false, true), false, "OFL-1.1",
        Some("https://fonts.google.com/noto"),
    ...
    fonts.push(serif("roboto-slab", "Roboto Slab", "Roboto Slab",
    ...
    fonts.push(FontMetadata {
        ...
        license: None,
        homepage_url: None,
        description: Some("Uses the OS default UI font (Segoe UI / SF Pro / Cantarell)".to_string()),
        tags: vec!["system".to_string(), "native".to_string(), "default".to_string()],
        year: None,
    ...
    fonts.push(FontMetadata {
        ...
        license: None,
        homepage_url: None,
        description: Some("Uses the OS default monospace (Consolas / SF Mono / DejaVu Sans Mono)".to_string()),
        tags: vec!["system".to_string(), "native".to_string(), "monospace".to_string()],
        year: None,
...
fn mono(
    id: &str, name: &str, css_family: &str,
    subcategory: Option<FontSubcategory>,
    ligatures: bool, nerd_font_available: bool,
    nerd_font_css: Option<&str>, nerd_font_package: Option<&str>,
    rec_size: f64, rec_lh: f64, rec_ls: f64,
    weights: Vec<u16>, has_italic: bool, is_variable: bool,
    platforms: PlatformAvailability, preinstalled: bool, license: &str,
    homepage_url: Option<&str>,
    description: &str,
    tags: Vec<&str>,
    year: u16, popularity_rank: u16,
) -> FontMetadata {
    FontMetadata {
        id: id.to_string(),
        name: name.to_string(),
...
fn plat(windows: bool, macos: bool, linux: bool, web: bool) -> PlatformAvailability {
    PlatformAvailability { windows, macos, linux, web_font: web }
}
