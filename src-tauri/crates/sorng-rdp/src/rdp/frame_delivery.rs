    ...
    for output in outputs {
        match output {
            ...
            ActiveStageOutput::GraphicsUpdate(region) => {
                ...
                if is_sync {
                    // Full-frame sync: send the entire desktop in one message.
                    // Skipping the dirty-region push avoids a redundant double-
                    // send that causes scanline artifacts on the Canvas 2D path
                    // (the compositor can snapshot the canvas between the two
                    // sequential putImageData calls).
