...
impl SessionManager {
    ...
    pub async fn create_session(client: &ConsulClient, req: &SessionCreateRequest) -> ConsulResult<String> {
    ...
    pub async fn get_session(client: &ConsulClient, session_id: &str) -> ConsulResult<ConsulSession> {
    ...
    pub async fn list_node_sessions(client: &ConsulClient, node: &str) -> ConsulResult<Vec<ConsulSession>> {
    ...
    pub async fn renew_session(client: &ConsulClient, session_id: &str) -> ConsulResult<ConsulSession> {
        let path = format!("/v1/session/renew/{}", session_id);
        debug!("CONSUL renew session: {session_id}");
        let raw: Vec<RawSession> = client.put_body(&path, &serde_json::json!({})).await
            .map_err(|e| {
                if matches!(e.kind, crate::error::ConsulErrorKind::NotFound) {
                    ConsulError::session_expired(format!("Session expired or not found: {session_id}"))
                } else {
                    e
                }
            })?;
        let entry = raw.into_iter()
            .next()
            .ok_or_else(|| ConsulError::session_expired(format!("Session not found after renew: {session_id}")))?;
        Ok(convert_raw_session(entry))
    }
}
...
fn build_session_create_body(req: &SessionCreateRequest) -> serde_json::Value {
    ...
    if let Some(ref name) = req.name { obj.insert("Name".into(), serde_json::json!(name)); }
    if let Some(ref node) = req.node { obj.insert("Node".into(), serde_json::json!(node)); }
    if let Some(ref ld) = req.lock_delay { obj.insert("LockDelay".into(), serde_json::json!(ld)); }
    if let Some(ref b) = req.behavior { obj.insert("Behavior".into(), serde_json::json!(b)); }
    if let Some(ref ttl) = req.ttl { obj.insert("TTL".into(), serde_json::json!(ttl)); }
    if let Some(ref checks) = req.checks { obj.insert("Checks".into(), serde_json::json!(checks)); }
    if let Some(ref nc) = req.node_checks { obj.insert("NodeChecks".into(), serde_json::json!(nc)); }
    if let Some(ref sc) = req.service_checks {
        let arr: Vec<serde_json::Value> = sc.iter().map(|s| {
            let mut m = serde_json::Map::new();
            m.insert("ID".into(), serde_json::json!(s.id));
            if let Some(ref ns) = s.namespace { m.insert("Namespace".into(), serde_json::json!(ns)); }
