...
impl ConsulKvManager {
    ...
    pub async fn get_key(client: &ConsulClient, key: &str) -> ConsulResult<ConsulKeyValue> {
        ...
        let entries: Vec<RawKvEntry> = client.get(&path).await
            .map_err(|e| match e.kind {
                crate::error::ConsulErrorKind::NotFound => ConsulError::not_found(format!("Key not found: {key}")),
                _ => e,
            })?;
    ...
    pub async fn list_keys(client: &ConsulClient, prefix: &str) -> ConsulResult<Vec<String>> {
        ...
        let keys: Vec<String> = client.get_with_params(&path, &[("keys", "true")]).await
            .unwrap_or_default();
        Ok(keys)
    }

    /// GET /v1/kv/:prefix?recurse — returns the full tree of KV entries.
    pub async fn get_tree(client: &ConsulClient, prefix: &str) -> ConsulResult<Vec<ConsulKeyValue>> {
        let path = format!("/v1/kv/{}", encode_key(prefix));
        let entries: Vec<RawKvEntry> = client.get_with_params(&path, &[("recurse", "true")]).await
            .unwrap_or_default();
        Ok(entries.into_iter().map(decode_kv_entry).collect())
    }

    /// GET /v1/kv/:key — returns metadata only (no value decode needed).
    pub async fn get_key_metadata(client: &ConsulClient, key: &str) -> ConsulResult<ConsulKeyMetadata> {
        let path = format!("/v1/kv/{}", encode_key(key));
        let entries: Vec<RawKvEntry> = client.get(&path).await
            .map_err(|e| match e.kind {
                crate::error::ConsulErrorKind::NotFound => ConsulError::not_found(format!("Key not found: {key}")),
    ...
    pub async fn cas_key(client: &ConsulClient, key: &str, value: &str, modify_index: u64) -> ConsulResult<bool> {
        let path = format!("/v1/kv/{}", encode_key(key));
        let cas_str = modify_index.to_string();
        debug!("CONSUL KV CAS {key} @{modify_index}");
        client.put_raw_with_params(&path, value, &[("cas", &cas_str)]).await
    }

    /// PUT /v1/kv/:key?acquire=:session — acquire a lock on the key.
    pub async fn lock_key(client: &ConsulClient, key: &str, session: &str, value: &str) -> ConsulResult<bool> {
        let path = format!("/v1/kv/{}", encode_key(key));
        debug!("CONSUL KV LOCK {key} session={session}");
        client.put_raw_with_params(&path, value, &[("acquire", session)]).await
    }

    /// PUT /v1/kv/:key?release=:session — release the lock on the key.
    pub async fn unlock_key(client: &ConsulClient, key: &str, session: &str, value: &str) -> ConsulResult<bool> {
        let path = format!("/v1/kv/{}", encode_key(key));
        debug!("CONSUL KV UNLOCK {key} session={session}");
        client.put_raw_with_params(&path, value, &[("release", session)]).await
    }
