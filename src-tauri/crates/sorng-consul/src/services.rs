...
impl ServiceDiscovery {
    ...
    pub async fn list_services(client: &ConsulClient) -> ConsulResult<HashMap<String, Vec<String>>> {
    ...
    pub async fn get_service(client: &ConsulClient, name: &str) -> ConsulResult<Vec<ConsulServiceEntry>> {
        ...
        for entry in entries {
            ...
            result.push(ConsulServiceEntry { node, service: svc, checks });
        }
        Ok(result)
    }

    /// GET /v1/health/service/:name — returns instances with health info.
    pub async fn list_service_instances(client: &ConsulClient, name: &str) -> ConsulResult<Vec<ConsulServiceEntry>> {
    ...
    pub async fn get_service_health(client: &ConsulClient, name: &str) -> ConsulResult<Vec<ConsulServiceEntry>> {
        let path = format!("/v1/health/service/{}", name);
    ...
    pub async fn register_service(client: &ConsulClient, reg: &ServiceRegistration) -> ConsulResult<()> {
    ...
    pub async fn enable_maintenance(client: &ConsulClient, service_id: &str, reason: &str) -> ConsulResult<()> {
...
fn build_agent_service_body(reg: &ServiceRegistration) -> serde_json::Value {
    ...
    if let Some(ref chks) = reg.checks {
        let arr: Vec<serde_json::Value> = chks.iter().map(|c| build_check_body(c)).collect();
...
fn build_check_body(chk: &ServiceCheckRegistration) -> serde_json::Value {
    let mut body = serde_json::json!({});
    let obj = body.as_object_mut().unwrap();
    if let Some(ref n) = chk.name { obj.insert("Name".into(), serde_json::json!(n)); }
    if let Some(ref id) = chk.check_id { obj.insert("CheckID".into(), serde_json::json!(id)); }
    if let Some(ref h) = chk.http { obj.insert("HTTP".into(), serde_json::json!(h)); }
    if let Some(ref t) = chk.tcp { obj.insert("TCP".into(), serde_json::json!(t)); }
    if let Some(ref g) = chk.grpc { obj.insert("GRPC".into(), serde_json::json!(g)); }
    if let Some(ref i) = chk.interval { obj.insert("Interval".into(), serde_json::json!(i)); }
    if let Some(ref t) = chk.timeout { obj.insert("Timeout".into(), serde_json::json!(t)); }
    if let Some(ref d) = chk.deregister_critical_service_after {
        obj.insert("DeregisterCriticalServiceAfter".into(), serde_json::json!(d));
    }
    if let Some(skip) = chk.tls_skip_verify { obj.insert("TLSSkipVerify".into(), serde_json::json!(skip)); }
    if let Some(ref s) = chk.status { obj.insert("Status".into(), serde_json::json!(s)); }
    body
}
