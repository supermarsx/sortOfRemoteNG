...
impl AgentManager {
    ...
    pub async fn list_agent_services(client: &ConsulClient) -> ConsulResult<HashMap<String, ConsulService>> {
    ...
    pub async fn register_agent_service(client: &ConsulClient, reg: &ServiceRegistration) -> ConsulResult<()> {
        debug!("CONSUL agent register service: {}", reg.name);
        let body = build_agent_service_body(reg);
        client.put_json_no_response("/v1/agent/service/register", &body).await
    }

    /// PUT /v1/agent/service/deregister/:id — remove a service from the agent.
    pub async fn deregister_agent_service(client: &ConsulClient, service_id: &str) -> ConsulResult<()> {
    ...
    pub async fn list_agent_checks(client: &ConsulClient) -> ConsulResult<HashMap<String, ConsulHealthCheck>> {
    ...
    pub async fn register_check(client: &ConsulClient, reg: &CheckRegistration) -> ConsulResult<()> {
        debug!("CONSUL agent register check: {}", reg.name);
        let body = build_check_register_body(reg);
        client.put_json_no_response("/v1/agent/check/register", &body).await
    }
...
fn parse_member(v: &serde_json::Value) -> AgentMember {
    AgentMember {
        ...
        addr: v.get("Addr").and_then(|v| v.as_str()).unwrap_or("").to_string(),
        ...
        protocol_min: v.get("ProtocolMin").and_then(|v| v.as_u64()).map(|n| n as u8),
        protocol_max: v.get("ProtocolMax").and_then(|v| v.as_u64()).map(|n| n as u8),
        protocol_cur: v.get("ProtocolCur").and_then(|v| v.as_u64()).map(|n| n as u8),
        delegate_min: v.get("DelegateMin").and_then(|v| v.as_u64()).map(|n| n as u8),
        delegate_max: v.get("DelegateMax").and_then(|v| v.as_u64()).map(|n| n as u8),
        delegate_cur: v.get("DelegateCur").and_then(|v| v.as_u64()).map(|n| n as u8),
    }
}
...
fn parse_agent_service(v: &serde_json::Value, key: &str) -> ConsulService {
    ConsulService {
        ...
        service: v.get("Service").and_then(|v| v.as_str()).unwrap_or(key).to_string(),
...
fn parse_agent_check(v: &serde_json::Value) -> ConsulHealthCheck {
    ConsulHealthCheck {
        node: v.get("Node").and_then(|v| v.as_str()).map(|s| s.to_string()),
        check_id: v.get("CheckID").and_then(|v| v.as_str()).map(|s| s.to_string()),
        name: v.get("Name").and_then(|v| v.as_str()).unwrap_or("").to_string(),
        status: v.get("Status").and_then(|v| v.as_str()).unwrap_or("unknown").to_string(),
        notes: v.get("Notes").and_then(|v| v.as_str()).map(|s| s.to_string()),
        output: v.get("Output").and_then(|v| v.as_str()).map(|s| s.to_string()),
        service_id: v.get("ServiceID").and_then(|v| v.as_str()).map(|s| s.to_string()),
        service_name: v.get("ServiceName").and_then(|v| v.as_str()).map(|s| s.to_string()),
        service_tags: v.get("ServiceTags").and_then(|v| v.as_array()).map(|arr| {
            arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
        }),
        check_type: v.get("Type").and_then(|v| v.as_str()).map(|s| s.to_string()),
...
fn build_agent_service_body(reg: &ServiceRegistration) -> serde_json::Value {
    ...
    if let Some(ref id) = reg.id { obj.insert("ID".into(), serde_json::json!(id)); }
    if let Some(ref tags) = reg.tags { obj.insert("Tags".into(), serde_json::json!(tags)); }
    if let Some(ref addr) = reg.address { obj.insert("Address".into(), serde_json::json!(addr)); }
    if let Some(port) = reg.port { obj.insert("Port".into(), serde_json::json!(port)); }
    if let Some(ref meta) = reg.meta { obj.insert("Meta".into(), serde_json::json!(meta)); }
    if let Some(eto) = reg.enable_tag_override { obj.insert("EnableTagOverride".into(), serde_json::json!(eto)); }
    if let Some(ref w) = reg.weights {
        obj.insert("Weights".into(), serde_json::json!({"Passing": w.passing, "Warning": w.warning}));
    }
    body
}
...
fn build_check_register_body(reg: &CheckRegistration) -> serde_json::Value {
    let mut body = serde_json::json!({"Name": reg.name});
    let obj = body.as_object_mut().unwrap();
    if let Some(ref id) = reg.check_id { obj.insert("CheckID".into(), serde_json::json!(id)); }
    if let Some(ref sid) = reg.service_id { obj.insert("ServiceID".into(), serde_json::json!(sid)); }
    if let Some(ref h) = reg.http { obj.insert("HTTP".into(), serde_json::json!(h)); }
    if let Some(ref t) = reg.tcp { obj.insert("TCP".into(), serde_json::json!(t)); }
    if let Some(ref g) = reg.grpc { obj.insert("GRPC".into(), serde_json::json!(g)); }
    if let Some(ref i) = reg.interval { obj.insert("Interval".into(), serde_json::json!(i)); }
    if let Some(ref t) = reg.timeout { obj.insert("Timeout".into(), serde_json::json!(t)); }
    if let Some(ref d) = reg.deregister_critical_service_after {
        obj.insert("DeregisterCriticalServiceAfter".into(), serde_json::json!(d));
    }
    if let Some(skip) = reg.tls_skip_verify { obj.insert("TLSSkipVerify".into(), serde_json::json!(skip)); }
    if let Some(ref s) = reg.status { obj.insert("Status".into(), serde_json::json!(s)); }
    if let Some(ref n) = reg.notes { obj.insert("Notes".into(), serde_json::json!(n)); }
    body
}
