...
impl IpmiSessionHandle {
    ...
    fn compute_v15_auth_code(&self, _request: &IpmiRequest) -> IpmiResult<Vec<u8>> {
        match self.session.negotiated_auth {
            ...
            AuthType::MD2 => {
                // IPMI 1.5 spec §22.17.1 defines the MD2 auth code identically
                // to MD5: MD2(password || session_id || message_data || session_seq || password).
                // The existing MD5 arm uses the simplified two-input form
                // (password || session_id) — mirror that here for consistency.
                //
                // MD2 is cryptographically broken (collision attacks published
                // since 2004) and forbidden by NIST SP 800-131A, but a number
                // of legacy BMCs still advertise it as their only auth option.
                // We emit a loud warning and compute the real hash instead of
                // falling back to password pass-through.
                warn!(
                    "BMC negotiated MD2 auth — this algorithm is cryptographically \
                     broken. Prefer MD5 or RAKP-HMAC-SHA1/SHA256 where supported."
                );
                let mut hasher = <Md2 as md2::Digest>::new();
                <Md2 as md2::Digest>::update(&mut hasher, self.session.config.password.as_bytes());
                <Md2 as md2::Digest>::update(
                    &mut hasher,
                    self.session.bmc_session_id.to_le_bytes(),
                );
                let result = <Md2 as md2::Digest>::finalize(hasher);
                Ok(result.to_vec())
            }
