...
mod tests {
    ...
    fn expiry_expired() {
        ...
        tracker.add_credential(rec).unwrap();
        let status = tracker.check_expiry("cred-1").unwrap();
        let ExpiryStatus::Expired { days_overdue } = status else {
            unreachable!("Expected Expired, got {:?}", status)
        };
        assert!(days_overdue >= 4);
    }
    ...
    fn expiry_soon() {
        ...
        tracker.add_credential(rec).unwrap();
        let status = tracker.check_expiry("cred-1").unwrap();
        let ExpiryStatus::ExpiringSoon { days_remaining } = status else {
            unreachable!("Expected ExpiringSoon, got {:?}", status)
        };
