...
pub fn derive_key(password: &str, email: &str, iterations: u32) -> Result<Vec<u8>, LastPassError> {
    use hmac::Hmac;
    use pbkdf2::pbkdf2;
    ...
    if iterations == 1 {
        ...
        Ok(second_hasher.finalize().to_vec())
    } else {
        let mut key = [0u8; 32];
        pbkdf2::<Hmac<Sha256>>(
            password.as_bytes(),
        ...
        .map_err(|_| LastPassError::decryption_error("PBKDF2 key derivation failed"))?;
        Ok(key.to_vec())
    }
}
...
pub fn compute_login_hash(key: &[u8], password: &str, iterations: u32) -> Result<String, LastPassError> {
    ...
    if iterations == 1 {
        ...
        pbkdf2::<Hmac<Sha256>>(key, password.as_bytes(), 1, &mut login_hash)
            .map_err(|_| LastPassError::decryption_error("PBKDF2 login hash computation failed"))?;
        Ok(hex::encode(login_hash))
    }
}
...
mod tests {
    ...
    fn test_derive_key_multiple_iterations() {
        let key = derive_key("master_password", "user@example.com", 100100).unwrap();
        assert_eq!(key.len(), 32);
    }

    #[test]
    fn test_derive_key_single_iteration() {
        let key = derive_key("master_password", "user@example.com", 1).unwrap();
        assert_eq!(key.len(), 32);
    }
    ...
    fn test_login_hash_multiple_iterations() {
        let key = derive_key("master_password", "user@example.com", 100100).unwrap();
        let hash = compute_login_hash(&key, "master_password", 100100).unwrap();
