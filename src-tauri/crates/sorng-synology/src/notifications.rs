...
impl NotificationsManager {
    ...
    pub async fn get_config(client: &SynoClient) -> SynologyResult<NotificationConfig> {
        let v = client
            .best_version("SYNO.Core.Notification.Setting", 1)
            .unwrap_or(1);
        client
            .api_call("SYNO.Core.Notification.Setting", v, "get", &[])
            .await
    }
    ...
    pub async fn test_email(client: &SynoClient) -> SynologyResult<()> {
        let v = client
            .best_version("SYNO.Core.Notification.Mail", 1)
            .unwrap_or(1);
        client
            .api_post_void("SYNO.Core.Notification.Mail", v, "test", &[])
            .await
    }
    ...
    pub async fn get_push_config(client: &SynoClient) -> SynologyResult<serde_json::Value> {
        let v = client
            .best_version("SYNO.Core.Notification.Push.Mobile", 1)
            .unwrap_or(1);
        client
            .api_call("SYNO.Core.Notification.Push.Mobile", v, "get", &[])
            .await
    }
    ...
    pub async fn test_push(client: &SynoClient) -> SynologyResult<()> {
        let v = client
            ...
            .unwrap_or(1);
        client
            .api_post_void("SYNO.Core.Notification.Push.Mobile", v, "test", &[])
            .await
    }
    ...
    pub async fn get_sms_config(client: &SynoClient) -> SynologyResult<serde_json::Value> {
        let v = client
            .best_version("SYNO.Core.Notification.SMS", 1)
            .unwrap_or(1);
        client
            .api_call("SYNO.Core.Notification.SMS", v, "get", &[])
            .await
    }
    ...
    pub async fn get_history(client: &SynoClient) -> SynologyResult<serde_json::Value> {
        let v = client
            .best_version("SYNO.Core.Notification.Event", 1)
            .unwrap_or(1);
        client
            .api_call(
                "SYNO.Core.Notification.Event",
                v,
                "list",
                &[("offset", "0"), ("limit", "100")],
            )
    ...
    pub async fn acknowledge_all(client: &SynoClient) -> SynologyResult<()> {
        let v = client
            ...
            .unwrap_or(1);
        client
            .api_post_void("SYNO.Core.Notification.Event", v, "acknowledge_all", &[])
            .await
    }
}
