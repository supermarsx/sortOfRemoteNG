...
impl PackagesManager {
    ...
    pub async fn list_installed(client: &SynoClient) -> SynologyResult<Vec<PackageInfo>> {
        ...
        client.api_call(
            ...
            v,
            "list",
            &[("additional", "[\"description\",\"description_enu\",\"dependent_packages\",\"status\"]")],
        )
        .await
    }
    ...
    pub async fn get_package(client: &SynoClient, id: &str) -> SynologyResult<PackageInfo> {
        let v = client.best_version("SYNO.Core.Package", 1).unwrap_or(1);
        client.api_call("SYNO.Core.Package", v, "get", &[("id", id)]).await
    }

    /// Start a package.
    pub async fn start(client: &SynoClient, id: &str) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Core.Package.Control", 1).unwrap_or(1);
        client.api_post_void("SYNO.Core.Package.Control", v, "start", &[("id", id)]).await
    }

    /// Stop a package.
    pub async fn stop(client: &SynoClient, id: &str) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Core.Package.Control", 1).unwrap_or(1);
        client.api_post_void("SYNO.Core.Package.Control", v, "stop", &[("id", id)]).await
    }

    /// Install a package from the Package Center.
    pub async fn install(client: &SynoClient, id: &str, volume: &str) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Core.Package.Installation", 1).unwrap_or(1);
    ...
    pub async fn uninstall(client: &SynoClient, id: &str) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Core.Package.Uninstallation", 1).unwrap_or(1);
        client.api_post_void("SYNO.Core.Package.Uninstallation", v, "uninstall", &[("id", id)]).await
    }

    /// Check for package updates.
    pub async fn check_updates(client: &SynoClient) -> SynologyResult<Vec<PackageInfo>> {
        let v = client.best_version("SYNO.Core.Package.Server", 1).unwrap_or(1);
        client.api_call("SYNO.Core.Package.Server", v, "list_upgradable", &[]).await
    }
    ...
    pub async fn list_available(client: &SynoClient) -> SynologyResult<Vec<PackageInfo>> {
        let v = client.best_version("SYNO.Core.Package.Feed", 1).unwrap_or(1);
        client.api_call("SYNO.Core.Package.Feed", v, "list", &[]).await
    }
    ...
    pub async fn is_running(client: &SynoClient, id: &str) -> SynologyResult<bool> {
        match Self::get_package(client, id).await {
            Ok(pkg) => Ok(pkg.status == Some("running".to_string())),
            Err(_) => Ok(false),
        }
    }
}
