...
impl UsersManager {
    ...
    pub async fn get_user(client: &SynoClient, name: &str) -> SynologyResult<SynoUser> {
        let v = client.best_version("SYNO.Core.User", 1).unwrap_or(1);
        client
            .api_call("SYNO.Core.User", v, "get", &[("name", name)])
            .await
    }

    /// Create a new user.
    pub async fn create_user(client: &SynoClient, params: &CreateUserParams) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Core.User", 1).unwrap_or(1);
        ...
        client
            .api_post_void("SYNO.Core.User", v, "create", &refs)
            .await
    }
    ...
    pub async fn delete_user(client: &SynoClient, name: &str) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Core.User", 1).unwrap_or(1);
        client
            .api_post_void("SYNO.Core.User", v, "delete", &[("name", name)])
            .await
    }
    ...
    pub async fn get_group_members(
        ...
        let v = client
            .best_version("SYNO.Core.Group.Member", 1)
            .unwrap_or(1);
        client
            .api_call("SYNO.Core.Group.Member", v, "list", &[("group", name)])
            .await
    }
    ...
    pub async fn get_quota(client: &SynoClient, name: &str) -> SynologyResult<Vec<UserQuota>> {
        ...
        client
            .api_call("SYNO.Core.Quota", v, "get", &[("name", name)])
            .await
    }
