...
impl BackupManager {
    ...
    pub async fn get_task(client: &SynoClient, task_id: &str) -> SynologyResult<BackupTaskInfo> {
        let v = client.best_version("SYNO.Backup.Task", 1).unwrap_or(1);
        client.api_call("SYNO.Backup.Task", v, "get", &[("task_id", task_id)]).await
    }

    /// Start a Hyper Backup task.
    pub async fn start_task(client: &SynoClient, task_id: &str) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Backup.Task", 1).unwrap_or(1);
        client.api_post_void("SYNO.Backup.Task", v, "backup", &[("task_id", task_id)]).await
    }

    /// Cancel a running Hyper Backup task.
    pub async fn cancel_task(client: &SynoClient, task_id: &str) -> SynologyResult<()> {
        let v = client.best_version("SYNO.Backup.Task", 1).unwrap_or(1);
        client.api_post_void("SYNO.Backup.Task", v, "cancel", &[("task_id", task_id)]).await
    }
    ...
    pub async fn list_versions(
        ...
        let v = client.best_version("SYNO.Backup.Task", 1).unwrap_or(1);
        client.api_call("SYNO.Backup.Task", v, "list_version", &[("task_id", task_id)]).await
    }
    ...
    pub async fn get_repository(
        ...
        let v = client.best_version("SYNO.Backup.Repository", 1).unwrap_or(1);
        client.api_call("SYNO.Backup.Repository", v, "get", &[("task_id", task_id)]).await
    }
    ...
    pub async fn list_active_backup_devices(
        ...
        let v = client.best_version("SYNO.ActiveBackup.Overview", 1).unwrap_or(1);
        client.api_call("SYNO.ActiveBackup.Overview", v, "list_device", &[]).await
    }
    ...
    pub async fn get_active_backup_overview(
        ...
        let v = client.best_version("SYNO.ActiveBackup.Overview", 1).unwrap_or(1);
        client.api_call("SYNO.ActiveBackup.Overview", v, "get", &[]).await
    }

    /// Run Active Backup task for a device.
    pub async fn run_active_backup(
        client: &SynoClient,
        device_id: &str,
    ) -> SynologyResult<()> {
        ...
        let v = client.best_version("SYNO.ActiveBackup.Device", 1).unwrap_or(1);
        client.api_post_void(
    ...
    pub async fn list_replication_tasks(
        ...
        let v = client.best_version("SYNO.Core.Share.Snapshot", 1).unwrap_or(1);
        client.api_call("SYNO.Core.Share.Snapshot", v, "list", &[]).await
    }
}
