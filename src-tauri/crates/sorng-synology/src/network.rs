...
impl NetworkManager {
    ...
    pub async fn list_interfaces(client: &SynoClient) -> SynologyResult<Vec<NetworkInterface>> {
        let v = client
            .best_version("SYNO.Core.Network.Interface", 1)
            .unwrap_or(1);
        client
            .api_call("SYNO.Core.Network.Interface", v, "list", &[])
            .await
    }
    ...
    pub async fn list_firewall_rules(client: &SynoClient) -> SynologyResult<Vec<FirewallRule>> {
        let v = client
            .best_version("SYNO.Core.Security.Firewall.Rules", 1)
            .unwrap_or(1);
        client
            .api_call("SYNO.Core.Security.Firewall.Rules", v, "list_all", &[])
            .await
    }
