...
impl ConfigManager {
    ...
    pub async fn get_snippet(client: &NginxClient, name: &str) -> NginxResult<NginxSnippet> {
        ...
        Ok(NginxSnippet {
            name: name.to_string(),
            path: format!("{}/{}", client.conf_d_dir(), name),
            content,
            description: None,
        })
    }
    ...
    pub async fn list_snippets(client: &NginxClient) -> NginxResult<Vec<NginxSnippet>> {
        ...
        for f in &files {
            ...
            let content = client.read_remote_file(&path).await?;
            snippets.push(NginxSnippet {
                name: f.clone(),
                path: format!("{}/{}", client.conf_d_dir(), f),
                content,
                description: None,
            });
        }
    ...
    pub async fn create_snippet(
        ...
        Ok(NginxSnippet {
            name: req.name.clone(),
            path: format!("{}/{}", client.conf_d_dir(), filename),
