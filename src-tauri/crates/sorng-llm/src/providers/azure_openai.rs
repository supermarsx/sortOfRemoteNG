...
impl AzureOpenAiProvider {
    ...
    fn deployment_url(&self, model: &str) -> String {
        ...
        let deployment = self.config.deployments.get(model).cloned().unwrap_or(model.to_string());
        format!("{}/openai/deployments/{}/chat/completions?api-version=2024-08-01-preview", base, deployment)
    }
}

#[async_trait]
impl LlmProvider for AzureOpenAiProvider {
    fn provider_type(&self) -> ProviderType { ProviderType::AzureOpenAi }
    ...
    async fn chat_completion(&self, request: &ChatCompletionRequest) -> LlmResult<ChatCompletionResponse> {
        ...
        if !status.is_success() {
            ...
            return Err(LlmError::provider_error("Azure OpenAI", &err, Some(status.as_u16())));
        }
            ...
            .map(|(i, c)| {
                ...
                Choice {
                    ...
                    message: ChatMessage {
                        ...
                        content: MessageContent::Text(msg["content"].as_str().unwrap_or("").to_string()),
                        name: None,
                        tool_calls,
                        tool_call_id: None,
                    },
                                ...
                                if let Some(choices) = val["choices"].as_array() {
                                    for c in choices {
                                        let chunk = StreamChunk {
                                            ...
                                            delta: StreamDelta {
                                                role: None,
                                                content: c["delta"]["content"].as_str().map(String::from),
                                                tool_calls: None,
                                            },
                                            finish_reason: c["finish_reason"].as_str().map(String::from),
                                            usage: None,
    ...
    async fn list_models(&self) -> LlmResult<Vec<ModelInfo>> { Ok(Vec::new()) }
