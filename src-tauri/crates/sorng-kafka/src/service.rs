...
use crate::runtime_check::{LibraryProbe, RealProbe};
...
impl KafkaService {
    ...
    pub fn connect(&mut self, config: KafkaConnectionConfig) -> KafkaResult<String> {
        // Gate on the native librdkafka presence probe (t3-e38). Under the
        // dynamic-linking feature this turns a confusing `rdkafka-sys`
        // dlopen failure into a typed `LibraryMissing` error carrying a
        // copy-pasteable per-OS install hint. Under `cmake-build` (static)
    ...
    pub async fn produce_message(
        ...
        // SAFETY: producer was just set to Some above if it was None
        session.producer.as_mut().expect("producer was just initialized").produce(message).await
    }
