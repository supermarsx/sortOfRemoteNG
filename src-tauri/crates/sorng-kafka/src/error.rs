...
pub enum KafkaErrorKind {
    ...
    PartitionError,
    OffsetError,
    /// Native `librdkafka` shared library missing at runtime. Paired with
    /// `detail` carrying a per-OS install hint (see `runtime_check.rs`).
    LibraryMissing,
}
...
impl KafkaError {
    ...
    /// Build a `LibraryMissing` error carrying a per-OS install hint in
    /// `detail`. The `message` is the user-facing summary; `install_hint`
    /// is the copy-pasteable install command list.
    pub fn library_missing(
        message: impl Into<String>,
        install_hint: impl Into<String>,
    ) -> Self {
        Self::new(KafkaErrorKind::LibraryMissing, message).with_detail(install_hint)
    }
}
