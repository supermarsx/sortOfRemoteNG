...
impl KnockCrypto {
    ...
    pub fn encrypt_payload(
        ...
        let nonce_size = match algorithm {
            KnockEncryption::Aes256Gcm => 12,
            KnockEncryption::ChaCha20Poly1305 => 12,
            KnockEncryption::Aes256Cbc | KnockEncryption::RijndaelCbc => 16,
            KnockEncryption::None => 0,
        };
    ...
    pub fn decrypt_payload(
        ...
        let stretched = stretch_key(key, &payload.nonce, payload.ciphertext.len());
...
/// Stretch `key` to `len` bytes using the nonce as additional entropy.
fn stretch_key(key: &[u8], nonce: &[u8], len: usize) -> Vec<u8> {
