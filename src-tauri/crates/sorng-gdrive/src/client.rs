...
impl GDriveClient {
    ...
    /// POST raw bytes and return the raw `Response` so callers can inspect
    /// headers (e.g. the `Location` header returned by resumable-upload
    /// session initiation). Uses the same auth, rate-limiting, and retry
    /// machinery as other verbs.
