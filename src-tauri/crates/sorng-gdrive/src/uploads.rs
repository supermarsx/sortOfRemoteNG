...
/// Initiate a resumable upload session per Google Drive API v3.
///
/// Protocol (https://developers.google.com/drive/api/guides/manage-uploads#resumable):
/// 1. POST `/upload/drive/v3/files?uploadType=resumable` with `Content-Type:
///    application/json; charset=UTF-8` and the file metadata as the body.
/// 2. The server responds `200 OK` with a `Location` header whose value is
///    the session URI. Subsequent chunk PUTs go there.
///
/// Returns the session URI extracted from the `Location` response header.
async fn initiate_resumable_session(
    ...
    // Optional but recommended upload-content hints. Google uses these to
    // pre-allocate the session if supplied. We don't always know the final
    // size here (callers pass it through `data.len()`), but we can at least
    // forward what the spec expects.
    ...
    let location = resp
        ...
        .ok_or_else(|| {
            GDriveError::new(
                GDriveErrorKind::UploadFailed,
                "Resumable session init: server returned success but no Location header",
            )
        })?
        .to_str()
        .map_err(|e| {
            GDriveError::new(
                GDriveErrorKind::UploadFailed,
                format!("Resumable session init: invalid Location header: {e}"),
            )
        })?
        .to_string();

    debug!("Resumable upload session initiated: {}", location);
    Ok(location)
}
