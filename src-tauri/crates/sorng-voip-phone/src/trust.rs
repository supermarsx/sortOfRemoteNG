//! Trust Center store handle resolution for the VoIP phone TLS client.
//!
...
//! Trust Center UI) uses. The phone HTTP client is built deep inside
//! `VoipPhoneService::connect` with no access to Tauri app state, so the store path is
//! resolved here:
//!
