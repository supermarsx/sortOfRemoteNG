...
use crate::types::{FileSharingHost, SshAuth, SshConfig};
use log::debug;
...
pub async fn exec(
    ...
    program: &str,
    args: &[&str],
) -> Result<(String, String, i32), FileSharingError> {
    debug!(
        "filesharing: executing {} command",
        if host.ssh.is_some() {
            "remote"
        } else {
            "local"
...
pub async fn exec_ok(
    ...
    program: &str,
    args: &[&str],
) -> Result<String, FileSharingError> {
    let (stdout, stderr, code) = exec(host, program, args).await?;
    if code != 0 {
        return Err(FileSharingError::CommandFailed {
...
pub async fn read_file(host: &FileSharingHost, path: &str) -> Result<String, FileSharingError> {
...
async fn exec_local(
    ...
    prog: &str,
    args: &[&str],
) -> Result<std::process::Output, FileSharingError> {
    Ok(if sudo {
        Command::new("sudo")
            .arg("-n")
            .arg("--")
...
async fn exec_remote(
    ...
    let raw = session
        ...
        .await
        .map_err(|error| FileSharingError::SshError(bounded_error(error)))?;
    parse_framed_output(raw, &marker).map_err(FileSharingError::SshError)
}

fn validate_ssh_config(ssh: &SshConfig) -> Result<(), FileSharingError> {
    if ssh.host.trim().is_empty() || ssh.username.trim().is_empty() || ssh.port == 0 {
        return Err(FileSharingError::SshError(
...
fn auth_material(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), FileSharingError> {
    match &ssh.auth {
