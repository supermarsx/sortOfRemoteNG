...
pub async fn test_config(host: &FileSharingHost) -> Result<String, FileSharingError> {
    client::exec_ok(host, "testparm", &["-s"]).await
}
pub async fn restart(host: &FileSharingHost) -> Result<(), FileSharingError> {
    client::exec_ok(host, "systemctl", &["restart", "smbd"]).await?;
    Ok(())
}

pub fn parse_smb_conf(content: &str) -> SambaFullConfig {
    let mut global = SambaGlobalConfig {
        workgroup: None,
        server_string: None,
        netbios_name: None,
        security: None,
    ...
    for line in content.lines() {
        ...
        if line.starts_with('[') && line.ends_with(']') {
            if let Some(ref section) = current_section {
                if section == "global" {
                    apply_global(&mut global, &current_settings);
                } else {
                    shares.push(settings_to_share(section, &current_settings));
                }
            }
...
fn settings_to_share(name: &str, s: &HashMap<String, String>) -> SambaShare {
    ...
    SambaShare {
        ...
        browseable: bool_val("browseable", true),
        writable: bool_val("writable", false),
        guest_ok: bool_val("guest ok", false),
        read_only: bool_val("read only", true),
        valid_users: s
            .get("valid users")
            .map(|v| v.split(',').map(|u| u.trim().to_string()).collect())
            .unwrap_or_default(),
        write_list: s
            .get("write list")
            .map(|v| v.split(',').map(|u| u.trim().to_string()).collect())
            .unwrap_or_default(),
        create_mask: s.get("create mask").cloned(),
        directory_mask: s.get("directory mask").cloned(),
        force_user: s.get("force user").cloned(),
        force_group: s.get("force group").cloned(),
        settings: s.clone(),
    }
}
