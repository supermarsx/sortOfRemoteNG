...
fn parse_pdbedit(output: &str) -> Vec<SambaUser> {
    ...
    for line in output.lines() {
        ...
        if line.starts_with("Unix username:") {
            username = line.split(':').nth(1).unwrap_or("").trim().into();
        } else if line.starts_with("User SID:") {
            sid = Some(line.split(':').nth(1).unwrap_or("").trim().to_string());
