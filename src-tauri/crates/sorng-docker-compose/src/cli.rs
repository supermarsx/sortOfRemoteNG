...
impl ComposeCli {
    ...
    pub fn version(&self) -> ComposeResult<ComposeVersionInfo> {
        ...
        let short = self
            .run(&[], &["version".to_string(), "--short".to_string()])
    ...
    pub fn up(&self, config: &ComposeUpConfig) -> ComposeResult<String> {
        ...
        if config.no_build.unwrap_or(false) {
            args.push("--no-build".to_string());
        }
        if config.no_start.unwrap_or(false) {
            args.push("--no-start".to_string());
        }
        if config.no_log_prefix.unwrap_or(false) {
            args.push("--no-log-prefix".to_string());
        }
        if config.abort_on_container_exit.unwrap_or(false) {
            args.push("--abort-on-container-exit".to_string());
        }
        if config.attach_dependencies.unwrap_or(false) {
            args.push("--attach-dependencies".to_string());
        }
        if config.always_recreate_deps.unwrap_or(false) {
            args.push("--always-recreate-deps".to_string());
        }
        if config.renew_anon_volumes.unwrap_or(false) {
            args.push("--renew-anon-volumes".to_string());
        }
        if config.quiet_pull.unwrap_or(false) {
            args.push("--quiet-pull".to_string());
        }
        ...
        if let Some(t) = config.wait_timeout {
        ...
        if let Some(ref ecf) = config.exit_code_from {
    ...
    pub fn build(&self, config: &ComposeBuildConfig) -> ComposeResult<String> {
        ...
        if config.with_dependencies.unwrap_or(false) {
            args.push("--with-dependencies".to_string());
        }
        if let Some(ref progress) = config.progress_output {
    ...
    pub fn pull(&self, config: &ComposePullConfig) -> ComposeResult<String> {
        ...
        if config.no_parallel.unwrap_or(false) {
    ...
    pub fn push(&self, config: &ComposePushConfig) -> ComposeResult<String> {
        ...
        if config.ignore_push_failures.unwrap_or(false) {
    ...
    pub fn compose_run(&self, config: &ComposeRunConfig) -> ComposeResult<String> {
        ...
        if config.detach.unwrap_or(false) {
            args.push("-d".to_string());
        }
        if config.rm.unwrap_or(false) {
            args.push("--rm".to_string());
        }
        if config.no_deps.unwrap_or(false) {
            args.push("--no-deps".to_string());
        }
        if config.service_ports.unwrap_or(false) {
            args.push("--service-ports".to_string());
        }
        if config.use_aliases.unwrap_or(false) {
            args.push("--use-aliases".to_string());
        }
        if config.build.unwrap_or(false) {
            args.push("--build".to_string());
        }
    ...
    pub fn rm(&self, config: &ComposeRmConfig) -> ComposeResult<String> {
        ...
        if config.force.unwrap_or(true) {
            args.push("--force".to_string());
        }
        if config.stop.unwrap_or(false) {
    ...
    pub fn cp(&self, config: &ComposeCpConfig) -> ComposeResult<String> {
        ...
        if config.follow_link.unwrap_or(false) {
            args.push("--follow-link".to_string());
        }
        if config.archive.unwrap_or(false) {
    ...
    pub fn config(&self, config: &ComposeConvertConfig) -> ComposeResult<String> {
        ...
        if config.resolve_image_digests.unwrap_or(false) {
            args.push("--resolve-image-digests".to_string());
        }
        if config.no_interpolate.unwrap_or(false) {
            args.push("--no-interpolate".to_string());
        }
        if config.no_normalize.unwrap_or(false) {
            args.push("--no-normalize".to_string());
        }
        if config.no_path_resolution.unwrap_or(false) {
            args.push("--no-path-resolution".to_string());
        }
        if config.services.unwrap_or(false) {
            args.push("--services".to_string());
        }
        if config.volumes_flag.unwrap_or(false) {
            args.push("--volumes".to_string());
        }
        if config.images.unwrap_or(false) {
            args.push("--images".to_string());
        }
        if config.quiet.unwrap_or(false) {
            args.push("--quiet".to_string());
        }
        if let Some(ref hash) = config.hash {
    ...
    pub fn watch(&self, config: &ComposeWatchConfig) -> ComposeResult<String> {
        ...
        if config.no_up.unwrap_or(false) {
    ...
    fn run_with_timeout(
        ...
        let output = cmd
            .output()
            .map_err(|e| ComposeError::command(&format!("Failed to execute compose CLI: {}", e)))?;
        ...
        if !output.status.success() {
            ...
            return Err(
                ComposeError::command(&format!("{}{}", stderr, stdout)).with_exit_code(exit_code)
            );
        }
