...
impl DeploymentManager {
    ...
    pub async fn create(
        ...
        info!(
            "Creating deployment '{}/{}' with {} replica(s)",
            namespace, config.name, config.replicas
        );
    ...
    pub async fn set_image(
        ...
        let containers: Vec<serde_json::Value> = deployment
            .spec
            .template
            .spec
            .containers
        ...
        info!(
            "Updating image for container '{}' in deployment '{}/{}' to '{}'",
    ...
    pub async fn rollback(
        ...
        let rs_url = format!(
            "{}?labelSelector=app={}",
            client.apps_v1_url(namespace, "replicasets"),
            name
        );
        ...
        let target_rs = items
            ...
            .ok_or_else(|| {
                K8sError::not_found(format!(
                    "ReplicaSet with revision {} not found",
                    target_revision
                ))
            })?;
    ...
    pub async fn list_statefulsets(
        ...
        let url = format!(
            "{}{}",
            client.apps_v1_url(namespace, "statefulsets"),
            K8sClient::list_query(opts)
        );
        let resp: serde_json::Value = client.get(&url).await?;
        let empty = vec![];
    ...
    pub async fn list_daemonsets(
        ...
        let url = format!(
            "{}{}",
            client.apps_v1_url(namespace, "daemonsets"),
            K8sClient::list_query(opts)
        );
        let resp: serde_json::Value = client.get(&url).await?;
        let empty = vec![];
