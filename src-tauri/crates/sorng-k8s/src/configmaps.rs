...
impl ConfigMapManager {
    ...
    pub async fn list(client: &K8sClient, namespace: &str, opts: &ListOptions) -> K8sResult<Vec<ConfigMapInfo>> {
        let url = format!("{}{}", client.namespaced_url(namespace, "configmaps"), K8sClient::list_query(opts));
    ...
    pub async fn get(client: &K8sClient, namespace: &str, name: &str) -> K8sResult<ConfigMapInfo> {
        let url = format!("{}/{}", client.namespaced_url(namespace, "configmaps"), name);
        client.get(&url).await
    }

    /// Create a ConfigMap.
    pub async fn create(client: &K8sClient, namespace: &str, config: &CreateConfigMapConfig) -> K8sResult<ConfigMapInfo> {
    ...
    pub async fn update(client: &K8sClient, namespace: &str, name: &str, manifest: &serde_json::Value) -> K8sResult<ConfigMapInfo> {
        let url = format!("{}/{}", client.namespaced_url(namespace, "configmaps"), name);
        client.put(&url, manifest).await
    }

    /// Patch a ConfigMap (merge new data keys).
    pub async fn patch(client: &K8sClient, namespace: &str, name: &str, patch: &serde_json::Value) -> K8sResult<ConfigMapInfo> {
        let url = format!("{}/{}", client.namespaced_url(namespace, "configmaps"), name);
