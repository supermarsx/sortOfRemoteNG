...
impl SecretManager {
    ...
    pub async fn list(client: &K8sClient, namespace: &str, opts: &ListOptions) -> K8sResult<Vec<SecretInfo>> {
        let url = format!("{}{}", client.namespaced_url(namespace, "secrets"), K8sClient::list_query(opts));
    ...
    /// Create a Secret.
    pub async fn create(client: &K8sClient, namespace: &str, config: &CreateSecretConfig) -> K8sResult<SecretInfo> {
        ...
        info!("Creating Secret '{}/{}' (type: {})", namespace, config.name, secret_type);
        client.post(&url, &body).await
    }

    /// Update (replace) a Secret.
    pub async fn update(client: &K8sClient, namespace: &str, name: &str, manifest: &serde_json::Value) -> K8sResult<SecretInfo> {
        let url = format!("{}/{}", client.namespaced_url(namespace, "secrets"), name);
        client.put(&url, manifest).await
    }

    /// Patch a Secret.
    pub async fn patch(client: &K8sClient, namespace: &str, name: &str, patch: &serde_json::Value) -> K8sResult<SecretInfo> {
        let url = format!("{}/{}", client.namespaced_url(namespace, "secrets"), name);
