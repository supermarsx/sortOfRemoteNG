...
impl EventManager {
    ...
    pub async fn list(
        ...
        let url = format!(
            "{}{}",
            client.namespaced_url(namespace, "events"),
    ...
    pub async fn list_for_resource(
        ...
        let field_selector = format!("involvedObject.kind={},involvedObject.name={}", kind, name);
    ...
    pub async fn list_crds(client: &K8sClient, opts: &ListOptions) -> K8sResult<Vec<CrdInfo>> {
        let url = format!(
            "{}{}",
            client.apiextensions_v1_url("customresourcedefinitions"),
            K8sClient::list_query(opts)
        );
        let resp: serde_json::Value = client.get(&url).await?;
        let empty = vec![];
    ...
    pub async fn list_hpas(
        ...
        let url = format!(
            "{}{}",
            client.autoscaling_v2_url(namespace, "horizontalpodautoscalers"),
            K8sClient::list_query(opts)
        );
        let resp: serde_json::Value = client.get(&url).await?;
        let empty = vec![];
