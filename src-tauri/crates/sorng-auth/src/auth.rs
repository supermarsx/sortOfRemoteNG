...
impl AuthService {
    ...
    pub async fn verify_user(&mut self, username: &str, password: &str) -> Result<bool, String> {
        ...
        // Constant-time path: always perform a password verify to prevent
        // a username-existence timing oracle. When the user doesn't exist we
        // verify against a real Argon2id hash (computed once, lazily) of a
        // random sentinel so CPU work is comparable to a valid lookup.
        use std::sync::OnceLock;
        static DUMMY_HASH: OnceLock<String> = OnceLock::new();
        let dummy = DUMMY_HASH.get_or_init(|| {
            hash_password("__sorng_nonexistent_user_sentinel__")
        ...
        let result = verify_password(password, &stored_hash).unwrap_or(false);
        ...
        if !valid && user_exists {
            ...
            // Opportunistically rehash legacy bcrypt hashes into Argon2id
            // so the store gradually migrates on its own. Failures here are
            // non-fatal: we've already authenticated the user.
            if needs_rehash(&stored_hash) {
                if let Ok(new_hash) = hash_password(password) {
                    self.users.insert(username.to_string(), new_hash);
                    if let Err(e) = self.persist() {
                        log::warn!("failed to persist rehashed password for {username}: {e}");
                    }
                }
            }
        }
