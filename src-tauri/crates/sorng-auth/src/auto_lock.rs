...
impl AutoLockService {
    ...
    /// Sets the unlock password (stores SHA-256 hash only)
    pub async fn set_unlock_password(&mut self, password: &str) -> Result<(), String> {
    ...
    /// Unlocks the application with password verification
    pub async fn unlock_with_password(&mut self, password: &str) -> Result<(), String> {
        if password.is_empty() {
            return Err("Password cannot be empty".to_string());
        }
        match &self.password_hash {
            ...
            None => {
                // No password set yet — accept and store for future verifications
...
mod tests {
    ...
    async fn unlock_after_lock_returns_to_unlocked() {
        let mut svc = make_service().await;
        svc.lock_application().await;
        svc.unlock_with_password("test_password").await.unwrap();
        assert!(matches!(svc.get_lock_state().await, LockState::Unlocked));
    }
    ...
    #[tokio::test]
    async fn unlock_requires_password_when_configured() {
        let mut svc = make_service().await;
    ...
    #[tokio::test]
    async fn unlock_with_password_succeeds() {
        let mut svc = make_service().await;
        let result = svc.unlock_with_password("test_password").await;
        assert!(result.is_ok());
        assert!(matches!(svc.get_lock_state().await, LockState::Unlocked));
    }
}
