...
impl LoginDetectionService {
    ...
    pub fn new() -> LoginDetectionServiceState {
        let mut service = LoginDetectionService {
            client: Client::builder()
                ...
                .build()
                .expect("default HTTP client builder should not fail"),
    ...
    fn initialize_patterns(&mut self) {
        ...
        self.username_patterns = vec![
            Regex::new(r"(?i)user(name|id|login|email|mail)").expect("valid regex literal"),
            Regex::new(r"(?i)login").expect("valid regex literal"),
            Regex::new(r"(?i)account").expect("valid regex literal"),
        ];

        // Password field patterns
        self.password_patterns = vec![
            Regex::new(r"(?i)pass(word|wd|phrase)").expect("valid regex literal"),
            Regex::new(r"(?i)pwd").expect("valid regex literal"),
        ];

        // Login form patterns
        self.login_form_patterns = vec![
            Regex::new(r"(?i)login").expect("valid regex literal"),
            Regex::new(r"(?i)sign.?in").expect("valid regex literal"),
            Regex::new(r"(?i)auth").expect("valid regex literal"),
        ];
    }
    ...
    fn extract_title(&self, html: &str) -> Option<String> {
        let title_regex = Regex::new(r"(?i)<title[^>]*>([^<]*)</title>").expect("valid regex literal");
        title_regex
            .captures(html)
    ...
    fn detect_forms(&self, html: &str, base_url: &str) -> Result<Vec<LoginForm>, String> {
        let form_regex = Regex::new(r"(?is)<form[^>]*>.*?</form>").expect("valid regex literal");
    ...
    fn extract_fields(&self, form_html: &str) -> Result<Vec<FormField>, String> {
        let input_regex = Regex::new(r"(?is)<input[^>]*>").expect("valid regex literal");
    ...
    fn extract_attribute(&self, html: &str, attr: &str) -> Result<Option<String>, String> {
        let pattern = format!(r#"(?i){}="([^"]*)""#, attr);
        let regex = Regex::new(&pattern).map_err(|e| format!("Invalid regex for attr '{}': {}", attr, e))?;
