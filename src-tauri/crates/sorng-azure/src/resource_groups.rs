...
use crate::types::{AzureResource, AzureResult, CreateResourceGroupRequest, ResourceGroup};
...
pub async fn list_resource_groups(client: &AzureClient) -> AzureResult<Vec<ResourceGroup>> {
    ...
    let url = client.subscription_url(&format!("/resourcegroups?api-version={}", api))?;
    debug!("list_resource_groups → {}", url);
    client.get_all_pages(&url).await
}

/// Get a single resource group by name.
pub async fn get_resource_group(client: &AzureClient, name: &str) -> AzureResult<ResourceGroup> {
    let api = &client.config().api_version_resources;
    let url = client.subscription_url(&format!("/resourcegroups/{}?api-version={}", name, api))?;
...
pub async fn delete_resource_group(client: &AzureClient, name: &str) -> AzureResult<()> {
    let api = &client.config().api_version_resources;
    let url = client.subscription_url(&format!("/resourcegroups/{}?api-version={}", name, api))?;
...
pub async fn export_template(client: &AzureClient, name: &str) -> AzureResult<serde_json::Value> {
    let api = &client.config().api_version_resources;
...
pub async fn list_resources_in_rg(
    ...
    let api = &client.config().api_version_resources;
    let url =
        client.resource_group_url(resource_group, &format!("/resources?api-version={}", api))?;
...
pub async fn list_all_resources(client: &AzureClient) -> AzureResult<Vec<AzureResource>> {
    let api = &client.config().api_version_resources;
    let url = client.subscription_url(&format!("/resources?api-version={}", api))?;
    debug!("list_all_resources → {}", url);
    client.get_all_pages(&url).await
}

/// Check if a resource group exists (HEAD).
pub async fn exists(client: &AzureClient, name: &str) -> AzureResult<bool> {
...
mod tests {
    ...
    fn url_construction() {
        ...
        let url = c
            .subscription_url("/resourcegroups?api-version=2024-03-01")
            .unwrap();
    ...
    fn rg_deserialization() {
        ...
        assert_eq!(
            rg.properties.unwrap().provisioning_state,
            Some("Succeeded".into())
        );
    }
