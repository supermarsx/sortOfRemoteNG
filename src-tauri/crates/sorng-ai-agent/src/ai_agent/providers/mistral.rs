...
impl MistralProvider {
    pub fn new(config: &ProviderConfig) -> Result<Self, String> {
        let api_key = config.api_key.clone().ok_or("Mistral requires an API key")?;
        let base_url = config.base_url.clone().unwrap_or_else(|| MISTRAL_API_BASE.to_string());
...
impl LlmProvider for MistralProvider {
    fn provider_type(&self) -> AiProvider { AiProvider::Mistral }
    ...
    async fn list_models(&self) -> Result<Vec<ModelSpec>, String> {
        ...
        if let Some(data) = body["data"].as_array() {
            for m in data {
                ...
                let ctx = m["max_context_length"].as_u64()
                    .or_else(|| m["context_length"].as_u64())
                    .unwrap_or(32_768) as u32;
                models.push(ModelSpec {
                    provider: AiProvider::Mistral, model_id: id.clone(),
                    display_name: Some(id.clone()), context_window: ctx,
                    supports_tools: true, supports_vision: id.contains("pixtral"),
            ...
            match self.client.post(&url).json(&body).send().await {
                Ok(resp) if resp.status().is_success() => {
                    ...
                    if let Some(arr) = m["tool_calls"].as_array() {
                        for tc in arr { tcs.push(ToolCall {
                    ...
                    let usage = TokenUsage {
                        ...
                        total_tokens: u["total_tokens"].as_u64().unwrap_or(0) as u32, estimated_cost: 0.0,
                    };
                    ...
                    return Ok(ChatResponse {
                        id: rb["id"].as_str().unwrap_or("").into(), provider: AiProvider::Mistral,
                ...
                Ok(resp) => {
                    ...
                    last_err = format!("Mistral API error {}: {}", s, e);
    ...
    async fn generate_embeddings(&self, texts: &[String], model: Option<&str>, _dim: Option<usize>) -> Result<EmbeddingResponse, String> {
        ...
        let model = model.unwrap_or("mistral-embed");
        let body = serde_json::json!({ "model": model, "input": texts });
        let resp = self.client.post(&url).json(&body).send().await.map_err(|e| format!("{}", e))?;
        if !resp.status().is_success() {
            return Err(format!("Mistral embedding error: {}", resp.text().await.unwrap_or_default()));
        }
        let rb: serde_json::Value = resp.json().await.map_err(|e| format!("{}", e))?;
