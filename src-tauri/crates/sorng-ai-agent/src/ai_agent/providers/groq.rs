                ...
                while let Some(nl) = buf.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        ...
                        if let Ok(p) = serde_json::from_str::<serde_json::Value>(data) {
                            ...
                            if let Some(u) = p.get("x_groq") {
                                if let Some(uu) = u.get("usage") {
                                    usage.prompt_tokens =
                                        uu["prompt_tokens"].as_u64().unwrap_or(0) as u32;
                                    usage.completion_tokens =
                                        uu["completion_tokens"].as_u64().unwrap_or(0) as u32;
                                    usage.total_tokens =
                                        uu["total_tokens"].as_u64().unwrap_or(0) as u32;
                                }
                            }
                        }
                    }
