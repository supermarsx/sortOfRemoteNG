...
impl ConversationStore {
    ...
    pub fn create(&mut self, req: CreateConversationRequest) -> Conversation {
        ...
        let title = req
            .title
            .unwrap_or_else(|| format!("Conversation {}", &id[..8]));
    ...
    pub fn list_summaries(
        &self,
        provider_map: &dyn Fn(&str) -> AiProvider,
    ...
    pub fn import_conversation(&mut self, data: serde_json::Value) -> Result<String, String> {
        let conv: Conversation =
            serde_json::from_value(data).map_err(|e| format!("Deserialization error: {}", e))?;
