...
impl MemoryStore {
    ...
    //
    // Two paths:
    //  * `build_summary` — synchronous, no LLM. Produces a structural digest
    //    (counts + namespaces + latest preview). Useful as a fallback and in
    //    tests.
    //  * `summarize_recent` — async, dispatches to an LLM provider to produce
    //    a natural-language summary of the most recent entries.
    ...
    /// LLM-backed summary of the last `limit` memory entries.
    ///
    /// Returns `Ok("")` when there are no entries. Propagates provider errors
    /// upstream; callers that want a fallback should use `build_summary`.
    pub async fn summarize_recent(
        ...
        let entries = self.get_context_messages(limit);
        if entries.is_empty() {
            return Ok(String::new());
        }

        // Concatenate entries into a numbered transcript.
        let mut transcript = String::new();
        for (i, entry) in entries.iter().enumerate() {
            transcript.push_str(&format!("[{}] ({}) {}\n", i + 1, entry.namespace, entry.content));
        }

        let system_msg = ChatMessage {
            id: "mem-summary-sys".into(),
            role: MessageRole::System,
            content: vec![ContentBlock::Text {
                text: "You are a concise summarisation assistant. Produce a 3-5 sentence \
                       summary of the supplied memory entries, preserving key entities, \
                       decisions, and open questions. Write in plain prose, no bullet points."
                    .into(),
            }],
            tool_call_id: None,
            tool_calls: Vec::new(),
        ...
        let user_msg = ChatMessage {
            id: "mem-summary-user".into(),
            role: MessageRole::User,
            content: vec![ContentBlock::Text {
                text: format!("Summarise the following memory entries:\n\n{}", transcript),
            }],
            tool_call_id: None,
            tool_calls: Vec::new(),
            name: None,
        ...
        let response = provider
            .chat_completion(&[system_msg, user_msg], model, &params, &[])
            .await?;
