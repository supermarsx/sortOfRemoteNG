...
use super::tools::{tool_results_to_messages, ToolRegistry};
use super::types::*;
...
pub async fn run_agent(
    ...
    match config.strategy {
        ...
        AgentStrategy::PlanAndExecute => {
            run_plan_and_execute(config, provider, tools, initial_messages, memory).await
        }
        AgentStrategy::ChainOfThought => {
            run_chain_of_thought(config, provider, initial_messages).await
        }
        AgentStrategy::Reflexion => {
            run_react(config, provider, tools, initial_messages, memory).await
        }
    }
}
...
async fn run_simple(
    ...
    let response = provider
        .chat_completion(&messages, &config.model, &config.params, &[])
        .await?;
...
async fn run_react(
    ...
    if let Some(mem) = memory {
        ...
        let insert_pos = if matches!(messages.first().map(|m| &m.role), Some(MessageRole::System)) {
            1
        } else {
            0
        };
    ...
    for iteration in 0..max_iterations {
        ...
        let response = provider
            .chat_completion(&messages, &config.model, &config.params, &tool_defs)
            .await?;
...
async fn run_plan_and_execute(
    ...
    let plan_messages = vec![
        ChatMessage {
            ...
            role: MessageRole::System,
            content: vec![ContentBlock::Text {
                text: "You are a planning agent. Create detailed step-by-step plans.".into(),
            }],
            tool_call_id: None,
            tool_calls: Vec::new(),
            name: None,
    ...
    let plan_response = provider
        .chat_completion(&plan_messages, &config.model, &config.params, &[])
        .await?;
    ...
    let mut exec_messages = vec![ChatMessage {
        id: "exec-system".into(),
        role: MessageRole::System,
        content: vec![ContentBlock::Text {
            text: execute_system,
    ...
    for _iteration in 0..max_iterations {
        let step_start = std::time::Instant::now();
        let response = provider
            .chat_completion(&exec_messages, &config.model, &config.params, &tool_defs)
            .await?;
        total_usage.prompt_tokens += response.usage.prompt_tokens;
        total_usage.completion_tokens += response.usage.completion_tokens;
        total_usage.total_tokens += response.usage.total_tokens;
