...
pub async fn get_config(host: &DhcpHost) -> Result<KeaDhcp4Config, DhcpError> {
    ...
    let val: serde_json::Value =
        serde_json::from_str(&content).map_err(|e| DhcpError::ConfigParseError(e.to_string()))?;
    parse_kea_config(&val)
}
pub async fn restart(host: &DhcpHost) -> Result<(), DhcpError> {
    client::exec_ok(host, "systemctl", &["restart", "kea-dhcp4-server"]).await?;
    Ok(())
}
...
fn parse_kea_config(val: &serde_json::Value) -> Result<KeaDhcp4Config, DhcpError> {
    ...
    let interfaces = dhcp4
        ...
        .unwrap_or_default();
    let valid_lifetime = dhcp4
        .get("valid-lifetime")
        .and_then(|v| v.as_u64())
        .map(|v| v as u32);
    let renew_timer = dhcp4
        .get("renew-timer")
        .and_then(|v| v.as_u64())
        .map(|v| v as u32);
    let rebind_timer = dhcp4
        .get("rebind-timer")
        .and_then(|v| v.as_u64())
        .map(|v| v as u32);
    Ok(KeaDhcp4Config {
