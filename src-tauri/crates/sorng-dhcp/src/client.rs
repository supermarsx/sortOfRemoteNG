...
async fn exec_remote(
    ...
    let raw = session
        ...
        .await
        .map_err(|error| DhcpError::SshError(bounded_error(error)))?;
    parse_framed_output(raw, &marker).map_err(DhcpError::SshError)
}

fn validate_ssh_config(ssh: &SshConfig) -> Result<(), DhcpError> {
    if ssh.host.trim().is_empty() || ssh.username.trim().is_empty() || ssh.port == 0 {
        return Err(DhcpError::SshError(
...
fn auth_material(ssh: &SshConfig) -> Result<(Option<&str>, Option<&str>), DhcpError> {
    match &ssh.auth {
