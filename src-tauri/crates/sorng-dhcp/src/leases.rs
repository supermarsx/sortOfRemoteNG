...
pub fn parse_isc_leases(content: &str) -> Vec<DhcpLease> {
    ...
    for line in content.lines() {
        ...
        if line.starts_with("lease ") && line.ends_with('{') {
            current_ip = line.strip_prefix("lease ").unwrap_or("").strip_suffix(" {").unwrap_or("").trim().to_string();
            in_lease = true; mac.clear(); hostname = None;
        } else if line == "}" && in_lease {
            leases.push(DhcpLease { ip_address: current_ip.clone(), mac_address: mac.clone(), hostname: hostname.clone(), starts: None, ends: None, state: LeaseState::Active, client_id: None });
            in_lease = false;
        } else if in_lease {
            if line.starts_with("hardware ethernet") { mac = line.split_whitespace().nth(2).unwrap_or("").trim_end_matches(';').to_string(); }
            else if line.starts_with("client-hostname") { hostname = Some(line.split('"').nth(1).unwrap_or("").to_string()); }
        }
    }
    leases
}
...
pub fn parse_dnsmasq_leases(content: &str) -> Vec<DhcpLease> {
    content.lines().filter_map(|line| {
        ...
        Some(DhcpLease { ip_address: cols[2].into(), mac_address: cols[1].into(), hostname: Some(cols[3].into()).filter(|s: &String| s != "*"), starts: None, ends: None, state: LeaseState::Active, client_id: cols.get(4).map(|s| s.to_string()) })
    }).collect()
}
