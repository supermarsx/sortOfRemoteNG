...
pub async fn restart(host: &DhcpHost) -> Result<(), DhcpError> {
    client::exec_ok(host, "systemctl", &["restart", "dnsmasq"]).await?;
    Ok(())
}

pub fn parse_dnsmasq_conf(content: &str) -> DnsmasqConfig {
    let mut cfg = DnsmasqConfig {
        interface: None,
        listen_address: None,
        dhcp_ranges: Vec::new(),
        dhcp_hosts: Vec::new(),
        ...
        if let Some((k, v)) = line.split_once('=') {
            ...
            match k {
                ...
                "dhcp-range" => {
                    ...
                    if parts.len() >= 3 {
                        cfg.dhcp_ranges.push(DnsmasqRange {
                            tag: None,
                            start: parts[0].trim().into(),
                            end: parts[1].trim().into(),
                            lease_time: parts[2].trim().into(),
                            netmask: parts.get(3).map(|s| s.trim().to_string()),
                        });
                    }
                }
                "dhcp-host" => {
                    let parts: Vec<&str> = v.split(',').collect();
                    if parts.len() >= 2 {
                        cfg.dhcp_hosts.push(DhcpReservation {
                            hostname: parts.get(2).unwrap_or(&"").to_string(),
                            mac_address: parts[0].trim().into(),
                            ip_address: parts[1].trim().into(),
                            options: HashMap::new(),
                        });
                    }
                }
