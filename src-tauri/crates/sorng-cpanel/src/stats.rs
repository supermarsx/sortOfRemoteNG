...
impl StatsManager {
    ...
    pub async fn get_bandwidth(client: &CpanelClient, user: &str) -> CpanelResult<BandwidthUsage> {
        let raw: serde_json::Value = client
            .whm_uapi(user, "Stats", "get_bandwidth", &[])
            .await?;
    ...
    pub async fn get_resource_usage(client: &CpanelClient, user: &str) -> CpanelResult<ResourceUsage> {
        let raw: serde_json::Value = client
    ...
    pub async fn get_visitor_logs(client: &CpanelClient, user: &str, domain: &str, lines: u32) -> CpanelResult<Vec<VisitorLog>> {
    ...
    pub async fn get_error_log(client: &CpanelClient, user: &str, lines: u32) -> CpanelResult<Vec<ErrorLogEntry>> {
        let lines_str = lines.to_string();
        let raw: serde_json::Value = client
            .whm_uapi(
                user,
    ...
    pub async fn get_awstats(client: &CpanelClient, user: &str, domain: &str) -> CpanelResult<AwestatsSummary> {
        let raw: serde_json::Value = client
            .whm_uapi(
                user,
                "Stats",
    ...
    pub async fn get_server_load(client: &CpanelClient) -> CpanelResult<ServerLoadStatus> {
        ...
        let cpu_count = data.get("cpu_count").and_then(|v| v.as_u64()).map(|v| v as u32);
        Ok(ServerLoadStatus {
            one,
            five,
            fifteen,
    ...
    pub async fn get_server_bandwidth(client: &CpanelClient, month: Option<&str>, year: Option<&str>) -> CpanelResult<serde_json::Value> {
