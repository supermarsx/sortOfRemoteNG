...
use crate::types::*;
use crate::{distro, hardware, init_system, kernel, locale, package_mgr, security, services, shell};
use crate::client;
...
pub async fn full_scan(host: &OsDetectHost) -> Result<OsCapabilities, OsDetectError> {
    let os_family = distro::detect_os_family(host).await.unwrap_or(OsFamily::Unknown);
    ...
    let version = match os_family {
        ...
        OsFamily::Windows => distro::detect_windows_version(host).await.unwrap_or_default(),
        _ => OsVersion::default(),
    };

    let init = init_system::detect_init_system(host).await.unwrap_or(InitSystem::Unknown);
    let pkg_managers = package_mgr::detect_package_managers(host).await.unwrap_or_default();
    let default_shell = shell::detect_default_shell(host).await.ok();
    let available_shells = shell::detect_available_shells(host).await.unwrap_or_default();
    let kern = kernel::detect_kernel(host).await.ok();
    let arch = kernel::detect_architecture(host).await.unwrap_or(Architecture::Unknown("unknown".to_string()));
    ...
    let svcs = services::detect_available_services(host).await.unwrap_or_default();
    let caps = services::detect_service_capabilities(host).await.unwrap_or_default();
    ...
    for section in sections {
        match section {
            ScanSection::OsFamily => {
                result.os_family = distro::detect_os_family(host).await.unwrap_or(OsFamily::Unknown);
            }
            ScanSection::Distro => {
            ...
            ScanSection::InitSystem => {
                result.init_system = init_system::detect_init_system(host).await.unwrap_or(InitSystem::Unknown);
            }
            ScanSection::PackageManagers => {
                result.package_managers = package_mgr::detect_package_managers(host).await.unwrap_or_default();
            }
            ScanSection::Shell => {
                result.default_shell = shell::detect_default_shell(host).await.ok();
                result.available_shells = shell::detect_available_shells(host).await.unwrap_or_default();
            }
            ScanSection::Kernel => {
                result.kernel = kernel::detect_kernel(host).await.ok();
                result.architecture = kernel::detect_architecture(host).await.unwrap_or(Architecture::Unknown("unknown".to_string()));
            }
            ...
            ScanSection::Services => {
                result.services = services::detect_available_services(host).await.unwrap_or_default();
            }
            ScanSection::Capabilities => {
                result.capabilities = services::detect_service_capabilities(host).await.unwrap_or_default();
            }
        }
    }
...
async fn detect_hostname(host: &OsDetectHost) -> (Option<String>, Option<String>, Option<String>) {
    ...
    let h = if hostname.trim().is_empty() { None } else { Some(hostname.trim().to_string()) };
    let d = if domain.trim().is_empty() { None } else { Some(domain.trim().to_string()) };
    let f = if fqdn.trim().is_empty() { None } else { Some(fqdn.trim().to_string()) };
