...
pub async fn detect_package_managers(host: &OsDetectHost) -> Result<Vec<PackageManager>, OsDetectError> {
...
pub async fn detect_default_package_manager(host: &OsDetectHost) -> Result<PackageManager, OsDetectError> {
    ...
    let priority = [
        ...
        PackageManager::Pacman, PackageManager::Zypper, PackageManager::Emerge,
        PackageManager::Apk, PackageManager::Xbps, PackageManager::Nix,
        PackageManager::Pkg, PackageManager::Brew, PackageManager::Ports,
        PackageManager::Winget, PackageManager::Chocolatey, PackageManager::Scoop,
    ];
    for pm in &priority {
    ...
    Ok(managers.into_iter().next().unwrap_or(PackageManager::Unknown))
}
...
pub async fn list_installed_packages(host: &OsDetectHost) -> Result<Vec<InstalledPackageInfo>, OsDetectError> {
    // dpkg (Debian/Ubuntu)
...
pub async fn check_updates_available(host: &OsDetectHost) -> Result<u64, OsDetectError> {
    ...
    // APK
    let apk = client::shell_exec(host, "apk upgrade --simulate 2>/dev/null | grep -c 'Upgrading' || true").await;
    if let Ok(n) = apk.trim().parse::<u64>() {
        return Ok(n);
    }
...
fn parse_dpkg_packages(stdout: &str) -> Vec<InstalledPackageInfo> {
    stdout.lines().filter_map(|line| {
        ...
        Some(InstalledPackageInfo {
            ...
            source: parts.get(2).map(|s| s.to_string()).filter(|s| !s.is_empty()),
        })
    }).collect()
}
...
fn parse_pacman_packages(stdout: &str) -> Vec<InstalledPackageInfo> {
    stdout.lines().filter_map(|line| {
        ...
        Some(InstalledPackageInfo { name, version, source: None })
    }).collect()
}
...
fn parse_apk_packages(stdout: &str) -> Vec<InstalledPackageInfo> {
    ...
    stdout.lines().filter_map(|line| {
        ...
        let source = parts.iter().find(|p| p.starts_with('{')).map(|s| {
            s.trim_matches('{').trim_matches('}').to_string()
        });
        Some(InstalledPackageInfo { name, version, source })
    }).collect()
}
