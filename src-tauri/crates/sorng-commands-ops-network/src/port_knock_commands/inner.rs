...
fn validate_nmap_request(host: &str, ports: &[u16]) -> Result<(), String> {
    ...
    for &port in ports {
        super::client::validate_port(port)
            .map_err(|_| format!("Nmap command requires 1 to {MAX_NMAP_PORTS} non-zero ports"))?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn probe_validation_accepts_safe_targets_and_bounds() {
        ...
        assert!(validate_probe_request("[2001:db8::1]", u16::MAX, MAX_SCANNER_TIMEOUT_MS).is_ok());
    }
    ...
    fn probe_validation_rejects_host_port_and_timeout_abuse() {
        ...
        assert!(validate_probe_request("host.example", 22, MAX_SCANNER_TIMEOUT_MS + 1).is_err());
    }
    ...
    fn nmap_validation_enforces_host_and_port_set_bounds() {
        ...
        assert!(validate_nmap_request("host.example", &vec![22; MAX_NMAP_PORTS + 1]).is_err());
    }
