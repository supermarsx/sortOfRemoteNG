...
mod tests {
    ...
    /// t40-f5 both-lists rule: each command must be in `COMMAND_NAMES`
    /// AND the generated `generate_handler!` list, or dispatch is dead.
    #[test]
    fn draytek_commands_are_in_both_handler_lists() {
        let handler_src = include_str!("../handler.rs");
        for name in DRAYTEK_COMMANDS {
            assert!(
                crate::handler::is_command(name),
                "{name} missing from COMMAND_NAMES"
            );
            let build_entry = format!("draytek_commands::{name},");
            assert!(
                handler_src.contains(&build_entry),
                "{name} missing from generate_handler! list"
            );
        }
        assert!(!crate::handler::is_command("draytek_run_cli"));
    }
