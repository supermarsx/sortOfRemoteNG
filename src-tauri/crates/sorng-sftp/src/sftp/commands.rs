...
//
// See `src-tauri/crates/sorng-sftp/src/sftp/upload_chunked.rs` for the writer
// task + timeout sweeper that back these commands.
...
pub async fn sftp_upload_begin(
    ...
    let mut svc = state.lock().await;
    svc.upload_begin(&session_id, &remote_path, total_bytes, overwrite.unwrap_or(true))
        .await
}
...
pub async fn sftp_upload_chunk(
    ...
    let mut svc = state.lock().await;
    svc.upload_chunk(&upload_id, offset, bytes).await
}
...
pub async fn sftp_upload_finish(
    ...
    let mut svc = state.lock().await;
    svc.upload_finish(&upload_id).await
}
...
pub async fn sftp_upload_abort(
    ...
    let mut svc = state.lock().await;
    svc.upload_abort(&upload_id).await
}
