...
impl<'a> ThermalManager<'a> {
    ...
    pub async fn get_thermal_summary(&self) -> LenovoResult<ThermalSummary> {
        ...
        let ambient = data.temperatures.iter()
            .find(|t| t.physical_context.as_deref() == Some("Intake") || t.name.to_lowercase().contains("ambient"))
            .and_then(|t| t.reading_celsius);

        let cpu_max = data.temperatures.iter()
            .filter(|t| t.physical_context.as_deref() == Some("CPU") || t.name.to_lowercase().contains("cpu"))
            .filter_map(|t| t.reading_celsius)
            .fold(None, |max: Option<f64>, r| Some(max.map_or(r, |m: f64| m.max(r))));

        let fans_ok = data.fans.iter()
            .filter(|f| f.status.health.as_deref() == Some("OK"))
            .count() as u32;

        let temp_warnings = data.temperatures.iter()
            .filter(|t| t.status.health.as_deref() == Some("Warning"))
            .count() as u32;

        let temp_critical = data.temperatures.iter()
