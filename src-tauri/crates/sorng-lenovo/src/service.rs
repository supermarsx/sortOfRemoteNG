...
impl LenovoService {
    ...
    fn require_client(&self) -> LenovoResult<&LenovoClient> {
        self.client.as_ref().filter(|c| c.is_connected()).ok_or_else(|| {
    ...
    pub async fn get_xcc_info(&self) -> LenovoResult<XccInfo> {
        SystemManager::new(self.require_client()?).get_xcc_info().await
    }
    ...
    pub async fn get_xcc_network(&self) -> LenovoResult<serde_json::Value> {
        NetworkManager::new(self.require_client()?).get_xcc_network().await
    }
    ...
    pub async fn insert_virtual_media(&self, slot: &str, image_url: &str) -> LenovoResult<()> {
        VirtualMediaManager::new(self.require_client()?).insert_media(slot, image_url).await
    }

    pub async fn eject_virtual_media(&self, slot: &str) -> LenovoResult<()> {
        VirtualMediaManager::new(self.require_client()?).eject_media(slot).await
    }
    ...
    pub async fn get_event_log(&self) -> LenovoResult<Vec<BmcEventLogEntry>> {
        EventLogManager::new(self.require_client()?).get_event_log().await
    }

    pub async fn get_audit_log(&self) -> LenovoResult<Vec<BmcEventLogEntry>> {
        EventLogManager::new(self.require_client()?).get_audit_log().await
    }

    pub async fn clear_event_log(&self) -> LenovoResult<()> {
        EventLogManager::new(self.require_client()?).clear_event_log().await
    }
    ...
    pub async fn update_password(&self, user_id: &str, password: &str) -> LenovoResult<()> {
        UserManager::new(self.require_client()?).update_password(user_id, password).await
    }
    ...
    pub async fn set_boot_override(&self, target: &str, mode: Option<&str>) -> LenovoResult<()> {
        BiosManager::new(self.require_client()?).set_boot_override(target, mode).await
    }
    ...
    pub async fn onecli_execute(&self, command: &str) -> LenovoResult<OnecliResult> {
        OnecliManager::new(self.require_client()?).execute(command).await
    }
