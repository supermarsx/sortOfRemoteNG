...
impl ApacheClient {
    ...
    pub async fn exec_ssh(&self, command: &str) -> ApacheResult<SshOutput> {
        debug!("Executing Apache SSH command on {}", self.config.host);
        ...
        let session = sorng_ssh::ssh::integration::IntegrationSshSession::new(
            sorng_ssh::ssh::integration::ExternalSshConfig {
                host: &self.config.host,
                username: ssh_user,
                port,
                private_key: self.config.ssh_key.as_deref(),
                password: self.config.ssh_password.as_deref(),
                connect_timeout_secs: timeout,
            },
        );
        let stdout = session
            .execute(command, Some(timeout.saturating_mul(1000)))
            .await
            .map_err(|e| ApacheError::ssh(format!("Apache SSH command failed: {}", e)))?;
