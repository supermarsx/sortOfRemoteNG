...
impl ModuleManager {
    pub async fn list(client: &ApacheClient) -> ApacheResult<Vec<ApacheModule>> {
        let out = client
            .exec_ssh(&format!("{} -M 2>&1", client.apache_bin()))
            .await?;
    ...
    pub async fn list_available(client: &ApacheClient) -> ApacheResult<Vec<String>> {
        ...
        Ok(files
            .into_iter()
            .filter(|f| f.ends_with(".load"))
            .map(|f| f.trim_end_matches(".load").to_string())
            .collect())
    }
...
fn parse_modules(output: &str) -> Vec<ApacheModule> {
    output
        ...
        .filter_map(|line| {
            let t = line.trim();
            if !t.ends_with("(static)") && !t.ends_with("(shared)") {
                return None;
            }
            ...
            let module_type = if t.ends_with("(static)") {
                ModuleType::Static
            } else {
                ModuleType::Shared
            };
