...
mod compositor_tests {
    ...
    fn softbuffer_compositor_update_and_flush() {
        ...
        // Note: frame.rgba includes 8-byte IPC header + pixel data
        assert_eq!(frame.rgba.len(), 8 + 64 * 48 * 4);

        // Verify pixel data: first pixel should be red (after 8-byte header)
        assert_eq!(frame.rgba[8], 255); // R
        assert_eq!(frame.rgba[9], 0); // G
        assert_eq!(frame.rgba[10], 0); // B
        assert_eq!(frame.rgba[11], 255); // A
    ...
    fn softbuffer_compositor_resize_desktop() {
        ...
        // Note: frame.rgba includes 8-byte IPC header + pixel data
        assert_eq!(frame.rgba.len(), 8 + 128 * 96 * 4);
    }
