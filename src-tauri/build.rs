fn main() {
  ...
  for (feature, flag) in &features {
    ...
    if target_features.split(',').any(|f| f.trim() == *feature || f.trim() == needle) {
  ...
  let has = |name: &str| -> bool {
    ...
    target_features.split(',').any(|f| f.trim() == name || f.trim() == needle)
  };
  ...
  // Print a summary for `cargo build -vv`
  let enabled: Vec<&str> = features.iter()
    .filter(|(feat, _)| has(feat))
    .map(|(_, flag)| *flag)
    .collect();
  if !enabled.is_empty() {
    println!(
      "cargo:warning=Compile-time CPU features enabled: {}",
      enabled.join(", ")
    );
  }
