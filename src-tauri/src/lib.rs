...
// Postfix — MTA management: main.cf, master.cf, queue, transport maps, virtual domains/aliases, relay, TLS, milters (dedicated crate)
pub use sorng_postfix as postfix;

// Dovecot — IMAP/POP3 server: mailboxes, sieve filters, quota, namespaces, auth, users, ACLs, replication, doveadm (dedicated crate)
pub use sorng_dovecot as dovecot;

// OpenDKIM — DKIM signing/verification: keys, signing tables, key tables, trusted hosts, internal hosts, config, stats (dedicated crate)
pub use sorng_opendkim as opendkim;

// Cyrus SASL — SASL authentication: mechanisms, saslauthd, users, realms, auxprop plugins, DB management (dedicated crate)
pub use sorng_cyrus_sasl as cyrus_sasl;

// Procmail — mail filtering: recipes, rules, variables, includes, config, logs (dedicated crate)
pub use sorng_procmail as procmail;

// SpamAssassin — spam filtering: rules, scores, Bayes, channels, whitelist, plugins, spamd, lint (dedicated crate)
pub use sorng_spamassassin as spamassassin;

// Rspamd — modern spam filter: scanning, stats, symbols, actions, maps, history, workers, fuzzy, Bayes (dedicated crate)
pub use sorng_rspamd as rspamd;

// ClamAV — antivirus: scanning, databases, quarantine, clamd/freshclam config, on-access, milter, scheduled scans (dedicated crate)
pub use sorng_clamav as clamav;

// Roundcube — webmail admin: users, identities, contacts, folders, filters, plugins, config, SMTP/IMAP (dedicated crate)
pub use sorng_roundcube as roundcube;

// Mailcow — all-in-one mail stack: domains, mailboxes, aliases, DKIM, transports, queue, quarantine, logs, status (dedicated crate)
pub use sorng_mailcow as mailcow;

// Amavis — content filter: config, policy banks, banned rules, whitelists/blacklists, quarantine, stats, process (dedicated crate)
...
// cPanel — WHM/cPanel web hosting: accounts, domains, email, databases, DNS, files, SSL, backups, FTP, cron, stats, PHP, security (dedicated crate)
pub use sorng_cpanel as cpanel;

// PHP — PHP server management: versions, FPM pools, php.ini, modules/extensions, OPcache, sessions, Composer, logs, process control (dedicated crate)
pub use sorng_php as php_mgmt;

// pfSense — firewall/router management: interfaces, firewall rules, NAT, DHCP, DNS, VPN, routing, services, certs, users, diagnostics, backups
pub use sorng_pfsense as pfsense;

// MySQL Admin — MySQL server administration: users, replication, databases, tables, queries, InnoDB, variables, backups, processes, binlogs
pub use sorng_mysql_admin as mysql_admin;

// PostgreSQL Admin — PostgreSQL server administration: roles, databases, HBA, replication, vacuum, extensions, stats, WAL, tablespaces, schemas, backups
pub use sorng_postgres_admin as pg_admin;

// Prometheus — monitoring/alerting: queries, targets, rules, alerts, config, TSDB, metadata, federation, silences
pub use sorng_prometheus as prometheus;

// Grafana — observability dashboards: dashboards, datasources, folders, orgs, users, teams, alerts, annotations, playlists, snapshots
pub use sorng_grafana as grafana;

// UPS — uninterruptible power supply management via NUT: devices, status, battery, events, outlets, schedules, thresholds, tests, config, notifications
pub use sorng_ups as ups_mgmt;

// NetBox — DCIM/IPAM: sites, racks, devices, interfaces, IPs, prefixes, VLANs, circuits, cables, tenants, contacts, VMs, clusters
...
use mysql_admin::service::MysqlAdminServiceState;
use pg_admin::service::PostgresAdminServiceState;
...
pub fn run() {
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      // Initialize Postfix service
      let postfix_state: PostfixServiceState = Arc::new(Mutex::new(postfix::service::PostfixService::new()));
      app.manage(postfix_state);

      // Initialize Dovecot service
      let dovecot_state: DovecotServiceState = Arc::new(Mutex::new(dovecot::service::DovecotServiceFacade::new()));
      app.manage(dovecot_state);

      // Initialize OpenDKIM service
      let opendkim_state: OpendkimServiceState = Arc::new(Mutex::new(opendkim::service::OpendkimService::new()));
      app.manage(opendkim_state);

      // Initialize Cyrus SASL service
      let cyrus_sasl_state: CyrusSaslServiceState = Arc::new(Mutex::new(cyrus_sasl::service::CyrusSaslService::new()));
      app.manage(cyrus_sasl_state);

      // Initialize Procmail service
      let procmail_state: ProcmailServiceState = Arc::new(Mutex::new(procmail::service::ProcmailService::new()));
      app.manage(procmail_state);

      // Initialize SpamAssassin service
      let spamassassin_state: SpamAssassinServiceState = Arc::new(Mutex::new(spamassassin::service::SpamAssassinService::new()));
      app.manage(spamassassin_state);

      // Initialize Rspamd service
      let rspamd_state: RspamdServiceState = Arc::new(Mutex::new(rspamd::service::RspamdService::new()));
      app.manage(rspamd_state);

      // Initialize ClamAV service
      let clamav_state: ClamavServiceState = Arc::new(Mutex::new(clamav::service::ClamavService::new()));
      app.manage(clamav_state);

      // Initialize Roundcube service
      let roundcube_state: RoundcubeServiceState = Arc::new(Mutex::new(roundcube::service::RoundcubeService::new()));
      app.manage(roundcube_state);

      // Initialize Mailcow service
      let mailcow_state: MailcowServiceState = Arc::new(Mutex::new(mailcow::service::MailcowService::new()));
      ...
      // Initialize cPanel service
      let cpanel_state: CpanelServiceState = Arc::new(Mutex::new(cpanel::service::CpanelService::new()));
      app.manage(cpanel_state);

      // Initialize PHP management service
      let php_state: PhpServiceState = Arc::new(Mutex::new(php_mgmt::service::PhpService::new()));
      app.manage(php_state);

      // Initialize pfSense service
      let pfsense_state: PfsenseServiceState = Arc::new(Mutex::new(pfsense::service::PfsenseServiceWrapper::new()));
      app.manage(pfsense_state);

      // Initialize MySQL admin service
      let mysql_admin_state: MysqlAdminServiceState = Arc::new(Mutex::new(mysql_admin::service::MysqlAdminService::new()));
      app.manage(mysql_admin_state);

      // Initialize PostgreSQL admin service
      let pg_admin_state: PostgresAdminServiceState = Arc::new(Mutex::new(pg_admin::service::PostgresAdminService::new()));
      app.manage(pg_admin_state);

      // Initialize Prometheus service
      let prometheus_state: PrometheusServiceState = Arc::new(Mutex::new(prometheus::service::PrometheusService::new()));
      app.manage(prometheus_state);

      // Initialize Grafana service
      let grafana_state: GrafanaServiceState = Arc::new(Mutex::new(grafana::service::GrafanaService::new()));
      app.manage(grafana_state);

      // Initialize UPS management service
      ...
      let netbox_state: NetboxServiceState = Arc::new(Mutex::new(netbox::service::NetboxService::new()));
      app.manage(netbox_state);

      // Initialize port knock service
    ...
    .invoke_handler(tauri::generate_handler![
        ...
        // cPanel
        cpanel::commands::cpanel_connect,
        cpanel::commands::cpanel_disconnect,
        cpanel::commands::cpanel_list_connections,
        cpanel::commands::cpanel_ping,
        cpanel::commands::cpanel_list_accounts,
        cpanel::commands::cpanel_get_account,
        cpanel::commands::cpanel_create_account,
        cpanel::commands::cpanel_suspend_account,
        cpanel::commands::cpanel_unsuspend_account,
        cpanel::commands::cpanel_terminate_account,
        cpanel::commands::cpanel_modify_account,
        cpanel::commands::cpanel_change_account_password,
        cpanel::commands::cpanel_list_packages,
        cpanel::commands::cpanel_get_account_summary,
        cpanel::commands::cpanel_list_suspended_accounts,
        cpanel::commands::cpanel_get_server_info,
        cpanel::commands::cpanel_list_domains,
        cpanel::commands::cpanel_list_all_domains,
        cpanel::commands::cpanel_create_addon_domain,
        cpanel::commands::cpanel_remove_addon_domain,
        cpanel::commands::cpanel_create_subdomain,
        cpanel::commands::cpanel_remove_subdomain,
        cpanel::commands::cpanel_park_domain,
        cpanel::commands::cpanel_unpark_domain,
        cpanel::commands::cpanel_list_email_accounts,
        cpanel::commands::cpanel_create_email_account,
        cpanel::commands::cpanel_delete_email_account,
        cpanel::commands::cpanel_change_email_password,
        cpanel::commands::cpanel_set_email_quota,
        cpanel::commands::cpanel_list_forwarders,
        cpanel::commands::cpanel_add_forwarder,
        cpanel::commands::cpanel_delete_forwarder,
        cpanel::commands::cpanel_list_autoresponders,
        cpanel::commands::cpanel_list_mailing_lists,
        cpanel::commands::cpanel_get_spam_settings,
        cpanel::commands::cpanel_list_mx_records,
        cpanel::commands::cpanel_list_databases,
        cpanel::commands::cpanel_create_database,
        cpanel::commands::cpanel_delete_database,
        cpanel::commands::cpanel_list_database_users,
        cpanel::commands::cpanel_create_database_user,
        cpanel::commands::cpanel_delete_database_user,
        cpanel::commands::cpanel_grant_database_privileges,
        cpanel::commands::cpanel_list_dns_zones,
        cpanel::commands::cpanel_get_dns_zone,
        cpanel::commands::cpanel_add_dns_record,
        cpanel::commands::cpanel_edit_dns_record,
        cpanel::commands::cpanel_remove_dns_record,
        cpanel::commands::cpanel_list_files,
        cpanel::commands::cpanel_create_directory,
        cpanel::commands::cpanel_delete_file,
        cpanel::commands::cpanel_get_disk_usage,
        cpanel::commands::cpanel_list_ssl_certs,
        cpanel::commands::cpanel_get_ssl_status,
        cpanel::commands::cpanel_install_ssl,
        cpanel::commands::cpanel_generate_csr,
        cpanel::commands::cpanel_autossl_check,
        cpanel::commands::cpanel_list_backups,
        cpanel::commands::cpanel_create_full_backup,
        cpanel::commands::cpanel_restore_file,
        cpanel::commands::cpanel_get_backup_config,
        cpanel::commands::cpanel_trigger_server_backup,
        cpanel::commands::cpanel_list_ftp_accounts,
        cpanel::commands::cpanel_create_ftp_account,
        cpanel::commands::cpanel_delete_ftp_account,
        cpanel::commands::cpanel_list_ftp_sessions,
        cpanel::commands::cpanel_list_cron_jobs,
        cpanel::commands::cpanel_add_cron_job,
        cpanel::commands::cpanel_edit_cron_job,
        cpanel::commands::cpanel_delete_cron_job,
        cpanel::commands::cpanel_get_bandwidth,
        cpanel::commands::cpanel_get_resource_usage,
        cpanel::commands::cpanel_get_error_log,
        cpanel::commands::cpanel_get_server_load,
        cpanel::commands::cpanel_list_php_versions,
        cpanel::commands::cpanel_get_domain_php_version,
        cpanel::commands::cpanel_set_domain_php_version,
        cpanel::commands::cpanel_get_php_config,
        cpanel::commands::cpanel_list_php_extensions,
        cpanel::commands::cpanel_list_blocked_ips,
        cpanel::commands::cpanel_block_ip,
        cpanel::commands::cpanel_unblock_ip,
        cpanel::commands::cpanel_list_ssh_keys,
        cpanel::commands::cpanel_import_ssh_key,
        cpanel::commands::cpanel_delete_ssh_key,
        cpanel::commands::cpanel_get_modsec_status,
        cpanel::commands::cpanel_set_modsec,
        // PHP management commands
        php_mgmt::commands::php_connect,
        php_mgmt::commands::php_disconnect,
        php_mgmt::commands::php_list_connections,
        php_mgmt::commands::php_list_versions,
        php_mgmt::commands::php_get_default_version,
        php_mgmt::commands::php_get_version_detail,
        php_mgmt::commands::php_set_default_version,
        php_mgmt::commands::php_list_sapis,
        php_mgmt::commands::php_get_config_path,
        php_mgmt::commands::php_get_extension_dir,
        php_mgmt::commands::php_check_version_installed,
        php_mgmt::commands::php_list_fpm_pools,
        php_mgmt::commands::php_get_fpm_pool,
        php_mgmt::commands::php_create_fpm_pool,
        php_mgmt::commands::php_update_fpm_pool,
        php_mgmt::commands::php_delete_fpm_pool,
        php_mgmt::commands::php_enable_fpm_pool,
        php_mgmt::commands::php_disable_fpm_pool,
        php_mgmt::commands::php_get_fpm_pool_status,
        php_mgmt::commands::php_list_fpm_pool_processes,
        php_mgmt::commands::php_get_ini_file,
        php_mgmt::commands::php_list_ini_directives,
        php_mgmt::commands::php_get_ini_directive,
        php_mgmt::commands::php_set_ini_directive,
        php_mgmt::commands::php_remove_ini_directive,
        php_mgmt::commands::php_get_ini_scan_dir,
        php_mgmt::commands::php_list_loaded_ini_files,
        php_mgmt::commands::php_backup_ini,
        php_mgmt::commands::php_restore_ini,
        php_mgmt::commands::php_validate_ini,
        php_mgmt::commands::php_list_modules,
        php_mgmt::commands::php_get_module,
        php_mgmt::commands::php_enable_module,
        php_mgmt::commands::php_disable_module,
        php_mgmt::commands::php_install_module,
        php_mgmt::commands::php_uninstall_module,
        php_mgmt::commands::php_is_module_loaded,
        php_mgmt::commands::php_list_available_modules,
        php_mgmt::commands::php_list_pecl_packages,
        php_mgmt::commands::php_install_pecl_package,
        php_mgmt::commands::php_uninstall_pecl_package,
        php_mgmt::commands::php_get_opcache_status,
        php_mgmt::commands::php_get_opcache_config,
        php_mgmt::commands::php_reset_opcache,
        php_mgmt::commands::php_list_cached_scripts,
        php_mgmt::commands::php_invalidate_cached_script,
        php_mgmt::commands::php_is_opcache_enabled,
        php_mgmt::commands::php_update_opcache_config,
        php_mgmt::commands::php_get_session_config,
        php_mgmt::commands::php_update_session_config,
        php_mgmt::commands::php_get_session_stats,
        php_mgmt::commands::php_cleanup_sessions,
        php_mgmt::commands::php_list_session_files,
        php_mgmt::commands::php_get_session_save_path,
        php_mgmt::commands::php_get_composer_info,
        php_mgmt::commands::php_is_composer_installed,
        php_mgmt::commands::php_list_composer_global_packages,
        php_mgmt::commands::php_install_composer_global_package,
        php_mgmt::commands::php_remove_composer_global_package,
        php_mgmt::commands::php_get_composer_project,
        php_mgmt::commands::php_composer_install,
        php_mgmt::commands::php_composer_update,
        php_mgmt::commands::php_composer_require,
        php_mgmt::commands::php_composer_remove,
        php_mgmt::commands::php_composer_dump_autoload,
        php_mgmt::commands::php_composer_validate,
        php_mgmt::commands::php_composer_outdated,
        php_mgmt::commands::php_composer_clear_cache,
        php_mgmt::commands::php_composer_self_update,
        php_mgmt::commands::php_read_log,
        php_mgmt::commands::php_get_log_config,
        php_mgmt::commands::php_get_fpm_log_config,
        php_mgmt::commands::php_get_log_path,
        php_mgmt::commands::php_get_fpm_log_path,
        php_mgmt::commands::php_clear_log,
        php_mgmt::commands::php_tail_log,
        php_mgmt::commands::php_get_log_size,
        php_mgmt::commands::php_rotate_log,
        php_mgmt::commands::php_get_fpm_service_status,
        php_mgmt::commands::php_start_fpm,
        php_mgmt::commands::php_stop_fpm,
        php_mgmt::commands::php_restart_fpm,
        php_mgmt::commands::php_reload_fpm,
        php_mgmt::commands::php_enable_fpm,
        php_mgmt::commands::php_disable_fpm,
        php_mgmt::commands::php_test_fpm_config,
        php_mgmt::commands::php_get_fpm_master_process,
        php_mgmt::commands::php_list_fpm_worker_pids,
        php_mgmt::commands::php_graceful_restart_fpm,
        php_mgmt::commands::php_reopen_fpm_logs,
        php_mgmt::commands::php_list_all_fpm_services,
        // pfSense commands
        pfsense::commands::pfsense_connect,
        pfsense::commands::pfsense_disconnect,
        pfsense::commands::pfsense_list_connections,
        pfsense::commands::pfsense_list_interfaces,
        pfsense::commands::pfsense_get_interface,
        pfsense::commands::pfsense_update_interface,
        pfsense::commands::pfsense_apply_interface_changes,
        pfsense::commands::pfsense_get_interface_stats,
        pfsense::commands::pfsense_list_firewall_rules,
        pfsense::commands::pfsense_get_firewall_rule,
        pfsense::commands::pfsense_create_firewall_rule,
        pfsense::commands::pfsense_update_firewall_rule,
        pfsense::commands::pfsense_delete_firewall_rule,
        pfsense::commands::pfsense_list_firewall_aliases,
        pfsense::commands::pfsense_get_firewall_alias,
        pfsense::commands::pfsense_create_firewall_alias,
        pfsense::commands::pfsense_update_firewall_alias,
        pfsense::commands::pfsense_delete_firewall_alias,
        pfsense::commands::pfsense_get_firewall_states,
        pfsense::commands::pfsense_flush_firewall_states,
        pfsense::commands::pfsense_list_nat_port_forwards,
        pfsense::commands::pfsense_create_nat_port_forward,
        pfsense::commands::pfsense_update_nat_port_forward,
        pfsense::commands::pfsense_delete_nat_port_forward,
        pfsense::commands::pfsense_list_nat_outbound,
        pfsense::commands::pfsense_create_nat_outbound,
        pfsense::commands::pfsense_update_nat_outbound,
        pfsense::commands::pfsense_delete_nat_outbound,
        pfsense::commands::pfsense_list_nat_1to1,
        pfsense::commands::pfsense_create_nat_1to1,
        pfsense::commands::pfsense_update_nat_1to1,
        pfsense::commands::pfsense_delete_nat_1to1,
        pfsense::commands::pfsense_get_dhcp_config,
        pfsense::commands::pfsense_update_dhcp_config,
        pfsense::commands::pfsense_list_dhcp_leases,
        pfsense::commands::pfsense_list_dhcp_static_mappings,
        pfsense::commands::pfsense_create_dhcp_static_mapping,
        pfsense::commands::pfsense_update_dhcp_static_mapping,
        pfsense::commands::pfsense_delete_dhcp_static_mapping,
        pfsense::commands::pfsense_get_dhcp_relay,
        pfsense::commands::pfsense_update_dhcp_relay,
        pfsense::commands::pfsense_get_dns_resolver_config,
        pfsense::commands::pfsense_update_dns_resolver_config,
        pfsense::commands::pfsense_get_dns_forwarder_config,
        pfsense::commands::pfsense_update_dns_forwarder_config,
        pfsense::commands::pfsense_list_dns_host_overrides,
        pfsense::commands::pfsense_create_dns_host_override,
        pfsense::commands::pfsense_update_dns_host_override,
        pfsense::commands::pfsense_delete_dns_host_override,
        pfsense::commands::pfsense_list_dns_domain_overrides,
        pfsense::commands::pfsense_create_dns_domain_override,
        pfsense::commands::pfsense_update_dns_domain_override,
        pfsense::commands::pfsense_delete_dns_domain_override,
        pfsense::commands::pfsense_flush_dns_cache,
        pfsense::commands::pfsense_get_dns_cache_stats,
        pfsense::commands::pfsense_list_openvpn_servers,
        pfsense::commands::pfsense_get_openvpn_server,
        pfsense::commands::pfsense_create_openvpn_server,
        pfsense::commands::pfsense_update_openvpn_server,
        pfsense::commands::pfsense_delete_openvpn_server,
        pfsense::commands::pfsense_list_openvpn_clients,
        pfsense::commands::pfsense_get_openvpn_client,
        pfsense::commands::pfsense_create_openvpn_client,
        pfsense::commands::pfsense_update_openvpn_client,
        pfsense::commands::pfsense_delete_openvpn_client,
        pfsense::commands::pfsense_list_ipsec_tunnels,
        pfsense::commands::pfsense_get_ipsec_tunnel,
        pfsense::commands::pfsense_create_ipsec_tunnel,
        pfsense::commands::pfsense_update_ipsec_tunnel,
        pfsense::commands::pfsense_delete_ipsec_tunnel,
        pfsense::commands::pfsense_list_wireguard_tunnels,
        pfsense::commands::pfsense_get_wireguard_tunnel,
        pfsense::commands::pfsense_create_wireguard_tunnel,
        pfsense::commands::pfsense_update_wireguard_tunnel,
        pfsense::commands::pfsense_delete_wireguard_tunnel,
        pfsense::commands::pfsense_list_wireguard_peers,
        pfsense::commands::pfsense_create_wireguard_peer,
        pfsense::commands::pfsense_update_wireguard_peer,
        pfsense::commands::pfsense_delete_wireguard_peer,
        pfsense::commands::pfsense_list_routes,
        pfsense::commands::pfsense_get_route,
        pfsense::commands::pfsense_create_route,
        pfsense::commands::pfsense_update_route,
        pfsense::commands::pfsense_delete_route,
        pfsense::commands::pfsense_list_gateways,
        pfsense::commands::pfsense_get_gateway,
        pfsense::commands::pfsense_create_gateway,
        pfsense::commands::pfsense_update_gateway,
        pfsense::commands::pfsense_delete_gateway,
        pfsense::commands::pfsense_list_gateway_groups,
        pfsense::commands::pfsense_get_gateway_group,
        pfsense::commands::pfsense_create_gateway_group,
        pfsense::commands::pfsense_update_gateway_group,
        pfsense::commands::pfsense_delete_gateway_group,
        pfsense::commands::pfsense_get_gateway_status,
        pfsense::commands::pfsense_get_routing_table,
        pfsense::commands::pfsense_list_services,
        pfsense::commands::pfsense_get_service_status,
        pfsense::commands::pfsense_start_service,
        pfsense::commands::pfsense_stop_service,
        pfsense::commands::pfsense_restart_service,
        pfsense::commands::pfsense_get_system_info,
        pfsense::commands::pfsense_get_system_updates,
        pfsense::commands::pfsense_get_general_config,
        pfsense::commands::pfsense_update_general_config,
        pfsense::commands::pfsense_get_advanced_config,
        pfsense::commands::pfsense_update_advanced_config,
        pfsense::commands::pfsense_reboot,
        pfsense::commands::pfsense_halt,
        pfsense::commands::pfsense_list_cas,
        pfsense::commands::pfsense_get_ca,
        pfsense::commands::pfsense_create_ca,
        pfsense::commands::pfsense_delete_ca,
        pfsense::commands::pfsense_list_certs,
        pfsense::commands::pfsense_get_cert,
        pfsense::commands::pfsense_create_cert,
        pfsense::commands::pfsense_delete_cert,
        pfsense::commands::pfsense_generate_csr,
        pfsense::commands::pfsense_list_users,
        pfsense::commands::pfsense_get_user,
        pfsense::commands::pfsense_create_user,
        pfsense::commands::pfsense_update_user,
        pfsense::commands::pfsense_delete_user,
        pfsense::commands::pfsense_list_groups,
        pfsense::commands::pfsense_get_group,
        pfsense::commands::pfsense_create_group,
        pfsense::commands::pfsense_update_group,
        pfsense::commands::pfsense_delete_group,
        pfsense::commands::pfsense_list_privileges,
        pfsense::commands::pfsense_get_arp_table,
        pfsense::commands::pfsense_flush_arp_table,
        pfsense::commands::pfsense_get_ndp_table,
        pfsense::commands::pfsense_dns_lookup,
        pfsense::commands::pfsense_ping,
        pfsense::commands::pfsense_traceroute,
        pfsense::commands::pfsense_get_if_stats,
        pfsense::commands::pfsense_get_pfinfo,
        pfsense::commands::pfsense_get_system_log,
        pfsense::commands::pfsense_list_backups,
        pfsense::commands::pfsense_create_backup,
        pfsense::commands::pfsense_download_backup,
        pfsense::commands::pfsense_restore_backup,
        pfsense::commands::pfsense_delete_backup,
        // MySQL admin commands
        mysql_admin::commands::mysql_admin_connect,
        mysql_admin::commands::mysql_admin_disconnect,
        mysql_admin::commands::mysql_admin_list_connections,
        mysql_admin::commands::mysql_admin_list_users,
        mysql_admin::commands::mysql_admin_get_user,
        mysql_admin::commands::mysql_admin_create_user,
        mysql_admin::commands::mysql_admin_drop_user,
        mysql_admin::commands::mysql_admin_set_password,
        mysql_admin::commands::mysql_admin_grant_privilege,
        mysql_admin::commands::mysql_admin_revoke_privilege,
        mysql_admin::commands::mysql_admin_show_grants,
        mysql_admin::commands::mysql_admin_flush_privileges,
        mysql_admin::commands::mysql_admin_get_replication_status,
        mysql_admin::commands::mysql_admin_get_slave_status,
        mysql_admin::commands::mysql_admin_start_slave,
        mysql_admin::commands::mysql_admin_stop_slave,
        mysql_admin::commands::mysql_admin_reset_slave,
        mysql_admin::commands::mysql_admin_change_master,
        mysql_admin::commands::mysql_admin_skip_counter,
        mysql_admin::commands::mysql_admin_list_databases,
        mysql_admin::commands::mysql_admin_create_database,
        mysql_admin::commands::mysql_admin_drop_database,
        mysql_admin::commands::mysql_admin_get_database_size,
        mysql_admin::commands::mysql_admin_get_database_tables,
        mysql_admin::commands::mysql_admin_get_all_sizes,
        mysql_admin::commands::mysql_admin_list_tables,
        mysql_admin::commands::mysql_admin_describe_table,
        mysql_admin::commands::mysql_admin_get_table_status,
        mysql_admin::commands::mysql_admin_get_create_table,
        mysql_admin::commands::mysql_admin_optimize_table,
        mysql_admin::commands::mysql_admin_repair_table,
        mysql_admin::commands::mysql_admin_analyze_table,
        mysql_admin::commands::mysql_admin_check_table,
        mysql_admin::commands::mysql_admin_get_indexes,
        mysql_admin::commands::mysql_admin_run_query,
        mysql_admin::commands::mysql_admin_explain_query,
        mysql_admin::commands::mysql_admin_get_slow_queries,
        mysql_admin::commands::mysql_admin_get_innodb_status,
        mysql_admin::commands::mysql_admin_get_innodb_metrics,
        mysql_admin::commands::mysql_admin_get_buffer_pool_stats,
        mysql_admin::commands::mysql_admin_get_variable,
        mysql_admin::commands::mysql_admin_set_variable,
        mysql_admin::commands::mysql_admin_get_all_variables,
        mysql_admin::commands::mysql_admin_get_global_status,
        mysql_admin::commands::mysql_admin_create_backup,
        mysql_admin::commands::mysql_admin_restore_backup,
        mysql_admin::commands::mysql_admin_list_backup_files,
        mysql_admin::commands::mysql_admin_list_processes,
        mysql_admin::commands::mysql_admin_kill_process,
        mysql_admin::commands::mysql_admin_get_process_info,
        mysql_admin::commands::mysql_admin_list_binlogs,
        mysql_admin::commands::mysql_admin_get_binlog_events,
        mysql_admin::commands::mysql_admin_purge_binlogs,
        mysql_admin::commands::mysql_admin_get_binlog_position,
        // PostgreSQL admin commands
        pg_admin::commands::pg_admin_connect,
        pg_admin::commands::pg_admin_disconnect,
        pg_admin::commands::pg_admin_list_connections,
        pg_admin::commands::pg_admin_list_roles,
        pg_admin::commands::pg_admin_get_role,
        pg_admin::commands::pg_admin_create_role,
        pg_admin::commands::pg_admin_drop_role,
        pg_admin::commands::pg_admin_alter_role,
        pg_admin::commands::pg_admin_set_role_password,
        pg_admin::commands::pg_admin_grant_role,
        pg_admin::commands::pg_admin_revoke_role,
        pg_admin::commands::pg_admin_list_databases,
        pg_admin::commands::pg_admin_get_database,
        pg_admin::commands::pg_admin_create_database,
        pg_admin::commands::pg_admin_drop_database,
        pg_admin::commands::pg_admin_get_database_size,
        pg_admin::commands::pg_admin_get_all_sizes,
        pg_admin::commands::pg_admin_list_hba_rules,
        pg_admin::commands::pg_admin_get_hba_config,
        pg_admin::commands::pg_admin_add_hba_rule,
        pg_admin::commands::pg_admin_remove_hba_rule,
        pg_admin::commands::pg_admin_update_hba_rule,
        pg_admin::commands::pg_admin_reload_hba,
        pg_admin::commands::pg_admin_get_replication_status,
        pg_admin::commands::pg_admin_list_replication_slots,
        pg_admin::commands::pg_admin_create_replication_slot,
        pg_admin::commands::pg_admin_drop_replication_slot,
        pg_admin::commands::pg_admin_get_replication_lag,
        pg_admin::commands::pg_admin_vacuum_table,
        pg_admin::commands::pg_admin_vacuum_database,
        pg_admin::commands::pg_admin_analyze_table,
        pg_admin::commands::pg_admin_analyze_database,
        pg_admin::commands::pg_admin_get_bloat,
        pg_admin::commands::pg_admin_list_extensions,
        pg_admin::commands::pg_admin_install_extension,
        pg_admin::commands::pg_admin_uninstall_extension,
        pg_admin::commands::pg_admin_get_extension,
        pg_admin::commands::pg_admin_list_available_extensions,
        pg_admin::commands::pg_admin_get_stats,
        pg_admin::commands::pg_admin_get_table_stats,
        pg_admin::commands::pg_admin_get_index_stats,
        pg_admin::commands::pg_admin_get_connection_stats,
        pg_admin::commands::pg_admin_get_bgwriter_stats,
        pg_admin::commands::pg_admin_get_cache_hit_ratio,
        pg_admin::commands::pg_admin_reset_stats,
        pg_admin::commands::pg_admin_get_wal_status,
        pg_admin::commands::pg_admin_list_wal_files,
        pg_admin::commands::pg_admin_get_wal_settings,
        pg_admin::commands::pg_admin_switch_wal,
        pg_admin::commands::pg_admin_create_checkpoint,
        pg_admin::commands::pg_admin_list_tablespaces,
        pg_admin::commands::pg_admin_get_tablespace,
        pg_admin::commands::pg_admin_create_tablespace,
        pg_admin::commands::pg_admin_drop_tablespace,
        pg_admin::commands::pg_admin_get_tablespace_size,
        pg_admin::commands::pg_admin_list_schemas,
        pg_admin::commands::pg_admin_get_schema,
        pg_admin::commands::pg_admin_create_schema,
        pg_admin::commands::pg_admin_drop_schema,
        pg_admin::commands::pg_admin_get_schema_size,
        pg_admin::commands::pg_admin_list_schema_tables,
        pg_admin::commands::pg_admin_create_backup,
        pg_admin::commands::pg_admin_restore_backup,
        pg_admin::commands::pg_admin_list_backup_files,
        pg_admin::commands::pg_admin_create_base_backup,
        // Prometheus commands
        prometheus::commands::prometheus_connect,
        prometheus::commands::prometheus_disconnect,
        prometheus::commands::prometheus_list_connections,
        prometheus::commands::prometheus_instant_query,
        prometheus::commands::prometheus_range_query,
        prometheus::commands::prometheus_label_values,
        prometheus::commands::prometheus_label_names,
        prometheus::commands::prometheus_series,
        prometheus::commands::prometheus_list_targets,
        prometheus::commands::prometheus_get_target_health,
        prometheus::commands::prometheus_get_active_targets,
        prometheus::commands::prometheus_get_dropped_targets,
        prometheus::commands::prometheus_list_rules,
        prometheus::commands::prometheus_get_alerting_rules,
        prometheus::commands::prometheus_get_recording_rules,
        prometheus::commands::prometheus_list_alerts,
        prometheus::commands::prometheus_get_active_alerts,
        prometheus::commands::prometheus_get_config,
        prometheus::commands::prometheus_reload_config,
        prometheus::commands::prometheus_get_flags,
        prometheus::commands::prometheus_get_tsdb_status,
        prometheus::commands::prometheus_get_tsdb_stats,
        prometheus::commands::prometheus_create_snapshot,
        prometheus::commands::prometheus_delete_series,
        prometheus::commands::prometheus_clean_tombstones,
        prometheus::commands::prometheus_list_metadata,
        prometheus::commands::prometheus_get_metric_metadata,
        prometheus::commands::prometheus_get_metric_type,
        prometheus::commands::prometheus_get_exemplars,
        prometheus::commands::prometheus_federate,
        prometheus::commands::prometheus_list_recording_rules,
        prometheus::commands::prometheus_get_recording_rule,
        prometheus::commands::prometheus_list_silences,
        prometheus::commands::prometheus_get_silence,
        prometheus::commands::prometheus_create_silence,
        prometheus::commands::prometheus_delete_silence,
        // Grafana commands
        grafana::commands::grafana_connect,
        grafana::commands::grafana_disconnect,
        grafana::commands::grafana_list_connections,
        grafana::commands::grafana_search_dashboards,
        grafana::commands::grafana_get_dashboard,
        grafana::commands::grafana_create_dashboard,
        grafana::commands::grafana_update_dashboard,
        grafana::commands::grafana_delete_dashboard,
        grafana::commands::grafana_get_dashboard_versions,
        grafana::commands::grafana_get_home_dashboard,
        grafana::commands::grafana_list_datasources,
        grafana::commands::grafana_get_datasource,
        grafana::commands::grafana_create_datasource,
        grafana::commands::grafana_update_datasource,
        grafana::commands::grafana_delete_datasource,
        grafana::commands::grafana_test_datasource,
        grafana::commands::grafana_get_datasource_by_name,
        grafana::commands::grafana_list_folders,
        grafana::commands::grafana_get_folder,
        grafana::commands::grafana_create_folder,
        grafana::commands::grafana_update_folder,
        grafana::commands::grafana_delete_folder,
        grafana::commands::grafana_get_current_org,
        grafana::commands::grafana_list_orgs,
        grafana::commands::grafana_get_org,
        grafana::commands::grafana_create_org,
        grafana::commands::grafana_update_org,
        grafana::commands::grafana_delete_org,
        grafana::commands::grafana_list_org_users,
        grafana::commands::grafana_add_org_user,
        grafana::commands::grafana_remove_org_user,
        grafana::commands::grafana_list_users,
        grafana::commands::grafana_get_user,
        grafana::commands::grafana_create_user,
        grafana::commands::grafana_update_user,
        grafana::commands::grafana_delete_user,
        grafana::commands::grafana_search_users,
        grafana::commands::grafana_list_teams,
        grafana::commands::grafana_get_team,
        grafana::commands::grafana_create_team,
        grafana::commands::grafana_update_team,
        grafana::commands::grafana_delete_team,
        grafana::commands::grafana_list_team_members,
        grafana::commands::grafana_add_team_member,
        grafana::commands::grafana_remove_team_member,
        grafana::commands::grafana_list_alert_rules,
        grafana::commands::grafana_get_alert_rule,
        grafana::commands::grafana_create_alert_rule,
        grafana::commands::grafana_update_alert_rule,
        grafana::commands::grafana_delete_alert_rule,
        grafana::commands::grafana_pause_alert_rule,
        grafana::commands::grafana_list_annotations,
        grafana::commands::grafana_create_annotation,
        grafana::commands::grafana_update_annotation,
        grafana::commands::grafana_delete_annotation,
        grafana::commands::grafana_list_playlists,
        grafana::commands::grafana_get_playlist,
        grafana::commands::grafana_create_playlist,
        grafana::commands::grafana_update_playlist,
        grafana::commands::grafana_delete_playlist,
        grafana::commands::grafana_get_playlist_items,
        grafana::commands::grafana_list_snapshots,
        grafana::commands::grafana_get_snapshot,
        grafana::commands::grafana_create_snapshot,
        grafana::commands::grafana_delete_snapshot,
        grafana::commands::grafana_get_snapshot_by_key,
        grafana::commands::grafana_list_panel_types,
        grafana::commands::grafana_get_panel_schema,
        // UPS management commands
        ups_mgmt::commands::ups_connect,
        ups_mgmt::commands::ups_disconnect,
        ups_mgmt::commands::ups_list_connections,
        ups_mgmt::commands::ups_list_devices,
        ups_mgmt::commands::ups_get_device,
        ups_mgmt::commands::ups_list_device_variables,
        ups_mgmt::commands::ups_get_device_variable,
        ups_mgmt::commands::ups_set_device_variable,
        ups_mgmt::commands::ups_list_device_commands,
        ups_mgmt::commands::ups_run_device_command,
        ups_mgmt::commands::ups_get_status,
        ups_mgmt::commands::ups_is_on_battery,
        ups_mgmt::commands::ups_is_online,
        ups_mgmt::commands::ups_get_load,
        ups_mgmt::commands::ups_get_input_voltage,
        ups_mgmt::commands::ups_get_output_voltage,
        ups_mgmt::commands::ups_get_temperature,
        ups_mgmt::commands::ups_list_all_status,
        ups_mgmt::commands::ups_get_battery_info,
        ups_mgmt::commands::ups_get_battery_charge,
        ups_mgmt::commands::ups_get_battery_runtime,
        ups_mgmt::commands::ups_get_battery_voltage,
        ups_mgmt::commands::ups_is_battery_low,
        ups_mgmt::commands::ups_battery_needs_replacement,
        ups_mgmt::commands::ups_get_battery_health,
        ups_mgmt::commands::ups_list_events,
        ups_mgmt::commands::ups_get_recent_events,
        ups_mgmt::commands::ups_clear_event_log,
        ups_mgmt::commands::ups_list_outlets,
        ups_mgmt::commands::ups_get_outlet,
        ups_mgmt::commands::ups_switch_outlet_on,
        ups_mgmt::commands::ups_switch_outlet_off,
        ups_mgmt::commands::ups_get_outlet_delay,
        ups_mgmt::commands::ups_set_outlet_delay,
        ups_mgmt::commands::ups_list_schedules,
        ups_mgmt::commands::ups_get_schedule,
        ups_mgmt::commands::ups_create_schedule,
        ups_mgmt::commands::ups_update_schedule,
        ups_mgmt::commands::ups_delete_schedule,
        ups_mgmt::commands::ups_enable_schedule,
        ups_mgmt::commands::ups_disable_schedule,
        ups_mgmt::commands::ups_list_thresholds,
        ups_mgmt::commands::ups_get_threshold,
        ups_mgmt::commands::ups_set_threshold,
        ups_mgmt::commands::ups_get_low_battery_threshold,
        ups_mgmt::commands::ups_set_low_battery_threshold,
        ups_mgmt::commands::ups_quick_test,
        ups_mgmt::commands::ups_deep_test,
        ups_mgmt::commands::ups_abort_test,
        ups_mgmt::commands::ups_get_last_test_result,
        ups_mgmt::commands::ups_calibrate_battery,
        ups_mgmt::commands::ups_get_test_history,
        ups_mgmt::commands::ups_get_nut_config,
        ups_mgmt::commands::ups_get_ups_conf,
        ups_mgmt::commands::ups_set_ups_conf,
        ups_mgmt::commands::ups_get_upsd_conf,
        ups_mgmt::commands::ups_set_upsd_conf,
        ups_mgmt::commands::ups_reload_upsd,
        ups_mgmt::commands::ups_reload_upsmon,
        ups_mgmt::commands::ups_restart_nut,
        ups_mgmt::commands::ups_get_nut_mode,
        ups_mgmt::commands::ups_set_nut_mode,
        ups_mgmt::commands::ups_list_notifications,
        ups_mgmt::commands::ups_get_notify_flags,
        ups_mgmt::commands::ups_set_notify_flags,
        ups_mgmt::commands::ups_get_notify_message,
        ups_mgmt::commands::ups_set_notify_message,
        ups_mgmt::commands::ups_get_notify_cmd,
        ups_mgmt::commands::ups_set_notify_cmd,
        ups_mgmt::commands::ups_test_notification,
        // NetBox commands
        netbox::commands::netbox_connect,
        netbox::commands::netbox_disconnect,
        netbox::commands::netbox_list_connections,
        netbox::commands::netbox_ping,
        netbox::commands::netbox_list_sites,
        netbox::commands::netbox_get_site,
        netbox::commands::netbox_create_site,
        netbox::commands::netbox_update_site,
        netbox::commands::netbox_partial_update_site,
        netbox::commands::netbox_delete_site,
        netbox::commands::netbox_list_sites_by_region,
        netbox::commands::netbox_list_sites_by_group,
        netbox::commands::netbox_list_racks,
        netbox::commands::netbox_get_rack,
        netbox::commands::netbox_create_rack,
        netbox::commands::netbox_update_rack,
        netbox::commands::netbox_partial_update_rack,
        netbox::commands::netbox_delete_rack,
        netbox::commands::netbox_get_rack_elevation,
        netbox::commands::netbox_list_rack_reservations,
        netbox::commands::netbox_list_devices,
        netbox::commands::netbox_get_device,
        netbox::commands::netbox_create_device,
        netbox::commands::netbox_update_device,
        netbox::commands::netbox_partial_update_device,
        netbox::commands::netbox_delete_device,
        netbox::commands::netbox_list_devices_by_site,
        netbox::commands::netbox_list_devices_by_rack,
        netbox::commands::netbox_list_device_types,
        netbox::commands::netbox_get_device_type,
        netbox::commands::netbox_list_manufacturers,
        netbox::commands::netbox_get_manufacturer,
        netbox::commands::netbox_list_platforms,
        netbox::commands::netbox_get_platform,
        netbox::commands::netbox_list_device_roles,
        netbox::commands::netbox_get_device_role,
        netbox::commands::netbox_render_device_config,
        netbox::commands::netbox_list_interfaces,
        netbox::commands::netbox_get_interface,
        netbox::commands::netbox_create_interface,
        netbox::commands::netbox_update_interface,
        netbox::commands::netbox_partial_update_interface,
        netbox::commands::netbox_delete_interface,
        netbox::commands::netbox_list_interface_connections,
        netbox::commands::netbox_list_ip_addresses,
        netbox::commands::netbox_get_ip_address,
        netbox::commands::netbox_create_ip_address,
        netbox::commands::netbox_update_ip_address,
        netbox::commands::netbox_delete_ip_address,
        netbox::commands::netbox_list_prefixes,
        netbox::commands::netbox_get_prefix,
        netbox::commands::netbox_create_prefix,
        netbox::commands::netbox_update_prefix,
        netbox::commands::netbox_delete_prefix,
        netbox::commands::netbox_get_available_ips,
        netbox::commands::netbox_create_available_ip,
        netbox::commands::netbox_get_available_prefixes,
        netbox::commands::netbox_list_vrfs,
        netbox::commands::netbox_get_vrf,
        netbox::commands::netbox_create_vrf,
        netbox::commands::netbox_update_vrf,
        netbox::commands::netbox_delete_vrf,
        netbox::commands::netbox_list_aggregates,
        netbox::commands::netbox_get_aggregate,
        netbox::commands::netbox_list_rirs,
        netbox::commands::netbox_get_rir,
        netbox::commands::netbox_list_ipam_roles,
        netbox::commands::netbox_get_ipam_role,
        netbox::commands::netbox_list_services,
        netbox::commands::netbox_list_vlans,
        netbox::commands::netbox_get_vlan,
        netbox::commands::netbox_create_vlan,
        netbox::commands::netbox_update_vlan,
        netbox::commands::netbox_partial_update_vlan,
        netbox::commands::netbox_delete_vlan,
        netbox::commands::netbox_list_vlans_by_site,
        netbox::commands::netbox_list_vlans_by_group,
        netbox::commands::netbox_list_vlan_groups,
        netbox::commands::netbox_get_vlan_group,
        netbox::commands::netbox_create_vlan_group,
        netbox::commands::netbox_update_vlan_group,
        netbox::commands::netbox_delete_vlan_group,
        netbox::commands::netbox_list_circuits,
        netbox::commands::netbox_get_circuit,
        netbox::commands::netbox_create_circuit,
        netbox::commands::netbox_update_circuit,
        netbox::commands::netbox_delete_circuit,
        netbox::commands::netbox_list_circuit_providers,
        netbox::commands::netbox_get_circuit_provider,
        netbox::commands::netbox_create_circuit_provider,
        netbox::commands::netbox_update_circuit_provider,
        netbox::commands::netbox_delete_circuit_provider,
        netbox::commands::netbox_list_circuit_types,
        netbox::commands::netbox_get_circuit_type,
        netbox::commands::netbox_list_circuit_terminations,
        netbox::commands::netbox_list_cables,
        netbox::commands::netbox_get_cable,
        netbox::commands::netbox_create_cable,
        netbox::commands::netbox_update_cable,
        netbox::commands::netbox_delete_cable,
        netbox::commands::netbox_trace_cable,
        netbox::commands::netbox_list_tenants,
        netbox::commands::netbox_get_tenant,
        netbox::commands::netbox_create_tenant,
        netbox::commands::netbox_update_tenant,
        netbox::commands::netbox_partial_update_tenant,
        netbox::commands::netbox_delete_tenant,
        netbox::commands::netbox_list_tenant_groups,
        netbox::commands::netbox_get_tenant_group,
        netbox::commands::netbox_create_tenant_group,
        netbox::commands::netbox_update_tenant_group,
        netbox::commands::netbox_delete_tenant_group,
        netbox::commands::netbox_list_contacts,
        netbox::commands::netbox_get_contact,
        netbox::commands::netbox_create_contact,
        netbox::commands::netbox_update_contact,
        netbox::commands::netbox_partial_update_contact,
        netbox::commands::netbox_delete_contact,
        netbox::commands::netbox_list_contact_groups,
        netbox::commands::netbox_get_contact_group,
        netbox::commands::netbox_create_contact_group,
        netbox::commands::netbox_update_contact_group,
        netbox::commands::netbox_delete_contact_group,
        netbox::commands::netbox_list_contact_roles,
        netbox::commands::netbox_list_contact_assignments,
        netbox::commands::netbox_list_vms,
        netbox::commands::netbox_get_vm,
        netbox::commands::netbox_create_vm,
        netbox::commands::netbox_update_vm,
        netbox::commands::netbox_delete_vm,
        netbox::commands::netbox_list_vm_interfaces,
        netbox::commands::netbox_create_vm_interface,
        netbox::commands::netbox_update_vm_interface,
        netbox::commands::netbox_delete_vm_interface,
        netbox::commands::netbox_list_clusters,
        netbox::commands::netbox_get_cluster,
        netbox::commands::netbox_create_cluster,
        netbox::commands::netbox_update_cluster,
        netbox::commands::netbox_delete_cluster,
        netbox::commands::netbox_list_cluster_types,
        netbox::commands::netbox_get_cluster_type,
        netbox::commands::netbox_create_cluster_type,
        netbox::commands::netbox_list_cluster_groups,
        // Port knock commands (54)
        port_knock::commands::port_knock_add_host,
        port_knock::commands::port_knock_remove_host,
        port_knock::commands::port_knock_update_host,
        port_knock::commands::port_knock_get_host,
        port_knock::commands::port_knock_list_hosts,
        port_knock::commands::port_knock_add_sequence,
        port_knock::commands::port_knock_remove_sequence,
        port_knock::commands::port_knock_get_sequence,
        port_knock::commands::port_knock_list_sequences,
        port_knock::commands::port_knock_generate_sequence,
        port_knock::commands::port_knock_encode_sequence_base64,
        port_knock::commands::port_knock_decode_sequence_base64,
        port_knock::commands::port_knock_calculate_complexity,
        port_knock::commands::port_knock_execute,
        port_knock::commands::port_knock_send_spa,
        port_knock::commands::port_knock_sequence_to_knockd,
        port_knock::commands::port_knock_encrypt_payload,
        port_knock::commands::port_knock_decrypt_payload,
        port_knock::commands::port_knock_generate_key,
        port_knock::commands::port_knock_detect_firewall,
        port_knock::commands::port_knock_firewall_accept_rule,
        port_knock::commands::port_knock_firewall_timed_rule,
        port_knock::commands::port_knock_firewall_remove_rule,
        port_knock::commands::port_knock_firewall_backup_command,
        port_knock::commands::port_knock_parse_knockd_config,
        port_knock::commands::port_knock_generate_knockd_config,
        port_knock::commands::port_knock_knockd_status_command,
        port_knock::commands::port_knock_knockd_install_command,
        port_knock::commands::port_knock_knockd_log_command,
        port_knock::commands::port_knock_parse_fwknop_access,
        port_knock::commands::port_knock_generate_fwknop_access,
        port_knock::commands::port_knock_build_fwknop_command,
        port_knock::commands::port_knock_fwknop_install_command,
        port_knock::commands::port_knock_generate_fwknop_keys,
        port_knock::commands::port_knock_generate_fwknop_client_rc,
        port_knock::commands::port_knock_create_profile,
        port_knock::commands::port_knock_update_profile,
        port_knock::commands::port_knock_delete_profile,
        port_knock::commands::port_knock_get_profile,
        port_knock::commands::port_knock_list_profiles,
        port_knock::commands::port_knock_export_profiles,
        port_knock::commands::port_knock_import_profiles,
        port_knock::commands::port_knock_search_profiles,
        port_knock::commands::port_knock_check_port_command,
        port_knock::commands::port_knock_banner_grab_command,
        port_knock::commands::port_knock_nmap_command,
        port_knock::commands::port_knock_rtt_command,
        port_knock::commands::port_knock_get_history,
        port_knock::commands::port_knock_filter_history,
        port_knock::commands::port_knock_get_statistics,
        port_knock::commands::port_knock_clear_history,
        port_knock::commands::port_knock_export_history_json,
        port_knock::commands::port_knock_export_history_csv,
        port_knock::commands::port_knock_get_recent_history,
        // About commands (14)
        about::commands::about_get_info,
        about::commands::about_get_app_info,
        about::commands::about_get_license_summary,
        about::commands::about_get_all_license_texts,
        about::commands::about_get_license_text,
        about::commands::about_get_rust_deps,
        about::commands::about_get_rust_deps_by_category,
        about::commands::about_get_js_deps,
        about::commands::about_get_js_deps_by_category,
        about::commands::about_get_workspace_crates,
        about::commands::about_get_workspace_crates_by_category,
        about::commands::about_get_credits,
        about::commands::about_search_deps,
        about::commands::about_get_deps_by_license,
    ])
