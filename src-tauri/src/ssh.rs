...
pub(crate) fn default_keepalive_probes() -> u32 { 2 } // Reduced for faster disconnect detection
...
lazy_static::lazy_static! {
    pub(crate) static ref FTP_TUNNELS: StdMutex<HashMap<String, FtpTunnelStatus>> = StdMutex::new(HashMap::new());
}
...
lazy_static::lazy_static! {
    pub(crate) static ref RDP_TUNNELS: StdMutex<HashMap<String, RdpTunnelStatus>> = StdMutex::new(HashMap::new());
}
...
lazy_static::lazy_static! {
    pub(crate) static ref VNC_TUNNELS: StdMutex<HashMap<String, VncTunnelStatus>> = StdMutex::new(HashMap::new());
}
...
// ─── Deep SSH Connection Diagnostics ────────────────────────────────────────
...
fn run_ssh_diagnostics(
    ...
    // SSH servers send their version string immediately upon connect
    // (e.g. "SSH-2.0-OpenSSH_9.6").

    let t = std::time::Instant::now();
    let _ = tcp_stream.set_read_timeout(Some(Duration::from_secs(5)));
    let mut banner_buf = [0u8; 512];
    let banner = match std::io::Read::read(&mut &tcp_stream, &mut banner_buf) {
        Ok(0) => {
    ...
    // If banner wasn't SSH, stop here
    if banner.is_none() {
        steps.push(DiagnosticStep {
    ...
    // Create a fresh connection for libssh2 (it needs to own the stream).
    ...
    if auth_methods.is_empty() {
        // Server might accept any (none auth), or the query failed
        steps.push(DiagnosticStep {
