...
/// Input events generated by the overlay window's WndProc.
///
/// Mouse coordinates are already scaled to the RDP desktop resolution.
#[derive(Debug)]
pub enum OverlayInputEvent {
    MouseMove { x: u16, y: u16 },
    MouseButton { x: u16, y: u16, button: u8, pressed: bool },
    MouseWheel { x: u16, y: u16, delta: i16, horizontal: bool },
...
pub trait NativeRenderer: Send {
    ...
    /// Attach an input sender so the overlay's WndProc can forward mouse
    /// and keyboard events directly to the RDP session.
    fn set_input_sender(
        &mut self,
        tx: tokio::sync::mpsc::UnboundedSender<OverlayInputEvent>,
        desktop_width: u16,
        desktop_height: u16,
    );
}
...
pub mod platform {
    ...
    /// Per-window input state stored in GWLP_USERDATA.
    struct OverlayInputData {
        input_tx: tokio::sync::mpsc::UnboundedSender<OverlayInputEvent>,
        desktop_width: u16,
        desktop_height: u16,
    }

    /// WndProc that captures mouse and keyboard input and forwards it
    /// to the RDP session via the attached `OverlayInputEvent` channel.
    ///
    /// When no input state is attached (before `set_input_sender`), all
    /// messages fall through to `DefWindowProcW`.
    unsafe extern "system" fn child_wnd_proc(
        ...
        let data_ptr = GetWindowLongPtrW(hwnd, GWLP_USERDATA) as *const OverlayInputData;

        if !data_ptr.is_null() {
            let data = &*data_ptr;

            // Overlay client-area size for coordinate scaling.
            let overlay_size = || -> (u32, u32) {
                let mut rect: RECT = std::mem::zeroed();
                let _ = GetClientRect(hwnd, &mut rect);
                (rect.right.max(1) as u32, rect.bottom.max(1) as u32)
            };

            let scale = |cx: i32, cy: i32| -> (u16, u16) {
                let (ow, oh) = overlay_size();
                let dw = data.desktop_width as u32;
                let dh = data.desktop_height as u32;
                let x = ((cx.max(0) as u32) * dw / ow).min(dw.saturating_sub(1)) as u16;
                let y = ((cy.max(0) as u32) * dh / oh).min(dh.saturating_sub(1)) as u16;
                (x, y)
            };

            let client_xy = || -> (i32, i32) {
                let lp = lparam.0 as usize;
            ...
            match msg {
                WM_MOUSEMOVE => {
                    let (cx, cy) = client_xy();
                    let (x, y) = scale(cx, cy);
                    let _ = data.input_tx.send(OverlayInputEvent::MouseMove { x, y });
                    return LRESULT(0);
                }
                WM_LBUTTONDOWN => {
                    ...
                    let (cx, cy) = client_xy();
                    let (x, y) = scale(cx, cy);
                    let _ = data.input_tx.send(OverlayInputEvent::MouseButton { x, y, button: 0, pressed: true });
                    return LRESULT(0);
                }
                WM_LBUTTONUP => {
                    ...
                    let (cx, cy) = client_xy();
                    let (x, y) = scale(cx, cy);
                    let _ = data.input_tx.send(OverlayInputEvent::MouseButton { x, y, button: 0, pressed: false });
                    return LRESULT(0);
                }
                WM_RBUTTONDOWN => {
                    let _ = SetCapture(hwnd);
                    let (cx, cy) = client_xy();
                    let (x, y) = scale(cx, cy);
                    let _ = data.input_tx.send(OverlayInputEvent::MouseButton { x, y, button: 2, pressed: true });
                    return LRESULT(0);
                }
                WM_RBUTTONUP => {
                    let _ = ReleaseCapture();
                    let (cx, cy) = client_xy();
                    let (x, y) = scale(cx, cy);
                    let _ = data.input_tx.send(OverlayInputEvent::MouseButton { x, y, button: 2, pressed: false });
                    return LRESULT(0);
                }
                WM_MBUTTONDOWN => {
                    let _ = SetCapture(hwnd);
                    let (cx, cy) = client_xy();
                    let (x, y) = scale(cx, cy);
                    let _ = data.input_tx.send(OverlayInputEvent::MouseButton { x, y, button: 1, pressed: true });
                    return LRESULT(0);
                }
                WM_MBUTTONUP => {
                    let _ = ReleaseCapture();
                    let (cx, cy) = client_xy();
                    let (x, y) = scale(cx, cy);
                    let _ = data.input_tx.send(OverlayInputEvent::MouseButton { x, y, button: 1, pressed: false });
                    return LRESULT(0);
                }
                WM_MOUSEWHEEL | WM_MOUSEHWHEEL => {
                    let delta = ((wparam.0 >> 16) & 0xFFFF) as i16;
                    // Wheel coords are screen-relative — convert to client.
                    let sp = lparam.0 as usize;
                    let sx = (sp & 0xFFFF) as i16 as i32;
                    let sy = ((sp >> 16) & 0xFFFF) as i16 as i32;
                    let mut wrect: RECT = std::mem::zeroed();
                    let _ = GetWindowRect(hwnd, &mut wrect);
                    let (x, y) = scale(sx - wrect.left, sy - wrect.top);
                    let horizontal = msg == WM_MOUSEHWHEEL;
                    let _ = data.input_tx.send(OverlayInputEvent::MouseWheel { x, y, delta, horizontal });
                    return LRESULT(0);
                }
                WM_KEYDOWN | WM_SYSKEYDOWN => {
                    let lp = lparam.0 as usize;
                    let scancode = ((lp >> 16) & 0xFF) as u8;
                    let extended = (lp >> 24) & 1 != 0;
                    let _ = data.input_tx.send(OverlayInputEvent::KeyDown { scancode, extended });
                    return LRESULT(0);
                }
                WM_KEYUP | WM_SYSKEYUP => {
                    ...
                    let scancode = ((lp >> 16) & 0xFF) as u8;
                    let extended = (lp >> 24) & 1 != 0;
                    let _ = data.input_tx.send(OverlayInputEvent::KeyUp { scancode, extended });
                    return LRESULT(0);
                }
                WM_CLOSE => {
                    // Prevent Alt+F4 from destroying the overlay.
                    return LRESULT(0);
                }
                WM_DESTROY => {
                    // Free the per-window input data.
                    let ptr = GetWindowLongPtrW(hwnd, GWLP_USERDATA) as *mut OverlayInputData;
                    if !ptr.is_null() {
    ...
    /// Attach an input channel to the overlay so its WndProc can forward
    /// mouse/keyboard events to the RDP session.
    pub fn attach_overlay_input(
        hwnd: isize,
        tx: tokio::sync::mpsc::UnboundedSender<OverlayInputEvent>,
        desktop_width: u16,
        desktop_height: u16,
    ) {
        let data = Box::new(OverlayInputData {
            input_tx: tx,
            desktop_width,
            desktop_height,
        });
    ...
    /// Update the desktop dimensions stored in the overlay's input state.
    /// Called when the remote desktop is resized.
    pub fn update_overlay_desktop_size(hwnd: isize, width: u16, height: u16) {
        unsafe {
            let ptr = GetWindowLongPtrW(HWND(hwnd as *mut _), GWLP_USERDATA)
                as *mut OverlayInputData;
            if !ptr.is_null() {
                (*ptr).desktop_width = width;
                (*ptr).desktop_height = height;
            }
        }
    }
...
impl NativeRenderer for SoftbufferRenderer {
    ...
    fn set_input_sender(
        ...
        platform::attach_overlay_input(self.hwnd, tx, desktop_width, desktop_height);
    }
}
