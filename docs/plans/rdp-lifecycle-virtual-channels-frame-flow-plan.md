...
- Preserve the `sorng-rdp-vendor` dependency boundary without requiring an
  unused runtime dynamic library.
...
| Runtime lifecycle observability | Mostly done               | `RdpStatsEvent` carries lifecycle snapshots and the runner emits `rdp://lifecycle` immediately on every phase transition. The runner now merges CLIPRDR/AUDIN counters into the lifecycle channel summary alongside RDPDR/RDPSND (`session_runner.rs`, t16-L1).                                                                                                                                                                                                                                                          | Remaining follow-up: the runner still drives lifecycle via phase-string projection rather than the typed `LifecycleEvent`/`apply_event` API, so `lastFailureClass` is not yet populated from typed cert/auth/protocol failures (UI renders it defensively). Typed-event refactor is a pending backend follow-up.                                                                                  |
| Frontend lifecycle consumption  | Done                      | `useRDPClient` listens for `rdp://lifecycle`; internals panel renders lifecycle state, substate, transition count, reconnect attempt, and (defensively) failure class.                                                                                                                                                                                                                                                                                                                                                   | Failure-class row stays empty until the typed-event backend follow-up lands.                                                                                                                                                                                                                                                                                                                      |
...
| Virtual channel manager         | Done                      | `virtual_channels/mod.rs` provides the registry with channel name/kind/priority/state, `ChannelSummary` accounting, and diagnostics. The runner merges RDPDR/RDPSND/CLIPRDR/AUDIN into the lifecycle channel summary. Backend integration tests cover registration, duplicate rejection, disabled channels, fault isolation, and summary counts (`tests/channel_registry.rs`, t16-L5).                                                                                                                                   | Remaining follow-up: AUDIN _live_ ready/fault counters (the merge currently reflects enabled-state only; live counters need an `audin.rs`-owned diagnostics handle, since the processor is moved into DRDYNVC and the runner holds no handle). RDPGFX diagnostics bridge remains deferred (GfxProcessor lives inside IronRDP DRDYNVC; a clean channel-state row needs a runner-owned GFX handle). |
| Backpressure frame controller   | Done                      | `frame_flow_control.rs` provides backend frame budgets, high/low watermark hysteresis, coalesce/drop dispositions, and counters. `useRdpFrameBackpressure` feeds capped-cadence frontend telemetry; frame telemetry incl. queued/delivered/dropped/coalesced and average render now persists into the lifecycle `FrameFlowSummary` (`commands_cmds.rs` + `session_state.rs`, t16-L2). Backend tests cover budget accounting, watermarks, and accounting-only no-behavior-change (`tests/frame_flow_control.rs`, t16-L5). | Keep budgets in sync if new frame classes are added.                                                                                                                                                                                                                                                                                                                                              |
| Diagnostics UI                  | Done                      | RDP internals panel now renders compact rows for channel enabled/ready/faults, frame-flow queued/delivered/dropped/coalesced, average render, backpressure state, queue depth, reconnect attempt, and failure class (`RDPInternalsPanel.tsx`/`RDPClient.tsx`, t16-L3/e13). Frontend diagnostics tests cover the rendered rows (t16-L4/e14).                                                                                                                                                                              | Failure-class row renders defensively (`—`) until the typed-event backend follow-up populates `lastFailureClass`.                                                                                                                                                                                                                                                                                 |
...
- Run focused Rust `sorng-rdp` tests. Landed: backend lifecycle/channel/frame integration tests across `tests/channel_registry.rs`, `tests/frame_flow_control.rs`, and `tests/lifecycle_diagnostics.rs` (29 tests, t16-L5), green alongside the existing `sorng-rdp` suite.
- Run `npx tsc --noEmit`.
- Run live Docker/xrdp smoke when the host environment is available. Status: the live Docker/xrdp RDPDR e2e smoke is **host-blocked** on this Windows host (requires Docker + xrdp container + port access not available here) — recorded as host-blocked, not failing.
- Update this tracker with pass/fail evidence and any deferred decisions.

Remaining follow-ups after this gate (tracked above, not blockers):

- AUDIN _live_ ready/fault counters — needs an `audin.rs`-owned diagnostics handle (currently enabled-state only).
- Typed lifecycle `LifecycleEvent` transitions + `lastFailureClass` population from cert/auth/protocol failures — the diagnostics "failure class" row renders defensively until this lands.
- RDPGFX diagnostics bridge — deferred; needs a runner-owned GFX diagnostics handle across the IronRDP DRDYNVC boundary.
- Live Docker/xrdp RDPDR e2e smoke — host-blocked.
