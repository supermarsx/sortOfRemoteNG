...
The current database's native protection panel can convert an unprotected or verified legacy-password database to the versioned `sorng-db` inner container. Its independent random database key supports **AES-256-GCM (recommended)**, **ChaCha20-Poly1305 (portable software option)**, and the advanced software-only **Twofish-256-EAX** and **Serpent-256-EAX** alternatives when the native backend reports them available. These are authenticated inner-payload ciphers, not VeraCrypt-compatible volumes or cipher cascades. Selecting one does not change outer HKDF-derived artifact encryption or the AES-based unlock-slot wrapping; password slots still use native Argon2id, and OS-vault slots remain tied to the device/account. The EAX 0.5 dependency keeps a short-lived private raw-key copy without a zeroizing destructor, although application-owned database/session key buffers and the enabled Twofish/Serpent key schedules use zeroization. This is not a guarantee of complete memory erasure, an independent security audit, or constant-time execution. Passkey/security-key and biometric methods are not offered as working encryption protectors by this frontend baseline. See [database cipher details and limitations]({{ '/security/database-ciphers/' | relative_url }}) for the native format and dependency notes.
