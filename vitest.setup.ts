...
// Provide a default SettingsContext mock so components using useSettings()
// don't throw when rendered outside a SettingsProvider in tests.
// Individual test files can override this with their own vi.mock().
// NOTE: Uses a factory in vi.mock rather than mockReturnValue so that
// vi.restoreAllMocks() in individual tests doesn't clear the implementation.
vi.mock("./src/contexts/SettingsContext", async (importOriginal) => {
  const actual = await importOriginal<Record<string, unknown>>();
  const defaults = (actual as { defaultSettings: unknown }).defaultSettings;
  return {
    ...actual,
    useSettings: () => ({
      settings: defaults,
