...
import type {
  WindowSessionSync,
  WindowCommand,
  WindowSessionCloseResult,
} from "../../src/types/windowManager";
import { WINDOW_SESSION_CLOSE_RESULT_EVENT } from "../../src/types/windowManager";
import { toSessionLifecyclePatch } from "../../src/utils/session/sessionLifecycle";
...
const SessionIcon: React.FC<{ protocol: string }> = ({ protocol }) => {
  ...
  switch (p) {
    case "rdp":
      return <Monitor size={14} className="mr-2 flex-shrink-0" />;
    case "ssh":
      return <Terminal size={14} className="mr-2 flex-shrink-0" />;
    case "vnc":
      return <Eye size={14} className="mr-2 flex-shrink-0" />;
    case "http":
    case "https":
      return <Globe size={14} className="mr-2 flex-shrink-0" />;
    case "telnet":
    case "rlogin":
      return <Phone size={14} className="mr-2 flex-shrink-0" />;
    case "raw":
      return <Network size={14} className="mr-2 flex-shrink-0" />;
    case "winrm":
      return <Server size={14} className="mr-2 flex-shrink-0" />;
    default:
      return <Monitor size={14} className="mr-2 flex-shrink-0" />;
  }
};
...
const reviveConnection = (connection: Connection): Connection => ({
  ...
  lastConnected:
    typeof connection.lastConnected === "string"
      ? connection.lastConnected
      : connection.lastConnected
        ? new Date(connection.lastConnected).toISOString()
        : undefined,
});
...
const DetachedSessionContent: React.FC<{
  ...
  const applyTransparency = useCallback(
    (enabled: boolean, opacity?: number) => {
      const targetOpacity = enabled
        ? Math.min(1, Math.max(0, opacity ?? 1))
        : 1;
  ...
  useEffect(() => {
    ...
    const unlistenPromise = listen<{ sessionId: string }>(
      ...
      async (event) => {
        ...
        const ownedSessions = sessionsRef.current;
        if (!ownedSessions.some((session) => session.id === closedSessionId))
          return;

        const remainingSessions = ownedSessions.filter(
          (session) => session.id !== closedSessionId,
        );
        sessionsRef.current = remainingSessions;
        dispatch({ type: "REMOVE_SESSION", payload: closedSessionId });
        localStorage.removeItem(`detached-session-${closedSessionId}`);
  ...
  const lifecycleSnapshotsRef = useRef(new Map<string, string>());
  useEffect(() => {
    ...
    state.sessions.forEach((session) => {
      ...
      if (lifecycleSnapshotsRef.current.get(session.id) === snapshot) return;
      lifecycleSnapshotsRef.current.set(session.id, snapshot);
      const command: WindowCommand = {
        type: "SYNC_SESSION_LIFECYCLE",
        sessionId: session.id,
        lifecycle,
      };
      emit("wm:command", command).catch(() => {});
    });
    lifecycleSnapshotsRef.current.forEach((_snapshot, id) => {
      if (!currentIds.has(id)) lifecycleSnapshotsRef.current.delete(id);
    });
  }, [detachedWindowId, state.sessions]);
  ...
  const emitCloseSession = useCallback(
    (sessionId: string) => {
      ...
      const command: WindowCommand = {
        ...
        sessionId,
        lifecycle: session
          ? toSessionLifecyclePatch(session, detachedWindowId)
          : undefined,
      };
  ...
  const requestAuthoritativeClose = useCallback(
    async (session: ConnectionSession): Promise<boolean> => {
      const sessionId = session.id;
      const requestId = generateId();
      const sourceWindow = getCurrentWindow().label as any;
      let resolveResult!: (success: boolean) => void;
      const result = new Promise<boolean>((resolve) => {
        resolveResult = resolve;
      });
      const unlisten = await listen<WindowSessionCloseResult>(
        ...
        (event) => {
          if (
            ...
            resolveResult(event.payload.success);
          }
        },
      );
      const timeout = setTimeout(() => resolveResult(false), 10_000);

      try {
        await emit("wm:command", {
          type: "CLOSE_SESSION",
          sessionId,
          lifecycle: toSessionLifecyclePatch(session, detachedWindowId),
          requestId,
          sourceWindow,
        } as WindowCommand);
        return await result;
      } catch (error) {
        console.error("Failed to request authoritative session close:", error);
        return false;
      } finally {
  ...
  const handleTabClose = useCallback(
    (sessionId: string) => {
      ...
      emitCloseSession(sessionId).catch(() => {});
    },
    [emitCloseSession],
  );
  ...
  const handleCommitRename = useCallback(() => {
    if (renamingTabId && renameValue.trim()) {
      ...
      if (sess)
        dispatch({
          type: "UPDATE_SESSION",
          payload: { ...sess, name: renameValue.trim() },
        });
    }
  ...
  const handleDetachedTabKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>, sessionId: string) => {
      ...
      switch (event.key) {
        case "ArrowLeft":
          nextIndex =
            currentIndex === 0 ? visibleTabIds.length - 1 : currentIndex - 1;
          break;
        case "ArrowRight":
          nextIndex =
            currentIndex === visibleTabIds.length - 1 ? 0 : currentIndex + 1;
          break;
        case "Home":
          nextIndex = 0;
          break;
      ...
      requestAnimationFrame(() => {
        const nextTab = document.getElementById(
          `detached-session-tab-${nextSessionId}`,
        );
  ...
  const resolveTabColor = useCallback(
    (sess: ConnectionSession): string | undefined => {
      const conn = connectionsRef.current.find(
        (c) => c.id === sess.connectionId,
      );
      if (conn?.tabColor) return conn.tabColor;
      if (conn?.parentId) {
        const parent = connectionsRef.current.find(
          (c) => c.id === conn.parentId,
        );
  ...
  const windowTitle =
    windowTitleOverride ??
    (activeSession
      ? `sortOfRemoteNG - ${activeSession.name}`
      : "sortOfRemoteNG");
  ...
  const disconnectOwnedSessions = useCallback(async (): Promise<boolean> => {
    ...
    for (const owned of ownedSessions) {
      try {
        const closed = isTauri
          ...
          : await emitCloseSession(owned.id).then(
              () => true,
              () => false,
            );
        if (!closed) {
        ...
        console.error("Failed to close detached session:", err);
        closingRef.current = false;
        return false;
      }
    }
    ...
    return true;
  }, [emitCloseSession, isTauri, requestAuthoritativeClose, sessionId]);
  ...
  const handleReattach = useCallback(async () => {
    ...
    try {
      ...
      if (!terminalBuffer && s.protocol !== "raw" && s.protocol !== "rlogin") {
        try {
          const bufferPromise = new Promise<string>((resolve) => {
            const timeout = setTimeout(() => resolve(""), 1000);
            listen<{ sessionId: string; buffer: string }>(
      ...
      const latestSession =
        sessionsRef.current.find((item) => item.id === s.id) ?? s;
      await emit("detached-session-reattach", {
        sessionId: s.id,
        terminalBuffer,
        lifecycle: toSessionLifecyclePatch(latestSession, detachedWindowId),
      });
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[var(--color-background,#111827)]">
        <div className="text-center max-w-sm px-6">
          ...
          <h3 className="text-sm font-semibold text-[var(--color-text,#f9fafb)] mb-1">
            Session Error
          </h3>
        ...
        <div className="text-center max-w-sm px-6">
          {loadingTimedOut ? (
            <>
              ...
              <p className="text-xs text-[var(--color-textSecondary,#9ca3af)] mb-5">
                The detached session failed to load within 30 seconds. The
                session data may be missing or corrupted.
              </p>
              <div className="flex items-center justify-center gap-2">
                <button
                  onClick={() => {
            ...
            <>
              ...
              <p className="text-xs text-[var(--color-textSecondary,#9ca3af)]">
                Preparing detached session...
              </p>
            </>
          )}
        </div>
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div
            className="group/bar flex items-center gap-2 min-w-0 max-w-[60%]"
          ...
          >
            ...
            {editingTitle ? (
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    const t = titleDraft.trim() || null;
                    setWindowTitleOverride(t);
        ...
        <div
          ...
          onDrop={(e) => {
            ...
            const draggedId = e.dataTransfer.getData(
              "application/x-detached-session",
            );
            if (!draggedId) return;
        ...
        >
          ...
          {state.sessions.map((sess) => {
            ...
            const isReal =
              !sess.protocol.startsWith("tool:") &&
              !sess.protocol.startsWith("winmgmt:");
            const group = sess.tabGroupId
              ? state.tabGroups.find((g) => g.id === sess.tabGroupId)
              : null;
            ...
            return (
                    ...
                    .then(({ getAllWindows }) =>
                      getAllWindows().then(async (wins) => {
                        ...
                        const entries = await Promise.all(
                          others.map(async (w) => {
                            if (w.label === "main")
                              return { label: w.label, title: "Main Window" };
              ...
              >
                ...
                {isPinned && (
                  <Pin
                    size={10}
                    className="mr-1 flex-shrink-0 text-[var(--color-textMuted)]"
                  />
                )}
                {renamingTabId === sess.id ? (
                  ...
                  <span className="truncate text-sm mr-2 max-w-[30vw]">
                    {sess.name || "Session"}
                  </span>
                )}
                {isReal && (
                  <>
                    {sess.status === "connected" && (
                      <div
                        className="w-2 h-2 rounded-full bg-success mr-1 flex-shrink-0"
                        role="status"
                    ...
                    {sess.status === "connecting" && (
                      <div
                        className="w-2 h-2 rounded-full bg-warning mr-1 flex-shrink-0"
                        role="status"
                    ...
                    {sess.status === "disconnected" && (
                      <div
                        className="w-2 h-2 rounded-full bg-[var(--color-textMuted)] mr-1 flex-shrink-0"
                        role="status"
                    ...
                    {sess.status === "error" && (
                      <div
                        className="w-2 h-2 rounded-full bg-error mr-1 flex-shrink-0"
                        role="status"
                        aria-label="Error"
                      />
                    )}
                ...
                <button
                  ...
                  className="flex-shrink-0 p-1 hover:bg-[var(--color-surface)] rounded transition-colors opacity-0 group-hover:opacity-100"
                  data-tooltip="Reattach"
                  aria-label={`Reattach ${sess.name || "session"}`}
                >
        ...
        >
          {tabContextMenu &&
            (() => {
              ...
              const conn = sess
                ? connectionsRef.current.find((c) => c.id === sess.connectionId)
                : null;
              const isFirst = idx === 0;
              ...
              return (
                <>
                      ...
                      <button
                        ...
                        onClick={() =>
                          act(() => {
                            const newGroup = {
                              id: generateId(),
                              name: `Group ${state.tabGroups.length + 1}`,
                              color: "#3b82f6",
                              collapsed: false,
                            };
                            dispatch({
                            ...
                            if (sess)
                              dispatch({
                                type: "UPDATE_SESSION",
                                payload: { ...sess, tabGroupId: newGroup.id },
                              });
                          })
                        }
                        className="sor-menu-item"
                      ...
                      onClick={() =>
                        act(() => {
                          if (sess)
                            dispatch({
                              type: "UPDATE_SESSION",
                              payload: { ...sess, tabGroupId: undefined },
                            });
                        })
                      }
                      className="sor-menu-item"
                    >
                      <FolderMinus size={14} className="mr-2" /> Remove from
                      Group
                    </button>
                  )}
                  ...
                  {/* ── Edit actions ── */}
                  <button
                    onClick={() => act(() => handleStartRename(sid))}
                    className="sor-menu-item"
                  >
                      ...
                      <button
                        onClick={() =>
                          act(() => {
                            if (sess?.hostname)
                              navigator.clipboard
                                .writeText(sess.hostname)
                                .catch(() => {});
                          })
                        }
                        className="sor-menu-item"
                      >
                        <ClipboardCopy size={14} className="mr-2" /> Copy
                        Hostname
                      </button>
                      <button
                        onClick={() =>
                          act(() => {
                            const info = [
                              sess?.name,
                              `${sess?.protocol ?? ""}://${sess?.hostname ?? ""}${conn?.port ? ":" + conn.port : ""}`,
                    ...
                    onClick={() =>
                      act(() => {
                        if (sess)
                          dispatch({
                            type: "UPDATE_SESSION",
                            payload: { ...sess, pinned: !isPinned } as any,
                          });
                      })
                    }
                    className="sor-menu-item"
                  ...
                  {/* ── Close actions ── */}
                  <button
                    onClick={() => act(() => handleTabClose(sid))}
                  ...
                  {hasTabsToRight && (
                    <button
                      onClick={() =>
                        act(() => {
                          state.sessions.slice(idx + 1).forEach((s) => {
                            emitCloseSession(s.id).catch(() => {});
                          });
                        })
                      }
                      className="sor-menu-item sor-menu-item-danger"
                    >
                      <ArrowRightFromLine size={14} className="mr-2" /> Close
                      Tabs to Right
                    </button>
                  )}
                  {hasTabsToLeft && (
                    <button
                      onClick={() =>
                        act(() => {
                          state.sessions.slice(0, idx).forEach((s) => {
                            emitCloseSession(s.id).catch(() => {});
                          });
                        })
                      }
                      className="sor-menu-item sor-menu-item-danger"
                    >
                      <ArrowLeftFromLine size={14} className="mr-2" /> Close
                      Tabs to Left
                    </button>
                  )}
                </>
        ...
        {(activeSession.status === "disconnected" ||
          ...
          <div
            className="flex items-center justify-between px-4 py-2 bg-warning/10 border-b border-warning/25 text-warning text-xs"
            role="alert"
            aria-live="assertive"
          >
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (tabCloseConfirm)
            emitCloseSession(tabCloseConfirm.sessionId).catch(() => {});
