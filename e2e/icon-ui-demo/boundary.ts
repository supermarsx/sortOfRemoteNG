...
declare global {
  interface Window {
    __ICON_UI_DEMO__: typeof demo;
  }
}
window.__ICON_UI_DEMO__ = demo;
export function refuse(operation: string): never {
  demo.refused.push(operation);
  throw new Error("Isolated icon demo refused " + operation);
}
...
window.addEventListener("error", (event) => demo.refused.push(event.message));
