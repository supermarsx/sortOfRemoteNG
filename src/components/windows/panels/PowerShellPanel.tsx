...
const PowerShellPanel: React.FC<PowerShellPanelProps> = ({ ctx }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#1a1a2e]">
      ...
      >
        {history.length === 0 && (
          <div className="text-[var(--color-textMuted)] mb-3">
            <p>WMI Query Console</p>
            <p className="mt-1">
              Enter WQL queries to execute on the remote machine.
            </p>
            <p className="text-[var(--color-textSecondary)] mt-2">Examples:</p>
        ...
        {history.map((entry, i) => (
          <div key={`ps-${entry.command.slice(0, 50)}-${i}`} className="mb-3">
            <div className="text-blue-400 flex items-center gap-1">
              <span className="text-[var(--color-textMuted)]">PS&gt;</span> {entry.command}
            </div>
            <pre
              className={`mt-1 whitespace-pre-wrap ${entry.error ? "text-error" : "text-[var(--color-textSecondary)]"}`}
            >
              {entry.output}
            </pre>
          </div>
        ...
        {running && (
          <div className="flex items-center gap-2 text-[var(--color-textMuted)]">
      ...
      <div className="border-t border-[var(--color-border)] bg-[#141428] px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none text-xs font-mono text-[var(--color-text)] placeholder:text-[var(--color-textSecondary)]"
            autoFocus
          />
          <button
            onClick={execute}
