  ...
  return (
    <div
      ...
      style={{
        ...
        height: pipH,
        transition:
          "top 300ms ease, bottom 300ms ease, left 300ms ease, right 300ms ease, width 200ms ease, height 200ms ease",
...
const ErrorOverlay: React.FC<{
  ...
  <RDPErrorScreen
    ...
    connectionDetails={{
      ...
      domain: (mgr.connection as Record<string, unknown> | undefined)
  ...
  React.useEffect(() => {
    ...
    const listenPromise = import("@tauri-apps/api/event").then(({ listen }) =>
      listen<{
        ...
        file_count: number;
        files_done: number;
      }>("rdp://file-transfer-progress", (event) => {
  ...
  const handleDrop = React.useCallback(
    async (e: React.DragEvent) => {
      ...
      if (!fileDragDropEnabled) {
        setDropStatus({
          message:
            "File drag & drop is disabled — enable it in the status bar or connection settings",
          type: "error",
        });
      ...
      if (totalSize > MAX_DROP_SIZE) {
        setDropStatus({
          message: `Total file size (${formatBytes(totalSize)}) exceeds the 100 MB limit`,
          type: "error",
        });
        if (dropDismissRef.current) clearTimeout(dropDismissRef.current);
        dropDismissRef.current = setTimeout(() => setDropStatus(null), 5000);
      ...
      if (
        !window.confirm(
          `Transfer ${fileCount} file${fileCount > 1 ? "s" : ""} (${formatBytes(totalSize)}) to remote session?`,
        )
      ) {
        return;
      }
      ...
      try {
        setDropStatus({
          message: "Scanning dropped items...",
          type: "info",
          spinning: true,
        });
        await yieldToBrowser();
        ...
        setDropStatus({
          message: `Sending ${countStr} (${sizeStr})...`,
          type: "info",
          spinning: true,
        });
  ...
  return (
    ...
    >
        ...
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-primary/20 border-2 border-dashed border-primary rounded-lg pointer-events-none">
          <div className="bg-[var(--color-surface)] px-6 py-4 rounded-xl shadow-2xl border border-primary/50 text-center">
            ...
            <div className="text-xs text-[var(--color-textSecondary)]">
              Files and folders will be transferred to remote clipboard
            </div>
          </div>
        </div>
      )}
      ...
      {(dropStatus || transferProgress) && (
        <div
          className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-50 max-w-md w-80 rounded-lg shadow-xl border overflow-hidden ${
            ...
            <div className="px-4 pb-3 pt-1 space-y-1.5">
              <div className="flex items-center justify-between text-[10px] text-[var(--color-textSecondary)]">
                <span
                  ...
                  title={transferProgress.fileName}
                >
                  {transferProgress.fileName}
                </span>
                <span className="text-[var(--color-textMuted)] flex-shrink-0">
                  {transferProgress.filesDone + 1}/{transferProgress.fileCount}{" "}
                  files
                </span>
              </div>
              <div className="h-1.5 bg-[var(--color-border)] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (transferProgress.transferred / transferProgress.total) * 100)}%`,
                  }}
                />
              </div>
      ...
      <canvas
        ...
        className={`${mgr.isFullscreen ? "max-h-full max-w-full border-0" : "border border-[var(--color-border)]"} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-black`}
        ...
        style={{
          ...
          ...(resizeToWindow && !mgr.isFullscreen
            ? {
                width: "100%",
                height: "100%",
                maxWidth: "100%",
...
const RDPClient: React.FC<RDPClientProps> = ({
  ...
  return (
    ...
    >
      ...
      {!mgr.isFullscreen && (
        <RDPClientHeader
          ...
          serverCertValidation={
            mgr.connection?.rdpSettings?.security?.serverCertValidation
          }
          ...
          totpUnavailableReason={
            mgr.connection?.credentialSource?.kind === "vault" &&
            !mgr.vaultTotp.available
              ? mgr.vaultTotp.unavailableReason
              : undefined
          }
      ...
      {!mgr.isFullscreen &&
        (mgr.connection?.osType === "windows" ||
          (!mgr.connection?.osType && session.protocol === "rdp")) && (
          <WindowsToolsBar
            connectionId={session.connectionId}
            connectionName={mgr.connection?.name || session.name}
            hostname={session.hostname}
