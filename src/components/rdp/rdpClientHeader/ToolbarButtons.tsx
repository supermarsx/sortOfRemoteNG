  ...
  <>
    ...
    {p.magnifierActive && (
      <div className="flex items-center gap-1 ml-2 px-2 py-0.5 bg-[var(--color-surface)] rounded border border-[var(--color-border)] text-xs">
        ...
        <button
          onClick={() => p.setMagnifierZoom(Math.max(2, p.magnifierZoom - 1))}
          className="px-1 hover:bg-[var(--color-surfaceHover)] rounded"
          data-tooltip="Decrease zoom"
        >
          &minus;
        </button>
        <span className="text-[var(--color-textSecondary)] w-6 text-center">
          {p.magnifierZoom}x
        </span>
        <button
          onClick={() => p.setMagnifierZoom(Math.min(8, p.magnifierZoom + 1))}
          className="px-1 hover:bg-[var(--color-surfaceHover)] rounded"
          data-tooltip="Increase zoom"
        >
          +
        </button>
        ...
        <button
          onClick={() =>
            p.setMagnifierPipSize(
              Math.max(150, (p.magnifierPipSize ?? 280) - 40),
            )
          }
          className="px-1 hover:bg-[var(--color-surfaceHover)] rounded"
          data-tooltip="Smaller window"
        >
          <Minimize2 size={11} />
        </button>
        <button
          onClick={() =>
            p.setMagnifierPipSize(
              Math.min(500, (p.magnifierPipSize ?? 280) + 40),
            )
          }
          className="px-1 hover:bg-[var(--color-surfaceHover)] rounded"
          data-tooltip="Larger window"
        >
          <Maximize2 size={11} />
        </button>
        <div className="w-px h-3 bg-[var(--color-border)] mx-1" />
        <button
          onClick={() => p.setMagnifierActive(false)}
          className="px-1 hover:bg-error/20 hover:text-error rounded"
          data-tooltip="Close magnifier"
        >
          <X size={11} />
        </button>
      </div>
    ...
    <button
      onClick={() => p.onOpenInternals("diagnostics")}
      className={btnDefault}
      data-tooltip="RDP Internals"
      aria-label="Open RDP Internals tab"
    >
      <Activity size={14} />
    </button>
    <button
      onClick={() => p.onOpenInternals("settings")}
      className={btnDefault}
      data-tooltip="RDP Settings"
      aria-label="Open RDP settings in Internals tab"
    >
      <Settings size={14} />
    </button>
    <button
