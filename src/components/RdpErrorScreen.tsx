  ...
  <div className={`flex-shrink-0 bg-gradient-to-r ${HEADER_COLOR[mgr.category]} border-b border-red-800/40 px-6 py-5`}>
    <div className="flex items-start gap-4 max-w-3xl mx-auto">
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs text-gray-500 mt-1 font-mono">Session {sessionId.slice(0, 8)}…</p>
      </div>
    </div>
  </div>
);
        ...
        return (
          <div key={idx} className={`rounded-lg border transition-colors ${isOpen ? 'border-[var(--color-border)] bg-[var(--color-background)]/80' : 'border-[var(--color-border)] bg-[var(--color-background)]/40 hover:border-[var(--color-border)]'}`}>
            <button onClick={() => mgr.toggleCause(idx)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
              {cause.icon}
              <span className="flex-1 text-sm font-medium text-gray-200">{cause.title}</span>
            ...
            {isOpen && (
              <div className="px-4 pb-4 space-y-3">
                ...
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">How to fix</p>
                  <ul className="space-y-1.5">
                    {cause.remediation.map((step, si) => (
                      <li key={si} className="flex items-start gap-2 text-sm text-[var(--color-textSecondary)]">
                        <span className="text-gray-600 select-none">{si + 1}.</span>
                        {step}
                      </li>
                    ))}
                  </ul>
...
const QuickActions: React.FC<{
  ...
  <section className="flex flex-wrap gap-3">
    ...
    {onEditConnection && (
      <button onClick={onEditConnection} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--color-border)] hover:bg-[var(--color-border)] text-gray-200 text-sm font-medium transition-colors">
            ...
            return (
              <div key={idx}>
                <button onClick={() => mgr.toggleStep(idx)} className="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-[var(--color-surface)]/40 transition-colors">
                  ...
                  <span className="flex items-center gap-1 text-xs text-gray-500"><Clock size={11} />{step.durationMs}ms</span>
                  {step.detail && (isExpanded ? <ChevronUp size={14} className="text-gray-600" /> : <ChevronDown size={14} className="text-gray-600" />)}
                </button>
                <div className="px-4 pb-1 -mt-1 pl-11">
                  <p className={`text-xs ${step.status === 'fail' ? 'text-red-400' : step.status === 'warn' ? 'text-yellow-400' : step.status === 'info' ? 'text-blue-400' : 'text-gray-500'}`}>{step.message}</p>
                </div>
                {isExpanded && step.detail && (
                  <div className="px-4 pb-3 pl-11">
                    <pre className="text-xs text-[var(--color-textSecondary)] whitespace-pre-wrap bg-gray-950/60 border border-[var(--color-border)] rounded p-2 mt-1">{step.detail}</pre>
                  </div>
                )}
              </div>
            );
...
const RawErrorToggle: React.FC<{ mgr: Mgr; errorMessage: string }> = ({ mgr, errorMessage }) => (
  <section>
    <button onClick={mgr.toggleRawError} className="flex items-center gap-2 text-xs text-gray-500 hover:text-[var(--color-textSecondary)] transition-colors">
