    ...
    <div className="p-6 overflow-y-auto flex-1">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg bg-success hover:bg-success/90 text-[var(--color-text)] text-xs font-medium transition-colors"
        >
      ...
      {mgr.filteredContainers.length === 0 ? (
        ...
        <div className="space-y-2">
          {mgr.filteredContainers.map((ct) => (
            <div
              key={ct.vmid}
              className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] overflow-hidden"
            >
              <button
                onClick={() => {
                ...
                className="w-full flex items-center gap-3 p-3 text-left hover:bg-[var(--color-surfaceHover)] transition-colors"
              >
                <div
                  className={`w-2 h-2 rounded-full ${statusColor(ct.status)}`}
                />
                ...
                <MoreHorizontal className="w-4 h-4 text-[var(--color-textSecondary)]" />
              </button>
              ...
              {expandedCt === ct.vmid && (
                <div className="border-t border-[var(--color-border)] p-3 flex flex-wrap gap-2">
                    ...
                    <>
                      <ActionBtn
                        ...
                        onClick={() =>
                          mgr.requestConfirm(
                            t(
                              "proxmox.lxc.shutdownTitle",
                              "Shutdown Container",
                            ),
                            t(
                              "proxmox.lxc.shutdownMsg",
                              `Gracefully shutdown container ${ct.vmid}?`,
                            ),
                  ...
                  <ActionBtn
                    ...
                    color="text-info"
                    testId={`proxmox-lxc-console-btn-${ct.vmid}`}
                    onClick={() =>
                      consoles.openTerm({
                        node,
                        vmid: ct.vmid,
                  ...
                  <ActionBtn
                    ...
                    onClick={() =>
                      mgr.requestConfirm(
                        ...
                        t(
                          "proxmox.lxc.deleteMsg",
                          `Permanently delete container ${ct.vmid}?`,
                        ),
                        async () => {
...
const ActionBtn: React.FC<{
  ...
  disabled?: boolean;
  testId?: string;
}> = ({ icon: Icon, label, color, onClick, disabled, testId }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    data-testid={testId}
    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors border border-[var(--color-border)] hover:bg-[var(--color-surfaceHover)] disabled:opacity-50 ${color}`}
  >
