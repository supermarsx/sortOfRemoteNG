...
const TasksView: React.FC<SubProps> = ({ mgr }) => {
  ...
  return (
    <div className="p-6 overflow-y-auto flex-1">
                  ...
                  <td className="py-2">
                    <div className="flex gap-1">
                      <button
                        ...
                        className="p-1 rounded border border-[var(--color-border)] text-[var(--color-textSecondary)] hover:text-[var(--color-text)] transition-colors"
      ...
      {/* Task detail panel */}
      {mgr.taskDetail && (
        <div className="mt-6 p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)]">
          ...
          <div className="font-mono text-[10px] max-h-48 overflow-y-auto space-y-0.5 bg-[var(--color-bg)] p-3 rounded-lg">
            {mgr.taskLog.map((line, i) => (
              <div key={`log-${line.n}-${i}`} className="text-[var(--color-textSecondary)]">
                <span className="text-[var(--color-textSecondary)] mr-2">{line.n}</span>
                {line.t}
              </div>
            ))}
            {mgr.taskLog.length === 0 && (
              <div className="text-[var(--color-textSecondary)]">No log entries</div>
            )}
          </div>
        </div>
      )}
