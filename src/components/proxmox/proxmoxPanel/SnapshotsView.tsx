...
const SnapshotsView: React.FC<SubProps> = ({ mgr }) => {
  ...
  return (
    <div className="p-6 overflow-y-auto flex-1">
      ...
      <div className="mb-6 p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)]">
        <h4 className="text-xs font-semibold text-[var(--color-text)] mb-3">
          {t("proxmox.snapshots.create", "Create Snapshot")}
        </h4>
        <div className="flex flex-col gap-2">
          <input
            className="w-full px-3 py-1.5 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] text-sm text-[var(--color-text)] focus:outline-none focus:ring-1 focus:ring-primary/50"
          ...
          <button
            ...
            className="w-full py-2 rounded-lg bg-primary hover:bg-primary disabled:bg-primary/50 text-[var(--color-text)] text-xs font-medium transition-colors flex items-center justify-center gap-1.5"
          >
            ...
            .map((snap) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {snap.description && (
                    <div className="text-[10px] text-[var(--color-textSecondary)]">{snap.description}</div>
                  )}
                  {snap.snaptime != null && (
