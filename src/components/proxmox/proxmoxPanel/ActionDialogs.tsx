...
const FooterButtons: React.FC<{ onCancel: () => void; submitLabel: string; submitting: boolean }> = ({
  ...
  <ModalFooter className="flex justify-end gap-3 border-t border-[var(--color-border)] p-4">
    ...
    <button
      type="submit"
      className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-[var(--color-text)] transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
      disabled={submitting}
    >
...
const ActionDialogs: React.FC<SubProps> = ({ mgr }) => {
  ...
  const suggestedVmId = useMemo(() => nextAvailableVmId(mgr), [mgr]);
