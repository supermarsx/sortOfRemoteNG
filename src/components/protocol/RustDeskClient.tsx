      ...
      <div className="flex-1 flex items-center justify-center bg-black">
        ...
        {mgr.connectionStatus === 'error' && (
          <div className="text-center max-w-lg px-4">
            ...
            <p className="text-[var(--color-textMuted)] text-sm mb-4">
              Unable to connect to {session.hostname}
            </p>
            {mgr.errorMessage && (
              <p className="text-[var(--color-textSecondary)] text-xs bg-[var(--color-surface)] border border-[var(--color-border)] rounded p-3 text-left">
                {mgr.errorMessage}
              </p>
            )}
            <button
              onClick={() => mgr.reconnect()}
              className="mt-4 px-3 py-1.5 text-xs bg-[var(--color-primary)] text-white rounded hover:opacity-90"
            >
              Retry
            </button>
          </div>
