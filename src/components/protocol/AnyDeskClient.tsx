      ...
      <div className="flex items-center justify-center h-full bg-[var(--color-background)] text-[var(--color-text)]">
        <div className="text-center">
          ...
          <p className="text-[var(--color-textSecondary)]">
            The connection for this session could not be found.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-2xl rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-8 shadow-sm">
          ...
          <p className="mx-auto mb-8 max-w-xl text-center text-[var(--color-textSecondary)]">
            Launch AnyDesk through the native Tauri backend when available, and
            fall back to the URL scheme when the desktop client cannot be
            spawned directly.
          </p>
              ...
              <div className="grid gap-2 md:grid-cols-2">
                ...
                <div>
                  ...
                  <span className="text-[var(--color-text)]">
                    {new Date(backendSession.start_time).toLocaleString()}
                  </span>
                </div>
                <div>
                  Remote ID:{" "}
                  <span className="text-[var(--color-text)]">
                    {backendSession.anydesk_id}
                  </span>
                </div>
                <div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            <button
              ...
              className="inline-flex items-center rounded-lg bg-primary px-6 py-3 font-medium text-[var(--color-text)] transition-colors duration-200 hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <button
              ...
              disabled={!backendSession && !session.backendSessionId}
          ...
          <div className="mt-6 grid gap-2 text-left text-xs text-[var(--color-textMuted)] md:grid-cols-2">
            ...
            <p>
              ...
              the desktop app can still open.
            </p>
          </div>
        </div>
      </div>
