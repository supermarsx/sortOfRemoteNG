...
function SettingsPanel({ m }: { m: Mgr }) {
  ...
  const toggle = (key: keyof VNCSettings) => (v: boolean) =>
    m.setSettings({ ...m.settings, [key]: v });
  return (
      ...
      {m.connectionStatus === "error" && (
        <div className="text-center">
          ...
          <p className="text-[var(--color-textMuted)] text-sm">
            Unable to connect to {m.session.hostname}
          </p>
          {m.errorMessage && (
            <p className="mt-3 max-w-md text-left text-xs text-[var(--color-textSecondary)] bg-[var(--color-surface)] border border-[var(--color-border)] rounded p-3">
              {m.errorMessage}
            </p>
          )}
        </div>
