...
const UnsupportedProtocol: React.FC<{ message: string }> = ({ message }) => (
  <div className="flex flex-1 items-center justify-center p-8">
    <EmptyState
      icon={FolderUp}
      iconSize={32}
      message={message}
      hint="Only SFTP has a wired frontend transfer runtime in this build."
    />
  </div>
);

const FileToolbar: React.FC<{ mgr: Mgr }> = ({ mgr }) => (
  <div className="bg-[var(--color-surfaceHover)] border-b border-[var(--color-border)] p-4">
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center space-x-2">
        ...
        <button
          onClick={() => mgr.loadDirectory(mgr.currentPath)}
          className="sor-icon-btn-sm"
          title="Refresh"
        >
          <RefreshCw size={16} />
      ...
      <div className="flex items-center space-x-2">
        <button
          onClick={() => mgr.setShowUploadDialog(true)}
      ...
      <table className="sor-data-table w-full">
        <thead className="bg-[var(--color-border)] sticky top-0">
          <tr>
            <th className="sor-th">
              <Checkbox
                checked={
                  mgr.selectedFiles.size === mgr.files.length &&
                  mgr.files.length > 0
                }
                onChange={(v: boolean) => mgr.handleSelectAll(v)}
        ...
        <tbody className="divide-y divide-[var(--color-border)]">
          {mgr.files.map((file) => (
            <tr
              key={file.name}
              className={`hover:bg-[var(--color-border)] cursor-pointer ${mgr.selectedFiles.has(file.name) ? "bg-primary/20" : ""}`}
    ...
    <div className="sor-selection-list flex-1 overflow-y-auto p-4">
      {mgr.transfers.length === 0 ? (
        ...
        mgr.transfers.map((transfer) => (
          <div
            key={transfer.id}
            className="sor-selection-row cursor-default bg-[var(--color-border)] p-3"
          >
            ...
            <div className="w-full bg-[var(--color-surfaceHover)] rounded-full h-2 mb-2">
              <div
                className={`h-2 rounded-full transition-all duration-300 ${transfer.status === "error" ? "bg-error" : transfer.status === "completed" ? "bg-success" : "bg-primary"}`}
            ...
            {transfer.error && (
              <p className="text-error text-xs mt-1">{transfer.error}</p>
            )}
            {transfer.status !== "active" &&
              transfer.status !== "completed" &&
              transfer.type === "download" && (
                <button
                  onClick={() => mgr.handleResumeTransfer(transfer.id)}
...
const UploadDialog: React.FC<{ mgr: Mgr }> = ({ mgr }) => (
  <Modal
    isOpen={mgr.showUploadDialog}
    onClose={() => mgr.setShowUploadDialog(false)}
    closeOnBackdrop={false}
    closeOnEscape={false}
    backdropClassName="z-[60] bg-black/60 p-4"
    panelClassName="max-w-md mx-4"
    dataTestId="file-transfer-upload-modal"
  >
    <div className="p-6">
      ...
      <div className="border-2 border-dashed border-[var(--color-border)] rounded-lg p-8 text-center">
        <Upload
          size={48}
          className="mx-auto text-[var(--color-textSecondary)] mb-4"
        />
        ...
        <input
          type="file"
          multiple
          onChange={(e) => e.target.files && mgr.handleUpload(e.target.files)}
          className="hidden"
          id="file-upload"
        />
        <div className="flex items-center justify-center gap-3 flex-wrap">
          <label
            htmlFor="file-upload"
            className="px-4 py-2 bg-primary hover:bg-primary/90 text-[var(--color-text)] rounded-md transition-colors cursor-pointer"
          >
            Select Files
          </label>
          {mgr.canPickAndUpload && (
            <button
              type="button"
              onClick={() => {
                void mgr.pickAndUpload();
              }}
              className="px-4 py-2 bg-[var(--color-surfaceHover)] hover:bg-[var(--color-border)] text-[var(--color-text)] rounded-md transition-colors"
              title="Open the native file picker (required until drag-and-drop is wired for Tauri)"
              data-testid="file-transfer-pick-and-upload"
            >
              Choose file…
            </button>
          )}
      ...
      <div className="flex justify-end space-x-3 mt-6">
        <button
          onClick={() => mgr.setShowUploadDialog(false)}
...
export const FileTransferManager: React.FC<FileTransferManagerProps> = ({
  ...
  // Pass `protocol` through so the hook auto-registers the right backend
  // adapter (SFTP → real Tauri `sftpApi` invoke chain; FTP/SCP → callers
  // still register their own until those paths are wired in follow-ups).
  const mgr = useFileTransfer(isOpen, connectionId, protocol);
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      closeOnEscape={false}
      panelClassName="max-w-6xl mx-4 max-h-[90vh]"
      contentClassName="overflow-hidden"
      dataTestId="file-transfer-manager-modal"
    >
      <div className="flex flex-col flex-1 min-h-0">
        <ManagerHeader protocol={protocol} />
        {mgr.isProtocolSupported ? (
          <>
          ...
          <div className="flex min-h-[320px] flex-1">
            <UnsupportedProtocol
              message={
                mgr.unsupportedReason ??
                "This file transfer protocol is not wired yet."
              }
            />
          </div>
        )}
