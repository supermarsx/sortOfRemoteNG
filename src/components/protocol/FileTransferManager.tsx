  ...
  <div className="bg-[var(--color-surfaceHover)] border-b border-[var(--color-border)] p-4">
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center space-x-2">
        <button onClick={() => mgr.navigateToPath("/")} className="sor-icon-btn-sm" title="Home"><Home size={16} /></button>
        ...
        <button onClick={() => mgr.loadDirectory(mgr.currentPath)} className="sor-icon-btn-sm" title="Refresh"><RefreshCw size={16} /></button>
      </div>
      <div className="flex items-center space-x-2">
    ...
    {mgr.isLoading ? (
      ...
      <table className="sor-data-table w-full">
        <thead className="bg-[var(--color-border)] sticky top-0">
          <tr>
            <th className="sor-th">
              <Checkbox checked={mgr.selectedFiles.size === mgr.files.length && mgr.files.length > 0} onChange={(v: boolean) => mgr.handleSelectAll(v)} className="rounded border-[var(--color-border)] bg-[var(--color-input)] text-primary" />
            </th>
        ...
        <tbody className="divide-y divide-[var(--color-border)]">
          {mgr.files.map((file) => (
            <tr key={file.name} className={`hover:bg-[var(--color-border)] cursor-pointer ${mgr.selectedFiles.has(file.name) ? "bg-primary/20" : ""}`} onClick={() => mgr.handleFileSelect(file.name)} onDoubleClick={() => mgr.handleDoubleClick(file)}>
              <td className="px-4 py-3"><Checkbox checked={mgr.selectedFiles.has(file.name)} onChange={() => mgr.handleFileSelect(file.name)} className="rounded border-[var(--color-border)] bg-[var(--color-input)] text-primary" /></td>
  ...
  <Modal isOpen={mgr.showUploadDialog} onClose={() => mgr.setShowUploadDialog(false)} closeOnBackdrop={false} closeOnEscape={false} backdropClassName="z-[60] bg-black/60 p-4" panelClassName="max-w-md mx-4" dataTestId="file-transfer-upload-modal">
    <div className="p-6">
      ...
      <div className="border-2 border-dashed border-[var(--color-border)] rounded-lg p-8 text-center">
        ...
        <div className="flex items-center justify-center gap-3 flex-wrap">
          ...
          {mgr.canPickAndUpload && (
            <button
              type="button"
              onClick={() => { void mgr.pickAndUpload(); }}
      ...
      <div className="flex justify-end space-x-3 mt-6">
        <button onClick={() => mgr.setShowUploadDialog(false)} className="sor-btn sor-btn-secondary">Cancel</button>
      </div>
    </div>
  </Modal>
);
