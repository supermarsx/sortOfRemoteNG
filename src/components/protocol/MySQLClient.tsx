...
const MySQLHeader: React.FC<{ mgr: Mgr }> = ({ mgr }) => (
  <div className="bg-[var(--color-surface)] border-b border-[var(--color-border)] p-4">
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center space-x-3">
        <Select value={mgr.selectedDatabase} onChange={(v: string) => mgr.setSelectedDatabase(v)} options={[...mgr.databases.map((db) => ({ value: db, label: db }))]} className="px-3 py-1 bg-[var(--color-input)] border border-[var(--color-border)] rounded text-[var(--color-text)] text-sm" />
        <button
...
const QueryEditor: React.FC<{ mgr: Mgr }> = ({ mgr }) => (
  <div className="bg-[var(--color-surface)] border-b border-[var(--color-border)] p-4">
    ...
    <Textarea
      value={mgr.query}
      onChange={(v) => mgr.setQuery(v)}
      className="w-full h-32 px-3 py-2 bg-[var(--color-input)] border border-[var(--color-border)] rounded text-[var(--color-text)] font-mono text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary"
...
const QueryResults: React.FC<{ mgr: Mgr }> = ({ mgr }) => {
  const exportCSV = () => {
    ...
    const headers = mgr.results.columns.map(sanitize).join(',');
    const rows = mgr.results.rows.map(row =>
      row.map(cell => sanitize(String(cell ?? ''))).join(',')
    );
              ...
              <thead className="bg-[var(--color-border)] sticky top-0">
                <tr>
                  {mgr.results.columns.map((column, index) => (
                    <th
                      key={index}
                      className="sor-th border-r border-[var(--color-border)] last:border-r-0"
                    >
                      {column}
                    </th>
                  ))}
