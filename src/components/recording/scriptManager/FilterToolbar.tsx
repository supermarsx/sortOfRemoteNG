  ...
  return (
    <div className="border-b border-[var(--color-border)] px-5 py-3 flex items-center gap-4 bg-[var(--color-surfaceHover)]/30">
      ...
      <div className="relative">
        <Select value={mgr.categoryFilter} onChange={(v: string) => mgr.setCategoryFilter(v)} options={[{ value: '', label: t('scriptManager.allCategories', 'All Categories') }, ...mgr.categories.map((cat) => ({ value: cat, label: cat }))]} className="appearance-none pl-3 pr-8 py-2 text-sm bg-[var(--color-input)] border border-[var(--color-border)] rounded-lg text-[var(--color-text)] focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer" />
        <ChevronDown size={14} className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--color-textSecondary)] pointer-events-none" />
      </div>

      {/* Language filter */}
      <div className="relative">
        <Select value={mgr.languageFilter} onChange={(v: string) => mgr.setLanguageFilter(v as ScriptLanguage | '')} options={[{ value: "", label: t('scriptManager.allLanguages', 'All Languages') }, { value: "bash", label: "Bash" }, { value: "sh", label: "Shell (sh)" }, { value: "powershell", label: "PowerShell" }, { value: "batch", label: "Batch (cmd)" }]} className="appearance-none pl-3 pr-8 py-2  bg-[var(--color-input)] border border-[var(--color-border)] rounded-lg text-[var(--color-text)] focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer" />
        <ChevronDown size={14} className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--color-textSecondary)] pointer-events-none" />
      </div>
