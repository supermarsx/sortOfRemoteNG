      ...
      <div className="space-y-4 max-w-3xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Select value={mgr.editLanguage} onChange={(v: string) => mgr.setEditLanguage(v as ScriptLanguage)} options={[{ value: "auto", label: "🔍 Auto Detect" }, { value: "bash", label: "🐚 Bash" }, { value: "sh", label: "📜 Shell (sh)" }, { value: "powershell", label: "⚡ PowerShell" }, { value: "batch", label: "🪟 Batch (cmd)" }]} className="w-full px-3 py-2  bg-[var(--color-input)] border border-[var(--color-border)] rounded-lg text-[var(--color-text)] focus:outline-none focus:ring-1 focus:ring-primary" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--color-text)] mb-1.5">
        ...
        <div>
          ...
          <div className="relative">
            <Textarea
              ...
              onChange={(v) => mgr.setEditScript(v)}
              placeholder={t('scriptManager.scriptPlaceholder', 'Enter your script here...')}
              className="w-full h-64 px-4 py-3 text-sm bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg text-[var(--color-text)] placeholder-[var(--color-textMuted)] focus:outline-none focus:ring-1 focus:ring-primary font-mono resize-y"
              spellCheck={false}
            />
          </div>
