  ...
  return (
    <div className="bg-[var(--color-surface)] border-b border-[var(--color-border)] px-4 py-2 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-2">
        ...
        <button onClick={() => m.setShowSettings(!m.showSettings)} className="p-1 hover:bg-[var(--color-border)] rounded transition-colors text-[var(--color-textSecondary)] hover:text-[var(--color-text)]" title="VNC Settings"><Settings size={14} /></button>
        <button onClick={m.toggleFullscreen} className="p-1 hover:bg-[var(--color-border)] rounded transition-colors text-[var(--color-textSecondary)] hover:text-[var(--color-text)]" title={m.isFullscreen ? 'Exit fullscreen' : 'Fullscreen'}>{m.isFullscreen ? <Minimize2 size={14} /> : <Maximize2 size={14} />}</button>
      </div>
    </div>
  );
}
    ...
    <div className="flex-1 flex items-center justify-center bg-black p-4">
      ...
      {m.connectionStatus === 'error' && (
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">Unable to connect to {m.session.hostname}</p>
        </div>
      )}
