    ...
    <div className="sor-credential-dialog-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="sor-credential-dialog bg-[var(--color-bgPrimary)] border border-[var(--color-border)] rounded-lg shadow-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} disabled={saving} className="sor-btn-icon p-1 rounded hover:bg-[var(--color-surfaceHover)]">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="space-y-3">
          <label className="block">
            ...
            <input value={form.label} onChange={(e) => set("label", e.target.value)} className="sor-input mt-1 w-full rounded border border-[var(--color-border)] bg-[var(--color-surfaceHover)] px-3 py-2 text-sm text-[var(--color-textPrimary)]" />
          </label>
          <label className="block">
            <span className="text-sm text-[var(--color-textSecondary)]">{t("credentials.fields.connectionName")} *</span>
            <input value={form.connectionName} onChange={(e) => set("connectionName", e.target.value)} className="sor-input mt-1 w-full rounded border border-[var(--color-border)] bg-[var(--color-surfaceHover)] px-3 py-2 text-sm text-[var(--color-textPrimary)]" />
          </label>
          <label className="block">
          ...
          <label className="block">
            ...
            <input type="date" value={form.expiresAt?.split("T")[0] ?? ""} onChange={(e) => set("expiresAt", e.target.value)} className="sor-input mt-1 w-full rounded border border-[var(--color-border)] bg-[var(--color-surfaceHover)] px-3 py-2 text-sm text-[var(--color-textPrimary)]" />
          </label>
        </div>

        <div className="flex justify-end gap-2 mt-6">
          <button onClick={onClose} disabled={saving} className="sor-btn px-4 py-2 text-sm rounded border border-[var(--color-border)] text-[var(--color-textSecondary)] hover:bg-[var(--color-surfaceHover)]">
            {t("common.cancel")}
          </button>
          <button onClick={handleSaveClick} disabled={!isValid || saving} className="sor-btn sor-btn-primary px-4 py-2 text-sm rounded bg-primary text-[var(--color-text)] hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
    ...
    <div className="sor-duplicates-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="sor-duplicates-panel bg-[var(--color-bgPrimary)] border border-[var(--color-border)] rounded-lg shadow-xl w-full max-w-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="sor-btn-icon p-1 rounded hover:bg-[var(--color-surfaceHover)]"><X size={16} /></button>
        </div>
        {groups.length === 0 ? (
...
export function CredentialManager() {
  ...
  useEffect(() => {
    ...
    creds.fetchAlerts();
    creds.fetchAuditLog();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- mount-only: initial data fetch
  }, []);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- tab and expiringDays are the only meaningful triggers
  }, [tab, expiringDays]);
    ...
    <div className="sor-credential-manager flex flex-col h-full overflow-hidden text-[var(--color-textPrimary)]">
      ...
      <div className="sor-header flex flex-wrap items-center gap-3 px-4 py-3 border-b border-[var(--color-border)]">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button onClick={() => { setEditingCred(null); setShowDialog(true); }} className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1.5 text-xs bg-primary text-[var(--color-text)] hover:bg-primary/90">
            <Plus size={14} /> {t("credentials.addBtn")}
          </button>
          <button onClick={() => creds.generateAlerts()} className="sor-btn flex items-center gap-1 rounded border border-[var(--color-border)] px-3 py-1.5 text-xs text-[var(--color-textSecondary)] hover:bg-[var(--color-surfaceHover)]">
            <Bell size={14} /> {t("credentials.generateAlerts")}
          </button>
          <button onClick={handleDetectDuplicates} className="sor-btn flex items-center gap-1 rounded border border-[var(--color-border)] px-3 py-1.5 text-xs text-[var(--color-textSecondary)] hover:bg-[var(--color-surfaceHover)]">
          ...
          creds.credentials.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="sor-table w-full text-sm">
                ...
                <tbody>
                  {sorted.map((c) => (
                    <tr key={c.id} className="sor-row border-b border-[var(--color-border)] hover:bg-[var(--color-surfaceHover)] transition-colors">
                      <td className="px-3 py-2 font-medium">{c.label}</td>
                      <td className="px-3 py-2 text-[var(--color-textSecondary)]">{c.connectionName}</td>
                      <td className="px-3 py-2"><span className="sor-kind-badge rounded bg-[var(--color-surfaceHover)] px-1.5 py-0.5 text-xs">{c.kind.replace(/_/g, " ")}</span></td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1">
                          <button onClick={() => creds.recordRotation(c.id)} title={t("credentials.rotate")} className="sor-btn-icon p-1 rounded hover:bg-[var(--color-surfaceHover)]"><RotateCw size={14} /></button>
                          <button onClick={() => { setEditingCred(c); setShowDialog(true); }} title={t("common.edit")} className="sor-btn-icon p-1 rounded hover:bg-[var(--color-surfaceHover)]"><Edit size={14} /></button>
        ...
        {tab === "expiring" && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <input type="number" value={expiringDays} onChange={(e) => setExpiringDays(Number(e.target.value))} min={1} max={365} className="sor-input w-20 rounded border border-[var(--color-border)] bg-[var(--color-surfaceHover)] px-2 py-1 text-xs text-[var(--color-textPrimary)]" />
            </div>
              ...
              <ul className="space-y-2">
                {expiringSoon.map((c) => (
                  <li key={c.id} className="sor-expiring-item flex items-center justify-between rounded border border-warning/30 bg-warning/5 px-4 py-2">
                    ...
                    <button onClick={() => creds.recordRotation(c.id)} className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1 text-xs bg-warning text-[var(--color-text)] hover:bg-warning/90">
            ...
            <ul className="space-y-2">
              {expiredList.map((c) => (
                <li key={c.id} className="sor-expired-item flex items-center justify-between rounded border border-error/30 bg-error/5 px-4 py-2">
                  ...
                  <button onClick={() => creds.recordRotation(c.id)} className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1 text-xs bg-error text-[var(--color-text)] hover:bg-error/90">
        ...
        {tab === "groups" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => { const name = prompt(t("credentials.groups.namePrompt")); if (name) creds.createGroup(name, ""); }} className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1.5 text-xs bg-primary text-[var(--color-text)] hover:bg-primary/90">
                    ...
                    <li key={g.id} className="sor-group-card border border-[var(--color-border)] rounded">
                      <div className="flex items-center justify-between px-4 py-2 hover:bg-[var(--color-surfaceHover)]">
                        ...
                        >
                          ...
                          <span className="sor-badge rounded-full bg-[var(--color-surfaceHover)] px-2 py-0.5 text-xs text-[var(--color-textSecondary)]">{g.credentialIds.length} members</span>
                        </button>
        ...
        {tab === "policies" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => { const name = prompt(t("credentials.policies.namePrompt")); if (name) creds.addPolicy({ name, kind: "password", maxAgeDays: 90, warningDays: 14, requireMinStrength: "fair", minLength: 12, requireUppercase: true, requireLowercase: true, requireDigits: true, requireSpecial: false, forbidReuse: 3, enabled: true }); }} className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1.5 text-xs bg-primary text-[var(--color-text)] hover:bg-primary/90">
                ...
                {creds.policies.map((p) => (
                  <div key={p.id} className="sor-policy-card border border-[var(--color-border)] rounded p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-1">
                        <span className={`sor-badge rounded-full px-2 py-0.5 text-xs ${p.enabled ? "bg-success/15 text-success" : "bg-[var(--color-surfaceHover)] text-[var(--color-textSecondary)]"}`}>
                    ...
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      ...
                      <dd className="flex flex-wrap gap-1">
                        {p.requireUppercase && <span className="sor-rule-tag rounded bg-[var(--color-surfaceHover)] px-1">A-Z</span>}
                        {p.requireLowercase && <span className="sor-rule-tag rounded bg-[var(--color-surfaceHover)] px-1">a-z</span>}
                        {p.requireDigits && <span className="sor-rule-tag rounded bg-[var(--color-surfaceHover)] px-1">0-9</span>}
                        {p.requireSpecial && <span className="sor-rule-tag rounded bg-[var(--color-surfaceHover)] px-1">!@#</span>}
                      </dd>
