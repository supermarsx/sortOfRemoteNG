...
function StrengthMeter({ strength }: { strength: CredentialStrength }) {
  ...
  return (
    <div className="sor-strength-meter flex items-center gap-2">
      <div className="h-2 w-20 rounded bg-[var(--color-surfaceHover)] overflow-hidden">
        <div
...
function CredentialDialog({ credential, onSave, onClose }: CredDialogProps) {
  ...
  const isValid =
    form.label.trim().length > 0 && form.connectionName.trim().length > 0;
      ...
      <div className="sor-credential-dialog bg-[var(--color-bgPrimary)] border border-[var(--color-border)] rounded-lg shadow-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={onClose}
            disabled={saving}
            className="sor-btn-icon p-1 rounded hover:bg-[var(--color-surfaceHover)]"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="space-y-3">
          <label className="block">
            <span className="text-sm text-[var(--color-textSecondary)]">
              {t("credentials.fields.name")} *
            </span>
            <input
              value={form.label}
              onChange={(e) => set("label", e.target.value)}
              className="sor-input mt-1 w-full rounded border border-[var(--color-border)] bg-[var(--color-surfaceHover)] px-3 py-2 text-sm text-[var(--color-textPrimary)]"
            />
          </label>
          <label className="block">
            <span className="text-sm text-[var(--color-textSecondary)]">
              {t("credentials.fields.connectionName")} *
            </span>
            <input
          ...
          <label className="block">
            <span className="text-sm text-[var(--color-textSecondary)]">
              {t("credentials.fields.expiresAt")}
            </span>
            <input
              type="date"
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button
            onClick={onClose}
            disabled={saving}
            className="sor-btn px-4 py-2 text-sm rounded border border-[var(--color-border)] text-[var(--color-textSecondary)] hover:bg-[var(--color-surfaceHover)]"
          >
            {t("common.cancel")}
          </button>
          <button
            ...
            className="sor-btn sor-btn-primary px-4 py-2 text-sm rounded bg-primary text-[var(--color-text)] hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
...
function DuplicatesPanel({
  ...
  const nameOf = (id: string) =>
    credentials.find((c) => c.id === id)?.label ?? id;
            ...
            {groups.map((g) => (
              ...
              >
                <p className="text-sm font-medium text-[var(--color-textPrimary)]">
                  {g.count} duplicates (hash: {g.hash.slice(0, 8)}…)
                </p>
...
export function CredentialRotationTracker() {
  const { t } = useTranslation();
  ...
  const [editingCred, setEditingCred] =
    useState<Partial<TrackedCredential> | null>(null);
  ...
  useEffect(() => {
    ...
    creds.fetchAlerts();
    creds.fetchAuditLog();
    // eslint-disable-next-line react-hooks/exhaustive-deps, react/exhaustive-deps -- mount-only: initial data fetch
  }, []);

  /* refresh expiring / expired when tab changes */
  useEffect(() => {
    if (tab === "expiring")
      creds.getExpiringSoon(expiringDays).then(setExpiringSoon);
    if (tab === "expired") creds.getExpired().then(setExpiredList);
    // eslint-disable-next-line react-hooks/exhaustive-deps, react/exhaustive-deps -- tab and expiringDays are the only meaningful triggers
  }, [tab, expiringDays]);
  ...
  const handleSave = useCallback(
    async (data: Partial<TrackedCredential>) => {
      try {
        if (data.id) {
        ...
        setEditingCred(null);
        toast.success(
          data.id ? t("credentials.saved") : t("credentials.added"),
        );
      } catch (err) {
        console.error(err);
        toast.error(t("credentials.operationFailed", "Operation failed"));
      }
    },
    [creds, toast, t],
  );
  ...
  return (
    <div className="sor-credential-manager flex flex-col h-full overflow-hidden text-[var(--color-textPrimary)]">
        ...
        <div className="sor-alerts mx-4 mt-2 space-y-1">
          {creds.alerts
            ...
            .map((a) => (
              <div
                key={a.id}
                className={`sor-alert flex items-center justify-between rounded px-3 py-1.5 text-xs ${a.severity === "critical" ? "bg-error/10 text-error" : a.severity === "warning" ? "bg-warning/10 text-warning" : "bg-primary/10 text-primary"}`}
              >
                <span className="flex items-center gap-1">
                  <Bell size={12} /> {a.message}
                </span>
                <button
                  onClick={() => creds.acknowledgeAlert(a.id)}
                  className="sor-btn-icon ml-2 hover:opacity-80"
                >
                  <Check size={12} />
                </button>
              </div>
      ...
      <div className="sor-header flex flex-wrap items-center gap-3 px-4 py-3 border-b border-[var(--color-border)]">
        ...
        {stats && (
          <div className="flex items-center gap-2 ml-2">
            <span className="sor-badge rounded-full bg-primary/15 text-primary px-2 py-0.5 text-xs">
              {stats.total} total
            </span>
            <span className="sor-badge rounded-full bg-warning/15 text-warning px-2 py-0.5 text-xs">
              {stats.expiringSoon} expiring
            </span>
            <span className="sor-badge rounded-full bg-error/15 text-error px-2 py-0.5 text-xs">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button
            ...
            className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1.5 text-xs bg-primary text-[var(--color-text)] hover:bg-primary/90"
          >
            <Plus size={14} /> {t("credentials.addBtn")}
          </button>
          <button
            onClick={() => creds.generateAlerts()}
            className="sor-btn flex items-center gap-1 rounded border border-[var(--color-border)] px-3 py-1.5 text-xs text-[var(--color-textSecondary)] hover:bg-[var(--color-surfaceHover)]"
          >
      ...
      {/* ---------- tabs ---------- */}
      <div
        className="sor-tab-bar flex gap-1 px-4 pt-2 border-b border-[var(--color-border)]"
        role="tablist"
        aria-label="Credential manager sections"
      >
        {TABS.map((tb) => (
          <button
            key={tb.id}
            ...
            className={`sor-tab px-3 py-2 text-xs font-medium rounded-t transition-colors ${tab === tb.id ? "bg-[var(--color-surfaceHover)] text-[var(--color-textPrimary)] border-b-2 border-primary" : "text-[var(--color-textSecondary)] hover:text-[var(--color-textPrimary)]"}`}
          >
            {t(tb.labelKey)}
          </button>
        ))}
      ...
      >
              ...
              <table className="sor-table w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--color-border)]">
                    ...
                    {sortHeader(
                      t("credentials.col.lastRotated"),
                      "lastRotated",
                    )}
                    <th className="sor-th px-3 py-2 text-left text-xs font-medium text-[var(--color-textSecondary)]">
                ...
                <tbody>
                  {sorted.map((c) => (
                    <tr
                      key={c.id}
                      className="sor-row border-b border-[var(--color-border)] hover:bg-[var(--color-surfaceHover)] transition-colors"
                    >
                      ...
                      <td className="px-3 py-2">
                        <span className="sor-kind-badge rounded bg-[var(--color-surfaceHover)] px-1.5 py-0.5 text-xs">
                          {c.kind.replace(/_/g, " ")}
                        </span>
                      </td>
                      <td className="px-3 py-2">{c.ageDays}d</td>
                      <td
                        className={`px-3 py-2 ${c.isExpired ? "text-error" : ""}`}
                      >
                        {c.expiresAt
                      ...
                      <td className="px-3 py-2">
                        <StrengthMeter strength={c.strength} />
                      </td>
                      <td className="px-3 py-2 text-[var(--color-textSecondary)]">
                        {c.lastRotated
                          ? new Date(c.lastRotated).toLocaleDateString()
                          : "—"}
                      </td>
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => creds.recordRotation(c.id)}
                            title={t("credentials.rotate")}
                            className="sor-btn-icon p-1 rounded hover:bg-[var(--color-surfaceHover)]"
                          >
                            <RotateCw size={14} />
                          </button>
                          ...
                          <button
                            onClick={() => creds.remove(c.id)}
                            title={t("common.delete")}
                            className="sor-btn-icon p-1 rounded hover:bg-error/10 text-error"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <input
                ...
                min={1}
                max={365}
                className="sor-input w-20 rounded border border-[var(--color-border)] bg-[var(--color-surfaceHover)] px-2 py-1 text-xs text-[var(--color-textPrimary)]"
              />
            </div>
            {expiringSoon.length === 0 ? (
              ...
              <ul className="space-y-2">
                {expiringSoon.map((c) => (
                  <li
                    key={c.id}
                    className="sor-expiring-item flex items-center justify-between rounded border border-warning/30 bg-warning/5 px-4 py-2"
                  >
                    <div>
                    ...
                    <button
                      onClick={() => creds.recordRotation(c.id)}
                      className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1 text-xs bg-warning text-[var(--color-text)] hover:bg-warning/90"
                    >
          ...
          (expiredList.length === 0 ? (
            ...
            <ul className="space-y-2">
              {expiredList.map((c) => (
                <li
                  key={c.id}
                  className="sor-expired-item flex items-center justify-between rounded border border-error/30 bg-error/5 px-4 py-2"
                >
                  <div>
                    <p className="text-sm font-medium text-error">{c.label}</p>
                    <p className="text-xs text-[var(--color-textSecondary)]">
                      {c.connectionName} — expired{" "}
                      {c.expiresAt
                        ? new Date(c.expiresAt).toLocaleDateString()
                        : ""}
                    </p>
                  ...
                  <button
                    onClick={() => creds.recordRotation(c.id)}
                    className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1 text-xs bg-error text-[var(--color-text)] hover:bg-error/90"
                  >
        ...
        {tab === "groups" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={() => {
                  const name = prompt(t("credentials.groups.namePrompt"));
                  if (name) creds.createGroup(name, "");
                }}
                className="sor-btn sor-btn-primary flex items-center gap-1 rounded px-3 py-1.5 text-xs bg-primary text-[var(--color-text)] hover:bg-primary/90"
              >
            ...
            {creds.groups.length === 0 ? (
              ...
              <ul className="space-y-2">
                {creds.groups.map((g) => {
                  ...
                  const members = creds.credentials.filter((c) =>
                    g.credentialIds.includes(c.id),
                  );
                  return (
                    <li
                      key={g.id}
                      className="sor-group-card border border-[var(--color-border)] rounded"
                    >
                      <div className="flex items-center justify-between px-4 py-2 hover:bg-[var(--color-surfaceHover)]">
                        <button
                          type="button"
                        ...
                        >
                          ...
                          <span className="sor-badge rounded-full bg-[var(--color-surfaceHover)] px-2 py-0.5 text-xs text-[var(--color-textSecondary)]">
                            {g.credentialIds.length} members
                          </span>
                        </button>
                        <button
                          onClick={() => creds.deleteGroup(g.id)}
                          className="sor-btn-icon p-1 rounded hover:bg-error/10 text-error"
                      ...
                      {expanded && (
                        <div
                          id={`credential-group-panel-${g.id}`}
                          className="px-4 pb-3 border-t border-[var(--color-border)]"
                        >
                          {members.length === 0 ? (
                            <p className="text-xs text-[var(--color-textSecondary)] py-2">
                              {t("credentials.groups.noMembers")}
                            </p>
                          ) : (
                            <ul className="mt-2 space-y-1">
                              {members.map((m) => (
                                <li
                                  key={m.id}
                                  className="flex items-center justify-between text-xs text-[var(--color-textSecondary)] py-1"
                                >
                                  <span>
                                    {m.label} ({m.connectionName})
                                  </span>
                                  <button
                                    onClick={() =>
                                      creds.removeFromGroup(g.id, m.id)
                                    }
                                    className="sor-btn-icon p-0.5 rounded hover:bg-error/10 text-error"
                                  >
                                    <X size={12} />
                                  </button>
                                </li>
                          ...
                          <div className="mt-2">
                            <Select
                              value=""
                              onChange={(v) => {
                                if (v) creds.addToGroup(g.id, v);
                              }}
                              variant="form-sm"
                              options={[
                                {
                                  value: "",
                                  label: t("credentials.groups.addMember"),
                                },
                                ...creds.credentials
                                  .filter(
                                    (c) => !g.credentialIds.includes(c.id),
                                  )
                                  .map((c) => ({
                                    value: c.id,
                                    label: c.label,
        ...
        {tab === "policies" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold">
                {t("credentials.policies.title")}
              </h3>
              <button
                onClick={() => {
                  const name = prompt(t("credentials.policies.namePrompt"));
                  if (name)
                    creds.addPolicy({
                      name,
                      kind: "password",
            ...
            {creds.policies.length === 0 ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {creds.policies.map((p) => (
                  <div
                    key={p.id}
                    className="sor-policy-card border border-[var(--color-border)] rounded p-4"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm font-medium">{p.name}</h4>
                      <div className="flex items-center gap-1">
                        <span
                          className={`sor-badge rounded-full px-2 py-0.5 text-xs ${p.enabled ? "bg-success/15 text-success" : "bg-[var(--color-surfaceHover)] text-[var(--color-textSecondary)]"}`}
                        >
                          {p.enabled
                            ? t("common.enabled")
                            : t("common.disabled")}
                        ...
                        <button
                          onClick={() => creds.removePolicy(p.id)}
                          className="sor-btn-icon p-1 rounded hover:bg-error/10 text-error"
                        >
                          <Trash2 size={14} />
                        </button>
                    ...
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      <dt className="text-[var(--color-textSecondary)]">
                        {t("credentials.policies.kind")}
                      </dt>
                      <dd>{p.kind.replace(/_/g, " ")}</dd>
                      <dt className="text-[var(--color-textSecondary)]">
                        {t("credentials.policies.maxAge")}
                      </dt>
                      <dd>{p.maxAgeDays} days</dd>
                      <dt className="text-[var(--color-textSecondary)]">
                        {t("credentials.policies.warning")}
                      </dt>
                      <dd>{p.warningDays} days</dd>
                      <dt className="text-[var(--color-textSecondary)]">
                        {t("credentials.policies.minStrength")}
                      </dt>
                      <dd>{p.requireMinStrength.replace(/_/g, " ")}</dd>
                      <dt className="text-[var(--color-textSecondary)]">
                        {t("credentials.policies.minLength")}
                      </dt>
                      <dd>{p.minLength} chars</dd>
                      <dt className="text-[var(--color-textSecondary)]">
                        {t("credentials.policies.rules")}
                      </dt>
                      <dd className="flex flex-wrap gap-1">
                        {p.requireUppercase && (
                          <span className="sor-rule-tag rounded bg-[var(--color-surfaceHover)] px-1">
                            A-Z
                          </span>
                        )}
                        {p.requireLowercase && (
                      ...
                      <dt className="text-[var(--color-textSecondary)]">
                        {t("credentials.policies.forbidReuse")}
                      </dt>
          ...
          (creds.auditLog.length === 0 ? (
            ...
            <div className="space-y-1">
              {creds.auditLog.map((entry) => {
                const cred = creds.credentials.find(
                  (c) => c.id === entry.credentialId,
                );
                return (
                  <div
                    key={entry.id}
                    className="sor-audit-entry flex items-start gap-3 border-b border-[var(--color-border)] py-2"
                  >
                    <span className="text-xs text-[var(--color-textSecondary)] whitespace-nowrap min-w-[140px]">
                      {new Date(entry.timestamp).toLocaleString()}
                    </span>
                    <span className="sor-audit-action rounded bg-[var(--color-surfaceHover)] px-1.5 py-0.5 text-xs font-medium min-w-[80px] text-center">
                      {entry.action}
                    </span>
