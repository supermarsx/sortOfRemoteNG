...
export default function TrustCenterTab({
  ...
  const [section, setSection] = useState<"identities" | "redirects">(
    "identities",
  );
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--color-border)] p-4">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--color-textMuted)]">
            {section === "redirects"
              ? "Saved-connection redirect destinations in the current database."
              : mgr.databaseName
          ...
          <p className="mt-1 max-w-3xl text-xs text-[var(--color-textMuted)]">
            {section === "redirects"
              ? "Redirect destination preferences are separate from certificate trust and saved-login permissions."
              : "Certificates and SSH host identities—not passwords, private keys, or the credential vault. Trust is database-scoped. Global verification policies remain in Settings → Trust Center."}
          </p>
        </div>
      ...
      <nav
        ...
        className="flex shrink-0 gap-1 border-b border-[var(--color-border)] px-4 py-2"
      >
        {(
          [
            ["identities", "Certificates & host keys"],
            ["redirects", "Redirect destinations"],
          ] as const
        ).map(([value, label]) => (
          <button
            key={value}
            ...
            className={`${button} ${section === value ? "!border-primary !text-primary bg-primary/10" : ""}`}
            onClick={() => {
              mgr.dismissReview();
              setDetail(null);
              setSection(value);
      ...
      {section === "redirects" ? (
        <TrustedRedirectDestinationsPanel />
      ) : (
        <>
          <div className="space-y-3 border-b border-[var(--color-border)] p-4">
            ...
            {mgr.summary && (
              ...
              >
                ...
                {mgr.summary.revoked_count} revoked ·{" "}
                {mgr.summary.expired_count} expired ·{" "}
                {mgr.summary.total_verifications} verifications ·{" "}
            ...
            {mgr.busy && (
              <p role="status" className="text-xs">
                Validating the reviewed database and identities. A record batch
                is applied as one write after every target passes validation.
              </p>
            )}
          </div>
          ...
          >
            <span>
              Page {currentPage + 1} of {pageCount} · up to 100 identities per
              page
            </span>
