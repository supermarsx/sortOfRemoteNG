...
const AuthMethodButton: React.FC<{
  ...
  <button
    ...
    className={`flex-1 flex items-center justify-center space-x-2 px-3 py-2.5 rounded-lg border transition-all ${
      active ? 'border-primary text-primary' : 'bg-[var(--color-input)] border-[var(--color-border)] text-[var(--color-textSecondary)] hover:border-[var(--color-border)]'
  ...
  <form onSubmit={mgr.handleSubmit} className="space-y-4">
    <div>
      ...
      <div className="relative">
        <input type={mgr.showPassword ? 'text' : 'password'} required value={mgr.password} onChange={(e) => mgr.setPassword(e.target.value)} disabled={noCollectionSelected} className="sor-form-input w-full pr-10 disabled:opacity-50" placeholder="Enter password" minLength={4} autoFocus aria-label="Password" data-testid="password-input" />
    ...
    <div className="flex justify-end space-x-3 pt-2">
      ...
      <button type="submit" disabled={mgr.passwordSubmitDisabled} data-testid="password-submit" className="px-4 py-2 bg-primary hover:bg-primary/90 disabled:bg-[var(--color-surfaceHover)] disabled:cursor-not-allowed text-[var(--color-text)] rounded-md transition-colors flex items-center space-x-2">
...
const PasskeyForm: React.FC<{ mgr: Mgr; mode: 'setup' | 'unlock'; noCollectionSelected: boolean }> = ({ mgr, mode, noCollectionSelected }) => (
  <div className="space-y-4">
    <div className="bg-[var(--color-border)] rounded-lg p-6 text-center">
      ...
      <p className="text-[var(--color-textSecondary)] mb-2">{mode === 'setup' ? `Use ${mgr.biometricInfo?.platformLabel ?? 'your device biometrics'} to secure your data` : `Authenticate with ${mgr.biometricInfo?.platformLabel ?? 'device biometrics'}`}</p>
...
export const PasswordDialog: React.FC<PasswordDialogProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={mgr.handleCancel} closeOnBackdrop closeOnEscape panelClassName="max-w-md mx-4 rounded-xl border border-[var(--color-border)] animate-in fade-in zoom-in-95 duration-200" dataTestId="password-dialog">
