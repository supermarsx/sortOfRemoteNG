...
function DropZone({ m }: { m: Mgr }) {
  return (
    ...
    >
      ...
      <span className="text-[10px] text-[var(--color-textMuted)] mt-1">{IMPORT_SOURCES.find((s) => s.id === m.source)?.extensions.join(", ") || ".json, .csv, .txt"}</span>
...
function ResultErrors({ m }: { m: Mgr }) {
  ...
  return (
    <div className="px-5 py-2 bg-warning/5 border-b border-warning/20">
      ...
      <div className="mt-1 max-h-16 overflow-y-auto">{m.result.errors.map((err, i) => <div key={`err-${err.slice(0, 50)}-${i}`} className="text-[10px] text-warning/70">{err}</div>)}</div>
    </div>
  );
}
...
function EntryList({ m }: { m: Mgr }) {
  if (!m.result) return null;
  if (m.result.entries.length === 0) return <div className="p-6 text-center text-[var(--color-textMuted)] text-sm">No TOTP entries found in this file</div>;
  return (
    <div className="sor-selection-list gap-0">
      {m.result.entries.map((entry, i) => {
        ...
        return (
          <div key={`totp-${entry.issuer}-${entry.account}-${i}`} onClick={() => m.toggleEntry(i)} className={`sor-selection-row rounded-none border-x-0 border-t-0 border-b border-[var(--color-border)]/30 ${isSelected ? "sor-selection-row-selected bg-primary/10" : "hover:bg-[var(--color-surface)]/40"} ${isDuplicate ? "opacity-50" : ""}`}>
    ...
    <Modal isOpen onClose={onClose} backdropClassName="bg-black/60 z-[10000]" panelClassName="w-[640px] max-w-[95vw] max-h-[80vh] rounded-xl border border-[var(--color-border)] overflow-hidden" contentClassName="bg-[var(--color-background)]">
      <div className="flex flex-1 min-h-0 flex-col">
        ...
        <div className="px-5 py-3 border-b border-[var(--color-border)]/50 space-y-3">
          <SourceSelector m={m} />
          <div className="text-[10px] text-[var(--color-textMuted)] ml-[76px]">{IMPORT_SOURCES.find((s) => s.id === m.source)?.description}</div>
          <DropZone m={m} />
          <QrHint m={m} />
          <QrPreview m={m} />
        </div>
