...
export const WebTerminal: React.FC<WebTerminalProps> = ({
  ...
  const serializeBuffer = useCallback(() => {
    ...
    const trimmedLines =
      lastNonEmptyLine >= 0 ? lines.slice(0, lastNonEmptyLine + 1) : [];
  ...
  const disconnectSsh = useCallback(() => {
    if (sshSessionId.current) {
      invoke("disconnect_ssh", { sessionId: sshSessionId.current }).catch(
        () => undefined,
      );
        ...
        switch (authMethod) {
          ...
          case "key":
            ...
            sshConfig.private_key_passphrase =
              currentConnection.passphrase || null;
            break;
          case "totp":
        ...
        if (sshTrustPolicy !== "always-trust") {
          try {
            ...
            if (result.status === "first-use" && sshTrustPolicy === "tofu") {
              ...
              writeLine(
                `\x1b[90mHost key fingerprint (SHA-256): ${keyInfo.fingerprint}\x1b[0m`,
              );
              writeLine(
                `\x1b[90mKey type: ${keyInfo.key_type ?? "unknown"}\x1b[0m`,
              );
              writeLine(
                "\x1b[33mHost key memorized (Trust-On-First-Use)\x1b[0m",
              );
            ...
            writeLine(
              `\x1b[33mCould not retrieve host key info: ${err}\x1b[0m`,
            );
          }
        }
  ...
  useEffect(() => {
    ...
    const canFit = () => {
      ...
      const dims = renderService.dimensions;
      if (
        typeof dims.css?.cell?.width !== "number" ||
    ...
    >
          ...
          {isSsh &&
            ...
            (() => {
              ...
              const TrustBadgeIcon = stored
                ...
                : ShieldAlert;
              const shortFp =
                formatFingerprint(hostKeyIdentity.fingerprint).slice(0, 23) +
                "…";
              return (
                <>
                  <span
        ...
        >
          <div className="bg-[var(--color-surface)] rounded-xl shadow-2xl w-full max-h-[70vh] flex flex-col border border-[var(--color-border)]">
            ...
            <div className="px-4 py-2 border-b border-[var(--color-border)] flex items-center gap-3">
              ...
              {/* Clear Filters */}
              {(scriptCategoryFilter !== "all" ||
                scriptLanguageFilter !== "all" ||
                scriptOsTagFilter !== "all") && (
                <button
                  onClick={() => {
                    setScriptCategoryFilter("all");
                    setScriptLanguageFilter("all");
              ...
              {Object.keys(scriptsByCategory).length === 0 ? (
                <div className="text-center py-8 text-[var(--color-textMuted)]">
                  ...
                  <p className="text-xs mt-1">
                    Add scripts in the Script Manager
                  </p>
                </div>
              ) : (
                Object.entries(scriptsByCategory).map(
      ...
      {sshTrustPrompt && hostKeyIdentity && (
        <TrustWarningDialog
          ...
          port={connection?.port || 22}
          reason={
            sshTrustPrompt.status === "mismatch" ? "mismatch" : "first-use"
          }
          receivedIdentity={hostKeyIdentity}
          storedIdentity={
            sshTrustPrompt.status === "mismatch"
              ? sshTrustPrompt.stored
              : undefined
          }
          onAccept={() => {
