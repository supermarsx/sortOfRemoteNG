...
`,
    ...
    if (results.pings.length > 0) {
      const successfulPings = results.pings.filter(
        (p) => p.success && p.time_ms,
      );
      const avgPing =
        successfulPings.length > 0
          ? successfulPings.reduce((sum, p) => sum + (p.time_ms || 0), 0) /
        ...
        <div className="sticky top-0 z-10 border-b border-[var(--color-border)] px-5 py-4 flex items-center justify-between bg-[var(--color-surface)]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isRunning}
              className="sor-option-chip p-2 text-[var(--color-textSecondary)] disabled:opacity-50"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="flex items-center gap-3 p-3 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-[var(--color-textMuted)] truncate">
                    {t("diagnostics.defaultGateway", "Default gateway")}
                  </div>
                </div>
                ...
                {results.dnsResult ? (
                  results.dnsResult.success ? (
                    ...
                    <div className="text-sm text-red-500">
                      {results.dnsResult.error ||
                        t("diagnostics.dnsFailedShort", "Resolution failed")}
                    </div>
                  )
                ) : isRunning ? (
                  <Loader2
              ...
              <>
                ...
                <div className="mb-3 p-3 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
                  {(() => {
                    ...
                    const points: { x: number; y: number; ping: PingResult }[] =
                      results.pings.map((ping, i) => {
                        const x = i * pointSpacing;
                        const y =
                          ping.success && ping.time_ms
                            ? graphHeight -
                              ((ping.time_ms - graphMin) / range) * graphHeight
                    ...
                    const avgY =
                      avgPingTime > 0
                        ? graphHeight -
                          ((avgPingTime - graphMin) / range) * graphHeight
                        : graphHeight / 2;
                            ...
                            <circle
                              ...
                              fill={
                                !p.ping.success
                                  ? "#ef4444"
                                  : p.ping.time_ms &&
                                      p.ping.time_ms > avgPingTime * 1.5
                                    ? "#eab308"
                                    : "#22c55e"
                              }
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-3">
                  <div className="text-center p-2.5 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
                    ...
                    <div className="text-[10px] text-[var(--color-textMuted)] uppercase">
                      {t("diagnostics.successRate", "Success Rate")}
                    </div>
                  </div>
                  <div className="text-center p-2.5 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
                    <div className="text-xl font-bold text-[var(--color-text)]">
                    ...
                    <div className="text-[10px] text-[var(--color-textMuted)] uppercase">
                      {t("diagnostics.avgLatency", "Avg Latency")}
                    </div>
                  </div>
                  <div className="text-center p-2.5 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
                    <div className="text-xl font-bold text-[var(--color-text)]">
                  ...
                  <div className="text-center p-2.5 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
                    ...
                    <div className="text-[10px] text-[var(--color-textMuted)] uppercase">
                      {t("diagnostics.successful", "Successful")}
                    </div>
                  </div>
                  <div className="text-center p-2.5 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
                  ...
                  {results.asymmetricRouting ? (
                    <div className="space-y-1">
                      ...
                      <div className="text-[10px] text-[var(--color-textMuted)]">
                        ...
                        {results.asymmetricRouting.latency_variance !==
                          undefined &&
                      ...
                      {results.asymmetricRouting.ttl_analysis.received_ttl && (
                        <div className="text-[10px] text-[var(--color-textMuted)]">
                          TTL:{" "}
                          {results.asymmetricRouting.ttl_analysis.received_ttl}
                          {results.asymmetricRouting.ttl_analysis
                            .estimated_hops &&
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {results.udpProbe.response_received
                            ...
                            : results.udpProbe.response_type ===
                                "icmp_unreachable"
