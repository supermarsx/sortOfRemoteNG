    ...
    <div className="space-y-3" data-testid="mcp-prompts-tab">
      ...
      {mgr.prompts.map((prompt) => (
        ...
        >
          ...
          {prompt.description && (
            <p className="text-[10px] text-[var(--color-textSecondary)] mb-2">{prompt.description}</p>
          )}
          {prompt.arguments && prompt.arguments.length > 0 && (
            <div className="space-y-1">
              <div className="flex items-center gap-1 text-[9px] font-semibold uppercase text-[var(--color-textSecondary)]">
                <ListChecks size={9} />
              ...
              {prompt.arguments.map((arg) => (
                <div key={arg.name} className="flex items-center gap-2 pl-2 text-[10px]">
                  ...
                  {arg.description && (
                    <span className="text-[var(--color-textSecondary)]">— {arg.description}</span>
                  )}
                </div>
              ))}
            </div>
