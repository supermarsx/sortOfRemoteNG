      ...
      <div className="px-4 py-2 border-b border-[var(--color-border)] flex items-center gap-3 bg-[var(--color-surfaceHover)]/30">
        ...
        {mgr.command.trim() && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm bg-primary hover:bg-primary/90 text-[var(--color-text)] rounded-md transition-colors"
          >
            <Save size={14} />
        ...
        <div className="px-4 py-3 border-b border-[var(--color-border)] bg-primary/5 space-y-2">
          <div className="flex gap-2">
            <input
              ...
              className="sor-form-input-sm flex-1 placeholder-[var(--color-textMuted)] focus:outline-none focus:ring-1 focus:ring-primary"
            />
            <Select value={mgr.newScriptCategory} onChange={(v: string) => mgr.setNewScriptCategory(v)} options={[...mgr.categories.map((cat) => ({ value: cat, label: cat })), { value: 'Custom', label: 'Custom' }]} className="px-3 py-1.5 text-sm bg-[var(--color-input)] border border-[var(--color-border)] rounded-md text-[var(--color-text)] focus:outline-none focus:ring-1 focus:ring-primary" />
          </div>
          <div className="flex gap-2">
            <input
              type="text"
