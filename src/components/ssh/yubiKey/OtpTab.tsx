...
import { Fingerprint, KeyRound, RefreshCw, RotateCcw } from "lucide-react";
...
export const OtpTab: React.FC<{ mgr: Mgr }> = ({ mgr }) => {
  ...
  const slotId = (idx: number) =>
    idx === 0 ? ("Short" as const) : ("Long" as const);
        ...
        {mgr.otpSlots.map((slotCfg, idx) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <StatusBadge
                status={slotCfg?.configured ? "success" : "error"}
                label={
                  slotCfg?.configured
            ...
            {slotCfg && (
              <div className="text-xs text-[var(--color-textSecondary)] space-y-0.5">
                {slotCfg.slot_type && (
                  <div>
                    {t("yubikey.otp.type", "Type")}: {slotCfg.slot_type}
                  </div>
                )}
                {slotCfg.require_touch && (
            ...
            <div className="flex flex-wrap gap-1">
              <button
                ...
                className="sor-btn sor-btn-secondary px-2 py-0.5 text-[10px] rounded"
              >
              ...
              <DangerConfirm
                ...
                onConfirm={() => void mgr.otpDeleteSlot(serial, slotId(idx))}
                disabled={!slotCfg?.configured || mgr.loading}
              />
            </div>
          </div>
        ))}
          ...
          {configType === "static" && (
            <div className="space-y-2">
              ...
              <Select
                ...
                onChange={(v) => setStaticForm((f) => ({ ...f, layout: v }))}
                variant="form-sm"
                className="w-full"
                options={[
                  { value: "US", label: "US" },
