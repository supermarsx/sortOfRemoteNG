    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className="flex items-center gap-1 text-xs px-2 py-1 rounded bg-primary hover:bg-primary/90 text-[var(--color-text)] transition-colors"
      ...
      {showInstall && (
        <div className="p-4 rounded-lg border border-primary/30 bg-primary/10">
          ...
          <button
            className="flex items-center gap-1 text-xs px-3 py-1.5 rounded bg-primary hover:bg-primary/90 text-[var(--color-text)] transition-colors disabled:opacity-50"
      ...
      {/* Status */}
      {config && (
        <div className="p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] text-xs space-y-1">
          <div className="flex items-center gap-2">
            ...
            {config.version && (
              <span className="text-[var(--color-textSecondary)]">v{config.version}</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)]">
        ...
        <div className="space-y-2 pt-2 border-t border-[var(--color-border)]">
          <div className="flex items-center gap-2 text-xs">
            ...
            />
            <button
              className="flex items-center gap-1 px-2 py-1 rounded bg-primary hover:bg-primary/90 text-[var(--color-text)] transition-colors disabled:opacity-50"
        ...
        {config?.globalAuthIds && config.globalAuthIds.length > 0 ? (
          <div className="space-y-2 mb-3">
            {config.globalAuthIds.map((a, i) => (
              <div
                key={`globalauth-${a.principal}-${a.identity}-${i}`}
                className="flex items-center justify-between p-2 rounded bg-black/10 border border-[var(--color-border)] text-xs"
              >
                <div>
                  ...
                  <div className="text-[var(--color-textSecondary)]">{a.issuer}</div>
                </div>
                <button
                  className="p-1 rounded text-error hover:bg-error/10 transition-colors"
        ...
        <div className="p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)]">
          ...
          <div className="space-y-2">
            {config.userAuthIds.map((a, i) => (
              <div
                key={`userauth-${a.principal}-${a.identity}-${i}`}
                className="flex items-center justify-between p-2 rounded bg-black/10 border border-[var(--color-border)] text-xs"
              >
                <div>
                  <div className="text-[var(--color-text)]">
      ...
      {config?.sshdConfigSnippet && (
        <div className="p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)]">
          ...
          <pre className="p-3 text-[10px] bg-black/20 rounded-lg overflow-auto max-h-40 text-[var(--color-textSecondary)] font-mono whitespace-pre-wrap border border-[var(--color-border)]">
            {config.sshdConfigSnippet}
          </pre>
        </div>
      )}
