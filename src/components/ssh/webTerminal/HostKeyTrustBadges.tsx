...
import { getStoredIdentity, formatFingerprint } from "../../../utils/auth/trustStore";
...
function HostKeyTrustBadges({ mgr }: { mgr: WebTerminalMgr }) {
  ...
  const TrustBadgeIcon = stored
    ...
    : ShieldAlert;
  const shortFp =
    formatFingerprint(mgr.hostKeyIdentity.fingerprint).slice(0, 16) + "…";
  ...
  return (
    <>
      ...
      <span
        className="app-badge app-badge--neutral normal-case tracking-normal font-mono cursor-pointer hover:opacity-80 max-w-[180px] truncate"
