...
const BackupStatusIcon: React.FC<{ mgr: Mgr }> = ({ mgr }) => {
  if (mgr.isBackingUp || mgr.backupStatus?.isRunning)
    return <Loader2 className="w-4 h-4 animate-spin text-primary" />;
  if (!mgr.backupStatus || mgr.backupStatus.backupCount === 0)
    return <Archive className="w-4 h-4 text-[var(--color-textMuted)]" />;
  if (mgr.backupStatus.lastBackupStatus === "failed")
    return <AlertCircle className="w-4 h-4 text-error" />;
  if (mgr.backupStatus.lastBackupStatus === "partial")
    return <AlertCircle className="w-4 h-4 text-warning" />;
  if (mgr.backupStatus.lastBackupStatus === "success")
    return <CheckCircle className="w-4 h-4 text-success" />;
  return <HardDrive className="w-4 h-4 text-[var(--color-textSecondary)]" />;
};

const ProviderRow: React.FC<{ provider: CloudSyncProvider; mgr: Mgr }> = ({
  provider,
  mgr,
}) => {
    ...
    <div className="sor-status-item flex items-center justify-between p-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          title={mgr.t("syncBackup.syncProvider", "Sync {{provider}}", {
  ...
  <div className="p-4 border-b border-[var(--color-border)]">
    ...
    {!mgr.hasSync ? (
      <p className="text-xs text-[var(--color-textMuted)]">
        {mgr.t("syncBackup.noSyncConfigured", "No sync providers configured")}
      </p>
    ) : (
    ...
    {mgr.hasSync && (
      <div className="mt-2 flex items-center gap-2 text-xs text-[var(--color-textMuted)]">
        <Clock className="w-3 h-3" />
        <span>
          {mgr.t("syncBackup.lastSync", "Last sync")}:{" "}
  ...
  <div className="p-4">
    <div className="flex items-center justify-between mb-3">
      ...
      <button
        ...
        className="flex items-center gap-1 px-2 py-1 text-xs bg-primary hover:bg-primary/90 text-[var(--color-text)] disabled:opacity-50 rounded"
      >
    ...
    {mgr.backupStatus ? (
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs">
          <span className="text-[var(--color-textSecondary)]">
            {mgr.t("syncBackup.lastBackup", "Last backup")}:
          </span>
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-center justify-between text-xs">
          <span className="text-[var(--color-textSecondary)]">
            {mgr.t("syncBackup.nextBackup", "Next backup")}:
          </span>
          <div className="flex items-center gap-1 text-[var(--color-textSecondary)]">
            <Timer className="w-3 h-3 text-[var(--color-textMuted)]" />
        ...
        <div className="flex items-center justify-between text-xs pt-2 border-t border-[var(--color-border)]">
          <span className="text-[var(--color-textMuted)]">
            {mgr.backupStatus.backupCount}{" "}
            {mgr.t("syncBackup.backups", "backups")}
          </span>
          <span className="text-[var(--color-textMuted)]">
      ...
      <p className="text-xs text-[var(--color-textMuted)]">
        {mgr.t("syncBackup.noBackups", "No backups yet")}
      </p>
    )}
  </div>
);
