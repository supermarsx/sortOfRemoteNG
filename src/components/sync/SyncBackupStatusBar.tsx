...
const ProviderRow: React.FC<{ provider: CloudSyncProvider; mgr: Mgr }> = ({ provider, mgr }) => {
  ...
  return (
    <div className="sor-status-item flex items-center justify-between p-2">
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs text-[var(--color-textMuted)]">{formatRelativeTime(status?.lastSyncTime)}</span>
...
const CloudSyncSection: React.FC<{ mgr: Mgr }> = ({ mgr }) => (
  <div className="p-4 border-b border-[var(--color-border)]">
    ...
    {!mgr.hasSync ? (
      <p className="text-xs text-[var(--color-textMuted)]">{mgr.t("syncBackup.noSyncConfigured", "No sync providers configured")}</p>
    ) : (
    ...
    {mgr.backupStatus ? (
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between text-xs pt-2 border-t border-[var(--color-border)]">
          <span className="text-[var(--color-textMuted)]">{mgr.backupStatus.backupCount} {mgr.t("syncBackup.backups", "backups")}</span>
      ...
      <p className="text-xs text-[var(--color-textMuted)]">{mgr.t("syncBackup.noBackups", "No backups yet")}</p>
    )}
  </div>
);
