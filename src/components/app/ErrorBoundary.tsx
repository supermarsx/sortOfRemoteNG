...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo): void {
    console.error(
      "[BSOD] ErrorBoundary caught a fatal error:",
      error,
      errorInfo,
    );
    this.setState((prev) => ({
  ...
  private buildFullReport(): string {
    ...
    for (let i = 0; i < errors.length; i++) {
      ...
      if (e.errorInfo?.componentStack) {
        sections.push(`Component Stack:`, e.errorInfo.componentStack, ``);
      }
    }
      ...
      >
        ...
        >
          ...
          >
            {errors.length === 1
              ? tr(
                  "errorBoundary.singleErrorDescription",
                  "An unrecoverable error occurred in the frontend. You can try the recovery options below.",
                )
              : tr(
                  "errorBoundary.multipleErrorsDescription",
                  "{{count}} errors were captured. Expand each to view details.",
                  { count: errors.length },
                )}
          </p>
          ...
          >
            ...
            >
              {tr("errorBoundary.stopCode", "Stop code")}
            </div>
            ...
            >
              ...
              {latestError?.error.message ??
                tr("common.unknownError", "Unknown error")}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              ...
              {errors.length > 1 && (
                <>
                  <button onClick={this.expandAll} style={btnLink(t)}>
                    {tr("errorBoundary.expandAll", "Expand all")}
                  </button>
                  <button onClick={this.collapseAll} style={btnLink(t)}>
                    {tr("errorBoundary.collapseAll", "Collapse all")}
                  </button>
                </>
              )}
            </div>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ marginBottom: 12 }}>
                      <div style={stackLabel(t)}>
                        {tr("errorBoundary.stackTrace", "Stack Trace")}
                      </div>
                      <pre style={stackPre(t)}>
                        {e.error.stack || tr("common.empty", "(empty)")}
                      </pre>
                    </div>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>
            {tr("errorBoundary.recoveryOptions", "Recovery options")}
          </div>
          ...
          >
            {tr("errorBoundary.destructiveOptions", "Destructive options")}
          </div>
              ...
              <div style={{ flex: 1, fontSize: 13, lineHeight: 1.5 }}>
                {confirmAction === "clear-cache" && (
                  <>
                    <strong>
                      {tr(
                        "errorBoundary.clearCacheTitle",
                        "Clear session cache?",
                      )}
                    </strong>{" "}
                    {tr(
                      "errorBoundary.clearCacheDescription",
                      "This will clear session storage and browser caches. Your settings and connections will be preserved.",
                    )}
                  </>
                )}
                {confirmAction === "clear-all" && (
                  <>
                    <strong>
                      {tr("errorBoundary.clearAllTitle", "Clear ALL app data?")}
                    </strong>{" "}
                    {tr(
                      "errorBoundary.clearAllDescription",
                      "This will delete all settings, saved connections, credentials, and cached data. This cannot be undone.",
                    )}
                  </>
                )}
                {confirmAction === "factory-reset" && (
                  <>
                    <strong>
                      {tr("errorBoundary.factoryResetTitle", "Factory reset?")}
                    </strong>{" "}
                    {tr(
                      "errorBoundary.factoryResetDescription",
                      "This will wipe everything — all data, settings, and backend state — and restore the app to its initial state. This cannot be undone.",
                    )}
                  </>
                )}
              </div>
              <button onClick={this.cancelConfirm} style={btnSecondary(t)}>
                {tr("dialogs.cancel", "Cancel")}
              </button>
              <button
                onClick={
                  confirmAction === "clear-cache"
                ...
                style={btnDanger(t)}
              >
                {tr("errorBoundary.confirmRestart", "Confirm & restart")}
              </button>
            </div>
          )}
          ...
          >
            ...
            >
              {tr("errorBoundary.clearCacheRestart", "Clear cache & restart")}
            </button>
            <button
            ...
            >
              {tr("errorBoundary.factoryReset", "Factory reset")}
            </button>
            <button onClick={this.handleQuitApp} style={btnDanger(t)}>
              {tr("errorBoundary.quitApp", "Quit app")}
            </button>
          </div>
