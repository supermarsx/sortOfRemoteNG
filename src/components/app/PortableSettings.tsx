      ...
      {migration && (
        <div className="sor-portable-migration">
          ...
          {migration.errors.length > 0 && (
            <ul className="sor-portable-issues">
              {migration.errors.map((e, i) => <li key={`err-${e.slice(0, 50)}-${i}`} className="sor-portable-error">{e}</li>)}
            </ul>
          )}
          {migration.warnings.length > 0 && (
            <ul className="sor-portable-issues">
              {migration.warnings.map((w, i) => <li key={`warn-${w.slice(0, 50)}-${i}`} className="sor-portable-warning">{w}</li>)}
            </ul>
          )}
        </div>
      )}
