...
import { Send, Loader2, Plus, RefreshCw, Trash2, CheckCircle2 } from "lucide-react";
...
const JsonAction: React.FC<{
  ...
  return (
    <div className={card}>
      ...
      />
      <button className={`${btn} mt-2`} onClick={run} disabled={disabled || !text}>
        {runLabel}
      </button>
...
const BotManager: React.FC<{ mgr: Mgr }> = ({ mgr }) => {
  ...
  const updateToken = useCallback(
    async (name: string) => {
      ...
      try {
        ...
        if (inst) await store.updateInstance(inst.id, { secret: token });
  ...
  const remove = useCallback(
    async (name: string) => {
      if (
        !window.confirm(
          t("integrations.telegram.removeBotConfirm", "Remove bot {{n}}?").replace(
            "{{n}}",
            name,
          ),
        )
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className={card}>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Labeled label={t("integrations.telegram.botName", "Name")}>
            <input className={field} value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="alerts-bot" />
          </Labeled>
          <Labeled label={t("integrations.telegram.token", "Bot token (@BotFather)")}>
            <input className={field} type="password" value={form.token} onChange={(e) => set("token", e.target.value)} />
          </Labeled>
          <Labeled label={t("integrations.telegram.apiBaseUrl", "API base URL (optional)")}>
            <input className={field} value={form.apiBaseUrl} onChange={(e) => set("apiBaseUrl", e.target.value)} placeholder="https://api.telegram.org" />
          </Labeled>
          <Labeled label={t("integrations.telegram.proxyUrl", "Proxy URL (optional)")}>
            <input className={field} value={form.proxyUrl} onChange={(e) => set("proxyUrl", e.target.value)} placeholder="socks5://…" />
          </Labeled>
          <Labeled label={t("integrations.telegram.timeout", "Timeout (seconds)")}>
            <input className={field} inputMode="numeric" value={form.timeoutSeconds} onChange={(e) => set("timeoutSeconds", e.target.value)} />
          </Labeled>
          <Labeled label={t("integrations.telegram.maxRetries", "Max retries")}>
            <input className={field} inputMode="numeric" value={form.maxRetries} onChange={(e) => set("maxRetries", e.target.value)} />
          </Labeled>
          <Labeled label={t("integrations.telegram.rateLimit", "Rate limit (ms)")}>
            <input className={field} inputMode="numeric" value={form.rateLimitMs} onChange={(e) => set("rateLimitMs", e.target.value)} />
          </Labeled>
        </div>
        <div className="mt-2 flex items-center gap-3">
          ...
          <button className={btn} onClick={addBot} disabled={mgr.isLoading || !form.name || !form.token}>
            {mgr.isLoading ? <Loader2 size={12} className="animate-spin" /> : <Plus size={12} />}
            {t("integrations.telegram.addBot", "Add bot")}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button className={btn} onClick={() => void mgr.refreshBots()} disabled={mgr.isLoading}>
          <RefreshCw size={12} />
          ...
          <div key={b.name} className={card}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="min-w-0">
                <span className="text-sm font-semibold text-[var(--color-text)]">{b.name}</span>{" "}
                <span className={b.connected ? "text-green-500 text-xs" : "text-[var(--color-textMuted)] text-xs"}>
                  {b.connected
                ...
                <div className="text-[10px] text-[var(--color-textMuted)]">
                  {b.botUser ? `@${b.botUser.username ?? b.botUser.first_name}` : b.apiBase} ·{" "}
                  {t("integrations.telegram.sent", "sent")} {b.messagesSent} ·{" "}
                  {t("integrations.telegram.failed", "failed")} {b.messagesFailed}
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-1">
                <button className={btn} onClick={() => void validate(b.name)} disabled={mgr.isLoading}>
                  <CheckCircle2 size={12} />
                  {t("integrations.telegram.validate", "Validate")}
                </button>
                <button className={btn} onClick={() => void toggle(b)} disabled={mgr.isLoading}>
                  {b.enabled ? t("integrations.telegram.disable", "Disable") : t("integrations.telegram.enable", "Enable")}
                </button>
                <button className={btn} onClick={() => void remove(b.name)} disabled={mgr.isLoading}>
                  <Trash2 size={12} />
                </button>
              </div>
            </div>
            <div className="mt-2 flex items-center gap-2">
              <input
                ...
                onChange={(e) => setTokenEdits((m) => ({ ...m, [b.name]: e.target.value }))}
              />
              <button className={btn} onClick={() => void updateToken(b.name)} disabled={!tokenEdits[b.name]}>
...
const SendTab: React.FC<{ mgr: Mgr; bot: string }> = ({ mgr, bot }) => {
  ...
  const sendMedia = guard(async () => {
    ...
    switch (media.kind) {
      ...
      case "document":
        return mgr.api.sendDocument(bot, { chatId: c, document: media.url, caption });
      case "video":
  ...
  return (
    <div className="flex flex-col gap-3">
      <Labeled label={t("integrations.telegram.chatId", "Chat ID or @username")}>
        <input className={field} value={chat} onChange={(e) => setChat(e.target.value)} placeholder="-1001234567890" />
      </Labeled>

      <div className={card}>
        <h4 className="mb-2 text-xs font-semibold text-[var(--color-text)]">{t("integrations.telegram.textMessage", "Text message")}</h4>
        <textarea className={field} rows={3} value={text} onChange={(e) => setText(e.target.value)} />
        <button
          className={`${btn} mt-2`}
          onClick={guard(() => mgr.api.sendMessage(bot, { chatId: cid(), text }))}
      ...
      <div className={card}>
        <h4 className="mb-2 text-xs font-semibold text-[var(--color-text)]">{t("integrations.telegram.media", "Media (file_id / URL)")}</h4>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          <select className={field} value={media.kind} onChange={(e) => setMedia((m) => ({ ...m, kind: e.target.value }))}>
            {["photo", "document", "video", "audio", "voice", "sticker"].map((k) => (
              <option key={k} value={k}>{k}</option>
            ))}
          </select>
          <input className={field} value={media.url} onChange={(e) => setMedia((m) => ({ ...m, url: e.target.value }))} placeholder="file_id or https://…" />
          <input className={field} value={media.caption} onChange={(e) => setMedia((m) => ({ ...m, caption: e.target.value }))} placeholder={t("integrations.telegram.caption", "Caption")} />
        </div>
        <button className={`${btn} mt-2`} onClick={sendMedia} disabled={mgr.isLoading || !chat || !media.url}>
          <Send size={12} />
          {t("integrations.telegram.send", "Send")}
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button className={btn} onClick={guard(() => mgr.api.sendDice(bot, { chatId: cid() }))} disabled={mgr.isLoading || !chat}>
          {t("integrations.telegram.sendDice", "Send dice 🎲")}
        </button>
        <select className={field} style={{ maxWidth: 180 }} value={action} onChange={(e) => setAction(e.target.value)}>
          {["typing", "upload_photo", "upload_document", "record_video", "find_location"].map((a) => (
            <option key={a} value={a}>{a}</option>
          ))}
        </select>
        <button
          className={btn}
          onClick={guard(() => mgr.api.sendChatAction(bot, cid(), action as never))}
      ...
      <JsonAction
        title={t("integrations.telegram.sendLocationTitle", "Send location (JSON)")}
        placeholder='{"latitude":51.5,"longitude":-0.12}'
        runLabel={t("integrations.telegram.send", "Send")}
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.sendLocation(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        title={t("integrations.telegram.sendContactTitle", "Send contact (JSON)")}
        placeholder='{"phoneNumber":"+15551234567","firstName":"Ada"}'
        runLabel={t("integrations.telegram.send", "Send")}
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.sendContact(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        ...
        runLabel={t("integrations.telegram.send", "Send")}
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.sendPoll(bot, { chatId: cid(), ...(p as object) } as never)}
      />
    </div>
  );
};
...
const MessagesTab: React.FC<{ mgr: Mgr; bot: string }> = ({ mgr, bot }) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <Labeled label={t("integrations.telegram.chatId", "Chat ID or @username")}>
          <input className={field} value={chat} onChange={(e) => setChat(e.target.value)} />
        </Labeled>
        <Labeled label={t("integrations.telegram.messageId", "Message ID")}>
          <input className={field} inputMode="numeric" value={msgId} onChange={(e) => setMsgId(e.target.value)} />
        </Labeled>
      </div>
      <div className="flex flex-wrap gap-2">
        <button className={btn} onClick={guard(() => mgr.api.deleteMessage(bot, cid(), mid()))} disabled={mgr.isLoading || !chat || !msgId}>
          <Trash2 size={12} />
          {t("integrations.telegram.deleteMessage", "Delete")}
        </button>
        <button className={btn} onClick={guard(() => mgr.api.pinMessage(bot, cid(), mid()))} disabled={mgr.isLoading || !chat || !msgId}>
          {t("integrations.telegram.pin", "Pin")}
        </button>
        <button className={btn} onClick={guard(() => mgr.api.unpinMessage(bot, cid(), mid()))} disabled={mgr.isLoading || !chat || !msgId}>
          {t("integrations.telegram.unpin", "Unpin")}
        </button>
        <button className={btn} onClick={guard(() => mgr.api.unpinAllMessages(bot, cid()))} disabled={mgr.isLoading || !chat}>
      ...
      <JsonAction
        ...
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.editMessageText(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        title={t("integrations.telegram.editCaption", "Edit message caption (JSON)")}
        placeholder='{"messageId":123,"caption":"new caption"}'
        runLabel={t("integrations.telegram.edit", "Edit")}
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.editMessageCaption(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        title={t("integrations.telegram.editReplyMarkup", "Edit reply markup (JSON)")}
        placeholder='{"messageId":123,"replyMarkup":{"inline_keyboard":[]}}'
        runLabel={t("integrations.telegram.edit", "Edit")}
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.editMessageReplyMarkup(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        ...
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.forwardMessage(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        ...
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.copyMessage(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        title={t("integrations.telegram.answerCallback", "Answer callback query (JSON)")}
    ...
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        ...
        <Labeled label={t("integrations.telegram.userId", "User ID")}>
          <input className={field} inputMode="numeric" value={userId} onChange={(e) => setUserId(e.target.value)} />
        </Labeled>
      </div>
      <div className="flex flex-wrap gap-2">
        <button className={btn} onClick={show(() => mgr.api.getChat(bot, cid()))} disabled={mgr.isLoading || !chat}>
          {t("integrations.telegram.getChat", "Get chat")}
        </button>
        <button className={btn} onClick={show(() => mgr.api.getChatMemberCount(bot, cid()))} disabled={mgr.isLoading || !chat}>
          {t("integrations.telegram.memberCount", "Member count")}
        </button>
        <button className={btn} onClick={show(() => mgr.api.getChatAdministrators(bot, cid()))} disabled={mgr.isLoading || !chat}>
          {t("integrations.telegram.admins", "Administrators")}
        </button>
        <button className={btn} onClick={show(() => mgr.api.getChatMember(bot, cid(), Number(userId)))} disabled={mgr.isLoading || !chat || !userId}>
          {t("integrations.telegram.getMember", "Get member")}
        </button>
        <button className={btn} onClick={show(() => mgr.api.exportChatInviteLink(bot, cid()))} disabled={mgr.isLoading || !chat}>
          {t("integrations.telegram.exportInvite", "Export invite link")}
        </button>
        <button className={btn} onClick={show(() => mgr.api.createInviteLink(bot, cid()))} disabled={mgr.isLoading || !chat}>
          {t("integrations.telegram.createInvite", "Create invite link")}
        </button>
        <button className={btn} onClick={act(() => mgr.api.leaveChat(bot, cid()))} disabled={mgr.isLoading || !chat}>
          {t("integrations.telegram.leaveChat", "Leave chat")}
        </button>
        <button className={btn} onClick={act(() => mgr.api.unbanChatMember(bot, cid(), Number(userId)))} disabled={mgr.isLoading || !chat || !userId}>
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div className="flex items-end gap-2">
          ...
          <button className={btn} onClick={act(() => mgr.api.setChatTitle(bot, cid(), title))} disabled={mgr.isLoading || !chat || !title}>
            {t("integrations.telegram.setTitle", "Set")}
          </button>
        </div>
        <div className="flex items-end gap-2">
          <Labeled label={t("integrations.telegram.description", "Chat description")}>
            <input className={field} value={desc} onChange={(e) => setDesc(e.target.value)} />
          </Labeled>
          <button className={btn} onClick={act(() => mgr.api.setChatDescription(bot, cid(), desc))} disabled={mgr.isLoading || !chat}>
      ...
      <JsonAction
        ...
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.banChatMember(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        ...
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.restrictChatMember(bot, { chatId: cid(), ...(p as object) } as never)}
      />
      <JsonAction
        ...
        disabled={mgr.isLoading || !chat}
        onRun={(p) => mgr.api.promoteChatMember(bot, { chatId: cid(), ...(p as object) } as never)}
      />
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-end gap-2">
        <Labeled label={t("integrations.telegram.fileId", "File ID")}>
          <input className={field} value={fileId} onChange={(e) => setFileId(e.target.value)} />
        </Labeled>
        <button className={btn} onClick={getFile} disabled={mgr.isLoading || !fileId}>
          {t("integrations.telegram.getFile", "Get file info")}
        </button>
      </div>
      <div className="flex items-end gap-2">
        <Labeled label={t("integrations.telegram.filePath", "File path (from getFile)")}>
          <input className={field} value={filePath} onChange={(e) => setFilePath(e.target.value)} />
        </Labeled>
        <button className={btn} onClick={download} disabled={mgr.isLoading || !filePath}>
          {t("integrations.telegram.download", "Download")}
        </button>
      </div>
      <JsonAction
        title={t("integrations.telegram.uploadFile", "Upload file (JSON: chatId, fileName, data[])")}
        placeholder='{"chatId":123,"fileName":"a.txt","data":[104,105]}'
        runLabel={t("integrations.telegram.upload", "Upload")}
        disabled={mgr.isLoading || !bot}
        onRun={(p) => {
          const o = p as { chatId: ChatId; fileName: string; data: number[]; caption?: string };
          return mgr.api.uploadFile(bot, o.chatId, o.fileName, o.data, o.caption);
        }}
      />
      <JsonView value={detail} />
    </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap gap-2">
        ...
        <button className={btn} onClick={updates} disabled={mgr.isLoading || !bot}>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        {rows.map((r) => (
          <div key={r.id} className="flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-1">
              <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.setNotificationRuleEnabled(r.id, !r.enabled)); await refresh(); } catch { /* surfaced */ } }}>
                {r.enabled ? t("integrations.telegram.disable", "Disable") : t("integrations.telegram.enable", "Enable")}
              </button>
              <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.removeNotificationRule(r.id)); await refresh(); } catch { /* surfaced */ } }}>
                <Trash2 size={12} />
              </button>
            </div>
          </div>
        ...
        {rows.length === 0 && (
          <span className="text-xs text-[var(--color-textMuted)]">{t("integrations.telegram.noRules", "No notification rules")}</span>
        )}
      </div>
      <JsonAction
        title={t("integrations.telegram.addRule", "Add notification rule (JSON)")}
        ...
        onRun={async (p) => { await mgr.api.addNotificationRule(p as never); await refresh(); }}
      />
      <JsonAction
        title={t("integrations.telegram.processEvent", "Process connection event (JSON) — test rules")}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap gap-2">
        ...
        <button className={btn} onClick={async () => { try { setDetail(await mgr.run(() => mgr.api.monitoringSummary())); } catch { /* surfaced */ } }} disabled={mgr.isLoading}>
          {t("integrations.telegram.summary", "Summary")}
        </button>
        <button className={btn} onClick={async () => { try { setDetail(await mgr.run(() => mgr.api.monitoringHistory())); } catch { /* surfaced */ } }} disabled={mgr.isLoading}>
      ...
      <div className="flex flex-col gap-1">
        {rows.map((c) => (
          <div key={c.id} className="flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-1">
              <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.setMonitoringCheckEnabled(c.id, !c.enabled)); await refresh(); } catch { /* surfaced */ } }}>
                {c.enabled ? t("integrations.telegram.disable", "Disable") : t("integrations.telegram.enable", "Enable")}
              </button>
              <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.removeMonitoringCheck(c.id)); await refresh(); } catch { /* surfaced */ } }}>
                <Trash2 size={12} />
              </button>
            </div>
          </div>
        ...
        {rows.length === 0 && (
          <span className="text-xs text-[var(--color-textMuted)]">{t("integrations.telegram.noChecks", "No monitoring checks")}</span>
        )}
      </div>
      <JsonAction
        title={t("integrations.telegram.addCheck", "Add monitoring check (JSON)")}
        ...
        runLabel={t("integrations.telegram.add", "Add")}
        disabled={mgr.isLoading}
        onRun={async (p) => { await mgr.api.addMonitoringCheck(p as never); await refresh(); }}
      />
      <JsonAction
        title={t("integrations.telegram.recordResult", "Record check result (JSON)")}
      ...
      <div className="flex flex-col gap-1">
        {rows.map((tpl) => (
          <div key={tpl.id} className="flex items-center justify-between text-xs">
            <span className="text-[var(--color-textSecondary)]">{tpl.name}</span>
            <div className="flex items-center gap-1">
              <button className={btn} onClick={async () => { try { setDetail(await mgr.run(() => mgr.api.renderTemplate(tpl.id, tpl.defaultVariables ?? {}))); } catch { /* surfaced */ } }}>
                {t("integrations.telegram.render", "Render")}
              </button>
              <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.removeTemplate(tpl.id)); await refresh(); } catch { /* surfaced */ } }}>
                <Trash2 size={12} />
              </button>
            </div>
          </div>
        ...
        {rows.length === 0 && (
          <span className="text-xs text-[var(--color-textMuted)]">{t("integrations.telegram.noTemplates", "No templates")}</span>
        )}
      </div>
      <div className={card}>
        <h4 className="mb-2 text-xs font-semibold text-[var(--color-text)]">{t("integrations.telegram.validateBody", "Validate template body")}</h4>
        <textarea className={`${field} font-mono`} rows={3} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Hello {{name}}" />
        <button className={`${btn} mt-2`} onClick={validate} disabled={mgr.isLoading || !body}>
          {t("integrations.telegram.validate", "Validate")}
        </button>
      </div>
      <JsonAction
        ...
        runLabel={t("integrations.telegram.add", "Add")}
        disabled={mgr.isLoading}
        onRun={async (p) => { await mgr.api.addTemplate(p as never); await refresh(); }}
      />
      <JsonAction
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap gap-2">
        ...
        <button className={btn} onClick={async () => { try { setDetail(await mgr.run(() => mgr.api.processScheduledMessages())); await refresh(); } catch { /* surfaced */ } }} disabled={mgr.isLoading}>
      ...
      <div className="flex flex-col gap-1">
        {rows.map((m) => (
          <div key={m.id} className="flex items-center justify-between text-xs">
            ...
            <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.cancelScheduledMessage(m.id)); await refresh(); } catch { /* surfaced */ } }}>
              <Trash2 size={12} />
            </button>
          </div>
        ))}
        {rows.length === 0 && (
          <span className="text-xs text-[var(--color-textMuted)]">{t("integrations.telegram.noScheduled", "No scheduled messages")}</span>
        )}
      </div>
      <JsonAction
        title={t("integrations.telegram.scheduleMessage", "Schedule message (JSON)")}
        ...
        disabled={mgr.isLoading}
        onRun={async (p) => { await mgr.api.scheduleMessage(p as never); await refresh(); }}
      />
      <JsonView value={detail} />
    </div>
  );
...
const BroadcastTab: React.FC<{ mgr: Mgr }> = ({ mgr }) => {
  ...
  return (
    <JsonAction
      title={t("integrations.telegram.broadcast", "Broadcast to many chats (JSON)")}
      ...
      <div className="flex flex-col gap-1">
        {rows.map((d) => (
          <div key={d.id} className="flex items-center justify-between text-xs">
            ...
            <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.removeDigest(d.id)); await refresh(); } catch { /* surfaced */ } }}>
              <Trash2 size={12} />
            </button>
          </div>
        ))}
        {rows.length === 0 && (
          <span className="text-xs text-[var(--color-textMuted)]">{t("integrations.telegram.noDigests", "No digests")}</span>
        )}
      </div>
      <JsonAction
        ...
        runLabel={t("integrations.telegram.add", "Add")}
        disabled={mgr.isLoading}
        onRun={async (p) => { await mgr.api.addDigest(p as never); await refresh(); }}
      />
    </div>
  );
};
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap gap-2">
        ...
        <button className={btn} onClick={async () => { try { setDetail(await mgr.run(() => mgr.api.notificationHistory())); } catch { /* surfaced */ } }} disabled={mgr.isLoading}>
          {t("integrations.telegram.notificationHistory", "Notification history")}
        </button>
        <button className={btn} onClick={async () => { try { await mgr.run(() => mgr.api.clearMessageLog()); await refresh(); } catch { /* surfaced */ } }} disabled={mgr.isLoading}>
      ...
      {stats && (
        <div className={card}>
          <div className="grid grid-cols-2 gap-1 text-xs text-[var(--color-textSecondary)] sm:grid-cols-4">
            <span>{t("integrations.telegram.bots", "Bots")}: {stats.configuredBots}</span>
            <span>{t("integrations.telegram.active", "Active")}: {stats.activeBots}</span>
            <span>{t("integrations.telegram.rules", "Rules")}: {stats.notificationRules}</span>
            <span>{t("integrations.telegram.checks", "Checks")}: {stats.monitoringChecks}</span>
            <span>{t("integrations.telegram.sent", "Sent")}: {stats.totalMessagesSent}</span>
            <span>{t("integrations.telegram.failed", "Failed")}: {stats.totalMessagesFailed}</span>
            <span>{t("integrations.telegram.notifications", "Notifs")}: {stats.totalNotificationsSent}</span>
            <span>{t("integrations.telegram.alerts", "Alerts")}: {stats.totalAlertsSent}</span>
          </div>
        </div>
      )}
      <div className="flex flex-col gap-1">
        {log.map((e) => (
          <div key={e.id} className="text-[10px] text-[var(--color-textMuted)]">
            {e.timestamp} · {e.botName} → {e.chatId} · {e.success ? "✓" : "✗"} {e.textPreview}
          </div>
        ))}
      </div>
      <JsonView value={detail} />
...
const TABS: { key: TabKey; label: string; fallback: string }[] = [
  ...
  { key: "messages", label: "integrations.telegram.tabMessages", fallback: "Messages" },
  ...
  { key: "webhooks", label: "integrations.telegram.tabWebhooks", fallback: "Webhooks" },
  { key: "rules", label: "integrations.telegram.tabRules", fallback: "Rules" },
  { key: "monitoring", label: "integrations.telegram.tabMonitoring", fallback: "Monitoring" },
  { key: "templates", label: "integrations.telegram.tabTemplates", fallback: "Templates" },
  { key: "scheduled", label: "integrations.telegram.tabScheduled", fallback: "Scheduled" },
  { key: "broadcast", label: "integrations.telegram.tabBroadcast", fallback: "Broadcast" },
  { key: "digests", label: "integrations.telegram.tabDigests", fallback: "Digests" },
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="border-t border-[var(--color-border)] pt-3">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            >
              {botNames.length === 0 && <option value="">{t("integrations.telegram.noBots", "No bots")}</option>}
              {botNames.map((n) => (
