      ...
      <SectionHeader
        ...
        title={
          <span className="flex items-center gap-1">
            Auto Lock{" "}
            <InfoTooltip text="Automatically lock the application after a period of inactivity, requiring the configured master-key unlock method to resume." />
          </span>
        }
      />
      ...
      <Card>
        {!mgr.hasMasterKey && (
          <div className="flex items-center gap-2 px-3 py-2 bg-warning/20 border border-warning/50 rounded-md text-warning text-sm">
            ...
            Set up global master-key protection to enable native auto lock.
          </div>
        )}

        <div
          className={
            mgr.hasMasterKey ? undefined : "opacity-50 pointer-events-none"
          }
        >
          <Toggle
            settingKey="autoLock.enabled"
            checked={settings.autoLock.enabled && mgr.hasMasterKey}
            onChange={(v) =>
              updateSettings({
                autoLock: { ...settings.autoLock, enabled: v },
              })
            }
            disabled={!mgr.hasMasterKey}
            icon={<Clock size={16} />}
        ...
        <div
          className={
            !mgr.hasMasterKey || !settings.autoLock.enabled
        ...
        >
          ...
          <Toggle
            ...
            disabled={!mgr.hasMasterKey || !settings.autoLock.enabled}
            ...
            infoTooltip="Useful on shared desktops — the configured global unlock method is required to bring the app back."
          />
