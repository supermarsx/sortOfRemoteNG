...
const EncryptionAtRestSection: React.FC = () => {
  ...
  // SORNG1 → v2 backup migration. Walks every enabled destination,
  // converts legacy / plaintext backups to the v2 envelope under the
  // master DEK. Required prerequisite for retiring the legacy reader.
  const [migrateBackupsBusy, setMigrateBackupsBusy] = useState(false);
  const [migrateBackupsError, setMigrateBackupsError] = useState<string | null>(null);
  const [migrateBackupsReport, setMigrateBackupsReport] =
    useState<BackupMigrationReport | null>(null);
  const [migrateBackupsRanAt, setMigrateBackupsRanAt] = useState<string | null>(null);
  const [migrateBackupsLegacyPw, setMigrateBackupsLegacyPw] = useState("");
  ...
  const handleMigrateBackups = async () => {
    setMigrateBackupsBusy(true);
    setMigrateBackupsError(null);
    try {
      const report = await enc.migrateBackups(
        migrateBackupsLegacyPw.length > 0 ? migrateBackupsLegacyPw : undefined,
      );
      setMigrateBackupsReport(report);
      setMigrateBackupsRanAt(formatNow());
      setMigrateBackupsLegacyPw("");
    } catch (e) {
      setMigrateBackupsError(e instanceof Error ? e.message : String(e));
    } finally {
      setMigrateBackupsBusy(false);
    }
  };
          ...
          <SectionHeader
            ...
            title={
              <span className="flex items-center gap-2">
                ...
                <InfoTooltip text="Walks every enabled backup destination and converts each backup file from the legacy SORNG1 envelope (or plaintext) to the v2 envelope under the master DEK. Required before the legacy reader can be retired." />
              </span>
            }
          />
          <Card>
            <p className="text-xs text-[var(--color-textMuted)]">
              Converts every <code>backup_*</code> file under each
              enabled destination from the legacy{" "}
              <code>SORNG1</code> envelope (PBKDF2/600k, separate
              backup password) to the v2 envelope under{" "}
              <code>sorng-v1::backups</code>. Originals are archived
              as <code>.v0.bak</code> for rollback. After migration
              the backup password is no longer load-bearing.
            </p>

            <SettingsPasswordRow
              icon={<Lock size={16} />}
              label="Legacy backup password"
              value={migrateBackupsLegacyPw}
              onChange={setMigrateBackupsLegacyPw}
              placeholder="Leave empty if no SORNG1 backups exist"
              infoTooltip="Required only when one or more files at any destination start with the SORNG1 prefix. Plaintext + already-v2 files migrate without a password."
            />

            {migrateBackupsRanAt && migrateBackupsReport && (
              <div className="mt-2 p-2 rounded bg-success/10 border border-success/30 text-xs">
                <div className="flex items-center gap-1.5 text-success mb-1">
                  <Check className="w-3.5 h-3.5" />
                  <span>Backups migration ran at {migrateBackupsRanAt}</span>
                </div>
                <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-[var(--color-textSecondary)] mt-1">
                  <span>Scanned</span>
                  <span className="text-[var(--color-text)] font-mono">
                    {migrateBackupsReport.total}
                  </span>
                  <span>Migrated</span>
                  <span className="text-[var(--color-text)] font-mono">
                    {migrateBackupsReport.migrated}
                  </span>
                  <span>Already v2</span>
                  <span className="text-[var(--color-text)] font-mono">
                    {migrateBackupsReport.alreadyV2}
                  </span>
                  {migrateBackupsReport.bytesIn > 0 && (
                    <>
                      <span>Bytes read</span>
                      <span className="text-[var(--color-text)] font-mono">
                        {migrateBackupsReport.bytesIn.toLocaleString()}
                      </span>
                      <span>Bytes written</span>
                      <span className="text-[var(--color-text)] font-mono">
                        {migrateBackupsReport.bytesOut.toLocaleString()}
                      </span>
                    </>
                  )}
                </div>
                {migrateBackupsReport.failed.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-warning/30">
                    <div className="flex items-center gap-1.5 text-warning mb-1">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      <span>
                        {migrateBackupsReport.failed.length} file(s)
                        could not be migrated
                      </span>
                    </div>
                    <ul className="text-[var(--color-textSecondary)] text-[10px] space-y-0.5">
                      {migrateBackupsReport.failed
                        .slice(0, 5)
                        .map((f, i) => (
                      ...
                      {migrateBackupsReport.failed.length > 5 && (
                        <li>
                          …and {migrateBackupsReport.failed.length - 5}{" "}
                          more
                        </li>
                      )}
                    </ul>
