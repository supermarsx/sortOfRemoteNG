...
export const Select: React.FC<SelectProps> = ({
  ...
  const updatePosition = useCallback(() => {
    ...
    const top =
      spaceBelow < ddHeight && spaceAbove > spaceBelow
        ? rect.top - ddHeight - 4
        : rect.bottom + 4;
  ...
  useEffect(() => {
    ...
    const target = itemsRef.current[highlightIdx];
    if (target && optionsScrollRef.current)
      scrollElementWithinContainer(optionsScrollRef.current, target, {
        axis: "vertical",
      });
  }, [isOpen, highlightIdx]);
    ...
    (e: React.KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "Home": {
          ...
          let first = 0;
          while (
            first < visibleOptions.length &&
            visibleOptions[first].disabled
          )
            first++;
