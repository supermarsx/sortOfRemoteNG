...
function ensureSpinKeyframes() {
  ...
  style.textContent =
    "@keyframes sorng-fib-spin { to { transform: rotateY(360deg); } }";
...
const FibonacciSphereVariant: React.FC<
  VariantRenderProps<"fibonacciSphere">
> = ({
  size,
  color,
  config,
      ...
      const tick = (now: number) => {
        ...
        for (let i = 0; i < arr.length; i++) {
          ...
          if (peak > 0.05) {
            ...
            ctx.beginPath();
            ctx.arc(px, py, r * (1 + 2.5 * peak), 0, TAU);
            ctx.fill();
          }
        }
        ctx.globalAlpha = 1;
  ...
  const sphereStyle: CSSProperties = {
    ...
    animation: reducedMotion
      ? undefined
      : `sorng-fib-spin ${config.spinSeconds}s linear infinite`,
...
export const descriptor: VariantDescriptor<"fibonacciSphere"> = {
  ...
  description:
    "Golden-angle dot sphere with brightness phase rolling through index spirals.",
