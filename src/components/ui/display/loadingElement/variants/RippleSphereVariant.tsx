...
function ensureSpinKeyframes() {
  ...
  style.textContent =
    "@keyframes sorng-ripple-spin { to { transform: rotateY(360deg); } }";
...
const RippleSphereVariant: React.FC<VariantRenderProps<"rippleSphere">> = ({
      ...
      const tick = (now: number) => {
        ...
        for (let i = 0; i < arr.length; i++) {
          ...
          if (peak > 0.05) {
            ...
            ctx.beginPath();
            ctx.arc(px, py, dotR * (1 + 2.0 * peak), 0, TAU);
            ctx.fill();
          }
        }
        ctx.globalAlpha = 1;
  ...
  const sphereStyle: CSSProperties = {
    ...
    animation: reducedMotion
      ? undefined
      : "sorng-ripple-spin 16s linear infinite",
...
export const descriptor: VariantDescriptor<"rippleSphere"> = {
  ...
  label: "Ripple sphere",
  description:
    "Fibonacci dot sphere displaced by two interfering traveling sine waves.",
