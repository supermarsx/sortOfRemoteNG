...
const HologramVariant: FC<VariantRenderProps<"hologram">> = ({
  ...
  const baseBg = `radial-gradient(circle at 50% 50%, color-mix(in srgb, ${color} 22%, #06070d) 0%, #06070d 80%)`;
    ...
    >
      ...
      <span
        ...
        style={{
          ...
          animation:
            animMode ?? `hg-flicker ${flickerSpeed}s steps(2, end) infinite`,
          animationPlayState: playState,
        }}
      />
    </span>
...
export const descriptor: VariantDescriptor<"hologram"> = {
  ...
  presets: [
    { id: "default", label: "Default", config: DEFAULT_HOLOGRAM },
    {
      id: "tight",
      label: "Tight mesh",
      config: { dotPitchPx: 5, scanSpeed: 1.4, flickerSpeed: 1.2 },
    },
    {
      id: "broadcast",
      label: "Broadcast",
      config: { dotPitchPx: 8, scanSpeed: 2.4, flickerSpeed: 2.0 },
    },
    {
      id: "malfunction",
      label: "Malfunction",
      config: { dotPitchPx: 6, scanSpeed: 0.8, flickerSpeed: 0.6 },
    },
  ],
  paramSchema: {
    fields: [
      {
        ...
        max: 14,
        step: 0.5,
        help: "Pixel-mesh grid spacing in px.",
      },
      {
        key: "scanSpeed",
        label: "Scan period",
