...
const ParticleStormVariant: FC<VariantRenderProps<"particleStorm">> = ({
  ...
  const baseBg = `radial-gradient(circle at 50% 50%, color-mix(in srgb, ${color} 14%, #06070d) 0%, #04050a 85%)`;

  const maskA =
    "radial-gradient(circle at 50% 50%, black 50%, transparent 78%)";
  const maskB =
    ...
    >
        ...
        style={
          {
            ...
            animation:
              animMode ??
              `ps-drift ${driftSpeed}s linear infinite, ps-pulse ${pulseSpeed}s ease-in-out infinite`,
        ...
        style={
          {
            ...
            animation:
              animMode ?? `ps-drift-rev ${driftSpeed * 1.5}s linear infinite`,
            animationPlayState: playState,
          } as CSSVars
        }
      />
      ...
      <span
        ...
        style={{
          ...
          animation:
            animMode ?? `ps-corebeat ${pulseSpeed / 2}s steps(2, end) infinite`,
          animationPlayState: playState,
        }}
      />
    </span>
...
export const descriptor: VariantDescriptor<"particleStorm"> = {
  ...
  presets: [
    { id: "default", label: "Default", config: DEFAULT_PARTICLE_STORM },
    {
      id: "sparse",
      label: "Sparse",
      config: { fieldDensity: 0.7, driftSpeed: 8, pulseSpeed: 3 },
    },
    {
      id: "dense",
      label: "Dense",
      config: { fieldDensity: 1.6, driftSpeed: 5, pulseSpeed: 2 },
    },
    {
      id: "lazy",
      label: "Lazy",
      config: { fieldDensity: 1.0, driftSpeed: 11, pulseSpeed: 4 },
    },
  ],
  paramSchema: {
    fields: [
      {
        ...
        max: 2,
        step: 0.1,
        help: "Multiplier on dot grid density.",
      },
      {
        key: "driftSpeed",
        label: "Drift speed",
