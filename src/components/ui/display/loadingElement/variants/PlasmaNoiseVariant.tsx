...
function ensureKeyframes(): void {
  ...
  style.textContent =
    "@keyframes sorng-plasma-spin{from{transform:rotateY(0)}to{transform:rotateY(360deg)}}";
...
function plasmaNoise(x: number, y: number, z: number, t: number): number {
  return (
    Math.sin(x * 1.7 + t * 1.3) *
      Math.sin(y * 1.9 - t * 0.8) *
      Math.sin(z * 2.1 + t * 0.6) +
    0.5 *
      Math.sin((x + y) * 2.3 + t * 1.7) *
      Math.sin((y - z) * 2.7 - t * 1.1) +
...
const PlasmaNoiseVariant: React.FC<VariantRenderProps<"plasmaNoise">> = ({
  ...
  const sphereStyle: CSSProperties = {
    ...
    animation: reducedMotion
      ? undefined
      : `sorng-plasma-spin 30s linear infinite`,
...
export const descriptor: VariantDescriptor<"plasmaNoise"> = {
  ...
  label: "Plasma noise",
  description:
    "Fibonacci-distributed dots driven by a flowing 3D pseudo-noise field.",
