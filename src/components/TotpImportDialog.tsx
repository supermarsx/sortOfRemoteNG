...
import {
  X, Upload, FileUp, Check, AlertTriangle,
  ChevronDown, Shield, CheckSquare, Square, QrCode, Loader2, Clipboard,
} from 'lucide-react';
    ...
    async (imageSource: Blob | string): Promise<void> => {
      ...
      try {
        ...
        if (uris.length > 0) {
          ...
          setQrError(`QR decoded but content is not an otpauth:// URI: "${data.slice(0, 80)}${data.length > 80 ? '...' : ''}"`);
        }
        ...
        <div className="px-5 py-3 border-b border-gray-700/50 space-y-3">
          ...
          >
            ...
            <span className="text-[10px] text-gray-500 mt-1">
              {IMPORT_SOURCES.find(s => s.id === source)?.extensions.join(', ') || '.json, .csv, .txt'}
            </span>
            <input
              ref={fileInputRef}
              type="file"
              className="hidden"
              accept={(IMPORT_SOURCES.find(s => s.id === source)?.extensions.join(',') || '.json,.csv,.txt,.2fas,.xml') + ',image/*'}
              onChange={(e) => {
          ...
          <div className="flex items-center gap-2 px-1">
            ...
            <span className="text-[10px] text-gray-500">
              Paste a QR code image (Ctrl+V) or drop/browse an image file to scan
            </span>
            {qrDecoding && <Loader2 size={12} className="text-blue-400 animate-spin flex-shrink-0" />}
          </div>
          ...
          <div className="flex-1 overflow-hidden flex flex-col min-h-0">
            ...
            <div className="flex items-center justify-between px-5 py-2 bg-gray-800/40 border-b border-gray-700/50 text-xs">
              ...
              {result.entries.length > 0 && (
                <button onClick={toggleAll} className="text-blue-400 hover:text-blue-300">
                  {selected.size === result.entries.length ? 'Deselect all' : 'Select all'}
                </button>
              )}
            </div>
              ...
              <div className="px-5 py-2 bg-yellow-500/5 border-b border-yellow-500/20">
                ...
                <div className="mt-1 max-h-16 overflow-y-auto">
                  {result.errors.map((err, i) => (
                    <div key={i} className="text-[10px] text-yellow-500/70">{err}</div>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto">
              {result.entries.length === 0 ? (
                ...
                result.entries.map((entry, i) => {
                  const isDuplicate = existingSet.has(entry.secret.toLowerCase());
