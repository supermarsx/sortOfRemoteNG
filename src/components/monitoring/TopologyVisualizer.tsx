...
import { useTopology } from "../../hooks/monitoring/useTopology";
import type {
  TopologyNode,
  LayoutAlgorithm,
  NodeKind,
...
const TopologyVisualizer: React.FC<TopologyVisualizerProps> = ({
  ...
  return (
    <div className="sor-topology-root flex flex-col h-full bg-[var(--color-surface)] text-[var(--color-text)]">
      {/* ---- Toolbar ---- */}
      <div className="sor-topology-toolbar flex items-center gap-2 px-3 py-2 border-b border-[var(--color-border)] bg-[var(--color-surface-raised)]">
      ...
      <div className="sor-topology-body flex flex-1 min-h-0">
        ...
        <div ref={containerRef} className="sor-topology-canvas-wrap relative flex-1 min-w-0">
          ...
          {/* Legend */}
          <div className="sor-topology-legend absolute top-2 left-2 bg-[var(--color-surface-raised)]/90 backdrop-blur rounded p-2 text-[10px] leading-relaxed border border-[var(--color-border)]">
          ...
          {/* Stats */}
          {topo.stats && (
            <div className="sor-topology-stats absolute bottom-2 right-2 bg-[var(--color-surface-raised)]/90 backdrop-blur rounded p-2 text-[10px] border border-[var(--color-border)] grid grid-cols-2 gap-x-3 gap-y-0.5">
        ...
        {/* ---- Details panel ---- */}
        {selected && (
          <aside className="sor-topology-details w-64 border-l border-[var(--color-border)] bg-[var(--color-surface-raised)] p-4 overflow-y-auto text-sm">
