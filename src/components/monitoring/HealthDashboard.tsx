...
const TopBar: React.FC<TopBarProps> = ({
  ...
  return (
    <header className="sor-dash-topbar">
      <h2 className="sor-dash-topbar__title">
        {t("dashboard.title", "Connection Health")}
      </h2>
      <div className="sor-dash-topbar__actions">
        <button
          ...
          title={
            monitoring
              ? t("dashboard.stopMonitoring", "Stop Monitoring")
              : t("dashboard.startMonitoring", "Start Monitoring")
          }
        >
          <IconCircle
            className={
              monitoring ? "sor-dash-icon--pulse" : "sor-dash-icon--muted"
            }
          />
          <span>
            {monitoring
        ...
        <span
          className={`sor-dash-status-dot ${error ? "sor-dash-status-dot--error" : monitoring ? "sor-dash-status-dot--ok" : "sor-dash-status-dot--idle"}`}
        />
      </div>
    </header>
  );
...
const QuickStatsCards: React.FC<QuickStatsCardsProps> = ({
  ...
  const cards = [
    {
      label: t("dashboard.totalConnections", "Total"),
      value: total,
    ...
    {
      label: t("dashboard.degraded", "Degraded"),
      value: degraded,
      color: STATUS_COLORS.degraded,
    },
    {
      label: t("dashboard.down", "Down"),
      value: down,
      color: STATUS_COLORS.unhealthy,
    },
    {
      label: t("dashboard.avgLatency", "Avg Latency"),
  ...
  return (
    <div className="sor-dash-stats-row">
      {cards.map((c) => (
        <div
          key={c.label}
          className="sor-dash-stat-card"
          style={{ borderTopColor: c.color }}
        >
          <span
            className="sor-dash-stat-card__value"
            style={{ color: c.color }}
          >
            {c.value}
          </span>
    ...
    <section className="sor-dash-alerts">
      ...
      <ul className="sor-dash-alerts__list">
        {active.map((alert) => (
          <li
            key={alert.id}
            className={`sor-dash-alert ${SEVERITY_CLASS[alert.severity] ?? ""}`}
          >
            <div className="sor-dash-alert__body">
              <span className="sor-dash-alert__conn">
                {alert.connectionName}
              </span>
              <span className="sor-dash-alert__msg">{alert.message}</span>
              <span className="sor-dash-alert__time">
                {relativeTime(alert.timestamp)}
              </span>
            </div>
            <button
              className="sor-dash-alert__ack"
              onClick={() => onAcknowledge(alert.id)}
              title={t("dashboard.acknowledge", "Acknowledge")}
            >
              <IconCheck />
            </button>
          </li>
...
const HeatmapGrid: React.FC<HeatmapGridProps> = ({
  ...
  if (cells.length === 0) {
    return (
      <p className="sor-dash-placeholder">
        {t("dashboard.noHeatmap", "No heatmap data available.")}
      </p>
    );
  }
  ...
  return (
    <section className="sor-dash-heatmap">
      <h3 className="sor-dash-section-title">
        {t("dashboard.heatmap", "Connection Heatmap")}
      </h3>
...
const SparklineMini: React.FC<SparklineMiniProps> = ({ data }) => {
  ...
  if (validPoints.length < 2)
    return <span className="sor-dash-sparkline--empty">—</span>;
  ...
  const pathD = validPoints
    .map((p, i) => {
      const x = i * step;
      const y =
        height - ((p.latencyMs! - data.minMs) / range) * (height - 4) - 2;
...
const TopLatencySection: React.FC<TopLatencyProps> = ({
  entries,
  sparklines,
}) => {
  const { t } = useTranslation();
...
const RecentList: React.FC<RecentListProps> = ({ recent }) => {
  ...
  if (recent.length === 0) {
    return (
      <p className="sor-dash-placeholder">
        {t("dashboard.noRecent", "No recent connections.")}
      </p>
    );
  }
  ...
  return (
    <section className="sor-dash-recent">
      <h3 className="sor-dash-section-title">
        {t("dashboard.recentConnections", "Recent Connections")}
      </h3>
      <ul className="sor-dash-recent__list">
        {recent.map((r, i) => (
          <li key={`${r.connectionId}-${i}`} className="sor-dash-recent__item">
            ...
            <span className="sor-dash-recent__time">
      ...
      <div className="sor-dash-config-panel">
        ...
        <div className="sor-dash-config-panel__body">
          <label className="sor-dash-config-field">
            <span>
              {t("dashboard.refreshInterval", "Refresh interval (s)")}
            </span>
            <input
              ...
              onChange={(e) =>
                onUpdate({ refreshIntervalMs: Number(e.target.value) * 1000 })
              }
            />
          </label>
          <label className="sor-dash-config-field">
            ...
            <input
              ...
              onChange={(e) =>
                onUpdate({ parallelChecks: Number(e.target.value) })
              }
            />
          </label>
          <button
          ...
          {showThresholds && (
            <div
              className="sor-dash-config-thresholds"
              role="group"
              aria-label="Alert threshold configuration"
            >
              <label className="sor-dash-config-field">
                <span>
                  {t("dashboard.latencyTarget", "Latency target (ms)")}
                </span>
                <input
                  className="sor-dash-input"
                  type="number"
              ...
              <label className="sor-dash-config-field">
                <span>
                  {t("dashboard.memoryThreshold", "Memory threshold (%)")}
                </span>
                <input
                  className="sor-dash-input"
                  type="number"
              ...
              <div className="flex gap-2 mt-2">
                <button
                  ...
                  onClick={() => {
                    onUpdate({
                      _thresholds: { latencyMs, cpuPercent, memoryPercent },
                    } as any);
...
export const HealthDashboard: React.FC<HealthDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      await Promise.all([
        ...
        dash.fetchAllHealth(),
        dash.fetchHeatmap(),
        dash.fetchQuickStats(),
      ]);

      const [top, rec] = await Promise.all([
        dash.fetchTopLatency(8),
        dash.fetchRecent(12),
      ]);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps, react/exhaustive-deps -- load dashboard data when panel opens
  }, [isOpen]);
    ...
    return (
      <div className="sor-dash-root sor-dash-root--error">
        <div className="sor-dash-error">
          ...
          <button className="sor-dash-btn" onClick={handleRefresh}>
  ...
  const selectedEntry = selectedConnection
    ? (dash.healthEntries.find((e) => e.connectionId === selectedConnection) ??
      null)
    : null;

  return (
    <div className="sor-dash-root" data-testid="health-dashboard">
      {/* Top bar */}
      <TopBar
      ...
      {dash.error && initialLoaded && (
        <div className="sor-dash-error-toast">
          ...
          <button
            className="sor-dash-btn sor-dash-btn--icon"
            onClick={() => {
              /* clear handled by next fetch */
            }}
          >
            <IconX />
          </button>
      ...
      <div className="sor-dash-body">
        ...
        <WidgetGrid columns={dash.layout.columns >= 8 ? 3 : 2}>
          {/* Heatmap widget */}
          <WidgetCard
            title={t("dashboard.heatmap", "Connection Heatmap")}
            span={2}
          >
            <HeatmapGrid
              cells={dash.heatmap}
          ...
          <WidgetCard
            title={t("dashboard.recentConnections", "Recent Connections")}
          >
          ...
          <WidgetCard
            title={t("dashboard.topLatency", "Top Connections by Latency")}
            span={2}
          >
            <TopLatencySection
              entries={topLatency}
              sparklines={dash.sparklines}
            />
          </WidgetCard>

          {/* Protocol breakdown widget */}
          <WidgetCard
            title={t("dashboard.protocolBreakdown", "Protocol Breakdown")}
          >
        ...
        {selectedEntry && (
          <section className="sor-dash-detail">
            <header className="sor-dash-detail__header">
              <h3 className="sor-dash-detail__title">
                {selectedEntry.connectionName}
              </h3>
              <button
                className="sor-dash-btn sor-dash-btn--icon"
            ...
            <div className="sor-dash-detail__grid">
              <DetailField
                label={t("dashboard.protocol", "Protocol")}
                value={selectedEntry.protocol}
              />
              <DetailField
                label={t("dashboard.hostname", "Hostname")}
                value={selectedEntry.hostname}
              />
              <DetailField
              ...
              />
              <DetailField
                label={t("dashboard.latency", "Latency")}
                value={formatLatency(selectedEntry.latencyMs)}
              />
              <DetailField
                label={t("dashboard.uptime", "Uptime")}
                value={`${selectedEntry.uptimePercent.toFixed(1)}%`}
              />
              <DetailField
                label={t("dashboard.checks", "Checks")}
                value={`${selectedEntry.checkCount} (${selectedEntry.failCount} fail)`}
              />
              <DetailField
                label={t("dashboard.lastChecked", "Last Checked")}
                value={relativeTime(selectedEntry.lastChecked)}
              />
              {selectedEntry.errorMessage && (
                <DetailField
                  label={t("dashboard.error", "Error")}
                  value={selectedEntry.errorMessage}
                  className="sor-dash-detail__error"
                />
              )}
            </div>
            {dash.sparklines[selectedEntry.connectionId] && (
              <div className="sor-dash-detail__sparkline">
                <SparklineMini
                  data={dash.sparklines[selectedEntry.connectionId]}
                />
              </div>
            )}
          </section>
...
const DetailField: React.FC<{
  ...
  <div className={`sor-dash-detail__field ${className ?? ""}`}>
    ...
    <span
      className="sor-dash-detail__field-value"
...
const ProtocolBreakdown: React.FC<{ stats: QuickStats | null }> = ({
  stats,
}) => {
  const { t } = useTranslation();
  ...
  if (!stats || Object.keys(stats.protocolBreakdown).length === 0) {
    return (
      <p className="sor-dash-placeholder">
        {t("dashboard.noProtocols", "No protocol data.")}
      </p>
    );
  }

  const total =
    Object.values(stats.protocolBreakdown).reduce((s, n) => s + n, 0) || 1;
