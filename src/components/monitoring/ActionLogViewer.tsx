  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div className="relative flex-1 max-w-md">
        ...
        <input type="text" placeholder="Search logs..." value={m.searchTerm} onChange={(e) => m.setSearchTerm(e.target.value)} aria-label="Search logs" className="w-full pl-9 pr-3 py-2 bg-[var(--color-border)] border border-[var(--color-border)] rounded-lg text-[var(--color-text)] placeholder-[var(--color-textMuted)] focus:outline-none focus:ring-2 focus:ring-warning/50 focus:border-warning/50 text-sm transition-all" />
      </div>
        ...
        <tbody className="divide-y divide-[var(--color-border)]">
          {m.filteredLogs.map((log) => (
            <tr key={log.id} className="hover:bg-[var(--color-border)]">
              <td className="px-4 py-3 text-sm text-[var(--color-textSecondary)]"><div><div>{log.timestamp.toLocaleDateString()}</div><div className="text-xs text-[var(--color-textMuted)]">{log.timestamp.toLocaleTimeString()}</div></div></td>
    ...
    <>
      <Modal isOpen={isOpen} onClose={onClose} backdropClassName="bg-black/50" panelClassName="max-w-6xl h-[90vh] rounded-lg overflow-hidden" contentClassName="bg-[var(--color-surface)] relative">
        <div className="flex flex-1 min-h-0 flex-col">
          ...
          <div className="border-b border-[var(--color-border)] px-4 py-3 bg-[var(--color-surfaceHover)] space-y-3">
            <SearchBar m={m} />
            <FilterBar m={m} />
          </div>
          <LogTable m={m} />
