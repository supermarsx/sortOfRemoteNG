...
const PasteArea: React.FC<{ secret: string; mgr: Mgr }> = ({ secret, mgr }) => (
  <div className="space-y-1.5">
    <Textarea
      value={mgr.pasteText}
      onChange={(v) => mgr.setPasteText(v)}
      placeholder="Paste recovery codes here (one per line, or comma-separated)"
      className="w-full h-24 px-2 py-1.5 bg-[var(--color-input)] border border-[var(--color-border)] rounded text-xs text-[var(--color-text)] font-mono placeholder-[var(--color-textMuted)] resize-none"
      autoFocus
    />
...
const AddSingleCodeInput: React.FC<{ secret: string; mgr: Mgr }> = ({ secret, mgr }) => (
  <div className="flex items-center space-x-2">
    <input
      ...
      className="flex-1 px-2 py-1 bg-[var(--color-input)] border border-[var(--color-border)] rounded text-xs text-[var(--color-text)] font-mono placeholder-[var(--color-textMuted)]"
      autoFocus
    />
    <button
      type="button"
