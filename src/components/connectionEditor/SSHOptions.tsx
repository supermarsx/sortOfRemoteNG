...
interface SSHOptionsProps {
  ...
  sections?: readonly SSHOptionsSection[];
}

export type SSHOptionsSection = "authentication" | "terminal" | "connection";
...
export const SSHOptions: React.FC<SSHOptionsProps> = ({
  ...
  const usesManagedSshSecrets =
    formData.protocol === "ssh" && !!sshSecretManager;
  ...
  const passphraseValue = usesManagedSshSecrets
    ...
    : formData.passphrase || "";
  const shows = (section: SSHOptionsSection) =>
  ...
  return (
    <>
        ...
        <>
          <div>
            <label className="sor-form-label">
              Username{" "}
              <InfoTooltip text="The SSH username used to authenticate with the remote host." />
            </label>
            <input
              type="text"
              data-testid={
                formData.protocol === "ssh" ? "editor-username" : undefined
              }
          ...
          {formData.protocol === "ssh" && (
            <div className="space-y-3">
              <div>
                <label className="sor-form-label">
                  Authentication Type{" "}
                  <InfoTooltip text="Choose between password-based authentication or public key authentication." />
                </label>
                <Select
                  value={formData.authType ?? "password"}
                  onChange={(v: string) =>
                    setFormData({ ...formData, authType: v as any })
                  }
                  options={[
              ...
              <label className="sor-form-inline-check">
                <Checkbox
                  checked={formData.ignoreSshSecurityErrors ?? false}
                  onChange={(v: boolean) =>
                    setFormData({
                      ...formData,
                ...
                <span>
                  ...
                  <InfoTooltip text="When enabled, host key mismatches and certificate errors are silently ignored. Convenient but less secure." />
                </span>
              </label>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="font-medium">
                        SSH identity verification is disabled.
                      </p>
                      <p className="text-error/90">
                        Host key mismatches and untrusted identities will be
                        accepted without verification. Use this only for
                        disposable lab systems or known test environments.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <label className="sor-form-label">
                  ...
                  <InfoTooltip text="Determines how unknown or changed host keys are handled for this connection." />
                </label>
                <Select
                  ...
                  options={[
                    ...
                    {
                      value: "always-trust",
                      label: "Always Trust (skip verification)",
                    },
                    {
                      value: "strict",
                      label: "Strict (reject unless pre-approved)",
                    },
                  ]}
                  variant="form"
                />
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <div>
                  <label className="sor-form-label">
                    Connect Timeout (sec){" "}
                    <InfoTooltip text="Maximum time in seconds to wait for the SSH connection to be established." />
                  </label>
                  <NumberInput
                ...
                <div>
                  <label className="sor-form-label">
                    Known Hosts Path{" "}
                    <InfoTooltip text="Path to the known_hosts file used for host key verification. Leave empty to use the default location." />
                  </label>
                  <input
                    type="text"
          ...
          {formData.authType === "password" && (
            <div>
              <label className="sor-form-label">
                Password{" "}
                <InfoTooltip text="The password used for SSH password authentication." />
              </label>
              {usesManagedSshSecrets ? (
                <PasswordInput
                  ref={sshSecretManager.passwordInputRef}
                  data-testid={
                    formData.protocol === "ssh" ? "editor-password" : undefined
                  }
                  value={passwordValue}
                  onChange={(e) =>
          ...
          {formData.protocol === "ssh" && formData.authType === "key" && (
            <>
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium text-[var(--color-textSecondary)]">
                    Private Key{" "}
                    <InfoTooltip text="The PEM or PPK private key used for public key authentication." />
                  </label>
                  <button
                    type="button"
                ...
                <Textarea
                  ref={
                    usesManagedSshSecrets
                      ? sshSecretManager.privateKeyInputRef
                      : undefined
                  }
              ...
              <div>
                <label className="sor-form-label">
                  Passphrase (optional){" "}
                  <InfoTooltip text="Decryption passphrase for the private key, if the key is encrypted." />
                </label>
                {usesManagedSshSecrets ? (
                  <PasswordInput
      ...
      {shows("connection") && formData.protocol === "ssh" && (
