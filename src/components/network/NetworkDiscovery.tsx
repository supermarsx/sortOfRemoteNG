...
const HostCard: React.FC<{ mgr: Mgr; host: DiscoveredHost }> = ({ mgr, host }) => (
  <div
    ...
    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); mgr.toggleHostSelection(host.ip); } }}
  >
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center space-x-3">
        <Checkbox checked={mgr.selectedHosts.has(host.ip)} onChange={() => mgr.toggleHostSelection(host.ip)} className="rounded border-[var(--color-border)] bg-[var(--color-input)] text-primary" />
        <div>
