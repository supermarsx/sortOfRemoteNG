      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <Labeled label={t("integrations.caddy.instanceName", "Saved name")}>
          <input
            ...
            placeholder={t(
              "integrations.caddy.instanceNameHint",
              "optional label",
            )}
          />
        </Labeled>
    ...
    async (kind: "set" | "patch" | "delete") => {
      try {
        if (kind === "delete") {
          ...
          if (kind === "set")
            await mgr.run(() => mgr.api.setConfigPath(cid, path, parsed));
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          className={btn}
          onClick={() => void mgr.api.ping(cid).then(setDetail)}
        >
      ...
      <div className={card}>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          <button
            className={`${btn} text-red-500`}
            onClick={() => void mutatePath("delete")}
            disabled={mgr.isLoading}
          >
      ...
      <div className={card}>
        ...
        <button
          className={`${btn} mt-2`}
          onClick={loadConfig}
          disabled={mgr.isLoading || !loadBody}
        >
...
const ServersTab: React.FC<{ mgr: CaddyManager; cid: string }> = ({
  ...
  const remove = useCallback(
    async (name: string) => {
      if (
        !window.confirm(
          t("integrations.caddy.deleteServerConfirm", "Delete this server?"),
        )
      )
        return;
      try {
          ...
          <thead className="text-[var(--color-textMuted)]">
            <tr>
              <th className="px-2 py-1">
                {t("integrations.caddy.name", "Name")}
              </th>
              <th className="px-2 py-1">
                {t("integrations.caddy.listen", "Listen")}
              </th>
              <th className="px-2 py-1">
                {t("integrations.caddy.routes", "Routes")}
              </th>
              <th className="px-2 py-1" />
            </tr>
          </thead>
...
const RoutesTab: React.FC<{ mgr: CaddyManager; cid: string }> = ({
  ...
  const setAt = useCallback(async () => {
    ...
    try {
      await mgr.run(() =>
        mgr.api.setRoute(cid, server, Number(editIndex), route),
      );
      await refresh();
    } catch {
      /* surfaced */
  ...
  const remove = useCallback(
    async (index: number) => {
      if (
        !window.confirm(
          t("integrations.caddy.deleteRouteConfirm", "Delete this route?"),
        )
      )
        return;
      try {
    ...
    <div className="flex flex-col gap-3">
        ...
        <table className="w-full text-left text-xs">
          <thead className="text-[var(--color-textMuted)]">
            <tr>
              ...
              <th className="px-2 py-1">
                {t("integrations.caddy.match", "Match")}
              </th>
              <th className="px-2 py-1">
                {t("integrations.caddy.handlers", "Handlers")}
              </th>
              <th className="px-2 py-1" />
            </tr>
          </thead>
      ...
      <div className={card}>
        ...
        <button
          className={`${btn} mt-2`}
          onClick={add}
          disabled={mgr.isLoading || !addBody}
        >
      ...
      <div className={card}>
        ...
        <button
          className={`${btn} mt-2`}
          onClick={setAt}
          disabled={mgr.isLoading || editIndex === ""}
        >
      ...
      <div className={card}>
        <h4 className="mb-2 text-xs font-semibold text-[var(--color-text)]">
          {t(
            "integrations.caddy.setAllRoutes",
            "Replace all routes (JSON array)",
          )}
        </h4>
        <textarea
          className={`${field} font-mono`}
        ...
        <button
          className={`${btn} mt-2`}
          onClick={setAll}
          disabled={mgr.isLoading || !allBody}
        >
...
const TlsTab: React.FC<{ mgr: CaddyManager; cid: string }> = ({ mgr, cid }) => {
  ...
  const saveTlsApp = useCallback(async () => {
    const app = parseJson<Parameters<typeof mgr.api.setTlsApp>[1]>(
      tlsAppBody,
      () => window.alert(t("integrations.caddy.invalidJson", "Invalid JSON")),
    );
  ...
  const saveAutomation = useCallback(async () => {
    const automation = parseJson<
      Parameters<typeof mgr.api.setTlsAutomation>[1]
    >(automationBody, () =>
      window.alert(t("integrations.caddy.invalidJson", "Invalid JSON")),
    );
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        <button
          className={btn}
          onClick={() => void call(() => mgr.api.getTlsApp(cid))}
          disabled={mgr.isLoading}
        >
        ...
        <button
          className={btn}
          onClick={() => void call(() => mgr.api.getTlsAutomation(cid))}
          disabled={mgr.isLoading}
        >
      ...
      <div className={card}>
        <h4 className="mb-2 text-xs font-semibold text-[var(--color-text)]">
          {t(
            "integrations.caddy.automateDomains",
            "Automated (managed) domains",
          )}
        </h4>
        <Labeled
          label={t(
            "integrations.caddy.domainsCsv",
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          <thead className="text-[var(--color-textMuted)]">
            <tr>
              <th className="px-2 py-1">
                {t("integrations.caddy.subjects", "Subjects (SANs)")}
              </th>
              <th className="px-2 py-1">
                {t("integrations.caddy.issuer", "Issuer")}
              </th>
              <th className="px-2 py-1">
                {t("integrations.caddy.managed", "Managed")}
              </th>
              <th className="px-2 py-1">
                {t("integrations.caddy.notAfter", "Expires")}
              </th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className={card}>
        ...
        <button
          className={`${btn} mt-2`}
          onClick={saveTlsApp}
          disabled={mgr.isLoading || !tlsAppBody}
        >
          {t("integrations.caddy.save", "Save")}
        </button>
      </div>
      ...
      <div className={card}>
        <h4 className="mb-2 text-xs font-semibold text-[var(--color-text)]">
          {t(
            "integrations.caddy.setTlsAutomation",
            "Replace TLS automation (JSON)",
          )}
        </h4>
        <textarea
          className={`${field} font-mono`}
        ...
        <button
          ...
          disabled={mgr.isLoading || !automationBody}
        >
          {t("integrations.caddy.save", "Save")}
        </button>
      </div>
...
const ProxyTab: React.FC<{ mgr: CaddyManager; cid: string }> = ({
  mgr,
  cid,
}) => {
  const { t } = useTranslation();
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className={card}>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Labeled label={t("integrations.caddy.server", "Server")}>
            <input
              className={field}
              value={rp.server}
              onChange={(e) => setRp((s) => ({ ...s, server: e.target.value }))}
            />
          </Labeled>
          <Labeled
            label={t("integrations.caddy.hostsCsv", "Hosts (comma-separated)")}
          >
            <input
              className={field}
              value={rp.hosts}
              onChange={(e) => setRp((s) => ({ ...s, hosts: e.target.value }))}
              placeholder="example.com"
            />
          </Labeled>
          <Labeled
            label={t(
              "integrations.caddy.upstreamsCsv",
              "Upstreams (comma-separated)",
            )}
          >
            <input
              className={field}
              value={rp.upstreams}
              onChange={(e) =>
                setRp((s) => ({ ...s, upstreams: e.target.value }))
              }
              placeholder="localhost:8080"
            />
          </Labeled>
          <Labeled
            label={t("integrations.caddy.healthCheckPath", "Health check path")}
          >
            <input
              className={field}
              value={rp.healthCheckPath}
              onChange={(e) =>
                setRp((s) => ({ ...s, healthCheckPath: e.target.value }))
              }
              placeholder="/healthz"
            />
          </Labeled>
          <Labeled
            label={t(
              "integrations.caddy.loadBalancing",
              "Load balancing policy",
            )}
          >
            <input
              className={field}
              value={rp.loadBalancing}
              onChange={(e) =>
                setRp((s) => ({ ...s, loadBalancing: e.target.value }))
              }
              placeholder="round_robin"
            />
          </Labeled>
          <Labeled
            label={t("integrations.caddy.stripPrefix", "Strip path prefix")}
          >
            <input
              className={field}
              value={rp.stripPrefix}
              onChange={(e) =>
                setRp((s) => ({ ...s, stripPrefix: e.target.value }))
              }
            />
          </Labeled>
        </div>
        <label className="mt-2 flex items-center gap-2 text-xs text-[var(--color-textSecondary)]">
          <input
            type="checkbox"
            checked={rp.tls}
            onChange={(e) => setRp((s) => ({ ...s, tls: e.target.checked }))}
          />
        ...
        <button
          className={`${btn} mt-2`}
          onClick={createRp}
          disabled={mgr.isLoading || !rp.hosts || !rp.upstreams}
        >
      ...
      <div className={card}>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Labeled label={t("integrations.caddy.server", "Server")}>
            <input
              className={field}
              value={fs.server}
              onChange={(e) => setFs((s) => ({ ...s, server: e.target.value }))}
            />
          </Labeled>
          <Labeled
            label={t("integrations.caddy.hostsCsv", "Hosts (comma-separated)")}
          >
            <input
              className={field}
              value={fs.hosts}
              onChange={(e) => setFs((s) => ({ ...s, hosts: e.target.value }))}
            />
          </Labeled>
          <Labeled label={t("integrations.caddy.root", "Root directory")}>
            <input
              className={field}
              value={fs.root}
              onChange={(e) => setFs((s) => ({ ...s, root: e.target.value }))}
              placeholder="/var/www"
            />
          </Labeled>
          <Labeled
            label={t(
              "integrations.caddy.indexNamesCsv",
              "Index files (comma-separated)",
            )}
          >
            <input
              className={field}
              value={fs.indexNames}
              onChange={(e) =>
                setFs((s) => ({ ...s, indexNames: e.target.value }))
              }
        ...
        <div className="mt-2 flex flex-wrap gap-4">
          <label className="flex items-center gap-2 text-xs text-[var(--color-textSecondary)]">
            <input
              ...
              checked={fs.browse}
              onChange={(e) =>
                setFs((s) => ({ ...s, browse: e.target.checked }))
              }
            />
          ...
          <label className="flex items-center gap-2 text-xs text-[var(--color-textSecondary)]">
            <input
              type="checkbox"
              checked={fs.tls}
              onChange={(e) => setFs((s) => ({ ...s, tls: e.target.checked }))}
            />
            {t("integrations.caddy.enableTls", "Enable automatic HTTPS")}
          </label>
        </div>
        <button
          className={`${btn} mt-2`}
          onClick={createFs}
          disabled={mgr.isLoading || !fs.hosts || !fs.root}
        >
          {t("integrations.caddy.create", "Create")}
        </button>
      </div>
      ...
      <div className={card}>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Labeled label={t("integrations.caddy.server", "Server")}>
            <input
              className={field}
              value={rd.server}
              onChange={(e) => setRd((s) => ({ ...s, server: e.target.value }))}
            />
          </Labeled>
          <Labeled
            label={t("integrations.caddy.hostsCsv", "Hosts (comma-separated)")}
          >
            <input
              className={field}
              value={rd.hosts}
              onChange={(e) => setRd((s) => ({ ...s, hosts: e.target.value }))}
            />
          </Labeled>
          <Labeled label={t("integrations.caddy.target", "Target URL")}>
            <input
              className={field}
              value={rd.target}
              onChange={(e) => setRd((s) => ({ ...s, target: e.target.value }))}
        ...
        <label className="mt-2 flex items-center gap-2 text-xs text-[var(--color-textSecondary)]">
          <input
            ...
            onChange={(e) =>
              setRd((s) => ({ ...s, permanent: e.target.checked }))
            }
          />
        ...
        <button
          className={`${btn} mt-2`}
          onClick={createRd}
          disabled={mgr.isLoading || !rd.hosts || !rd.target}
        >
          {t("integrations.caddy.create", "Create")}
        </button>
      </div>
...
const TABS: {
  ...
  {
    key: "config",
    labelKey: "integrations.caddy.tabConfig",
    labelDefault: "Config",
    icon: FileText,
  },
  {
    key: "caddyfile",
    labelKey: "integrations.caddy.tabCaddyfile",
    labelDefault: "Caddyfile",
    icon: FileCode2,
  },
  {
    key: "servers",
    labelKey: "integrations.caddy.tabServers",
    labelDefault: "Servers",
    icon: ServerCog,
  },
  {
    key: "routes",
    labelKey: "integrations.caddy.tabRoutes",
    labelDefault: "Routes",
    icon: RouteIcon,
  },
  {
    key: "tls",
    labelKey: "integrations.caddy.tabTls",
    labelDefault: "TLS",
    icon: ShieldCheck,
  },
  {
    key: "proxy",
    labelKey: "integrations.caddy.tabProxy",
    labelDefault: "Proxy",
    icon: Network,
  },
];

const CaddyPanel: React.FC<IntegrationPanelProps> = ({
  isOpen,
  instanceId,
}) => {
  const { t } = useTranslation();
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          >
            ...
            />
            {mgr.isConnected
              ? (mgr.summary?.admin_url ??
                t("integrations.caddy.connected", "Connected"))
