...
const Sidebar: React.FC<SubProps> = ({ mgr }) => {
  ...
  return (
    <div className="w-52 shrink-0 border-r border-[var(--color-border)] flex flex-col bg-[var(--color-surfaceHover)]">
      {/* Search */}
      <div className="p-3 border-b border-[var(--color-border)]">
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--color-textSecondary)]" />
          <input
            className="w-full pl-8 pr-3 py-1.5 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] text-xs text-[var(--color-text)] focus:outline-none focus:ring-1 focus:ring-primary/50"
        ...
        {TABS.filter(
          ...
          return (
            <button
              ...
              data-testid={`idrac-tab-${tab.key}`}
              className={`w-full flex items-center gap-2.5 px-4 py-2 text-xs transition-colors ${
                isActive
                  ? "bg-warning/10 text-warning border-r-2 border-warning"
                  : "text-[var(--color-textSecondary)] hover:bg-[var(--color-bg)] hover:text-[var(--color-text)]"
              }`}
            >
      ...
      <div className="p-3 border-t border-[var(--color-border)] space-y-1">
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] text-[var(--color-textSecondary)] hover:bg-[var(--color-bg)] hover:text-[var(--color-text)] transition-colors disabled:opacity-50"
        >
          <RefreshCw
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] text-[var(--color-textSecondary)] hover:bg-[var(--color-bg)] hover:text-error transition-colors"
        >
