...
export const NetworkView: React.FC<SubProps> = ({ mgr }) => {
  ...
  if (mgr.loading && mgr.networkAdapters.length === 0) {
    return <div className="flex items-center justify-center flex-1"><Loader2 className="w-6 h-6 animate-spin text-[var(--color-textSecondary)]" /></div>;
  }
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      {mgr.networkAdapters.map((adapter) => (
        <div key={adapter.id} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
          <div className="flex items-center gap-2 mb-3">
            <Network className="w-4 h-4 text-primary" />
            <h3 className="text-xs font-semibold text-[var(--color-text)]">{adapter.name}</h3>
            <span className="text-[10px] text-[var(--color-textSecondary)]">{adapter.model ?? ""}</span>
          </div>
              ...
              <tbody>
                {adapter.ports.map((p) => (
                  <tr key={p.id} className="border-b border-[var(--color-border)] last:border-0">
                    ...
                    <td className="py-1 text-[var(--color-textSecondary)]">{p.macAddress ?? "—"}</td>
                    <td className="py-1">
                      <span className={p.linkStatus?.toLowerCase() === "up" ? "text-success" : "text-[var(--color-textSecondary)]"}>
                        {p.linkStatus ?? "—"}
                      </span>
                    </td>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-1.5">
            {[
              ...
              <div key={label} className="flex items-baseline gap-2">
                <span className="text-[10px] text-[var(--color-textSecondary)] w-20 shrink-0">{label}:</span>
    ...
    <div className="flex-1 overflow-y-auto p-4">
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] text-[var(--color-textSecondary)]">({mgr.firmware.length} components)</span>
        </div>
        <table className="w-full text-[10px]">
          <thead>
            <tr className="text-[var(--color-textSecondary)] border-b border-[var(--color-border)]">
          ...
          <tbody>
            {mgr.firmware.map((fw) => (
              <tr key={fw.id} className="border-b border-[var(--color-border)] last:border-0">
                ...
                <td className="py-1 text-[var(--color-textSecondary)]">{fw.installDate ?? "—"}</td>
                <td className="py-1 text-center">{fw.updateable ? <CheckCircle className="w-3 h-3 text-success inline" /> : <XCircle className="w-3 h-3 text-[var(--color-textSecondary)] inline" />}</td>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <div className="flex items-center gap-3 mb-2">
        <span className="text-[10px] text-[var(--color-textSecondary)]">LC Status: <span className="text-[var(--color-text)]">{mgr.lcStatus ?? "Unknown"}</span></span>
      ...
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
        ...
        {mgr.jobs.length === 0 ? (
          <p className="text-[10px] text-[var(--color-textSecondary)]">{t("idrac.lifecycle.no_jobs", "No jobs found")}</p>
        ) : (
          <table className="w-full text-[10px]">
            <thead>
              <tr className="text-[var(--color-textSecondary)] border-b border-[var(--color-border)]">
            ...
            <tbody>
              {mgr.jobs.map((j) => (
                <tr key={j.id} className="border-b border-[var(--color-border)] last:border-0">
                  ...
                  <td className="py-1 text-[var(--color-textSecondary)] truncate max-w-[200px]">{j.message ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      {mgr.virtualMedia.map((vm) => (
        <div key={vm.id} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
          <div className="flex items-center gap-2 mb-2">
            <Disc className="w-4 h-4 text-primary" />
            <h3 className="text-xs font-semibold text-[var(--color-text)]">{vm.name}</h3>
            <span className={`text-[10px] px-1.5 py-0.5 rounded ${vm.inserted ? "bg-success/10 text-success" : "bg-[var(--color-bg)] text-[var(--color-textSecondary)]"}`}>
              {vm.inserted ? "Inserted" : "Empty"}
            </span>
          </div>
          {vm.image && <p className="text-[10px] text-[var(--color-textSecondary)] mb-2">Image: {vm.image}</p>}
        ...
        {ci ? (
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2 text-[10px]">
              <div><span className="text-[var(--color-textSecondary)]">Type:</span> <span className="text-[var(--color-text)]">{ci.consoleType}</span></div>
              <div><span className="text-[var(--color-textSecondary)]">Enabled:</span> <span className={ci.enabled ? "text-success" : "text-error"}>{ci.enabled ? "Yes" : "No"}</span></div>
              <div><span className="text-[var(--color-textSecondary)]">Max Sessions:</span> <span className="text-[var(--color-text)]">{ci.maxSessions ?? "—"}</span></div>
              <div><span className="text-[var(--color-textSecondary)]">Encryption:</span> <span className="text-[var(--color-text)]">{ci.sslEncryptionBits ? `${ci.sslEncryptionBits}-bit` : "—"}</span></div>
            </div>
            {ci.url && (
          ...
          <p className="text-[10px] text-[var(--color-textSecondary)]">{t("idrac.no_data", "No data available")}</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-3">
      <div className="flex items-center gap-2">
        <button onClick={() => setLogTab("sel")} className={`text-[10px] px-3 py-1.5 rounded-lg border ${logTab === "sel" ? "border-warning text-warning bg-warning/10" : "border-[var(--color-border)] text-[var(--color-textSecondary)]"}`}>
          SEL ({mgr.selEntries.length})
        </button>
        <button onClick={() => setLogTab("lc")} className={`text-[10px] px-3 py-1.5 rounded-lg border ${logTab === "lc" ? "border-warning text-warning bg-warning/10" : "border-[var(--color-border)] text-[var(--color-textSecondary)]"}`}>
      ...
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4 max-h-[70vh] overflow-y-auto">
        <table className="w-full text-[10px]">
          <thead>
            <tr className="text-[var(--color-textSecondary)] border-b border-[var(--color-border)]">
              <th className="text-left py-1">ID</th>
          ...
          <tbody>
            {entries.map((e) => (
              <tr key={e.id} className="border-b border-[var(--color-border)] last:border-0">
                ...
                <td className="py-1"><span className={e.severity.toLowerCase() === "critical" ? "text-error" : e.severity.toLowerCase() === "warning" ? "text-warning" : "text-[var(--color-textSecondary)]"}>{e.severity}</span></td>
                <td className="py-1 text-[var(--color-text)] truncate max-w-[400px]">{e.message}</td>
                <td className="py-1 text-[var(--color-textSecondary)]">{e.created ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <tbody>
            {mgr.users.map((u) => (
              <tr key={u.id} className="border-b border-[var(--color-border)] last:border-0">
                ...
                <td className="py-1 text-[var(--color-text)]">{u.name || <span className="text-[var(--color-textSecondary)] italic">empty</span>}</td>
                <td className="py-1 text-[var(--color-textSecondary)]">{u.roleId}</td>
                <td className="py-1 text-center">{u.enabled ? <CheckCircle className="w-3 h-3 text-success inline" /> : <XCircle className="w-3 h-3 text-[var(--color-textSecondary)] inline" />}</td>
...
export const BiosView: React.FC<SubProps> = ({ mgr }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-3">
      {mgr.bootConfig && (
        <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4 mb-3">
          <h3 className="text-xs font-semibold text-[var(--color-text)] mb-2">{t("idrac.bios.boot", "Boot Configuration")}</h3>
          <div className="grid grid-cols-2 gap-2 text-[10px]">
            <div><span className="text-[var(--color-textSecondary)]">Boot Mode:</span> <span className="text-[var(--color-text)]">{mgr.bootConfig.bootMode}</span></div>
            <div><span className="text-[var(--color-textSecondary)]">Override:</span> <span className="text-[var(--color-text)]">{mgr.bootConfig.bootSourceOverrideTarget ?? "None"}</span></div>
          </div>
          <div className="mt-2 text-[10px] text-[var(--color-textSecondary)]">
            Boot Order: {mgr.bootConfig.bootOrder.join(" → ") || "N/A"}
          </div>
        </div>
      )}
      <div className="flex items-center gap-2">
        <input className="flex-1 px-3 py-1.5 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] text-xs text-[var(--color-text)] focus:outline-none focus:ring-1 focus:ring-primary/50" placeholder="Filter BIOS attributes..." value={filter} onChange={(e) => setFilter(e.target.value)} />
        <span className="text-[10px] text-[var(--color-textSecondary)]">{filtered.length} attrs</span>
      </div>
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4 max-h-[65vh] overflow-y-auto">
        <table className="w-full text-[10px]">
          <thead>
            <tr className="text-[var(--color-textSecondary)] border-b border-[var(--color-border)]">
          ...
          <tbody>
            {filtered.map((a) => (
              <tr key={a.name} className="border-b border-[var(--color-border)] last:border-0">
                ...
                <td className="py-1 text-[var(--color-textSecondary)]">{a.attributeType ?? "—"}</td>
    ...
    <div className="flex-1 overflow-y-auto p-4">
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
        ...
        {mgr.certificates.length === 0 ? (
          <p className="text-[10px] text-[var(--color-textSecondary)]">No certificates found</p>
        ) : (
          <div className="space-y-3">
            {mgr.certificates.map((c) => (
              <div key={c.id} className="border border-[var(--color-border)] rounded-lg p-3 space-y-1">
                <div className="text-[10px]"><span className="text-[var(--color-textSecondary)]">Subject:</span> <span className="text-[var(--color-text)]">{c.subject}</span></div>
                <div className="text-[10px]"><span className="text-[var(--color-textSecondary)]">Issuer:</span> <span className="text-[var(--color-text)]">{c.issuer}</span></div>
                <div className="text-[10px]"><span className="text-[var(--color-textSecondary)]">Valid:</span> <span className="text-[var(--color-text)]">{c.validFrom} → {c.validTo}</span></div>
                <div className="text-[10px]"><span className="text-[var(--color-textSecondary)]">Serial:</span> <span className="text-[var(--color-text)]">{c.serialNumber}</span></div>
              </div>
            ))}
          </div>
        )}
...
export const HealthView: React.FC<SubProps> = ({ mgr }) => {
  ...
  if (!hr) {
    return <div className="flex items-center justify-center flex-1 text-xs text-[var(--color-textSecondary)]">{t("idrac.no_data", "No data available")}</div>;
  }
        ...
        <div className="grid grid-cols-3 gap-3">
          {components.map(([label, health]) => (
            <div key={label} className="flex items-center gap-2 p-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)]">
              ...
              <span className="text-[10px] text-[var(--color-textSecondary)]">{label}</span>
      ...
      {pt && (
        <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Current</p><p className="text-sm font-semibold text-[var(--color-text)]">{pt.currentWatts} W</p></div>
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Peak</p><p className="text-sm font-semibold text-[var(--color-text)]">{pt.peakWatts} W</p></div>
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Min</p><p className="text-sm font-semibold text-[var(--color-text)]">{pt.minWatts} W</p></div>
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Average</p><p className="text-sm font-semibold text-[var(--color-text)]">{pt.averageWatts} W</p></div>
          </div>
          {pt.history.length > 0 && (
            <div className="mt-3 text-[10px] text-[var(--color-textSecondary)]">{pt.history.length} data points over {pt.timeWindowMinutes} min</div>
          )}
        </div>
      )}
      {tt && (
        <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Inlet</p><p className="text-sm font-semibold text-[var(--color-text)]">{tt.inletTempCelsius ?? "N/A"} °C</p></div>
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Exhaust</p><p className="text-sm font-semibold text-[var(--color-text)]">{tt.exhaustTempCelsius ?? "N/A"} °C</p></div>
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Peak Inlet</p><p className="text-sm font-semibold text-[var(--color-text)]">{tt.peakInletCelsius ?? "N/A"} °C</p></div>
            <div><p className="text-[10px] text-[var(--color-textSecondary)]">Avg Inlet</p><p className="text-sm font-semibold text-[var(--color-text)]">{tt.averageInletCelsius ?? "N/A"} °C</p></div>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surfaceHover)] p-4">
        ...
        <div className="flex gap-2 mb-3">
          <input
            className="flex-1 px-3 py-2 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] text-xs text-[var(--color-text)] font-mono focus:outline-none focus:ring-1 focus:ring-primary/50"
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg bg-warning hover:bg-warning/90 text-[var(--color-text)] text-xs font-medium transition-colors disabled:opacity-50 flex items-center gap-1"
          >
        ...
        {mgr.racadmOutput && (
          <div className="rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] p-3">
            <div className="flex items-center gap-2 mb-2 text-[10px]">
              <span className="text-[var(--color-textSecondary)]">$ {mgr.racadmOutput.command}</span>
