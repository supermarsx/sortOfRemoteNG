...
export const RecordingManager: React.FC<RecordingManagerProps> = ({
  ...
  // ---- Stats ----
  const sshTotalDuration = sshRecordings.reduce(
    (s, r) => s + r.recording.metadata.duration_ms,
    0,
  );
        ...
        {activeTab === "ssh" && (
          <>
            <span className="flex items-center gap-1">
              <HardDrive size={10} /> {sshRecordings.length} recording
              {sshRecordings.length !== 1 ? "s" : ""}
            </span>
            <span className="flex items-center gap-1">
              <Clock size={10} /> {formatDuration(sshTotalDuration)} total
            </span>
          </>
        )}
        {activeTab === "rdp" && (
          <>
            <span className="flex items-center gap-1">
              <Film size={10} /> {rdpRecordings.length} recording
              {rdpRecordings.length !== 1 ? "s" : ""}
            </span>
            <span className="flex items-center gap-1">
              <Clock size={10} /> {formatDuration(rdpTotalDuration)} total
            </span>
            <span className="flex items-center gap-1">
              <HardDrive size={10} /> {formatBytes(rdpTotalSize)}
            </span>
          </>
        )}
        {activeTab === "web" && (
          <>
            <span className="flex items-center gap-1">
              <Globe size={10} /> {webRecordings.length} recording
              {webRecordings.length !== 1 ? "s" : ""}
            </span>
          </>
        )}
        {activeTab === "webVideo" && (
          <>
            <span className="flex items-center gap-1">
              <Film size={10} /> {webVideoRecordings.length} recording
              {webVideoRecordings.length !== 1 ? "s" : ""}
            </span>
            {webVideoRecordings.length > 0 && (
              <span className="flex items-center gap-1">
                <HardDrive size={10} />{" "}
                {formatBytes(
                  webVideoRecordings.reduce((s, r) => s + r.sizeBytes, 0),
                )}
              </span>
            )}
          </>
        ...
        {activeTab === "rdp" &&
          (filteredRdp.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-500">
              ...
              {!searchQuery && (
                <p className="text-xs mt-1">
                  Enable auto-save in Recording settings, or save from the RDP
                  toolbar
                </p>
              )}
            </div>
