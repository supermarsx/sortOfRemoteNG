...
/**
 * Helper prefix check: detects legacy bcrypt hashes ($2a$/$2b$/$2x$/$2y$).
 * New hashes produced via the backend are Argon2id (PHC format,
 * `$argon2id$...`); legacy bcrypt hashes from older stores continue to be
 * verifiable via the backend's dispatch logic.
 */
function isBcryptHash(h: string): boolean {
  return /^\$2[abxy]\$/.test(h);
}
...
export class AuthService {
  ...
  /**
   * Adds a new user. Passwords are hashed by the Rust backend using Argon2id
   * (OWASP params) via the `auth_hash_password` command.
   */
  async addUser(username: string, password: string): Promise<void> {
    if (this.useTauri) {
      ...
      await this.ready();
      const hash = await invoke<string>('auth_hash_password', { password });
   ...
   * username. Uses the Rust backend (`auth_verify_password`), which accepts
   * both Argon2id (current) and bcrypt (legacy) hashes. On a successful
   * verification against a legacy bcrypt hash the entry is opportunistically
   * rehashed with Argon2id and persisted.
   */
  async verifyUser(username: string, password: string): Promise<boolean> {
    if (this.useTauri) {
      ...
      const ok = await invoke<boolean>('auth_verify_password', {
        password,
        hash,
      });
      if (ok && isBcryptHash(hash)) {
        try {
          const newHash = await invoke<string>('auth_hash_password', { password });
          this.users[username] = newHash;
          await this.persist();
        } catch (error) {
          // Rehash is opportunistic; log but don't fail the login.
          console.warn("Failed to opportunistically rehash password", error);
        }
      }
      return ok;
    }
  ...
  async updatePassword(username: string, newPassword: string): Promise<boolean> {
    if (this.useTauri) {
      ...
      const hash = await invoke<string>('auth_hash_password', { password: newPassword });
      this.users[username] = hash;
      try {
        await this.persist();
      } catch (error) {
