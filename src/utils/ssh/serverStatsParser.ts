...
export function parseServerStatsOutput(
  ...
  // Extract which detection method worked for each section
  const detectedMethods: Record<string, string> = {};
  const extractMethod = (section: string, key: string) => {
    const match = section.match(/^method:(.+)$/m);
    if (match) detectedMethods[key] = match[1];
  };
  extractMethod(cpuSection, 'cpu');
  extractMethod(memSection, 'memory');
  extractMethod(diskSection, 'disk');
  // firewall backend is already captured
  if (firewall.backend !== 'none') detectedMethods['firewall'] = firewall.backend;
  // port tool
  const portToolMatch = portSection.match(/^port_tool:(.+)$/m);
  if (portToolMatch) detectedMethods['ports'] = portToolMatch[1];
