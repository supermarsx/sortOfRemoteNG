...
} from "../../components/rdp/rdpRenderers";
...
export function useRDPConnection({
  ...
  const initializeRDPConnection = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (existing) {
          debugLog(
            `Re-attaching to existing session ${existing.id} for ${conn.id}`,
          );
          ...
          setStatusMessage(
            `Re-attached (${sessionInfo.desktop_width}x${sessionInfo.desktop_height})`,
          );
          return;
        }
      } catch {
      ...
      debugLog(
        `Attempting RDP connection to ${connectionDetails.host}:${connectionDetails.port}`,
      );
  ...
  const cleanup = useCallback(async () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps, react/exhaustive-deps -- reads connection from ref
  }, []);
