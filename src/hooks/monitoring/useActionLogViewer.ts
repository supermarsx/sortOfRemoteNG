...
export function useActionLogViewer(isOpen: boolean) {
  ...
  const filterLogs = useCallback(() => {
    ...
    if (dateFilter !== "all") {
      ...
      switch (dateFilter) {
        case "today":
          filtered = filtered.filter((log) => new Date(log.timestamp) >= startOfToday);
          break;
        case "yesterday": {
          ...
          filtered = filtered.filter(
            (log) =>
              new Date(log.timestamp) >= startOfYesterday && new Date(log.timestamp) < startOfToday,
          );
          break;
        }
        case "week": {
  ...
  const exportLogs = () => {
    try {
      const csvContent = [
        ...
        ...filteredLogs.map(
          (log) =>
            `"${log.timestamp}","${log.level}","${log.action}","${log.connectionName || ""}","${log.details.replace(/"/g, '""')}","${log.duration || ""}"`,
        ),
      ].join("\n");
