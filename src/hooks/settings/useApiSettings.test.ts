...
import type {
  ApiSecretStatusResult,
  ApiServerStatusResult,
} from "./useApiSettings";
...
const SECURE_SECRETS: ApiSecretStatusResult = {
  vaultAvailable: true,
...
function defaultInvoke(cmd: string): Promise<unknown> {
  switch (cmd) {
    ...
    case "api_secret_status":
      return Promise.resolve(SECURE_SECRETS);
    default:
      return Promise.resolve(undefined);
  }
}
...
describe("useApiSettings — real server control wiring", () => {
  ...
  it("generateApiKey rotates only in native secure storage", async () => {
    invokeMock.mockImplementation((cmd: string) =>
      cmd === "api_regenerate_key"
        ? Promise.resolve(SECURE_SECRETS)
        : defaultInvoke(cmd),
    );
