...
// backend. Pairs 1:1 with the `vault_*` commands registered by
// `sorng-commands-ops-identity` (see its `commands.json`).
...
export const vaultApi = {
  ...
  getDashboard: (id: string) =>
    invoke<VaultDashboard>("vault_get_dashboard", { id }),

  // Sys
  sealStatus: (id: string) =>
    invoke<VaultSealStatus>("vault_seal_status", { id }),
  ...
  transitListKeys: (id: string) =>
    invoke<string[]>("vault_transit_list_keys", { id }),
  ...
  pkiIssueCert: (
    ...
    invoke<VaultCertificate>("vault_pki_issue_cert", {
      id,
      mount,
      role,
      params,
