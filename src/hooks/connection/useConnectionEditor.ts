...
const BUILT_IN_PROTOCOL_METADATA: Omit<ProtocolOption, "icon">[] = [
  {
    value: "rdp",
    label: "RDP",
    desc: "Remote Desktop",
...
export const PROTOCOL_OPTIONS: ProtocolOption[] =
  BUILT_IN_PROTOCOL_METADATA.map((option) => ({
    ...option,
    icon: getProtocolDefaultIcon(option.value),
  }));
...
export const INTEGRATION_PROTOCOL_OPTIONS: ProtocolOption[] =
  integrationRegistry.map((descriptor) => ({
    ...
    icon: getProtocolDefaultIcon(
      toIntegrationProtocol(descriptor.key),
      descriptor,
    ),
    color: "cyan",
  ...
  useEffect(() => {
    ...
    if (connection) {
      ...
      const resolved = {
        ...
        sshKeepAliveInterval: connection.sshKeepAliveInterval ?? 30,
