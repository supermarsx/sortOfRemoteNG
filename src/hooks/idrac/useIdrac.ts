...
export function useIdrac(): UseIdracReturn {
  ...
  const getSystemInfo = useCallback(
    () => wrap(() => invoke<SystemInfo>("idrac_get_system_info")),
    [wrap],
  );
  const getIdracInfo = useCallback(
    () => wrap(() => invoke<IdracInfo>("idrac_get_idrac_info")),
    [wrap],
  );
  const setAssetTag = useCallback(
    (tag: string) => wrap(() => invoke<void>("idrac_set_asset_tag", { tag })),
    [wrap],
  );
  const setIndicatorLed = useCallback(
    (state: string) =>
      wrap(() => invoke<void>("idrac_set_indicator_led", { state })),
    [wrap],
  );
  ...
  const powerAction = useCallback(
    (action: PowerAction) =>
      wrap(() => invoke<string | null>("idrac_power_action", { action })),
    [wrap],
  );
  const getPowerState = useCallback(
    () => wrap(() => invoke<string>("idrac_get_power_state")),
    [wrap],
  );
  const getPowerMetrics = useCallback(
    () => wrap(() => invoke<PowerMetrics>("idrac_get_power_metrics")),
    [wrap],
  );
  const listPowerSupplies = useCallback(
    () => wrap(() => invoke<PowerSupply[]>("idrac_list_power_supplies")),
    [wrap],
  );
  const setPowerCap = useCallback(
    (watts: number, enabled: boolean) =>
      wrap(() => invoke<void>("idrac_set_power_cap", { watts, enabled })),
    [wrap],
  );
  ...
  const getThermalData = useCallback(
    () => wrap(() => invoke<ThermalData>("idrac_get_thermal_data")),
    [wrap],
  );
  const getThermalSummary = useCallback(
    () => wrap(() => invoke<ThermalSummary>("idrac_get_thermal_summary")),
    [wrap],
  );
  const setFanOffset = useCallback(
    (offset: number) =>
      wrap(() => invoke<void>("idrac_set_fan_offset", { offset })),
    [wrap],
  );

  // ── Hardware ─────────────────────────────────────

  const listProcessors = useCallback(
    () => wrap(() => invoke<Processor[]>("idrac_list_processors")),
    [wrap],
  );
  const listMemory = useCallback(
    () => wrap(() => invoke<MemoryDimm[]>("idrac_list_memory")),
    [wrap],
  );
  const listPcieDevices = useCallback(
    () => wrap(() => invoke<PcieDevice[]>("idrac_list_pcie_devices")),
    [wrap],
  );
  const getTotalMemory = useCallback(
    () => wrap(() => invoke<number>("idrac_get_total_memory")),
    [wrap],
  );
  const getProcessorCount = useCallback(
    () => wrap(() => invoke<number>("idrac_get_processor_count")),
    [wrap],
  );

  // ── Storage ──────────────────────────────────────

  const listStorageControllers = useCallback(
    () =>
      wrap(() => invoke<StorageController[]>("idrac_list_storage_controllers")),
    [wrap],
  );
  const listVirtualDisks = useCallback(
    (controllerId?: string) =>
      wrap(() =>
        invoke<VirtualDisk[]>("idrac_list_virtual_disks", {
          controllerId: controllerId ?? null,
        }),
      ),
    [wrap],
  );
  const listPhysicalDisks = useCallback(
    (controllerId?: string) =>
      wrap(() =>
        invoke<PhysicalDisk[]>("idrac_list_physical_disks", {
          controllerId: controllerId ?? null,
        }),
      ),
    [wrap],
  );
  const listEnclosures = useCallback(
    () => wrap(() => invoke<StorageEnclosure[]>("idrac_list_enclosures")),
    [wrap],
  );
  const createVirtualDisk = useCallback(
    (params: CreateVirtualDiskParams) =>
      wrap(() =>
        invoke<string | null>("idrac_create_virtual_disk", { params }),
      ),
    [wrap],
  );
  const deleteVirtualDisk = useCallback(
    (id: string) =>
      wrap(() => invoke<void>("idrac_delete_virtual_disk", { id })),
    [wrap],
  );
  const assignHotspare = useCallback(
    (diskId: string, controllerId: string, vdId?: string) =>
      wrap(() =>
        invoke<void>("idrac_assign_hotspare", {
          diskId,
          controllerId,
          virtualDiskId: vdId ?? null,
        }),
      ),
    [wrap],
  );
  const initializeVirtualDisk = useCallback(
    (id: string, initType?: string) =>
      wrap(() =>
        invoke<string | null>("idrac_initialize_virtual_disk", {
          id,
  ...
  const listNetworkAdapters = useCallback(
    () => wrap(() => invoke<NetworkAdapter[]>("idrac_list_network_adapters")),
    [wrap],
  );
  const listNetworkPorts = useCallback(
    (adapterId: string) =>
      wrap(() =>
        invoke<NetworkPort[]>("idrac_list_network_ports", { adapterId }),
      ),
    [wrap],
  );
  const getNetworkConfig = useCallback(
    () => wrap(() => invoke<IdracNetworkConfig>("idrac_get_network_config")),
    [wrap],
  );
  const updateNetworkConfig = useCallback(
    (cfg: Partial<IdracNetworkConfig>) =>
      wrap(() => invoke<void>("idrac_update_network_config", { config: cfg })),
    [wrap],
  );

  // ── Firmware ─────────────────────────────────────

  const listFirmware = useCallback(
    () => wrap(() => invoke<FirmwareInventory[]>("idrac_list_firmware")),
    [wrap],
  );
  const updateFirmware = useCallback(
    (params: FirmwareUpdateParams) =>
      wrap(() => invoke<string | null>("idrac_update_firmware", { params })),
    [wrap],
  );
  const getComponentVersion = useCallback(
    (componentId: string) =>
      wrap(() =>
        invoke<string | null>("idrac_get_component_version", { componentId }),
      ),
    [wrap],
  );
  ...
  const listJobs = useCallback(
    () => wrap(() => invoke<LifecycleJob[]>("idrac_list_jobs")),
    [wrap],
  );
  const getJob = useCallback(
    (jobId: string) =>
      wrap(() => invoke<LifecycleJob>("idrac_get_job", { jobId })),
    [wrap],
  );
  const deleteJob = useCallback(
    (jobId: string) => wrap(() => invoke<void>("idrac_delete_job", { jobId })),
    [wrap],
  );
  const purgeJobQueue = useCallback(
    () => wrap(() => invoke<void>("idrac_purge_job_queue")),
    [wrap],
  );
  const exportScp = useCallback(
    (params: ScpExportParams) =>
      wrap(() => invoke<string>("idrac_export_scp", { params })),
    [wrap],
  );
  const importScp = useCallback(
    (params: ScpImportParams) =>
      wrap(() => invoke<string>("idrac_import_scp", { params })),
    [wrap],
  );
  const getLcStatus = useCallback(
    () => wrap(() => invoke<string>("idrac_get_lc_status")),
    [wrap],
  );
  const waitForJob = useCallback(
    (jobId: string, timeoutSecs?: number) =>
      wrap(() =>
        invoke<LifecycleJob>("idrac_wait_for_job", {
          jobId,
          timeoutSecs: timeoutSecs ?? 600,
        }),
      ),
    [wrap],
  );
  ...
  const listVirtualMedia = useCallback(
    () => wrap(() => invoke<VirtualMediaStatus[]>("idrac_list_virtual_media")),
    [wrap],
  );
  const mountVirtualMedia = useCallback(
    (params: VirtualMediaMountParams) =>
      wrap(() => invoke<void>("idrac_mount_virtual_media", { params })),
    [wrap],
  );
  const unmountVirtualMedia = useCallback(
    (id: string) =>
      wrap(() => invoke<void>("idrac_unmount_virtual_media", { id })),
    [wrap],
  );
  const bootFromVirtualCd = useCallback(
    () => wrap(() => invoke<void>("idrac_boot_from_virtual_cd")),
    [wrap],
  );
  ...
  const getConsoleInfo = useCallback(
    () => wrap(() => invoke<ConsoleInfo>("idrac_get_console_info")),
    [wrap],
  );
  const setConsoleEnabled = useCallback(
    (enabled: boolean) =>
      wrap(() => invoke<void>("idrac_set_console_enabled", { enabled })),
    [wrap],
  );
  const setConsoleType = useCallback(
    (consoleType: string) =>
      wrap(() => invoke<void>("idrac_set_console_type", { consoleType })),
    [wrap],
  );
  const setVncEnabled = useCallback(
    (enabled: boolean) =>
      wrap(() => invoke<void>("idrac_set_vnc_enabled", { enabled })),
    [wrap],
  );
  const setVncPassword = useCallback(
    (password: string) =>
      wrap(() => invoke<void>("idrac_set_vnc_password", { password })),
    [wrap],
  );

  // ── Event Log ────────────────────────────────────

  const getSelEntries = useCallback(
    (limit?: number) =>
      wrap(() =>
        invoke<SelEntry[]>("idrac_get_sel_entries", { limit: limit ?? 100 }),
      ),
    [wrap],
  );
  const getLcLogEntries = useCallback(
    (limit?: number) =>
      wrap(() =>
        invoke<LcLogEntry[]>("idrac_get_lc_log_entries", {
          limit: limit ?? 100,
        }),
      ),
    [wrap],
  ...
  const clearSel = useCallback(
    () => wrap(() => invoke<void>("idrac_clear_sel")),
    [wrap],
  );
  const clearLcLog = useCallback(
    () => wrap(() => invoke<void>("idrac_clear_lc_log")),
    [wrap],
  );
  ...
  const listUsers = useCallback(
    () => wrap(() => invoke<IdracUser[]>("idrac_list_users")),
    [wrap],
  );
  const createOrUpdateUser = useCallback(
    (slotId: string, params: IdracUserParams) =>
      wrap(() =>
        invoke<void>("idrac_create_or_update_user", { slotId, params }),
      ),
    [wrap],
  );
  const deleteUser = useCallback(
    (slotId: string) =>
      wrap(() => invoke<void>("idrac_delete_user", { slotId })),
    [wrap],
  );
  const unlockUser = useCallback(
    (slotId: string) =>
      wrap(() => invoke<void>("idrac_unlock_user", { slotId })),
    [wrap],
  );
  const changeUserPassword = useCallback(
    (slotId: string, password: string) =>
      wrap(() =>
        invoke<void>("idrac_change_user_password", { slotId, password }),
      ),
    [wrap],
  );
  const getLdapConfig = useCallback(
    () => wrap(() => invoke<LdapConfig>("idrac_get_ldap_config")),
    [wrap],
  );
  const getAdConfig = useCallback(
    () => wrap(() => invoke<ActiveDirectoryConfig>("idrac_get_ad_config")),
    [wrap],
  );
  ...
  const getBiosAttributes = useCallback(
    () => wrap(() => invoke<BiosAttribute[]>("idrac_get_bios_attributes")),
    [wrap],
  );
  const getBiosAttribute = useCallback(
    (name: string) =>
      wrap(() =>
        invoke<BiosAttribute | null>("idrac_get_bios_attribute", { name }),
      ),
    [wrap],
  );
  const setBiosAttributes = useCallback(
    (attrs: Record<string, unknown>) =>
      wrap(() =>
        invoke<string | null>("idrac_set_bios_attributes", {
          attributes: attrs,
        }),
      ),
    [wrap],
  );
  const getBootOrder = useCallback(
    () => wrap(() => invoke<BootConfig>("idrac_get_boot_order")),
    [wrap],
  );
  const setBootOrder = useCallback(
    (order: string[]) =>
      wrap(() => invoke<void>("idrac_set_boot_order", { order })),
    [wrap],
  );
  const setBootOnce = useCallback(
    (target: string) =>
      wrap(() => invoke<void>("idrac_set_boot_once", { target })),
    [wrap],
  );
  const setBootMode = useCallback(
    (mode: string) => wrap(() => invoke<void>("idrac_set_boot_mode", { mode })),
    [wrap],
  );
  ...
  const listCertificates = useCallback(
    () => wrap(() => invoke<IdracCertificate[]>("idrac_list_certificates")),
    [wrap],
  );
  const generateCsr = useCallback(
    (params: CsrParams) =>
      wrap(() => invoke<string>("idrac_generate_csr", { params })),
    [wrap],
  );
  const importCertificate = useCallback(
    (certType: string, certData: string) =>
      wrap(() =>
        invoke<void>("idrac_import_certificate", { certType, certData }),
      ),
    [wrap],
  );
  const deleteCertificate = useCallback(
    (certId: string) =>
      wrap(() => invoke<void>("idrac_delete_certificate", { certId })),
    [wrap],
  );
  const replaceSslCertificate = useCallback(
    (certData: string, keyData: string) =>
      wrap(() =>
        invoke<void>("idrac_replace_ssl_certificate", { certData, keyData }),
      ),
    [wrap],
  );
  ...
  const getHealthRollup = useCallback(
    () => wrap(() => invoke<ServerHealthRollup>("idrac_get_health_rollup")),
    [wrap],
  );
  const getComponentHealth = useCallback(
    () => wrap(() => invoke<ComponentHealth[]>("idrac_get_component_health")),
    [wrap],
  );
  const isHealthy = useCallback(
    () => wrap(() => invoke<boolean>("idrac_is_healthy")),
    [wrap],
  );

  // ── Telemetry ────────────────────────────────────

  const getPowerTelemetry = useCallback(
    () => wrap(() => invoke<PowerTelemetry>("idrac_get_power_telemetry")),
    [wrap],
  );
  const getThermalTelemetry = useCallback(
    () => wrap(() => invoke<ThermalTelemetry>("idrac_get_thermal_telemetry")),
    [wrap],
  );
  const listTelemetryReports = useCallback(
    () => wrap(() => invoke<TelemetryReport[]>("idrac_list_telemetry_reports")),
    [wrap],
  );
  const getTelemetryReport = useCallback(
    (metricId: string) =>
      wrap(() =>
        invoke<TelemetryReport>("idrac_get_telemetry_report", { metricId }),
      ),
    [wrap],
  );
  ...
  const racadmExecute = useCallback(
    (command: string) =>
      wrap(() => invoke<RacadmResult>("idrac_racadm_execute", { command })),
    [wrap],
  );
  const resetIdrac = useCallback(
    () => wrap(() => invoke<void>("idrac_reset")),
    [wrap],
  );
  const getAttribute = useCallback(
    (group: string, name: string) =>
      wrap(() => invoke<string>("idrac_get_attribute", { group, name })),
    [wrap],
  );
  const setAttribute = useCallback(
    (group: string, name: string, value: string) =>
      wrap(() => invoke<void>("idrac_set_attribute", { group, name, value })),
    [wrap],
  );
  ...
  const getDashboard = useCallback(
    () => wrap(() => invoke<IdracDashboard>("idrac_get_dashboard")),
    [wrap],
  );
