...
export function useProxmoxManager(isOpen: boolean) {
  ...
  const [selectedVmType, setSelectedVmType] = useState<"qemu" | "lxc" | null>(
    null,
  );
  ...
  const [firewallOptions, setFirewallOptions] =
    useState<FirewallOptions | null>(null);
    ...
    try {
      ...
      if (nodeList.length > 0) {
        ...
        const [vms, cts, stor, cluster, resources] = await Promise.all([
          safe(() =>
            invoke<QemuVmSummary[]>("proxmox_list_qemu_vms", {
              node: targetNode,
            }),
          ),
          safe(() =>
            invoke<LxcSummary[]>("proxmox_list_lxc_containers", {
              node: targetNode,
            }),
          ),
          safe(() =>
            invoke<StorageSummary[]>("proxmox_list_storage", {
              node: targetNode,
            }),
          ),
          safe(() => invoke<ClusterStatus[]>("proxmox_get_cluster_status")),
          safe(() =>
            invoke<ClusterResource[]>("proxmox_list_cluster_resources"),
          ),
        ]);
        if (!mountedRef.current) return;
  ...
  const refreshNodeVms = useCallback(
    async (node: string) => {
      setRefreshing(true);
      try {
        const [vms, cts] = await Promise.all([
          safe(() =>
            invoke<QemuVmSummary[]>("proxmox_list_qemu_vms", { node }),
          ),
          safe(() =>
            invoke<LxcSummary[]>("proxmox_list_lxc_containers", { node }),
          ),
        ]);
        if (mountedRef.current) {
          if (vms) setQemuVms(vms);
  ...
  const refreshTasks = useCallback(
    async (node: string) => {
      ...
      try {
        const t = await safe(() =>
          invoke<TaskSummary[]>("proxmox_list_tasks", { node, limit: 50 }),
        );
  ...
  const refreshStorage = useCallback(
    async (node: string) => {
      ...
      try {
        const s = await safe(() =>
          invoke<StorageSummary[]>("proxmox_list_storage", { node }),
        );
  ...
  const refreshFirewall = useCallback(async () => {
    ...
    try {
      const [rules, opts] = await Promise.all([
        safe(() =>
          invoke<FirewallRule[]>("proxmox_list_cluster_firewall_rules"),
        ),
        safe(() =>
          invoke<FirewallOptions>("proxmox_get_cluster_firewall_options"),
        ),
      ]);
      if (mountedRef.current) {
  ...
  const refreshBackups = useCallback(async () => {
    ...
    try {
      const jobs = await safe(() =>
        invoke<BackupJobConfig[]>("proxmox_list_backup_jobs"),
      );
  ...
  const refreshNetwork = useCallback(
    async (node: string) => {
      setRefreshing(true);
      try {
        const ifaces = await safe(() =>
          invoke<NetworkInterface[]>("proxmox_list_network_interfaces", {
            node,
          }),
        );
  ...
  const refreshSnapshots = useCallback(
    async (node: string, vmid: number, vmType: "qemu" | "lxc") => {
      setRefreshing(true);
      try {
        const cmd =
          vmType === "qemu"
        ...
        const snaps = await safe(() =>
          invoke<SnapshotSummary[]>(cmd, { node, vmid }),
        );
  ...
  const vmAction = useCallback(
    async (node: string, vmid: number, action: string) => {
      ...
      setDataError(null);
      try {
        await invoke<string | null>(`proxmox_${action}_qemu_vm`, {
          node,
          vmid,
        });
  ...
  const lxcAction = useCallback(
    async (node: string, vmid: number, action: string) => {
      ...
      setDataError(null);
      try {
        await invoke<string | null>(`proxmox_${action}_lxc_container`, {
          node,
          vmid,
        });
        await refreshNodeVms(node);
  ...
  const createQemuVm = useCallback(
    async (node: string, params: QemuCreateParams) => {
      setLoading(true);
      setDataError(null);
      try {
        await invoke<string | null>("proxmox_create_qemu_vm", { node, params });
        await Promise.all([refreshNodeVms(node), refreshTasks(node)]);
      } catch (e) {
        const msg =
          typeof e === "string" ? e : ((e as Error).message ?? String(e));
        if (mountedRef.current) setDataError(msg);
  ...
  const createLxcContainer = useCallback(
    async (node: string, params: LxcCreateParams) => {
      setLoading(true);
      setDataError(null);
      try {
        await invoke<string | null>("proxmox_create_lxc_container", {
          node,
          params,
        });
        await Promise.all([refreshNodeVms(node), refreshTasks(node)]);
  ...
  const cloneQemuVm = useCallback(
    async (node: string, vmid: number, params: QemuCloneParams) => {
      setLoading(true);
      setDataError(null);
      try {
        await invoke<string | null>("proxmox_clone_qemu_vm", {
          node,
          vmid,
          params,
        });
  ...
  const cloneLxcContainer = useCallback(
    async (node: string, vmid: number, params: LxcCloneParams) => {
      setLoading(true);
      setDataError(null);
      try {
        await invoke<string | null>("proxmox_clone_lxc_container", {
          node,
          vmid,
          params,
        });
  ...
  const migrateQemuVm = useCallback(
    async (node: string, vmid: number, params: QemuMigrateParams) => {
      setLoading(true);
      setDataError(null);
      try {
        await invoke<string | null>("proxmox_migrate_qemu_vm", {
          node,
          vmid,
          params,
        });
        await Promise.all([
          refreshNodeVms(node),
          refreshTasks(node),
          params.target && params.target !== node
            ? refreshNodeVms(params.target)
            : Promise.resolve(),
        ]);
      } catch (e) {
        const msg =
  ...
  const migrateLxcContainer = useCallback(
    async (node: string, vmid: number, params: LxcMigrateParams) => {
      setLoading(true);
      setDataError(null);
      try {
        await invoke<string | null>("proxmox_migrate_lxc_container", {
          node,
          vmid,
          params,
        });
  ...
  const openVncConsole = useCallback(
    async (node: string, vmid: number, vmType: "qemu" | "lxc") => {
      const cmd =
        vmType === "qemu" ? "proxmox_qemu_vnc_proxy" : "proxmox_lxc_vnc_proxy";
      return invoke<VncTicket>(cmd, { node, vmid });
    },
    [],
  );

  const openTermConsole = useCallback(
    async (node: string, vmid: number, vmType: "qemu" | "lxc") => {
      const cmd =
        vmType === "qemu" ? "proxmox_qemu_termproxy" : "proxmox_lxc_termproxy";
  ...
  const loadNodeMetrics = useCallback(
    async (node: string, timeframe = "hour") => {
      const data = await safe(() =>
        invoke<RrdDataPoint[]>("proxmox_node_rrd", { node, timeframe }),
      );
  ...
  const loadVmMetrics = useCallback(
    async (
      node: string,
      vmid: number,
      vmType: "qemu" | "lxc",
      ...
      const data = await safe(() =>
        invoke<RrdDataPoint[]>(cmd, { node, vmid, timeframe }),
      );
      if (mountedRef.current && data) setRrdData(data);
    },
    [safe],
  ...
  const loadTaskDetail = useCallback(
    async (node: string, upid: string) => {
      const [status, log] = await Promise.all([
        safe(() =>
          invoke<TaskStatus>("proxmox_get_task_status", { node, upid }),
        ),
        safe(() =>
          invoke<TaskLogLine[]>("proxmox_get_task_log", { node, upid }),
        ),
      ]);
      if (mountedRef.current) {
  ...
  const loadVmConfig = useCallback(
    async (node: string, vmid: number) => {
      const cfg = await safe(() =>
        invoke<QemuConfig>("proxmox_get_qemu_config", { node, vmid }),
      );
  ...
  const loadLxcConfig = useCallback(
    async (node: string, vmid: number) => {
      const cfg = await safe(() =>
        invoke<LxcConfig>("proxmox_get_lxc_config", { node, vmid }),
      );
  ...
  const openCreateVmDialog = useCallback(
    (node?: string | null) => {
  ...
  const openCreateLxcDialog = useCallback(
    (node?: string | null) => {
      setDialogNode(node ?? selectedNode);
      setDialogVmId(null);
      setDialogVmType("lxc");
  ...
  const openCloneDialog = useCallback(
    (node: string, vmid: number, vmType: "qemu" | "lxc") => {
      setDialogNode(node);
      setDialogVmId(vmid);
  ...
  return {
    ...
    setShowCreateVm,
    showCreateLxcDialog,
    setShowCreateLxcDialog,
    showCloneDialog,
    setShowCloneDialog,
    showMigrateDialog,
    setShowMigrateDialog,
    showSnapshotDialog,
    setShowSnapshotDialog,
    dialogNode,
    dialogVmId,
    dialogVmType,
    openCreateVmDialog,
