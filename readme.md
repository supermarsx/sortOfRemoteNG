...
Public bundles can be published without operating-system signing certificates. Windows SmartScreen or macOS Gatekeeper may therefore show an unknown-publisher warning on first launch. Every release publishes `latest.json` so installed apps can discover the current version. Automatic in-app installation remains a separate, cryptographically signed channel: without the protected Tauri updater key, the feed is explicitly discovery-only and links to the matching public artifacts for manual installation.
...
The release workflow builds Windows x64 and ARM64 installers plus an architecture-matched portable ZIP for each, Linux x64 and ARM64 AppImage, Debian, RPM, and Flatpak bundles, macOS Intel bundles, and macOS Apple Silicon bundles. OS signing certificates are optional; the Tauri updater private key is required only for signed updater artifacts and automatic installation, while every successfully promoted release includes `latest.json`. An unsigned release includes discovery-only metadata, but signing-key continuity is required after a signed updater release because CI refuses to replace a usable signed feed with an unsigned one. Signed in-app self-update is limited to the feed-compatible AppImage, NSIS, MSI, and macOS app-bundle formats. Debian, RPM, Flatpak, portable, and unsigned-release users download and reinstall or replace the newer matching GitHub release asset. See the [release guide](docs/releases.md) for publication and recovery details and the [updater setup](docs/release/updater-setup.md) for signature and feed requirements.
