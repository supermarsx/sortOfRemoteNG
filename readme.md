...
- **VNC** opens a native TCP/RFB session with framebuffer rendering, keyboard and pointer input, clipboard transfer, reconnect, and explicit opt-in exceptions for legacy security modes. VeNCrypt/TLS and Tight security negotiation are not yet supported.
...
Public bundles can be published without operating-system signing certificates. Windows SmartScreen or macOS Gatekeeper may therefore show an unknown-publisher warning on first launch. Every release publishes `latest.json` so installed apps can discover the current version. Signed entries use the verified automatic updater path and must pass the Ed25519/minisign check against the key pinned in the app. If the fixed official public feed has no updater signature, a compatible installation can run its exact canonical GitHub release artifact only after showing the unsigned-update warning and receiving an explicit acknowledgement for that attempt; the artifact is never trusted automatically. Windows hands the installer to the operating system and Linux replaces the AppImage. On macOS, the action only downloads and opens the DMG, closes sortOfRemoteNG, and requires the user to drag the app to Applications and reopen it. The unsigned command always re-resolves that fixed public feed and ignores configured private endpoints. Private unsigned entries and artifacts remain non-executable manual-download links unless a future release defines an additional pinned trust policy.
...
- signed application updates require a valid Ed25519/minisign signature from the key pinned in the app; the narrowly scoped official-public unsigned path requires an explicit warning acknowledgement for every attempt and never executes a private-feed artifact.
...
The release workflow builds Windows x64 and ARM64 installers plus an architecture-matched portable ZIP for each, Linux x64 and ARM64 AppImage, Debian, RPM, and Flatpak bundles, macOS Intel bundles, and macOS Apple Silicon bundles. OS signing certificates are optional; the Tauri updater private key is required for signed updater artifacts and the verified automatic installation path, while every successfully promoted release includes `latest.json`. An unsigned artifact from the fixed official public feed is available only through the separate warning-gated action with explicit per-use acknowledgement; it is not trusted automatically. That action launches Windows installers, replaces Linux AppImages, and opens the macOS DMG for a clearly described manual drag-to-Applications step. Signing-key continuity is still required after a signed updater release because CI refuses to replace a usable signed feed with an unsigned one. In-app update formats are limited to AppImage, NSIS, MSI, and the corresponding macOS format. Debian, RPM, Flatpak, portable, private unsigned feed artifacts, and completion of an unsigned macOS DMG update are handled manually. See the [release guide](docs/releases.md) for publication and recovery details and the [updater setup](docs/release/updater-setup.md) for signature and feed requirements.
