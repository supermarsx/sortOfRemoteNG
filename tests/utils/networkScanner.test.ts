...
describe("NetworkScanner helper methods", () => {
  ...
  it("throws on malformed CIDR strings", async () => {
    await expect(
      scanner.generateIPRange("192.168.0.0").next(),
    ).rejects.toThrow();
    await expect(
      scanner.generateIPRange("192.168.0.0/abc").next(),
    ).rejects.toThrow();
  });

  it("throws when IP does not have four octets", async () => {
    await expect(
      scanner.generateIPRange("192.168.0/24").next(),
    ).rejects.toThrow();
  });

  it("throws when prefix is outside supported range", async () => {
    await expect(
      scanner.generateIPRange("192.168.0.0/23").next(),
    ).rejects.toThrow();
    await expect(
      scanner.generateIPRange("192.168.0.0/31").next(),
    ).rejects.toThrow();
    await expect(
      scanner.generateIPRange("2001:db8::/111").next(),
    ).rejects.toThrow();
  });
