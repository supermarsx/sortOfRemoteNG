...
describe("mergeRdpSettings", () => {
  ...
  describe("basic merge — compile-time defaults only (no globals, no per-connection)", () => {
    ...
    it("deviceRedirection matches compile-time defaults", () => {
      ...
      expect(result.deviceRedirection?.clipboardDirection).toBe(
        "bidirectional",
      );
      ...
      const result = mergeRdpSettings(
        {
          gateway: {
            enabled: true,
            hostname: "conn-gw.example.com",
            port: 8443,
          },
        },
        {
          gatewayEnabled: false,
          gatewayHostname: "global-gw.example.com",
          gatewayPort: 443,
        },
      );
  ...
  describe("display section", () => {
    ...
    it("per-connection display fields do not clobber unrelated fields from base", () => {
      const result = mergeRdpSettings({ display: { width: 800 } }, {});
      // Only width was set; everything else should come from compile-time default
      expect(result.display?.width).toBe(800);
      expect(result.display?.height).toBe(DEFAULT_RDP_SETTINGS.display?.height);
      expect(result.display?.colorDepth).toBe(
        DEFAULT_RDP_SETTINGS.display?.colorDepth,
      );
      expect(result.display?.resizeToWindow).toBe(
        DEFAULT_RDP_SETTINGS.display?.resizeToWindow,
      );
      expect(result.display?.smartSizing).toBe(
        DEFAULT_RDP_SETTINGS.display?.smartSizing,
      );
      expect(result.display?.desktopScaleFactor).toBe(
        DEFAULT_RDP_SETTINGS.display?.desktopScaleFactor,
      );
      expect(result.display?.lossyCompression).toBe(
        DEFAULT_RDP_SETTINGS.display?.lossyCompression,
      );
    });
  });
  ...
  describe("input section", () => {
    ...
    it("autoDetectLayout (mapped from autoDetectKeyboardLayout) inherits from global", () => {
      const result = mergeRdpSettings(undefined, {
        autoDetectKeyboardLayout: false,
      });
  ...
  describe("security section", () => {
    ...
    it("sspiPackageList uses || (falsy check) so empty string falls back to base", () => {
      ...
      expect(result.security?.sspiPackageList).toBe(
        DEFAULT_RDP_SETTINGS.security?.sspiPackageList,
      );
    });
  });
  ...
  describe("performance section", () => {
    ...
    describe('renderBackend "inherit" resolution', () => {
      ...
      it('resolves to compile-time default when per-connection is "inherit" and global is unset', () => {
        ...
        expect(result.performance?.renderBackend).toBe(
          DEFAULT_RDP_SETTINGS.performance?.renderBackend,
        );
      });
  ...
  describe("gateway section", () => {
    ...
    it("gatewayHostname uses || (falsy check) so empty string falls back to base", () => {
      ...
      expect(result.gateway?.hostname).toBe(
        DEFAULT_RDP_SETTINGS.gateway?.hostname,
      );
    });
  ...
  describe("advanced section", () => {
    ...
    it("clientName uses || (falsy check) so empty string falls back to base", () => {
      ...
      expect(result.advanced?.clientName).toBe(
        DEFAULT_RDP_SETTINGS.advanced?.clientName,
      );
    });
  ...
  describe("deviceRedirection section", () => {
    ...
    it("false values from global (falsy booleans) are preserved via ??", () => {
      ...
      expect(result.deviceRedirection?.clipboard).toBe(false);
      expect(result.deviceRedirection?.clipboardDirection).toBe(
        DEFAULT_RDP_SETTINGS.deviceRedirection?.clipboardDirection,
      );
      expect(result.deviceRedirection?.printers).toBe(false);
      expect(result.deviceRedirection?.printerOutputMode).toBe(
        DEFAULT_RDP_SETTINGS.deviceRedirection?.printerOutputMode,
      );
    });
  });
  ...
  describe("edge cases", () => {
    it("null-ish connSettings (undefined) produces compile-time defaults with empty globals", () => {
      const result = mergeRdpSettings(undefined, {});
      expect(result.display?.width).toBe(DEFAULT_RDP_SETTINGS.display?.width);
      expect(result.audio?.playbackMode).toBe(
        DEFAULT_RDP_SETTINGS.audio?.playbackMode,
      );
      expect(result.performance?.renderBackend).toBe(
        DEFAULT_RDP_SETTINGS.performance?.renderBackend,
      );
    ...
    it("empty connection settings object (no sections) uses globals and defaults", () => {
      const result = mergeRdpSettings(
        {},
        { defaultWidth: 800, audioPlaybackMode: "remote" },
      );
      expect(result.display?.width).toBe(800);
      expect(result.audio?.playbackMode).toBe("remote");
      // Other fields from compile-time defaults
      expect(result.input?.mouseMode).toBe(
        DEFAULT_RDP_SETTINGS.input?.mouseMode,
      );
    });
    ...
    it("empty globalDefaults object results in compile-time defaults everywhere", () => {
      ...
      expect(result.deviceRedirection?.clipboard).toBe(
        DEFAULT_RDP_SETTINGS.deviceRedirection?.clipboard,
      );
      expect(result.performance?.targetFps).toBe(
        DEFAULT_RDP_SETTINGS.performance?.targetFps,
      );
      expect(result.security?.useCredSsp).toBe(
        DEFAULT_RDP_SETTINGS.security?.useCredSsp,
      );
      expect(result.gateway?.enabled).toBe(
        DEFAULT_RDP_SETTINGS.gateway?.enabled,
      );
      expect(result.hyperv?.enhancedSessionMode).toBe(
        DEFAULT_RDP_SETTINGS.hyperv?.enhancedSessionMode,
      );
      expect(result.negotiation?.strategy).toBe(
        DEFAULT_RDP_SETTINGS.negotiation?.strategy,
      );
      expect(result.advanced?.clientName).toBe(
        DEFAULT_RDP_SETTINGS.advanced?.clientName,
      );
  ...
  describe("full three-layer inheritance chain: compile-time → global → per-connection", () => {
    ...
    it("fields not in per-conn fall through to global; fields not in global fall through to compile-time", () => {
      ...
      expect(result.audio?.recordingMode).toBe(
        DEFAULT_RDP_SETTINGS.audio?.recordingMode,
      ); // compile-time
      expect(result.input?.mouseMode).toBe(
        DEFAULT_RDP_SETTINGS.input?.mouseMode,
      ); // compile-time
