...
describe("classifyTabKind", () => {
  it.each([
    ...
    ["integration:netbox", "integration"],
  ...
  it("does not match colon-prefix on protocols that merely contain the substring", () => {
    ...
    expect(classifyTabKind({ protocol: "super-winmgmt:foo" })).toBe(
      "connection",
    );
  });
  ...
  it("is case sensitive — uppercase prefixes are NOT tools", () => {
    ...
    expect(classifyTabKind({ protocol: "Winmgmt:services" })).toBe(
      "connection",
    );
  });
});
...
describe("predicates", () => {
  it("isRealConnectionSession agrees with classifyTabKind", () => {
    ...
    expect(isRealConnectionSession(s("integration:netbox"))).toBe(false);
  });
  ...
  it("isWinmgmtTabSession only true for winmgmt: prefix", () => {
    ...
    expect(isWinmgmtTabSession(s("integration:netbox"))).toBe(false);
  });

  it("isIntegrationTabSession only true for integration: prefix", () => {
    expect(isIntegrationTabSession(s("ssh"))).toBe(false);
...
describe("partitionSessions", () => {
  it("partitions a mixed list into three buckets", () => {
    ...
    expect(out.integrations.map((x) => x.id)).toEqual(["f"]);
  });
...
describe("realConnectionCount", () => {
  ...
  it("returns 0 for an all-tool tab list", () => {
    expect(
      realConnectionCount([s("tool:a"), s("tool:b"), s("integration:netbox")]),
    ).toBe(0);
  });
...
describe("performance", () => {
  it("classifies 10k sessions in under 10ms", () => {
    ...
    // Sanity: 3 of every 5 are tool/winmgmt/integration panels.
    expect(out.connections).toHaveLength(4000);
    expect(out.tools).toHaveLength(2000);
    expect(out.winmgmt).toHaveLength(2000);
    expect(out.integrations).toHaveLength(2000);
  });
});
