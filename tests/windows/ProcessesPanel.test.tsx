...
describe("ProcessesPanel", () => {
  ...
  it("renders labeled process table and fetches process list on mount", async () => {
    ...
    expect(
      screen.getByRole("textbox", { name: /Search processes/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Refresh processes/i }),
    ).toBeInTheDocument();
  });
  ...
  it("updates aria-sort on sortable headers and exposes terminate busy state", async () => {
    ...
    const cmd = vi.fn((command: string) => {
      if (command === "winmgmt_list_processes")
    ...
    const memoryHeader = screen.getByRole("columnheader", {
      name: /^Memory$/i,
    });
    ...
    expect(cmd).toHaveBeenCalledWith("winmgmt_terminate_process", {
      confirmed: true,
      pid: 101,
    });
    await waitFor(() => {
