...
describe('InternalProxyManager', () => {
  ...
  it('does not render when closed', () => {
    ...
    expect(screen.queryByText('Internal Proxy Manager')).not.toBeInTheDocument();
  });

  it('renders sessions and supports tab switching', async () => {
    render(<InternalProxyManager isOpen onClose={() => {}} />);

    expect(await screen.findByText('Internal Proxy Manager')).toBeInTheDocument();
    expect(await screen.findByText('https://example.com')).toBeInTheDocument();

    fireEvent.click(screen.getByText('Request Log'));
    expect(await screen.findByText('https://example.com/api/status')).toBeInTheDocument();

    fireEvent.click(screen.getByText('Statistics'));
    expect(await screen.findByText('Per-Session Breakdown')).toBeInTheDocument();
  });
  ...
  it('closes on backdrop click', async () => {
    const onClose = vi.fn();
    const { container } = render(<InternalProxyManager isOpen onClose={onClose} />);
