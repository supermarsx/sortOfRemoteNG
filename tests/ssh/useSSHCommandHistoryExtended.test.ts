...
describe("useSSHCommandHistory – extended", () => {
  ...
  it("deletes an entry by id", () => {
    ...
    expect(result.current.allEntries).toHaveLength(1);
    expect(
      result.current.allEntries.find((e) => e.id === idToDelete),
    ).toBeUndefined();
  });
  ...
  it("clears only non-starred entries by default", () => {
    ...
    const keepId = result.current.allEntries.find(
      (e) => e.command === "keep",
    )!.id;
  ...
  it("filters entries by search query (fuzzy)", () => {
    ...
    expect(
      result.current.entries.every((e) => e.command.includes("docker")),
    ).toBe(true);
  });
  ...
  it("imports JSON history", () => {
    ...
    const data = JSON.stringify([
      { command: "imported-cmd", executionCount: 3 },
    ]);
    ...
    expect(
      result.current.allEntries.find((e) => e.command === "imported-cmd"),
    ).toBeDefined();
  });
  ...
  it("preserves starred entries during retention enforcement", () => {
    ...
    const starId = result.current.allEntries.find(
      (e) => e.command === "cmd-a",
    )!.id;
    act(() => {
    ...
    expect(
      result.current.allEntries.find((e) => e.command === "cmd-a"),
    ).toBeDefined();
    expect(
      result.current.allEntries.find((e) => e.command === "cmd-a")!.starred,
    ).toBe(true);
  });
