...
describe("useTunnelChainEditor", () => {
    ...
    const initial = [
      {
        ...
        type: "proxy" as const,
        enabled: true,
        proxy: { proxyType: "socks5" as const, host: "test", port: 1080 },
      },
    ];
  ...
  it("toggles a layer", () => {
    const { result } = renderHook(() => useTunnelChainEditor());
    act(() => {
      result.current.addLayer("ssh-jump");
    });
    const id = result.current.layers[0].id;
    expect(result.current.layers[0].enabled).toBe(true);
    act(() => {
      result.current.toggleLayer(id);
    });
  ...
  it("moves a layer up", () => {
    ...
    act(() => {
      result.current.moveLayer(secondId, "up");
    });
  ...
  it("moves a layer down", () => {
    ...
    act(() => {
      result.current.moveLayer(firstId, "down");
    });
  ...
  it("does not move last layer down", () => {
    ...
    act(() => {
      result.current.moveLayer(lastId, "down");
    });
  ...
  it("updates a layer", () => {
    ...
    const id = result.current.layers[0].id;
    act(() => {
      result.current.updateLayer(id, { name: "My Proxy" });
    });
  ...
  it("creates correct defaults for each tunnel type", () => {
    ...
    for (const vpnType of ["pptp", "l2tp", "ikev2", "ipsec", "sstp"] as const) {
      const vpnLayer = result.current.layers.find(
        (layer) => layer.type === vpnType,
      );
      expect(vpnLayer?.vpn).toEqual({ configId: "" });
      expect(vpnLayer?.tunnel).toBeUndefined();
    }
  });
});
