...
import { renderDiagnosticsPrintDocument } from "../../src/components/connection/diagnostics/renderDiagnosticsPrintDocument";
...
describe("ConnectionDiagnostics", () => {
  ...
  it("renders the diagnostics dialog", () => {
    renderWithProviders(
      <ConnectionDiagnostics connection={mockConnection} onClose={() => {}} />,
    );
  ...
  it("calls onClose when close button is clicked", () => {
    const onClose = vi.fn();
    renderWithProviders(
      <ConnectionDiagnostics connection={mockConnection} onClose={onClose} />,
    );
  ...
  it("renders with different protocols", () => {
    ...
    expect(() => {
      renderWithProviders(
        <ConnectionDiagnostics connection={rdpConnection} onClose={() => {}} />,
      );
    }).not.toThrow();
  });
  ...
  it("writes attacker-controlled diagnostics as text in the print document", () => {
    const payload =
      '</td><img src=x onerror="alert(1)"><script>alert(2)</script>';
    ...
    const manager = {
      results: {
        ...
        pings: [],
        tlsCheck: {
          certificate_subject: payload,
          certificate_issuer: payload,
          certificate_expiry: payload,
          tls_version: payload,
        },
      },
      protocolReport: {
        summary: payload,
        rootCauseHint: payload,
        steps: [
          {
            name: payload,
            status: payload,
      ...
      avgPingTime: null,
      maxPing: null,
    } as unknown as Parameters<typeof renderDiagnosticsPrintDocument>[2];

    renderDiagnosticsPrintDocument(
      printDocument,
      { ...mockConnection, name: payload, hostname: payload },
      manager,
      new Date("2026-07-30T12:00:00Z"),
    );

    expect(printDocument.querySelector("img, script, [onerror]")).toBeNull();
    expect(printDocument.body.textContent).toContain(payload);
    expect(printDocument.title).toContain(payload);
    const csp = printDocument.querySelector(
      'meta[http-equiv="Content-Security-Policy"]',
    );
    expect(csp).not.toBeNull();
    expect(printDocument.head.contains(csp)).toBe(true);
