...
const csvHeaders =
  "ID,Name,Protocol,Hostname,Port,Username,Domain,Description,ParentId,IsGroup,Tags,CreatedAt,UpdatedAt";
const csvRow =
  "1,Test,RDP,example.com,3389,user,,desc,,false,tag1;tag2,2024-01-01T00:00:00.000Z,2024-01-01T00:00:00.000Z";
...
describe("imports connections from CSV", () => {
  ...
  it("never guesses RDP when the protocol is missing (falls back to raw)", async () => {
    const csv = [csvHeaders, "1,NoProto,,host.local,,,,,,false,,,"].join("\n");
    const connections = await importFromCSV(csv);
    expect(connections).toHaveLength(1);
    expect(connections[0].protocol).toBe("raw");
    expect(connections[0].port).not.toBe(3389);
  });

  it("classifies a missing protocol from the port when the port is evidence", async () => {
    const csv = [
      csvHeaders,
      "1,NoProto443,,host.local,443,,,,,false,,,",
      "2,NoProto3389,,host.local,3389,,,,,false,,,",
    ].join("\n");
    const connections = await importFromCSV(csv);
    expect(connections.map((c) => c.protocol)).toEqual(["https", "rdp"]);
  });
  ...
  it("fills default fields when optional CSV values are missing", async () => {
    ...
    expect(new Date(connections[0].createdAt).toString()).not.toBe(
      "Invalid Date",
    );
    expect(new Date(connections[0].updatedAt).toString()).not.toBe(
      "Invalid Date",
    );
  });
...
describe("handles import errors gracefully", () => {
  it("throws on empty CSV (no data rows)", async () => {
    await expect(importFromCSV("")).rejects.toThrow(
      "CSV file must have headers and at least one data row",
    );
  });

  it("throws on CSV with only headers", async () => {
    await expect(importFromCSV(csvHeaders)).rejects.toThrow(
      "CSV file must have headers and at least one data row",
    );
  });
