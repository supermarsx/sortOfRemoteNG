...
describe("detectImportFormat", () => {
  it("detects mRemoteNG XML", () => {
    expect(
      detectImportFormat('<Connections ConfVersion="2.6">', "confCons.xml"),
    ).toBe("mremoteng");
  });
  ...
  it("detects generic mRemoteNG node XML without a Connections root", () => {
    expect(
      detectImportFormat(
        '<Node Name="Direct" Protocol="SSH2" Hostname="direct.example.com" />',
      ),
    ).toBe("mremoteng");
  });
  ...
  it("detects RDCMan XML", () => {
    expect(
      detectImportFormat('<RDCMan programVersion="2.7">', "group.rdg"),
    ).toBe("rdcman");
  });

  it("detects MobaXterm INI", () => {
    expect(detectImportFormat("[Bookmarks]\nSubRep=", "MobaXterm.ini")).toBe(
      "mobaxterm",
    );
  });
  ...
  it("detects Termius JSON by filename", () => {
    expect(
      detectImportFormat(
        JSON.stringify({ hosts: [{ address: "a" }] }),
        "termius-data.json",
      ),
    ).toBe("termius");
  });
  ...
  it("detects Royal TS JSON", () => {
    expect(
      detectImportFormat(
        JSON.stringify({ Objects: [{ Type: "RoyalFolder" }] }),
        "export.rtsz",
      ),
    ).toBe("royalts");
  });
  ...
  it("detects SecureCRT XML", () => {
    expect(
      detectImportFormat(
        "<VanDyke><Sessions></Sessions></VanDyke>",
        "sessions.xml",
      ),
    ).toBe("securecrt");
  });
  ...
  it("detects CSV", () => {
    expect(
      detectImportFormat("Name,Protocol,Hostname,Port\n", "connections.csv"),
    ).toBe("csv");
  });
  ...
  it("defaults unmatched plain text to CSV", () => {
    expect(
      detectImportFormat("plain text export with no structured markers"),
    ).toBe("csv");
  });
  ...
  it("detects content-only vendor markers that rely on secondary branches", () => {
    ...
    expect(detectImportFormat(JSON.stringify({ Type: "RoyalFolder" }))).toBe(
      "royalts",
    );
    ...
    expect(
      detectImportFormat(
        '<Node Name="Direct" Hostname="host-only.example.com" />',
      ),
    ).toBe("mremoteng");
  });
});
...
<Connections ConfVersion="2.6">
  ...
  it("throws when mRemoteNG XML is invalid", async () => {
    await expect(
      importFromMRemoteNG("<Connections><Node></Connections>"),
...
describe("importFromTermius", () => {
  ...
  it("creates unnamed Termius groups when labels are missing", async () => {
    const conns = await importFromTermius(
      JSON.stringify({ groups: [{ id: "grp-1" }] }),
    );
    expect(conns).toHaveLength(1);
...
describe("importFromRoyalTS", () => {
  ...
  it("fills unnamed Royal TS folders and sparse connections with safe fallbacks", async () => {
    ...
    const computerOnly = conns.find(
      (c) => c.hostname === "computer-only.example.com",
    );
  ...
  it("rejects Royal TS exports that do not define Objects", async () => {
    await expect(
      importFromRoyalTS(JSON.stringify({ version: "1.0" })),
    ).rejects.toThrow("Royal TS import must contain an object array");
  });
});
...
describe("importFromJSON", () => {
  ...
  it("throws for unsupported JSON object shapes", async () => {
    await expect(
      importFromJSON(JSON.stringify({ invalid: true })),
    ).rejects.toThrow(
...
<Connections ConfVersion="2.6">
  <Node Name="Direct SSH" Protocol="SSH2" Hostname="mremoteng.example.com" />
    ...
    const conns = await importConnections(
      content,
      "mremoteng.xml",
      "mremoteng",
    );
...
"Protocol"="telnet"`;
  ...
  it("routes termius format to the Termius parser", async () => {
    const content = JSON.stringify({
      hosts: [
        { label: "Termius Host", address: "termius.example.com", port: 22 },
      ],
    });
