...
const buildManager = (count: number) => ({
  ...
  rdpRec: {
    enabled: true,
    autoRecordRdpSessions: true,
    autoSaveToLibrary: true,
    defaultVideoFormat: "gif",
    recordingFps: 30,
    videoBitrateMbps: 5,
    maxRdpRecordingDurationMinutes: 60,
    maxStoredRdpRecordings: 10,
  },
  webRec: {
    enabled: true,
    autoRecordWebSessions: true,
    recordHeaders: true,
    maxWebRecordingDurationMinutes: 60,
    maxStoredWebRecordings: 10,
...
describe("RecordingSettings translations", () => {
  it("renders default-on deletion confirmation and writes the persistent setting", () => {
    useRecordingSettingsMock.mockReturnValue(buildManager(0));
    ...
    const toggle = screen.getByRole("checkbox", {
      name: "Confirm recording deletion",
    });
    ...
    view.rerender(
      <RecordingSettings
        settings={{ confirmDeleteRecording: false } as GlobalSettings}
        updateSettings={updateSettings}
      />,
    );
  ...
  it("localizes all recording copy and preserves format identifiers", () => {
    ...
    for (const fallback of technicalFallbacks) {
      expect(recordingT.mock.calls.some((call) => call[1] === fallback)).toBe(
        false,
      );
    }
  });
